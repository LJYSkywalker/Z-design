...
| REG-004 | checked-in registry entry 指向真实可打包插件源码 | `community/registry-starter` 的 source 指向 `plugins/community/registry-starter`，源码 `z-design.json` 带 `plugin.repo` | `apps/daemon/tests/plugins-marketplaces.test.ts` |
...
| REG-C04 | `open-design-marketplace.json` 生成器 | 输入多个 `plugins/community/**/z-design.json`，输出排序稳定、schema 通过、source/digest 完整 |
