...
- **Fail-closed.** Two coordinated mechanisms prevent the gate from
  ...
  - An **orchestrator-pinned mode** via the `ZD_REQUIRE_DESKTOP_AUTH=1`
...
- **Daemon restart edge.** If the daemon is restarted while desktop
  keeps running, the new daemon process will be in `ZD_REQUIRE_DESKTOP_AUTH`
...
- **tools-dev split-start hardening.** `tools-dev start desktop`
  ...
  `ZD_REQUIRE_DESKTOP_AUTH=1` (the env var is only injected when
...
services:
  ...
  web:
    ...
    ports: ["3000:3000"]
    environment: [ "ZD_DAEMON_URL=http://daemon:7456" ]
```
...
# flip VERCEL env flag ZD_MODE=direct to hide daemon-connect UI
```
