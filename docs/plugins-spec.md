...
1. **Plugin author ships portable capability.** `SKILL.md` remains the executable agent contract; `z-design.json` adds OD marketplace metadata, input fields, defaults, previews, and context wiring.
...
sequenceDiagram
  ...
  participant P as Plugin Manifest<br/>(SKILL.md + z-design.json)
...
5. [`z-design.json` schema](#5-open-designjson--schema-v1)
...
- Defaults locked from the planning round (overridable in review):
  - **Compatibility = wrap-then-extend.** Existing `SKILL.md` and `.claude-plugin/plugin.json` repos run as-is; `z-design.json` is an additive sidecar.
...
Each catalog needs a different listing format, but all of them index `SKILL.md`-shaped folders. By keeping `SKILL.md` canonical and `z-design.json` strictly sidecar, a single repo lands in every catalog without per-target rewrites.
...
2. A vanilla skill or claude-plugin repo becomes an OD plugin by adding an optional `z-design.json` sidecar — no rename, no body changes.
...
| `z-design.json` only                                            | metadata-only  | no                                        | no                | no                   | no      | no        |
| `SKILL.md` + `z-design.json`                                    | enriched       | yes                                       | yes               | yes                  | yes     | yes       |
| `.claude-plugin/...` + `z-design.json`                          | enriched       | yes (claude)                              | partial           | listable             | listable| listable  |
| `SKILL.md` + `.claude-plugin/...` + `z-design.json`             | fully enriched | yes                                       | yes               | yes                  | yes     | yes       |

The takeaway: **`SKILL.md` is the lowest common denominator**. Every plugin recommended for distribution should ship a `SKILL.md` so it lands cleanly in every major catalog, then add `z-design.json` to gain OD's product surface.

A folder that contains only `z-design.json` is not a runnable plugin in v1; it is a **metadata-only preset**. OD may read it to show a marketplace card, aggregate remote references, or act as a future install stub, but it cannot trigger an agent run and cannot be listed in cross-agent catalogs. `od plugin doctor` must mark this shape as `metadata-only` and prompt the author to add `SKILL.md` or `.claude-plugin/plugin.json` before publishing it as a runnable plugin.
...
│   └── plugin.json
├── z-design.json                  # optional sidecar — unlocks OD product surface
...
- `z-design.json` only ever **points** at SKILL.md / DESIGN.md / craft files; it never duplicates their bodies.
- Existing OD-specific frontmatter on SKILL.md (the `od:` namespace already documented in [`skills-protocol.md`](skills-protocol.md) and used in [`skills/blog-post/SKILL.md`](../skills/blog-post/SKILL.md)) is honored as a fallback for plugins without `z-design.json`. We do not deprecate it; we layer over it.
- A runnable v1 plugin must contain at least one of `SKILL.md` or `.claude-plugin/plugin.json`. `z-design.json` does not define agent behavior by itself; it only tells OD how to display, resolve, and apply that behavior.

## 5. `z-design.json` — schema v1
...
If `z-design.json` and `SKILL.md` frontmatter both exist, `z-design.json` wins, but the loader must preserve adapter warnings. Authors can migrate incrementally: first keep the old skill runnable as-is, then add OD marketplace metadata.
...
The marketplace top-level `version` is the catalog snapshot version; every `plugins[]` entry also declares the listed plugin version. Installers still verify the target folder's own `z-design.json` after fetching, but registry search, audit logs, and marketplace refresh events can now reason about catalog and plugin versions before install.
...
- **Iteration ceiling.** The daemon enforces `iterations <= 10` (configurable via `ZD_MAX_DEVLOOP_ITERATIONS`) to keep a buggy plugin from burning provider quota in an infinite loop.
...
- `parsers/manifest.ts` — read `z-design.json` → `PluginManifest` (Zod-validated).
...
- `merge.ts` — merge sidecar + adapters with `z-design.json` winning; foreign content lands in `compat.*`.
...
| New `apps/daemon/src/plugins/pipeline.ts` | Parses `od.pipeline` (including the `until` expression evaluator), schedules stages, and drives §10.2 devloop (with `ZD_MAX_DEVLOOP_ITERATIONS` ceiling and break signaling). |
...
CREATE TABLE installed_plugins (
  ...
  manifest_json        TEXT NOT NULL,    -- cached z-design.json (or synthesized)
...
CREATE TABLE applied_plugin_snapshots (
  ...
  expires_at              INTEGER,                       -- NULL when referenced by any run / conversation / project (pinned forever); otherwise applied_at + ZD_SNAPSHOT_UNREFERENCED_TTL_DAYS at insert time. GC worker (Phase 5) deletes rows where expires_at <= now()
  FOREIGN KEY (project_id)      REFERENCES projects(id)      ON DELETE CASCADE,
  FOREIGN KEY (conversation_id) REFERENCES conversations(id) ON DELETE SET NULL,
  FOREIGN KEY (run_id)          REFERENCES runs(id)          ON DELETE SET NULL
);
...
`od daemon start --headless` is the entry for the headless mode in §11.7 (no web bundle, no electron). `od daemon start --serve-web` adds the local web UI without electron. Both keep using the existing tools-dev port conventions ([`ZD_PORT`, `ZD_WEB_PORT`](../AGENTS.md)).
...
- Same plugin manifests (`z-design.json` inside each repo).
...
| `open-design-marketplace.json`                                                                   | entry referencing `github:owner/repo`                                      | `z-design.json` enriches the listing                |

### 14.1 Author tooling

- `od plugin scaffold` — writes a starter folder containing both `SKILL.md` (industry-standard, with `od:` frontmatter for backward compat) and `z-design.json` (OD enrichment with `compat.agentSkills` pointing at the SKILL.md).
...
1. **Skill-only consumption (no OD).** A Cursor user runs `npx skills add open-design/make-a-deck`. Cursor reads `SKILL.md` and runs the workflow. No OD CLI, no OD daemon. The plugin's marketplace polish (`z-design.json`) is ignored — Cursor sees a vanilla skill.
...
| Plugin / skill format | `.cursor/rules/`, MCP servers                | `SKILL.md` + `z-design.json` + atoms              |
...
- **Contents**:
  ...
  - Bundled code-agent CLIs that OD supports as agent backends: Claude Code, Codex CLI, Gemini CLI. Selectable per run; default is `ZD_AGENT_BACKEND`.
...
```env
ZD_PORT=17456
ZD_BIND_HOST=0.0.0.0                 # the variable the daemon already reads ([`apps/daemon/src/server.ts`](../apps/daemon/src/server.ts))
# Set daemon storage env vars only after reading root AGENTS.md -> Daemon data directory contract.
ZD_TRUST_DEFAULT=restricted          # safe default for hosted (§9) — introduced in Phase 5
ZD_AGENT_BACKEND=claude              # default code agent backend
ZD_API_TOKEN=<random>                # required when ZD_BIND_HOST != 127.0.0.1 — Phase 5 introduces the bearer middleware
ZD_SNAPSHOT_UNREFERENCED_TTL_DAYS=30 # see §11.4: unreferenced applied_plugin_snapshots expire after this window; set to 0 to keep forever
ZD_SNAPSHOT_RETENTION_DAYS=          # opt-in (default unset): also retire referenced snapshots once their run/conversation/project is terminal and applied_at is older than the window
ZD_SNAPSHOT_GC_INTERVAL_MS=21600000  # snapshot GC worker tick (Phase 5)
...
>
> - `ZD_BIND_HOST` already exists in the daemon ([`apps/daemon/src/server.ts`](../apps/daemon/src/server.ts), [`apps/daemon/src/origin-validation.ts`](../apps/daemon/src/origin-validation.ts)). Earlier draft text referred to the same variable as `ZD_HOST`; the correct name is `ZD_BIND_HOST`, and this spec uses it everywhere. **No `ZD_HOST` alias is introduced** — that would invite double-name drift.
> - `ZD_TRUST_DEFAULT`, `ZD_API_TOKEN`, and the corresponding bearer-token middleware are **not yet implemented**; they are part of Phase 5 "Cloud deployment + pluggable storage" (§15.7, §16 Phase 5). Until landed, hosted deployments must rely on a reverse proxy / network ACL for access control; §15.7 explicitly calls out this prerequisite.
...
- `ZD_TRUST_DEFAULT=restricted` is the recommended default. Capabilities (`mcp`, `subprocess`, `bash`, `network`) require explicit operator opt-in via `od plugin trust <id>` or a `ZD_TRUSTED_PLUGINS` allow-list env var.
- The image runs as a non-root user; plugin sandboxes inherit this.
- The HTTP API listens on `ZD_BIND_HOST`; when set to `0.0.0.0`, **once Phase 5 lands** `ZD_API_TOKEN` is required and is checked on every request via `Authorization: Bearer <token>`. When unset, the daemon refuses to bind to a public interface and exits with an error. Before Phase 5 (i.e., today's implementation), hosted deployments must isolate the daemon port behind a reverse proxy / network ACL.
...
- **Authentication scope (v1):** single-tenant only — one shared `ZD_API_TOKEN`. **Multi-tenant auth** (per-user OAuth, RBAC, project ownership, billing) is **explicitly out of scope for v1** and tracked as an open item in §18.
...
- `pnpm --filter @open-design/plugin-runtime test` (parser fixtures: pure SKILL.md, pure claude plugin, metadata-only z-design.json, all three combined, SKILL frontmatter mapping).
...
- **Bound-API-token guard (new in Phase 5):** daemon refuses to bind `ZD_BIND_HOST=0.0.0.0` without `ZD_API_TOKEN`; bearer-token middleware on `/api/*` (skipped only when host is loopback).
...
- **`AppliedPluginSnapshot` retention enforcement worker:** the `expires_at` column added in Phase 1 is now enforced. A daemon background job (default every 6 h, knob `ZD_SNAPSHOT_GC_INTERVAL_MS`) deletes rows where `expires_at IS NOT NULL AND expires_at <= now()`. Unreferenced snapshots get `expires_at = applied_at + ZD_SNAPSHOT_UNREFERENCED_TTL_DAYS` (default `30`; `0` disables) at insert time; referenced snapshots stay `NULL` (pinned per §8.2.1). Operators may set `ZD_SNAPSHOT_RETENTION_DAYS` to additionally retire referenced rows once their referencing run/conversation/project is terminal. Each deletion writes an audit log entry. CLI escape hatch: `od plugin snapshots prune --before <ts>` for forced cleanup.
...
OD reads it as a plugin via the existing `od:` frontmatter loader documented in [`skills-protocol.md`](skills-protocol.md). No `z-design.json` needed — the plugin lacks marketplace polish but is fully runnable.
...
`SKILL.md` stays portable — Cursor / Codex / OpenClaw read it directly. `z-design.json` adds preview, query, chip strip, capabilities. The repo lists cleanly on every catalog in §14 without modification.
...
├── z-design.json                  # kind: 'bundle'; lists nested skills
├── skills/
...
| Risk                                                        | Mitigation                                                                                          |
| ----------------------------------------------------------- | --------------------------------------------------------------------------------------------------- |
| Schema drift between OD plugin and the broader skill spec   | `z-design.json` is sidecar-only; it never modifies SKILL.md. CI tests run against the public anthropics/skills repo. |
...
| `od.pipeline` devloop infinite loop burning quota | `until` is required and uses a restricted syntax; `ZD_MAX_DEVLOOP_ITERATIONS` ceiling (default 10); both UI and CLI expose a "Stop refining" break action. |
| `ZD_HOST` / `ZD_BIND_HOST` naming drift | Spec uses the variable the daemon already reads, `ZD_BIND_HOST`; no `ZD_HOST` alias is introduced; §15.3 explicitly notes the deviation from earlier draft text. |
| Hosted deployments without the bound-API-token guard could leak the API publicly (pre-Phase 5 must rely on a reverse proxy) | Once Phase 5 lands, daemon refuses to bind `ZD_BIND_HOST=0.0.0.0` without `ZD_API_TOKEN`; bearer-token middleware enforced on `/api/*`; §15.3 / §15.7 record the current vs. target gap. |
...
| Cross-conversation reuse via `genui_surfaces` may make users "forget what they authorized" | The web `GenUIInbox` and `od ui list --project <id>` must enumerate every `persist=project` resolved row with revoke entry points; hosted mode can default-expire via `ZD_GENUI_PROJECT_TTL_DAYS`; revoke writes an audit log entry. |
...
- **Multi-tenant auth (per-user OAuth, RBAC, project ownership, billing)** is explicitly out of scope for v1. The Docker image is single-tenant by design (one `ZD_API_TOKEN`). Multi-tenancy is a post-v1 story that needs its own spec — confirm this scoping is acceptable for the first ecosystem release.
- **Trust propagation in hosted mode** — current spec locks arbitrary GitHub / URL / local plugins to `restricted` by default, and third-party marketplaces do not propagate trust by default. Confirm whether hosted deployments may trust individual plugins through `ZD_TRUSTED_PLUGINS`, or whether operators must first trust the source marketplace.
...
- ~~**`AppliedPluginSnapshot` retention**~~ — **resolved (PB2, see `docs/plans/plugins-implementation.md` §7).** Snapshots referenced by any run / conversation / project stay pinned forever (`expires_at = NULL`); unreferenced snapshots get `expires_at = applied_at + ZD_SNAPSHOT_UNREFERENCED_TTL_DAYS` (default `30`, `0` disables). The "expire even referenced rows" knob `ZD_SNAPSHOT_RETENTION_DAYS` is operator-opt-in only (default unset), and applies only when the referencing row is terminal. The `expires_at` column lands in Phase 1 (§11.4); the GC worker lands in Phase 5 (§16). The `od plugin snapshots prune` CLI remains as a forced-cleanup escape hatch.
...
- **Inherited supply.** Every public agent skill on `anthropics/skills`, `awesome-agent-skills`, `clawhub`, and `skills.sh` is one optional `z-design.json` away from being an OD plugin — and reciprocally, every OD plugin is publishable to all four catalogs without modification.
...
- An atom plugin is a folder with `z-design.json` (`od.kind: 'atom'`) plus `SKILL.md`.
...
- `plugins/_official/scenarios/od-new-generation/z-design.json`
- `plugins/_official/scenarios/od-figma-migration/z-design.json` (after Phase 6)
- `plugins/_official/scenarios/od-code-migration/z-design.json` (after Phase 7)
- `plugins/_official/scenarios/od-tune-collab/z-design.json`
