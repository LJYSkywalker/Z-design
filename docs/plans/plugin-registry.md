...
- [ ] **R3. `SKILL.md` floor stays portable.** A plugin published to OD's
  ...
  Code / Cursor / Codex / Gemini CLI / OpenClaw / Hermes. `z-design.json`
...
```text
Plugin source repo
  z-design.json includes plugin.repo
        |
...
the user describe the plugin, writes `SKILL.md` and `z-design.json`, adds
...
├── plugins/
│   └── <vendor>/<plugin-name>/
│       ├── manifest.json              ← latest copy of z-design.json
│       ├── versions/
...
manifest must include `plugin.repo` in `z-design.json`, pointing to the
...
- [x] **P2.4 Agent-assisted Create plugin flow.** The `Create plugin` action
  ...
  writes `SKILL.md`/`z-design.json`, validates, installs a local test copy,
...
2. **Plugin source-of-truth repo.** The source repo can be any shape that
   ...
   requires a `plugin.repo` field in `z-design.json` pointing to source.
