...
1. An external orchestrator launches Open Design with `ZD_SANDBOX_MODE=1` and
...
`ZD_SANDBOX_MODE=1` is the spine of the sandbox runtime contract. Several
...
Without `ZD_SANDBOX_MODE=1`, Open Design is the normal local product runtime, not
...
- Add one tools-dev/e2e variant that launches with `ZD_SANDBOX_MODE=1` and
...
- Plumb `ZD_SANDBOX_MODE=1` and isolated daemon storage through a
...
2. Sandbox-mode fail-closed media and e2e smoke: explicit run media policy for
   ...
   `ZD_SANDBOX_MODE=1` tools-dev smoke.
