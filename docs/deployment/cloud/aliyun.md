...
server {
  ...
  location / {
    ...
    # generates an ZD_API_TOKEN and binds the daemon to 0.0.0.0 inside
    ...
    # localhost bypass.
    proxy_set_header Authorization "Bearer <ZD_API_TOKEN from deploy/.env>";
  }
}
```

Set `OPEN_DESIGN_ALLOWED_ORIGINS` in `deploy/.env` to the public URL so CORS clears the proxy. The `ZD_API_TOKEN` referenced in the Nginx block above is generated automatically by `deploy/scripts/install.sh` and written into `deploy/.env`; copy that value (or wire `proxy_set_header Authorization` to read it from a secrets manager) so the proxy authenticates on every request.
...
> **Required env for Kubernetes:** the daemon defaults to `ZD_BIND_HOST=127.0.0.1`, which makes the readiness probe (and the Service) unable to reach the container. To make the Pod reachable inside the cluster you must set `ZD_BIND_HOST=0.0.0.0`, and the daemon's bound-API-token guard then requires `ZD_API_TOKEN` to be set whenever it binds to a non-loopback interface. Both env vars are reflected in the manifest below.
>
> Also note that the daemon reads `ZD_ALLOWED_ORIGINS` directly. The `OPEN_DESIGN_ALLOWED_ORIGINS` name documented in `deploy/.env.example` is a Compose-only alias mapped in `deploy/docker-compose.yml`; for the direct-container ACK path you must set `ZD_ALLOWED_ORIGINS` instead.
            ...
            - name: ZD_API_TOKEN
              valueFrom:
                secretKeyRef:
                  ...
                  key: api-token            # required whenever ZD_BIND_HOST is non-loopback
            - name: ZD_ALLOWED_ORIGINS
              value: "https://design.example.cn"  # set when fronting with an Ingress; daemon reads ZD_*, not OPEN_DESIGN_*
...
3. CORS allowlist set via `OPEN_DESIGN_ALLOWED_ORIGINS` (Compose / `deploy/.env` path) or `ZD_ALLOWED_ORIGINS` (direct-container / Kubernetes path).
...
| 200 from `/api/health` but UI fails to load | CORS rejecting the proxied origin, **or** the reverse proxy not forwarding the bearer token (Compose always generates `ZD_API_TOKEN`; only `/api/health`, `/api/version`, `/api/daemon/status` skip auth) | Set `OPEN_DESIGN_ALLOWED_ORIGINS` (Compose / `deploy/.env`) or `ZD_ALLOWED_ORIGINS` (direct container / ACK) to the public URL, **and** add `proxy_set_header Authorization "Bearer <ZD_API_TOKEN>"` to the Nginx / SLB upstream config |
| ACK Pod stuck in `NotReady`, readiness probe fails | Daemon defaulting to `ZD_BIND_HOST=127.0.0.1` so the kubelet can't reach it | Set `ZD_BIND_HOST=0.0.0.0` and `ZD_API_TOKEN` in the Pod env (the daemon refuses non-loopback binds without a token) |
