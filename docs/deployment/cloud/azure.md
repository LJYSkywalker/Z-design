...
ACI is the daemon upstream in this topology. The browser-facing app URL must be served by an authenticated TLS reverse proxy that forwards traffic to ACI, injects the daemon bearer token on `/api/*` requests, and sends a browser origin listed in `ZD_ALLOWED_ORIGINS`.
...
export ZD_API_TOKEN="$(openssl rand -hex 32)"
```
...
Do not open this URL directly in a browser. The daemon requires `Authorization: Bearer <ZD_API_TOKEN>` on non-loopback `/api/*` requests, and the web UI does not put that secret in browser requests.
...
server {
  ...
  location /api/ {
    proxy_set_header Authorization "Bearer <ZD_API_TOKEN>";
...
- Set `ZD_API_TOKEN` as a secret pipeline variable.
...
- Keep `ZD_API_TOKEN` secret. The proxy may use it upstream, but browser clients must not receive it. Rotate it by redeploying with a new value.
