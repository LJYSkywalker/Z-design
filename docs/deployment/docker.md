...
- recommended default: paste the generated token into `ZD_API_TOKEN=`
- trusted authenticated reverse proxy only: leave `ZD_API_TOKEN=` empty and set `OPEN_DESIGN_DISABLE_API_AUTH=1`
...
- reverse proxy + `ZD_API_TOKEN`: either inject `Authorization: Bearer <ZD_API_TOKEN>` at the proxy, or set `OPEN_DESIGN_DISABLE_API_AUTH=1` only when that proxy already authenticates every request and the daemon is not directly exposed.
- `Authorization: Bearer <ZD_API_TOKEN> required` on macOS: Docker Desktop bridge networking makes the daemon see requests as non-loopback. See [Docker Desktop on macOS](../../deploy/README.md#docker-desktop-on-macos) for the host networking workaround.
