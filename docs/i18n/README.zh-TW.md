<h1 align="center">Z-design：開源的 Claude Design 替代方案</h1>

> 🔥 **Z-design 1.0.0 正式發布：All-in-one 的 Agentic 設計工作台。** 設計的完整流程從此只需一個視窗——從一個模糊的想法出發，到發現參考、蒐集素材、互動式編輯、留言排入佇列、打磨動效，再交付給編輯器或 Code Agent，全程無須離開應用程式。搭配多會話並行，它不再只是一個助手，而是一支為你工作的本地設計團隊。[下載 1.0.0](https://github.com/LJYSkywalker/Z-design/releases) · [參與討論](https://github.com/LJYSkywalker/Z-design/discussions/4153)
>
<p align="center">
...
> 🍃 **本專案基於 [Open Design](https://github.com/nexu-io/open-design)，針對 [GLM（智譜）](https://www.zhipuai.cn/) 進行了優化與改進。** 它保留了上游 Open Design 完整的 Agentic 設計工作台，同時對 Agent 迴圈、提示詞與模型接入做了針對 GLM-5.2 工具呼叫的適配，讓整套流程原生執行在 GLM 系列模型上，而不再只依賴 Claude。

原始產品、技能、設計系統與外掛的功勞歸屬上游 Open Design 團隊及其貢獻者。本 Fork（由 [Aqu-210L](https://github.com/Aqu-210L) 維護）主要做的是：

- 將預設的 Agent 執行階段切換到 **GLM-5.2**。
- 針對 GLM 的工具呼叫行為調校系統提示詞、輸出契約與 artifact 串流輸出。
- 保留上游的技能、設計系統與外掛，讓開放的生態繼續可用。

上游產品請見 [`nexu-io/open-design`](https://github.com/nexu-io/open-design)。

---

## 部署

Z-Design 提供三種形態：本地優先的桌面應用、Docker 映像檔、以及從原始碼執行。下面所有命令都基於本倉庫。

### 前置需求

| 方式 | 需求 |
|---|---|
| 桌面應用 | macOS（Apple Silicon / Intel x64）或 Windows（x64），無需安裝其他相依套件。 |
| Docker | Docker Desktop + Docker Compose v2 |
| 從原始碼執行 | Node.js `~24`、pnpm `10.33.x`（透過 Corepack）。主支援 macOS / Linux / WSL2；Windows 原生為盡力而為。 |

### 方式 A — 桌面應用（零設定，推薦）

從 [GitHub Releases](https://github.com/LJYSkywalker/Z-design/releases) 下載對應平台最新建置：

- **macOS** — Apple Silicon 與 Intel x64
- **Windows** — x64
- **Linux** — AppImage（可選通道）

安裝後，應用會自動偵測 `PATH` 上的編碼 Agent CLI，載入 100+ 技能與 150 個設計系統，直接在入口檢視輸入簡報即可。若要讓 GLM 作為後端，把執行模式切換為 **BYOK / API 模式**，填入 GLM 的 API Key 與端點即可。

### 方式 B — Docker（單容器，無頭部署）

發布映像檔會在 `7456` 連接埠執行守護行程並提供靜態 Web 建置。
...
docker compose pull && docker compose up -d   # 更新到最新映像檔
docker compose down           # 停止
docker compose down -v        # 停止並清除本地資料
```

`deploy/.env` 關鍵變數：

```env
OPEN_DESIGN_PORT=7456                       # 宿主連接埠（繫結到 127.0.0.1）
OPEN_DESIGN_ALLOWED_ORIGINS=https://yourdomain.com   # 網域後方的 CORS 來源
OPEN_DESIGN_IMAGE=ghcr.io/nexu-io/od:latest # 映像檔 tag
ZD_API_TOKEN=                                # 必填 — 用 openssl rand -hex 32 產生
```

> **公開暴露：** 維持 Compose 繫結到 localhost，透過帶鑑權的反向代理、SSH 通道或 VPN 再暴露到公開網路。若在前方放 nginx，務必關閉 SSE 路由的緩衝（`proxy_buffering off; gzip off;`），否則 Agent 長連線串流會被截斷。
...
# 2. 工具鏈（Node 24 + pnpm 10.33.x，透過 Corepack）
corepack enable
pnpm install

# 3. 建置 daemon CLI（`od` 命令依賴它）
pnpm --filter @z-design/daemon build

# 4. 前景啟動 daemon + web
pnpm tools-dev run web
# 開啟 tools-dev 印出的 web 位址
```

其他生命週期命令：

```bash
pnpm tools-dev                       # 背景啟動 daemon + web + desktop
pnpm tools-dev start web             # 背景啟動 daemon + web
pnpm tools-dev status                # 檢視託管的執行階段狀態
pnpm tools-dev logs                  # 檢視 daemon/web/desktop 日誌
pnpm tools-dev stop                  # 停止託管的執行階段
pnpm typecheck                       # 工作區型別檢查
```

> 請勿使用已移除的舊根命令（`pnpm dev`、`pnpm start`、`pnpm daemon`）。`pnpm tools-dev` 是唯一的本地生命週期入口。

### 設定 GLM 作為後端

Z-Design 啟動後，把它指向 GLM，讓所有生成都走 GLM-5.2：

1. 開啟應用，進入 **設定 → 執行模式**。
2. 選擇 **BYOK / API 模式**（OpenAI 相容端點）。
3. 填入：
   - **baseUrl** — `https://open.bigmodel.cn/api/paas/v4`（智譜 GLM 的 OpenAI 相容端點）
   - **apiKey** — 你在 [open.bigmodel.cn](https://open.bigmodel.cn/) 的 GLM API Key
   - **model** — `glm-5.2`（或其他你有權限的 GLM 系列模型）
4. 儲存，然後在入口檢視輸入簡報。Agent 會在左側面板串流輸出，`<artifact>` 在右側即時呈現。

> 在 Docker / 無頭部署中，相同的值透過 `POST /api/proxy/openai/stream` 傳給同樣的 `baseUrl` / `apiKey` / `model` 欄位，無需單獨設定。

### 驗證是否正常

```bash
# 原始碼 / 開發模式 — daemon 健康檢查
curl -s http://127.0.0.1:7456/api/health

# Docker — 從 compose 網路內部檢查
docker compose exec open-design wget -qO- http://127.0.0.1:7456/api/health
```

`/api/health` 回傳 `200 OK` 表示守護行程已啟動，Web 建置已就緒。完整快速開始、環境變數、Nix flake 與打包建置流程請見 [`QUICKSTART.zh-TW.md`](QUICKSTART.zh-TW.md)。
...
Z-design 是這樣誕生的：當 Anthropic 隨 Claude Design 推出的那套 **agent 原生** 迴圈——釐清需求、鎖定方向、串流產出 artifact、評析、交付——不再封閉，而是化為一個由 **skills、設計系統與外掛組成的檔案系統**，讓你筆電上既有的編碼 agent 都能讀取、寫入、再混搭。你的 CLI 成為設計引擎，你的筆電成為工作室，而你團隊的 `DESIGN.md` 則成為品牌契約。
...
快速看看 Z-design 是什麼、能做什麼。從 **首頁** 起步，用 **自動化** 編排重複的工作流程，在 **設計系統** 中提煉品牌契約，再以 **外掛** 與 **整合** 擴充；在任何專案的 **Studio** 中，同一套設計系統就能串流產出原型、即時 artifact、HyperFrames、簡報與圖片。
...
<sub><b>整合</b>——連接外部系統與 MCP 工具，並從任何 IDE、腳本或自動化流程使用 Z-design。</sub>
</td>
</tr>
</table>
...
> Z-design 以 **skills、CLI 與 MCP server** 的形式發行，主流編碼 agent 都能原生使用。安裝 OD 後，只要一句 `od mcp install <agent>` 就能把 MCP server 接進該 agent 的設定，你便能在任何 agent 內呼叫同一組工具。
...
**[HyperFrames][hyperframes]** 是 HeyGen 開源、agent 原生的影片框架，在 Z-design 中以一等公民的身分整合。agent 撰寫 HTML + CSS + GSAP，HyperFrames 透過無頭 Chrome + FFmpeg 將其渲染成一支確定性的 MP4。搭配 **Seedance 2.0** 可做電影感的 t2v / i2v，**Veo 3 / Sora 2 / Kling 2** 提供路由後的模型變體，**Suno v5 / Lyria 2** 則負責音訊層。
...
Z-design（OD）就是那個開源的替代方案。同樣的迴圈、同樣以 artifact 為先的思維模式，卻沒有任何鎖定：
...
使用 Z-design 最快的方式。無須 Node、無須 pnpm、無須 clone。

- **macOS**（Apple Silicon · Intel x64）→ [**open-design.ai**](https://open-design.ai/) 或 [GitHub Releases](https://github.com/LJYSkywalker/Z-design/releases)
- **Windows**（x64）→ [**open-design.ai**](https://open-design.ai/) 或 [GitHub Releases](https://github.com/LJYSkywalker/Z-design/releases)
- **Linux**（AppImage，選用通道）→ [GitHub Releases](https://github.com/LJYSkywalker/Z-design/releases)

安裝後：應用程式會自動偵測你 `PATH` 上的每一個編碼 agent CLI、載入 100+ 個 skills 與 150 套設計系統，並讓你在入口視圖中輸入需求。

### 🤖 安裝進你的編碼 agent（無 UI）

你完全不必打開 GUI 也能使用 Z-design——在 Claude Code、Codex、Cursor、Copilot、OpenClaw、Antigravity、Hermes、Kimi 等等之中，把它當成 skill、外掛或 MCP server 來呼叫。
...
Sealos App Store 範本會執行已發布的 Z-design Docker 映像，提供持久化工作區儲存，並在公開代理層啟用 Basic Auth。自訂公開或共享 Docker 部署請遵循 [`deploy/README.md`](../../deploy/README.md#local-compose) 中的反向代理與 `OPEN_DESIGN_ALLOWED_ORIGINS` 指引。
...
5. **Z-design 會隨著你的使用愈來愈聰明。** 你的截圖、字型、色票與確認過的 artifact 會累積成下一次工作階段的預設值。更少返工、更少偏移。

---

## 從你的編碼 agent 使用 Z-design

Z-design 隨附一個 **stdio MCP server** 與各 agent 專屬的 **安裝腳本**。任何在其他儲存庫中、相容 MCP 的 agent，都能直接讀取你本地 Z-design 專案的檔案——tokens CSS、JSX 元件、入口 HTML——當成一個可按名稱查詢的結構化 API。agent 看到的永遠是即時檔案，而非過期的匯出檔。
...
**對於從零開始的 agent，** 安裝程式會放置 `~/.config/<agent>/z-design.json`（或對應平台的等價檔案），外加一段可複製貼上的 MCP 片段。Cursor 會得到一鍵 deeplink；Claude Code 會得到一行 `claude mcp add-json`；其餘每一個 agent 都會得到符合其設定所需 schema 的 JSON。完整的各 agent 流程 → 桌面應用程式中的 **Settings → MCP server**，或 [`docs/agent-adapters.md`](../../docs/agent-adapters.md)。

**安全模型。** 預設唯讀，daemon 綁定於 `127.0.0.1`，SSRF 在代理邊界被阻擋。要在區域網路曝露，需明確設定 `ZD_BIND_HOST` 加上 `ZD_ALLOWED_ORIGINS`。連接器憑證與即時 artifact 預覽路由無論如何都僅限 loopback。
...
**261 個官方外掛** 位於 [`plugins/_official/`](../../plugins/_official/)。每個外掛都是一個 **可攜的 agent-skill 資料夾**——含一份 `SKILL.md`（任何支援 Agent Skills 的 agent 都能讀取），外加一份選用的 `z-design.json` manifest，為 Z-design 提供市集中繼資料、輸入、預覽、pipeline 與能力宣告。直接跳到某個分類：
...
一個外掛 **至少只需要一份 `SKILL.md`**；若要把它列入 Z-design 市集，再加上一份 `z-design.json`：
...
1. 把外掛資料夾放進 [`plugins/community/`](../../plugins/community/)（第三方外掛），或——若要讓它隨 Z-design 一同附帶——放進 [`plugins/_official/`](../../plugins/_official/) 中對應的層級。
2. 通過驗證：`od plugin validate`、`pnpm guard`、`pnpm --filter @z-design/plugin-runtime typecheck`。
...
- [x] **0.9.0**——Z-design AMR（內建於應用程式的官方 Model Router：零設定、一鍵登入）
...
- 🗣️ **GitHub Discussions**——深入問答、RFC、「秀出你的成果」 → [**Discussions**](https://github.com/LJYSkywalker/Z-design/discussions)
- 🐛 **GitHub Issues**——錯誤回報、功能請求 → [**Issues**](https://github.com/LJYSkywalker/Z-design/issues)

[`good-first-issue`](https://github.com/LJYSkywalker/Z-design/issues?q=is%3Aissue+is%3Aopen+label%3A%22good+first+issue%22) 與 [`help-wanted`](https://github.com/LJYSkywalker/Z-design/issues?q=is%3Aissue+is%3Aopen+label%3A%22help+wanted%22) 這兩個標籤是最容易的入門方式。

---

## 貢獻

Z-design 之所以能持續前進，是因為貢獻者——設計師、工程師、prompt 作者——不斷現身。許多最常用的 skills、設計系統與外掛，都出自核心團隊以外的人之手。
...
| 一個新的 **外掛** | 在某個分類資料夾下放進 `z-design.json` + manifest | [`plugins/community/`](../../plugins/community/) · 規格見 [`plugins/spec/SPEC.md`](../../plugins/spec/SPEC.md) · agent 開發指南見 [`plugins/spec/AGENT-DEVELOPMENT.md`](../../plugins/spec/AGENT-DEVELOPMENT.md) |
| 支援一個新的 **編碼 agent CLI** | 一筆 adapter 設定 + stream parser | [`apps/daemon/src/agents.ts`](../../apps/daemon/src/agents.ts) |
| 修錯誤或打磨 UI | 瀏覽 [`good-first-issue`](https://github.com/LJYSkywalker/Z-design/issues?q=is%3Aissue+is%3Aopen+label%3A%22good+first+issue%22) 標籤 | [Issues →](https://github.com/LJYSkywalker/Z-design/issues) |
...
我們正在全球招募 **Z-design Fellows**——Fellows 與核心團隊一同形塑產品、在各自地區正式代表 Z-design，並在當地壯大社群，背後有經費支援（$1,000 / MR）、免費的 LLM credits，以及一條直通的審查通道。詳情請見 → [`MAINTAINERS.md`](../../MAINTAINERS.md) 與 [Discord](https://discord.gg/9ptkbbqRu) 上的公告。
...
| [`heygen-com/hyperframes`](https://github.com/heygen-com/hyperframes) | HTML→MP4 動態圖像框架，在 Z-design 中整合為一等公民的 `hyperframes-html`。 |
