...
3. 에디터에서 `.env`를 열고 `ZD_API_TOKEN=`을 찾아, 방금 생성한 토큰을 붙여 넣습니다.
...
- `ZD_BIN` — `apps/daemon/dist/cli.js`의 절대 경로.
- `ZD_DAEMON_URL` — 실행 중인 daemon URL.
- `ZD_PROJECT_ID` — 활성 프로젝트 id.
- `ZD_PROJECT_DIR` — 활성 프로젝트의 파일 디렉터리.

미디어 생성이 `ZD_BIN: parameter not set`, `apps/daemon/dist/cli.js` 누락, `failed to reach daemon at http://127.0.0.1:0` 같은 메시지로 실패하면, daemon CLI를 다시 빌드하고 관리 중인 런타임을 재시작하세요.
...
`ZD_DAEMON_URL`은 `http://127.0.0.1:7457`처럼 실제 daemon 포트여야 하며, `http://127.0.0.1:0`이어서는 안 됩니다. `:0` 값은 "빈 포트를 골라라"라는 내부 실행 힌트일 뿐이라, 에이전트 세션으로 새어 나가면 안 됩니다.
...
- **미디어 생성이 `ZD_BIN` 누락 또는 daemon URL이 `:0`이라고 할 때** — 위의 미디어 dispatcher 점검을 실행하세요. 옛 CLI 세션을 이어 가지 말고, Open Design 앱에서 프로젝트를 다시 열어 daemon이 새 `ZD_*` 변수를 주입하게 하세요.
- **Codex가 플러그인 컨텍스트를 너무 많이 로드할 때** — `ZD_CODEX_DISABLE_PLUGINS=1 pnpm tools-dev`로 Open Design을 시작하면, daemon이 spawn하는 Codex 프로세스가 `--disable plugins`로 실행됩니다.
