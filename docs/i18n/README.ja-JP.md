<h1 align="center">Z-design: オープンソースの Claude Design 代替ツール</h1>

> 🔥 **Z-design 1.0.0 が登場——オールインワンの Agentic デザインワークスペース。** デザインの全工程が、ひとつのウィンドウに。曖昧なアイデアから、リファレンス探し、素材集め、インタラクティブな編集、コメントのキュー投入、モーションの仕上げ、そしてエディタや Code Agent への受け渡しまで——アプリを離れることなく完結します。複数セッションの並行実行で、それはもはや一人のアシスタントではなく、あなたのために働くローカルなデザインチームです。[1.0.0 をダウンロード](https://github.com/LJYSkywalker/Z-design/releases) · [ディスカッションに参加](https://github.com/LJYSkywalker/Z-design/discussions/4153)
>
<p align="center">
...
> 🍃 **本プロジェクトは [Open Design](https://github.com/nexu-io/open-design) をベースに、[GLM（Zhipu）](https://www.zhipuai.cn/) 向けに最適化・改良したものです。** 上流 Open Design の完全な Agentic デザインワークスペースを維持しつつ、エージェントループ・プロンプト・モデル連携を GLM-5.2 のツール呼び出し向けに調整し、Claude 専用ではなく GLM ファミリーでネイティブに動作するようにしています。

オリジナルのプロダクト、スキル、デザインシステム、プラグインの功績は upstream Open Design チームとコントリビューターに帰属します。本フォーク（[Aqu-210L](https://github.com/Aqu-210L) が保守）が主に行うのは：

- デフォルトのエージェントランタイムを **GLM-5.2** に切替。
- GLM のツール呼び出し挙動に合わせてシステムプロンプト・出力契約・artifact ストリーミングを調整。
- 上流のスキル・デザインシステム・プラグションを維持し、オープンなエコシステムを引き続き利用可能に。

上流プロダクトは [`nexu-io/open-design`](https://github.com/nexu-io/open-design) を参照してください。

---

## デプロイ

Z-Design は、ローカルファーストのデスクトップアプリ、Docker イメージ、ソースからの実行という 3 つの形態で提供されます。以下のコマンドはすべて本リポジトリを使用します。

### 前提条件

| 方式 | 要件 |
|---|---|
| デスクトップアプリ | macOS（Apple Silicon / Intel x64）または Windows（x64）。他の依存は不要。 |
| Docker | Docker Desktop + Docker Compose v2 |
| ソースから | Node.js `~24`、pnpm `10.33.x`（Corepack 経由）。macOS / Linux / WSL2 が主対応、Windows ネイティブはベストエフォート。 |

### 方式 A — デスクトップアプリ（設定不要、推奨）

[GitHub Releases](https://github.com/LJYSkywalker/Z-design/releases) から各プラットフォームの最新ビルドをダウンロード：

- **macOS** — Apple Silicon と Intel x64
- **Windows** — x64
- **Linux** — AppImage（オプション）

インストール後、アプリは `PATH` 上のすべてのコーディングエージェント CLI を自動検出し、100+ のスキルと 150 のデザインシステムを読み込み、エントリビューでブリーフを入力できます。GLM をバックエンドにするには、実行モードを **BYOK / API モード** に切替え、GLM の API Key とエンドポイントを入力します。

### 方式 B — Docker（シングルコンテナ、ヘッドレス）

公開イメージはデーモンを実行し、`7456` ポートで静的 Web ビルドを提供します。
...
docker compose down -v        # 停止してローカルデータを削除
```

`deploy/.env` の主要変数：

```env
OPEN_DESIGN_PORT=7456                       # ホストポート（127.0.0.1 にバインド）
OPEN_DESIGN_ALLOWED_ORIGINS=https://yourdomain.com   # ドメイン背後の CORS オリジン
OPEN_DESIGN_IMAGE=ghcr.io/nexu-io/od:latest # イメージタグ
ZD_API_TOKEN=                                # 必須 — openssl rand -hex 32 で生成
```

> **公開公開：** Compose は localhost にバインドしたまま、認証付きリバースプロキシ、SSH トンネル、VPN 経由で Z-Design を公開ネットワークに公開してください。手前に nginx を置く場合、SSE ルートのバッファリングを無効化（`proxy_buffering off; gzip off;`）しないと、エージェントの長いストリームが切断されます。
...
# 2. ツールチェーン（Node 24 + pnpm 10.33.x、Corepack 経由）
corepack enable
pnpm install

# 3. daemon CLI をビルド（`od` コマンドが依存）
pnpm --filter @z-design/daemon build

# 4. フォアグラウンドで daemon + web を起動
...
```bash
pnpm tools-dev                       # バックグラウンドで daemon + web + desktop 起動
pnpm tools-dev start web             # バックグラウンドで daemon + web 起動
pnpm tools-dev status                # 管理ランタイムの状態を確認
pnpm tools-dev logs                  # daemon/web/desktop のログを表示
pnpm tools-dev stop                  # 管理ランタイムを停止
pnpm typecheck                       # ワークスペースの型チェック
```

> 削除されたレガシーのルートコマンド（`pnpm dev`、`pnpm start`、`pnpm daemon`）は使わないでください。`pnpm tools-dev` が唯一のローカルライフサイクルエントリポイントです。

### GLM をバックエンドに設定

Z-Design 起動後、GLM に向け、すべての生成を GLM-5.2 経由にします：

1. アプリを開き、**設定 → 実行モード** に進みます。
2. **BYOK / API モード**（OpenAI 互換エンドポイント）を選択。
3. 入力：
   - **baseUrl** — `https://open.bigmodel.cn/api/paas/v4`（Zhipu GLM の OpenAI 互換エンドポイント）
   - **apiKey** — [open.bigmodel.cn](https://open.bigmodel.cn/) での GLM API Key
   - **model** — `glm-5.2`（またはアクセス権のある他の GLM ファミリーモデル）
4. 保存し、エントリビューでブリーフを入力。エージェントは左ペインにストリーム出力し、`<artifact>` が右側にリアルタイム描画されます。

> Docker / ヘッドレスデプロイでも、同じ値を `POST /api/proxy/openai/stream` に同じ `baseUrl` / `apiKey` / `model` フィールドで送信します。個別の設定面は不要です。

### 動作確認

```bash
# ソース / 開発モード — daemon ヘルスチェック
curl -s http://127.0.0.1:7456/api/health

# Docker — compose ネットワーク内からチェック
docker compose exec open-design wget -qO- http://127.0.0.1:7456/api/health
```

`/api/health` が `200 OK` を返せばデーモン起動済み、Web ビルドも提供中です。完全なクイックスタート、環境変数、Nix flake、パッケージビルドフローは [`QUICKSTART.ja-JP.md`](QUICKSTART.ja-JP.md) を参照してください。
...
Z-design は、Anthropic が Claude Design とともに送り出した **エージェントネイティブ**なループ——ブリーフを探り、方向性を確定し、アーティファクトをストリーミングし、批評し、納品する——がクローズドであることをやめ、ラップトップにすでにあるコーディングエージェントが読み・書き・リミックスできる **スキル・デザインシステム・プラグインのファイルシステム** になったときに得られるものです。あなたの CLI がデザインエンジンに、ラップトップがスタジオに、そしてチームの `DESIGN.md` がブランドの契約書になります。
...
Z-design が何であり、何ができるのかを手早く見ていきましょう。**Home** から始め、**Automation** で繰り返しのワークフローをオーケストレーションし、**Design System** でブランドの契約書を抽出し、**Plugins** と **連携機能**で拡張します。どのプロジェクトの **Studio** 内でも、同じデザインシステムがプロトタイプ、ライブアーティファクト、HyperFrames、スライド、画像をストリーミング出力します。
...
<sub><b>Integrations</b> — 外部システムや MCP ツールを接続し、任意の IDE・スクリプト・自動化から Z-design を利用できます。</sub>
</td>
</tr>
</table>
...
> Z-design は、主流のコーディングエージェントがネイティブに利用する **スキル、CLI、MCP サーバー**として提供されます。OD をインストールすれば、`od mcp install <agent>` 一発で MCP サーバーがそのエージェントの設定に組み込まれ、どのエージェントの内部からでも同じツールを呼び出せます。
...
Z-design (OD) はそのオープンソースの代替ツールです。同じループ、同じアーティファクトファーストのメンタルモデル、ロックインは一切なし:
...
Z-design を使う最も速い方法。Node も、pnpm も、クローンも不要です。

- **macOS**（Apple Silicon · Intel x64）→ [**open-design.ai**](https://open-design.ai/) または [GitHub Releases](https://github.com/LJYSkywalker/Z-design/releases)
- **Windows**（x64）→ [**open-design.ai**](https://open-design.ai/) または [GitHub Releases](https://github.com/LJYSkywalker/Z-design/releases)
- **Linux**（AppImage、オプションのレーン）→ [GitHub Releases](https://github.com/LJYSkywalker/Z-design/releases)
...
GUI を一度も開くことなく Z-design を使えます——Claude Code、Codex、Cursor、Copilot、OpenClaw、Antigravity、Hermes、Kimi などの内部で、スキル、プラグイン、または MCP サーバーとして呼び出せます。
...
Sealos App Store テンプレートは、公開済みの Z-design Docker イメージを永続的なワークスペースストレージと公開プロキシの Basic Auth 付きで実行します。独自の公開または共有 Docker デプロイでは、[`deploy/README.md`](../../deploy/README.md#local-compose) のリバースプロキシと `OPEN_DESIGN_ALLOWED_ORIGINS` の手順に従ってください。
...
5. **Z-design は使うほど賢くなります。** あなたのスクリーンショット、フォント、パレット、確定したアーティファクトは、次回のセッションのデフォルトとして蓄積されます。やり直しは減り、ブレも減ります。

---

## コーディングエージェントから Z-design を使う

Z-design は **stdio MCP サーバー**とエージェントごとの**インストールスクリプト**を提供します。別のリポジトリにある MCP 互換のエージェントは、あなたのローカルの Z-design プロジェクトからファイル——トークン CSS、JSX コンポーネント、エントリー HTML——を、名前で問い合わせ可能な構造化 API として直接読み取れます。エージェントは常に、古いエクスポートではなくライブのファイルを見ます。
...
**ゼロから始めるエージェントのために、**インストーラーは `~/.config/<agent>/z-design.json`（またはプラットフォーム相当のもの）と、コピー＆ペースト用の MCP スニペットを配置します。Cursor にはワンクリックのディープリンク、Claude Code には `claude mcp add-json` のワンライナー、その他すべてのエージェントには、その設定が期待するスキーマの JSON が提供されます。エージェントごとの完全なフロー → デスクトップアプリの **Settings → MCP server**、または [`docs/agent-adapters.md`](../../docs/agent-adapters.md)。

**セキュリティモデル。** デフォルトで読み取り専用、デーモンは `127.0.0.1` にバインドし、SSRF はプロキシのエッジでブロックされます。LAN への公開には、明示的な `ZD_BIND_HOST` と `ZD_ALLOWED_ORIGINS` が必要です。コネクターの認証情報とライブアーティファクトのプレビュールートは、いずれにせよループバック専用のままです。
...
**261 の公式プラグイン**が [`plugins/_official/`](../../plugins/_official/) にあります。各プラグインは**ポータブルなエージェントスキルのフォルダ**です——`SKILL.md`（Agent Skills をサポートする任意のエージェントが読める）に加え、Z-design にマーケットプレイスのメタデータ、入力、プレビュー、パイプライン、機能宣言を与える任意の `z-design.json` マニフェストを備えています。カテゴリーに直接ジャンプ:
...
プラグインは **最低限 `SKILL.md` が 1 つあれば成り立ちます**。Z-design のマーケットプレイスに掲載するには `z-design.json` を追加します:
...
1. プラグインフォルダを [`plugins/community/`](../../plugins/community/)（サードパーティプラグイン）にドロップするか、または——Z-design に同梱して提供するには——[`plugins/_official/`](../../plugins/_official/) の該当するティアにドロップします。
2. 検証を通過させます: `od plugin validate`、`pnpm guard`、`pnpm --filter @z-design/plugin-runtime typecheck`。
...
- [x] **0.9.0** — Z-design AMR（アプリに組み込まれた公式 Model Router: 設定ゼロ、ワンクリックのサインイン）
...
- 🗣️ **GitHub Discussions** — 深い Q&A、RFC、「成果を見せて」 → [**Discussions**](https://github.com/LJYSkywalker/Z-design/discussions)
- 🐛 **GitHub Issues** — バグ報告、機能リクエスト → [**Issues**](https://github.com/LJYSkywalker/Z-design/issues)

[`good-first-issue`](https://github.com/LJYSkywalker/Z-design/issues?q=is%3Aissue+is%3Aopen+label%3A%22good+first+issue%22) と [`help-wanted`](https://github.com/LJYSkywalker/Z-design/issues?q=is%3Aissue+is%3Aopen+label%3A%22help+wanted%22) のラベルが、最も入りやすい入口です。

---

## 貢献

Z-design が動き続けるのは、貢献者——デザイナー、エンジニア、プロンプトの作者——が現れ続けるからです。最もよく使われるスキル、デザインシステム、プラグインの多くは、コアチームの外の人々によって書かれました。
...
| 新しい**プラグイン** | カテゴリーフォルダの下に `z-design.json` + マニフェストをドロップ | [`plugins/community/`](../../plugins/community/) · 仕様は [`plugins/spec/SPEC.md`](../../plugins/spec/SPEC.md) · エージェント開発ガイドは [`plugins/spec/AGENT-DEVELOPMENT.md`](../../plugins/spec/AGENT-DEVELOPMENT.md) |
| 新しい**コーディングエージェント CLI** をサポート | アダプターエントリー 1 つ + ストリームパーサー | [`apps/daemon/src/agents.ts`](../../apps/daemon/src/agents.ts) |
| バグ修正や UI の磨き上げ | [`good-first-issue`](https://github.com/LJYSkywalker/Z-design/issues?q=is%3Aissue+is%3Aopen+label%3A%22good+first+issue%22) ラベルを閲覧 | [Issues →](https://github.com/LJYSkywalker/Z-design/issues) |
...
### 🏅 Z-design Fellow プログラム

私たちは世界中で **Z-design Fellow** を募集しています——Fellow はコアチームとともにプロダクトを形づくり、各地域で Z-design を公式に代表し、資金面のサポート（$1,000 / MR）、無料の LLM クレジット、直通のレビュートラックに支えられながら、ローカルでコミュニティを育てます。詳細 → [`MAINTAINERS.md`](../../MAINTAINERS.md) と [Discord](https://discord.gg/9ptkbbqRu) のアナウンス。
...
| [`heygen-com/hyperframes`](https://github.com/heygen-com/hyperframes) | HTML→MP4 のモーショングラフィックスフレームワークで、Z-design に第一級の `hyperframes-html` として統合されています。 |
