<h1 align="center">Z-design: a alternativa open-source ao Claude Design</h1>

> 🔥 **O Z-design 1.0.0 chegou: o espaço de trabalho de design Agentic tudo-em-um.** Todo o ofício agora vive em uma única janela — vá de uma ideia vaga à descoberta de referências, coleta de material, edição interativa, fila de comentários, polimento de animações e entrega para um editor ou um Code Agent — sem sair do app. Com sessões em paralelo, deixa de parecer um assistente e passa a parecer um time de design local trabalhando para você. [Baixar 1.0.0](https://github.com/LJYSkywalker/Z-design/releases) · [Participar da discussão](https://github.com/LJYSkywalker/Z-design/discussions/4153)
>
<p align="center">
...
> 🍃 **Este projeto é baseado no [Z-design](https://github.com/nexu-io/open-design) e foi otimizado e melhorado para o [GLM (Zhipu)](https://www.zhipuai.cn/).** Ele mantém o workspace de design agentic completo do Z-design upstream, ao mesmo tempo em que adapta o loop do agente, os prompts e a integração de modelo para as chamadas de ferramenta do GLM-5.2, de modo que todo o fluxo rode nativamente na família GLM em vez de apenas no Claude.

O crédito pelo produto original, pelas skills, pelos sistemas de design e pelos plugins pertence à equipe upstream do Open Design e aos seus contribuidores. Este fork (mantido por [Aqu-210L](https://github.com/Aqu-210L)) foca em:

- Redirecionar o runtime padrão do agente para o **GLM-5.2**.
- Ajustar o prompt do sistema, o contrato de saída e o streaming de artifacts ao comportamento de chamadas de ferramenta do GLM.
- Manter intactas as skills, os sistemas de design e os plugins do upstream para que o ecossistema aberto continue funcionando.

Para o produto upstream, veja [`nexu-io/open-design`](https://github.com/nexu-io/open-design).

---

## Implantação

O Z-Design é distribuído como um app de desktop local-first, uma imagem Docker e um projeto para rodar a partir do código-fonte. Escolha o caminho que melhor se adapta a você. Todos os comandos abaixo usam este repositório.

### Pré-requisitos

| Caminho | Requisitos |
|---|---|
| App de desktop | macOS (Apple Silicon / Intel x64) ou Windows (x64). Nada mais a instalar. |
| Docker | Docker Desktop + Docker Compose v2 |
| A partir do código-fonte | Node.js `~24`, pnpm `10.33.x` (via Corepack). macOS / Linux / WSL2 são os caminhos principais; Windows nativo é best-effort. |

### Opção A — App de desktop (zero config, recomendado)

Baixe o build mais recente para sua plataforma no [GitHub Releases](https://github.com/LJYSkywalker/Z-design/releases):

- **macOS** — Apple Silicon e Intel x64
- **Windows** — x64
- **Linux** — AppImage (carril opcional)

Após a instalação, o app detecta automaticamente toda CLI de agente de codificação no seu `PATH`, carrega 100+ skills e 150 sistemas de design e permite que você digite um brief na view de entrada. Para usar o GLM como backend, alterne o modo de execução para **modo BYOK / API** e cole sua GLM API Key e endpoint.

### Opção B — Docker (um único contêiner, headless)

A imagem publicada executa o daemon e atende o build estático web na porta `7456`.
...
# 2. Preparar variáveis de ambiente (gera um token de API seguro)
cd deploy
cp .env.example .env
...
docker compose pull && docker compose up -d   # atualizar para a imagem mais recente
docker compose down           # parar
docker compose down -v        # parar e apagar dados locais
```

Principais variáveis do `deploy/.env`:

```env
OPEN_DESIGN_PORT=7456                       # porta do host (vinculada a 127.0.0.1)
OPEN_DESIGN_ALLOWED_ORIGINS=https://yourdomain.com   # origens CORS atrás de um domínio
OPEN_DESIGN_IMAGE=ghcr.io/nexu-io/od:latest # tag da imagem
ZD_API_TOKEN=                                # OBRIGATÓRIO — gere com: openssl rand -hex 32
```

> **Exposição pública:** mantenha o Compose vinculado a localhost e coloque um proxy reverso autenticado, túnel SSH ou VPN na frente antes de expor o Z-Design publicamente. Se colocar nginx na frente, mantenha as rotas SSE sem buffer (`proxy_buffering off; gzip off;`) ou os streams longos do agente serão truncados.

### Opção C — Rodar a partir do código-fonte (desenvolvimento)
...
# 3. Buildar a CLI do daemon (requerida pelo comando `od`)
pnpm --filter @z-design/daemon build

# 4. Iniciar daemon + web em primeiro plano
...
```bash
pnpm tools-dev                       # daemon + web + desktop em segundo plano
pnpm tools-dev start web             # daemon + web em segundo plano
pnpm tools-dev status                # inspecionar runtimes gerenciados
pnpm tools-dev logs                  # mostrar logs de daemon/web/desktop
pnpm tools-dev stop                  # parar runtimes gerenciados
pnpm typecheck                       # typecheck do workspace
```

> Não use os alias raiz legados removidos (`pnpm dev`, `pnpm start`, `pnpm daemon`). `pnpm tools-dev` é o único ponto de entrada do ciclo de vida local.

### Configurar o GLM como backend

Depois que o Z-Design estiver rodando, aponte-o para o GLM para que toda geração passe pelo GLM-5.2:

1. Abra o app e vá para **Configurações → Modo de execução**.
2. Escolha **modo BYOK / API** (endpoint compatível com OpenAI).
3. Preencha:
   - **baseUrl** — `https://open.bigmodel.cn/api/paas/v4` (endpoint compatível OpenAI do Zhipu GLM)
   - **apiKey** — sua GLM API Key de [open.bigmodel.cn](https://open.bigmodel.cn/)
   - **model** — `glm-5.2` (ou outro modelo da família GLM ao qual você tenha acesso)
4. Salve e digite um brief na view de entrada. O agente faz stream no painel esquerdo; o `<artifact>` renderiza ao vivo na direita.

> Em implantações Docker / headless, os mesmos valores são enviados para `POST /api/proxy/openai/stream` com os mesmos campos `baseUrl` / `apiKey` / `model`; não é necessária uma superfície de configuração separada.

### Verificar se está funcionando

```bash
# Código-fonte / modo dev — health do daemon
curl -s http://127.0.0.1:7456/api/health

# Docker — health de dentro da rede do compose
docker compose exec open-design wget -qO- http://127.0.0.1:7456/api/health
```

Um `200 OK` de `/api/health` indica que o daemon subiu e o build web está sendo servido. Para o quickstart completo, variáveis de ambiente, Nix flake e o fluxo de build empacotado, veja [`QUICKSTART.pt-BR.md`](QUICKSTART.pt-BR.md).
...
<sub><b>Integrations</b> — conecte sistemas externos e ferramentas MCP, e use o Z-design a partir de qualquer IDE, script ou automação.</sub>
</td>
</tr>
</table>
...
> O Z-design é entregue como **skills, uma CLI e um servidor MCP** que os coding agents populares consomem nativamente. Depois que o OD está instalado, um único `od mcp install <agent>` conecta o servidor MCP à configuração daquele agente, e você chama as mesmas ferramentas de dentro de qualquer agente.
...
O **[HyperFrames][hyperframes]** é o framework de vídeo open-source e agent-native da HeyGen, integrado como cidadão de primeira classe no Z-design. O agente escreve HTML + CSS + GSAP, e o HyperFrames renderiza tudo para um MP4 determinístico via Chrome headless + FFmpeg. Combine-o com o **Seedance 2.0** para t2v / i2v cinematográfico, **Veo 3 / Sora 2 / Kling 2** para variantes de modelo roteadas e **Suno v5 / Lyria 2** para a camada de áudio.
...
O Z-design (OD) é a alternativa open-source. Mesmo loop, mesmo modelo mental artifact-first, nada do lock-in:
...
A forma mais rápida de usar o Z-design. Sem Node, sem pnpm, sem clone.
...
- **Linux** (AppImage, trilha opcional) → [GitHub Releases](https://github.com/LJYSkywalker/Z-design/releases)
...
Você pode usar o Z-design sem nunca abrir a GUI — chame-o como skill, plugin ou servidor MCP dentro do Claude Code, Codex, Cursor, Copilot, OpenClaw, Antigravity, Hermes, Kimi e mais.
...
O template da Sealos App Store executa a imagem Docker publicada do Z-design com armazenamento persistente para o workspace e Basic Auth no proxy público. Para implantações Docker públicas ou compartilhadas personalizadas, siga as orientações de proxy reverso e `OPEN_DESIGN_ALLOWED_ORIGINS` em [`deploy/README.md`](../../deploy/README.md#local-compose).
...
5. **O Z-design fica mais inteligente conforme você usa.** Seus screenshots, fontes, paletas e artefatos confirmados se acumulam como padrões para a próxima sessão. Menos retrabalho, menos desvio.

---

## Use o Z-design a partir do seu coding agent

O Z-design entrega um **servidor MCP stdio** e **scripts de instalação** por agente. Qualquer agente compatível com MCP em outro repositório pode ler arquivos diretamente dos seus projetos locais do Z-design — tokens CSS, componentes JSX, HTML de entrada — como uma API estruturada consultável por nome. O agente sempre vê o arquivo ao vivo, não uma exportação desatualizada.
...
**261 plugins oficiais** ficam em [`plugins/_official/`](../../plugins/_official/). Cada plugin é uma **pasta portátil de agent-skill** — um `SKILL.md` (legível por qualquer agente que suporte Agent Skills), além de um manifesto opcional `z-design.json` que dá ao Z-design metadados de marketplace, inputs, prévias, pipelines e declarações de capacidades. Vá direto para uma categoria:
...
Um plugin **precisa apenas de um `SKILL.md` no mínimo**; para listá-lo no marketplace do Z-design, adicione um `z-design.json`:
...
1. Solte a pasta do plugin em [`plugins/community/`](../../plugins/community/) (plugins de terceiros), ou — para entregá-lo empacotado com o Z-design — no tier correspondente de [`plugins/_official/`](../../plugins/_official/).
2. Passe na validação: `od plugin validate`, `pnpm guard`, `pnpm --filter @z-design/plugin-runtime typecheck`.
...
- [x] **0.9.0** — Z-design AMR (Model Router oficial embutido no app: zero configuração, login de um clique)
...
- 🗣️ **GitHub Discussions** — Q&A aprofundado, RFCs, "mostre seu trabalho" → [**Discussions**](https://github.com/LJYSkywalker/Z-design/discussions)
- 🐛 **GitHub Issues** — relatos de bugs, pedidos de features → [**Issues**](https://github.com/LJYSkywalker/Z-design/issues)

As labels [`good-first-issue`](https://github.com/LJYSkywalker/Z-design/issues?q=is%3Aissue+is%3Aopen+label%3A%22good+first+issue%22) e [`help-wanted`](https://github.com/LJYSkywalker/Z-design/issues?q=is%3Aissue+is%3Aopen+label%3A%22help+wanted%22) são a forma mais fácil de começar.

---

## Contribuindo

O Z-design continua avançando porque os contribuidores — designers, engenheiros, autores de prompts — continuam aparecendo. Muitas das skills, design systems e plugins mais usados foram escritos por pessoas fora do time central.
...
| Um novo **plugin** | Solte `z-design.json` + manifesto numa pasta de categoria | [`plugins/community/`](../../plugins/community/) · spec em [`plugins/spec/SPEC.md`](../../plugins/spec/SPEC.md) · guia de dev por agente em [`plugins/spec/AGENT-DEVELOPMENT.md`](../../plugins/spec/AGENT-DEVELOPMENT.md) |
...
| Corrigir um bug ou polir a UI | Explore a label [`good-first-issue`](https://github.com/LJYSkywalker/Z-design/issues?q=is%3Aissue+is%3Aopen+label%3A%22good+first+issue%22) | [Issues →](https://github.com/LJYSkywalker/Z-design/issues) |
...
### 🏅 Programa Z-design Fellow

Estamos recrutando **Z-design Fellows** ao redor do mundo — os Fellows moldam o produto ao lado do time central, representam oficialmente o Z-design em sua região e fazem a comunidade crescer localmente, com suporte financiado ($1,000 / MR), créditos gratuitos de LLM e uma trilha de revisão direta. Detalhes → [`MAINTAINERS.md`](../../MAINTAINERS.md) e o anúncio no [Discord](https://discord.gg/9ptkbbqRu).
...
| [`heygen-com/hyperframes`](https://github.com/heygen-com/hyperframes) | O framework de motion graphics HTML→MP4, integrado como o `hyperframes-html` de primeira classe no Z-design. |
