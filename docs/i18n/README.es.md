...
> 🍃 **Este proyecto se basa en [Z-design](https://github.com/nexu-io/open-design) y está optimizado y mejorado para [GLM (Zhipu)](https://www.zhipuai.cn/).** Conserva el workspace de diseño agentic completo de Z-design upstream, mientras adapta el bucle del agente, los prompts y la integración del modelo a las llamadas a herramientas de GLM-5.2, de modo que todo el flujo se ejecuta de forma nativa en la familia GLM en lugar de solo en Claude.

El crédito del producto original, las skills, los sistemas de diseño y los plugins corresponde al equipo upstream de Open Design y a sus contribuidores. Este fork (mantenido por [Aqu-210L](https://github.com/Aqu-210L)) se centra en:

- Reorientar el runtime del agente por defecto hacia **GLM-5.2**.
- Ajustar el prompt del sistema, el contrato de salida y el streaming de artifacts al comportamiento de llamadas a herramientas de GLM.
- Mantener intactas las skills, los sistemas de diseño y los plugins del upstream para que el ecosistema abierto siga funcionando.

Para el producto upstream, consulta [`nexu-io/open-design`](https://github.com/nexu-io/open-design).

---

## Despliegue

Z-Design se distribuye como app de escritorio local-first, imagen Docker y proyecto para ejecutar desde el código fuente. Elige la vía que te convenga. Todos los comandos siguientes usan este repositorio.

### Requisitos previos

| Vía | Requisitos |
|---|---|
| App de escritorio | macOS (Apple Silicon / Intel x64) o Windows (x64). Nada más que instalar. |
| Docker | Docker Desktop + Docker Compose v2 |
| Desde el código fuente | Node.js `~24`, pnpm `10.33.x` (vía Corepack). macOS / Linux / WSL2 primarios; Windows nativo best-effort. |
...
Tras la instalación, la app detecta automáticamente todas las CLIs de agentes de codificación en tu `PATH`, carga 100+ skills y 150 sistemas de diseño, y te permite escribir un brief en la vista de entrada. Para usar GLM como backend, cambia el modo de ejecución a **modo BYOK / API** y pega tu GLM API Key y endpoint.

### Opción B — Docker (un solo contenedor, headless)

La imagen publicada ejecuta el daemon y sirve el build estático web en el puerto `7456`.
...
Variables clave de `deploy/.env`:

```env
OPEN_DESIGN_PORT=7456                       # puerto del host (ligado a 127.0.0.1)
OPEN_DESIGN_ALLOWED_ORIGINS=https://yourdomain.com   # orígenes CORS tras un dominio
OPEN_DESIGN_IMAGE=ghcr.io/nexu-io/od:latest # tag de imagen
ZD_API_TOKEN=                                # OBLIGATORIO — genera con: openssl rand -hex 32
```

> **Exposición pública:** mantén Compose ligado a localhost y pon un proxy inverso autenticado, túnel SSH o VPN delante antes de exponer Z-Design a una red pública. Si pones nginx por delante, mantén las rutas SSE sin búfer (`proxy_buffering off; gzip off;`) o los streams largos del agente se truncarán.

### Opción C — Ejecutar desde el código fuente (desarrollo)
...
# abre la URL web que imprime tools-dev
```

Otros comandos del ciclo de vida:

```bash
pnpm tools-dev                       # daemon + web + desktop en segundo plano
pnpm tools-dev start web             # daemon + web en segundo plano
pnpm tools-dev status                # inspeccionar runtimes gestionados
pnpm tools-dev logs                  # mostrar logs de daemon/web/desktop
pnpm tools-dev stop                  # detener runtimes gestionados
pnpm typecheck                       # typecheck del workspace
```

> No uses los alias raíz heredados eliminados (`pnpm dev`, `pnpm start`, `pnpm daemon`). `pnpm tools-dev` es el único punto de entrada del ciclo de vida local.

### Configurar GLM como backend

Una vez que Z-Design esté corriendo, apúntalo a GLM para que cada generación pase por GLM-5.2:

1. Abre la app y ve a **Ajustes → Modo de ejecución**.
2. Elige **modo BYOK / API** (endpoint compatible con OpenAI).
3. Rellena:
   - **baseUrl** — `https://open.bigmodel.cn/api/paas/v4` (endpoint compatible OpenAI de Zhipu GLM)
   - **apiKey** — tu GLM API Key de [open.bigmodel.cn](https://open.bigmodel.cn/)
   - **model** — `glm-5.2` (u otro modelo de la familia GLM al que tengas acceso)
4. Guarda y escribe un brief en la vista de entrada. El agente hace stream en el panel izquierdo; el `<artifact>` se renderiza en vivo a la derecha.

> En despliegues Docker / headless los mismos valores se envían a `POST /api/proxy/openai/stream` con los mismos campos `baseUrl` / `apiKey` / `model`; no hace falta una superficie de configuración aparte.

### Verificar que funciona

```bash
# Código fuente / modo dev — salud del daemon
curl -s http://127.0.0.1:7456/api/health

# Docker — salud desde dentro de la red compose
docker compose exec open-design wget -qO- http://127.0.0.1:7456/api/health
```

Un `200 OK` de `/api/health` indica que el daemon está activo y se sirve el build web. Para el quickstart completo, variables de entorno, Nix flake y el flujo de build empaquetado, consulta [`QUICKSTART.es.md`](QUICKSTART.es.md).
...
<sub><b>Integrations</b> — conecta sistemas externos y herramientas MCP, y usa Z-design desde cualquier IDE, script o automatización.</sub>
</td>
</tr>
</table>
...
<p align="center">
  <img src="https://repo-assets.open-design.ai/resources/images/coding-agents.png" alt="Las 21 CLIs de agentes de codificación que admite Z-design — Claude Code · Codex · OpenCode · Hermes · Antigravity · Gemini · Grok Build · Kimi · Cursor Agent · Qwen · Qoder · GitHub Copilot · Pi · Kiro · Kilo · Mistral Vibe · DeepSeek · Reasonix · Aider · Devin · Trae" width="100%" />
</p>
...
**[HyperFrames][hyperframes]** es el framework de video de código abierto y agent-native de HeyGen, integrado como ciudadano de primera clase en Z-design. El agente escribe HTML + CSS + GSAP, y HyperFrames lo renderiza a un MP4 determinista mediante Chrome headless + FFmpeg. Combínalo con **Seedance 2.0** para t2v / i2v cinematográfico, **Veo 3 / Sora 2 / Kling 2** para variantes de modelo enrutadas, y **Suno v5 / Lyria 2** para la capa de audio.
...
Z-design (OD) es la alternativa de código abierto. El mismo bucle, el mismo modelo mental centrado en el artefacto, sin nada del lock-in:
...
Puedes usar Z-design sin abrir nunca la GUI — llámalo como skill, plugin o servidor MCP dentro de Claude Code, Codex, Cursor, Copilot, OpenClaw, Antigravity, Hermes, Kimi y más.
...
La plantilla de Sealos App Store ejecuta la imagen Docker publicada de Z-design con almacenamiento persistente para el espacio de trabajo y Basic Auth en el proxy público. Para despliegues Docker públicos o compartidos personalizados, sigue la guía de proxy inverso y `OPEN_DESIGN_ALLOWED_ORIGINS` en [`deploy/README.md`](../../deploy/README.md#local-compose).
...
5. **Z-design se vuelve más inteligente conforme lo usas.** Tus capturas, fuentes, paletas y artefactos confirmados se acumulan como valores por defecto para la próxima sesión. Menos retrabajo, menos deriva.

---

## Usa Z-design desde tu agente de codificación

Z-design distribuye un **servidor MCP por stdio** y **scripts de instalación** por agente. Cualquier agente compatible con MCP en otro repo puede leer archivos de tus proyectos locales de Z-design directamente — tokens CSS, componentes JSX, HTML de entrada — como una API estructurada consultable por nombre. El agente siempre ve el archivo en vivo, no una exportación obsoleta.
...
**Para un agente que empieza desde cero,** el instalador coloca `~/.config/<agent>/z-design.json` (o el equivalente de la plataforma) más un fragmento MCP para copiar y pegar. Cursor obtiene un deeplink de un clic; Claude Code obtiene un comando `claude mcp add-json` de una línea; cada otro agente obtiene JSON en el esquema que su configuración espera. Flujo completo por agente → **Settings → MCP server** en la app de escritorio, o [`docs/agent-adapters.md`](../../docs/agent-adapters.md).

**Modelo de seguridad.** De solo lectura por defecto, el daemon se enlaza a `127.0.0.1` y el SSRF se bloquea en el borde del proxy. La exposición a la LAN requiere un `ZD_BIND_HOST` explícito más `ZD_ALLOWED_ORIGINS`. Las credenciales de conectores y las rutas de vista previa de artefactos en vivo permanecen solo en loopback independientemente de ello.
...
Un plugin **necesita como mínimo solo un `SKILL.md`**; para listarlo en el marketplace de Z-design, añade un `z-design.json`:
...
- [x] **0.9.0** — Z-design AMR (Model Router oficial integrado en la app: cero configuración, inicio de sesión de un clic)
...
Z-design sigue avanzando porque los colaboradores — diseñadores, ingenieros, autores de prompts — siguen apareciendo. Muchos de los skills, sistemas de diseño y plugins más usados fueron escritos por personas ajenas al equipo principal.
...
<picture>
  <img alt="Z-design — métricas del repositorio" src="https://repo-assets.open-design.ai/resources/images/github-metrics.svg" />
</picture>
...
| [`heygen-com/hyperframes`](https://github.com/heygen-com/hyperframes) | El framework de gráficos en movimiento HTML→MP4, integrado como el `hyperframes-html` de primera clase en Z-design. |
