<h1 align="center">Z-design : l'alternative open source à Claude Design</h1>

> 🔥 **Z-design 1.0.0 est là : l'espace de travail design Agentic tout-en-un.** Tout le métier tient désormais dans une seule fenêtre — partez d'une idée vague, découvrez des références, rassemblez la matière, éditez de manière interactive, mettez vos commentaires en file d'attente, peaufinez les animations, puis transmettez le résultat à un éditeur ou à un Code Agent — sans quitter l'application. Avec les sessions parallèles, ce n'est plus un assistant, mais une équipe de design locale qui travaille pour vous. [Télécharger 1.0.0](https://github.com/LJYSkywalker/Z-design/releases) · [Rejoindre la discussion](https://github.com/LJYSkywalker/Z-design/discussions/4153)
>
<p align="center">
  <img src="https://repo-assets.open-design.ai/resources/images/hero.png" alt="Z-design — l'alternative open source à Claude Design · 150 systèmes de design · 261 plugins · 21 agents de code · 14 fournisseurs de médias" width="100%" />
</p>
...
> 🍃 **Ce projet est basé sur [Z-design](https://github.com/nexu-io/open-design) et optimisé et amélioré pour [GLM (Zhipu)](https://www.zhipuai.cn/).** Il conserve le workspace de design agentique complet d'Z-design en amont tout en adaptant la boucle de l'agent, les prompts et l'intégration du modèle aux appels d'outils de GLM-5.2, afin que tout le flux s'exécute nativement sur la famille GLM plutôt que sur Claude uniquement.

Le mérite du produit original, des skills, des systèmes de design et des plugins revient à l'équipe upstream Open Design et à ses contributeurs. Ce fork (maintenu par [Aqu-210L](https://github.com/Aqu-210L)) se concentre sur :

- Réorienter le runtime d'agent par défaut vers **GLM-5.2**.
- Ajuster le prompt système, le contrat de sortie et le streaming d'artifacts au comportement d'appels d'outils de GLM.
- Conserver intacts les skills, systèmes de design et plugins amont pour que l'écosystème ouvert continue de fonctionner.

Pour le produit amont, voir [`nexu-io/open-design`](https://github.com/nexu-io/open-design).

---

## Déploiement

Z-Design est distribué comme une application desktop locale d'abord, comme image Docker et comme projet à exécuter depuis les sources. Choisissez la voie qui vous convient. Toutes les commandes ci-dessous utilisent ce dépôt.

### Prérequis

| Voie | Prérequis |
|---|---|
| Application desktop | macOS (Apple Silicon / Intel x64) ou Windows (x64). Rien d'autre à installer. |
| Docker | Docker Desktop + Docker Compose v2 |
| Depuis les sources | Node.js `~24`, pnpm `10.33.x` (via Corepack). macOS / Linux / WSL2 pris en charge en premier ; Windows natif au mieux. |

### Option A — Application desktop (zéro configuration, recommandée)

Téléchargez la build la plus récente pour votre plateforme depuis [GitHub Releases](https://github.com/LJYSkywalker/Z-design/releases) :

- **macOS** — Apple Silicon et Intel x64
- **Windows** — x64
- **Linux** — AppImage (voie optionnelle)

Après l'installation, l'application détecte automatiquement chaque CLI d'agent de codage sur votre `PATH`, charge 100+ skills et 150 systèmes de design, et vous permet de saisir un brief dans la vue d'entrée. Pour utiliser GLM comme backend, basculez le mode d'exécution en **mode BYOK / API** et collez votre clé API GLM et votre endpoint.

### Option B — Docker (conteneur unique, headless)

L'image publiée exécute le daemon et sert le build web statique sur le port `7456`.
...
# 2. Préparer les variables d'environnement (génère un token API sécurisé)
cd deploy
cp .env.example .env
...
docker compose down -v        # arrêter et effacer les données locales
```

Variables clés de `deploy/.env` :

```env
OPEN_DESIGN_PORT=7456                       # port hôte (lié à 127.0.0.1)
OPEN_DESIGN_ALLOWED_ORIGINS=https://yourdomain.com   # origines CORS derrière un domaine
OPEN_DESIGN_IMAGE=ghcr.io/nexu-io/od:latest # tag d'image
ZD_API_TOKEN=                                # OBLIGATOIRE — générer avec : openssl rand -hex 32
```

> **Exposition publique :** gardez Compose lié à localhost et placez un reverse proxy authentifié, un tunnel SSH ou un VPN devant avant d'exposer Z-Design publiquement. Si vous placez nginx devant, gardez les routes SSE sans buffer (`proxy_buffering off; gzip off;`) sinon les longs flux de l'agent seront tronqués.

### Option C — Exécuter depuis les sources (développement)
...
# 3. Builder la CLI du daemon (requise par la commande `od`)
pnpm --filter @z-design/daemon build

# 4. Démarrer daemon + web au premier plan
pnpm tools-dev run web
# ouvrez l'URL web affichée par tools-dev
```

Autres commandes de cycle de vie :

```bash
pnpm tools-dev                       # daemon + web + desktop en arrière-plan
pnpm tools-dev start web             # daemon + web en arrière-plan
pnpm tools-dev status                # inspecter les runtimes gérés
pnpm tools-dev logs                  # afficher les logs daemon/web/desktop
pnpm tools-dev stop                  # arrêter les runtimes gérés
pnpm typecheck                       # typecheck du workspace
```

> N'utilisez pas les alias racine historiques supprimés (`pnpm dev`, `pnpm start`, `pnpm daemon`). `pnpm tools-dev` est le seul point d'entrée du cycle de vie local.

### Configurer GLM comme backend

Une fois Z-Design démarré, pointez-le vers GLM pour que chaque génération passe par GLM-5.2 :

1. Ouvrez l'application et allez dans **Réglages → Mode d'exécution**.
2. Choisissez **mode BYOK / API** (endpoint compatible OpenAI).
3. Renseignez :
   - **baseUrl** — `https://open.bigmodel.cn/api/paas/v4` (endpoint compatible OpenAI de Zhipu GLM)
   - **apiKey** — votre clé API GLM depuis [open.bigmodel.cn](https://open.bigmodel.cn/)
   - **model** — `glm-5.2` (ou un autre modèle de la famille GLM auquel vous avez accès)
4. Enregistrez puis saisissez un brief dans la vue d'entrée. L'agent stream dans le panneau de gauche ; l'`<artifact>` se rend en direct à droite.

> Dans les déploiements Docker / headless, les mêmes valeurs sont envoyées à `POST /api/proxy/openai/stream` avec les mêmes champs `baseUrl` / `apiKey` / `model` ; aucune surface de configuration séparée n'est nécessaire.

### Vérifier que cela fonctionne

```bash
# Sources / mode dev — health du daemon
curl -s http://127.0.0.1:7456/api/health

# Docker — health depuis l'intérieur du réseau compose
docker compose exec open-design wget -qO- http://127.0.0.1:7456/api/health
```

Un `200 OK` de `/api/health` signifie que le daemon est démarré et que le build web est servi. Pour le quickstart complet, les variables d'environnement, le flake Nix et le flux de build empaqueté, voir [`QUICKSTART.fr.md`](QUICKSTART.fr.md).

---

## Qu'est-ce qu'Z-design
...
<sub><b>Intégrations</b> — connectez des systèmes externes et des outils MCP, et utilisez Z-design depuis n'importe quel IDE, script ou automatisation.</sub>
</td>
</tr>
</table>
...
<p align="center">
  <img src="https://repo-assets.open-design.ai/resources/images/coding-agents.png" alt="Les 21 CLI d'agents de code pris en charge par Z-design — Claude Code · Codex · OpenCode · Hermes · Antigravity · Gemini · Grok Build · Kimi · Cursor Agent · Qwen · Qoder · GitHub Copilot · Pi · Kiro · Kilo · Mistral Vibe · DeepSeek · Reasonix · Aider · Devin · Trae" width="100%" />
</p>
...
Z-design (OD) est l'alternative open source. La même boucle, le même modèle mental orienté artefact, sans aucun verrouillage :
...
Le moyen le plus rapide d'utiliser Z-design. Pas de Node, pas de pnpm, pas de clone.

- **macOS** (Apple Silicon · Intel x64) → [**open-design.ai**](https://open-design.ai/) ou [GitHub Releases](https://github.com/LJYSkywalker/Z-design/releases)
- **Windows** (x64) → [**open-design.ai**](https://open-design.ai/) ou [GitHub Releases](https://github.com/LJYSkywalker/Z-design/releases)
- **Linux** (AppImage, voie optionnelle) → [GitHub Releases](https://github.com/LJYSkywalker/Z-design/releases)
...
Vous pouvez utiliser Z-design sans jamais ouvrir l'interface graphique — appelez-le en tant que skill, plugin ou serveur MCP à l'intérieur de Claude Code, Codex, Cursor, Copilot, OpenClaw, Antigravity, Hermes, Kimi, et bien d'autres.
...
5. **Z-design devient plus intelligent à mesure que vous l'utilisez.** Vos captures d'écran, polices, palettes et artefacts confirmés s'accumulent comme valeurs par défaut pour la session suivante. Moins de retouches, moins de dérive.

---

## Utilisez Z-design depuis votre agent de code

Z-design livre un **serveur MCP en stdio** et des **scripts d'installation** par agent. Tout agent compatible MCP dans un autre dépôt peut lire directement des fichiers de vos projets Z-design locaux — tokens CSS, composants JSX, HTML d'entrée — comme une API structurée interrogeable par nom. L'agent voit toujours le fichier en direct, jamais un export périmé.
...
**Pour un agent partant de zéro,** l'installeur place `~/.config/<agent>/z-design.json` (ou l'équivalent de la plateforme) plus un extrait MCP à copier-coller. Cursor obtient un deeplink en un clic ; Claude Code obtient une ligne `claude mcp add-json` ; chaque autre agent obtient du JSON dans le schéma attendu par sa configuration. Flux complet par agent → **Settings → MCP server** dans l'application de bureau, ou [`docs/agent-adapters.md`](../../docs/agent-adapters.md).

**Modèle de sécurité.** En lecture seule par défaut, le daemon se lie à `127.0.0.1`, et la SSRF est bloquée à la périphérie du proxy. L'exposition au réseau local nécessite un `ZD_BIND_HOST` explicite plus `ZD_ALLOWED_ORIGINS`. Les identifiants de connecteurs et les routes d'aperçu d'artefacts en direct restent en loopback uniquement, quoi qu'il en soit.
...
**261 plugins officiels** se trouvent dans [`plugins/_official/`](../../plugins/_official/). Chaque plugin est un **dossier d'agent-skill portable** — un `SKILL.md` (lisible par tout agent prenant en charge les Agent Skills), plus un manifeste `z-design.json` optionnel qui donne à Z-design des métadonnées de marketplace, des entrées, des aperçus, des pipelines et des déclarations de capacités. Accédez directement à une catégorie :
...
1. Déposez le dossier du plugin dans [`plugins/community/`](../../plugins/community/) (plugins tiers), ou — pour le livrer empaqueté avec Z-design — dans le tier correspondant de [`plugins/_official/`](../../plugins/_official/).
2. Passez la validation : `od plugin validate`, `pnpm guard`, `pnpm --filter @z-design/plugin-runtime typecheck`.
...
- [x] **0.9.0** — Z-design AMR (Model Router officiel intégré à l'application : zéro configuration, connexion en un clic)
...
- 🗣️ **GitHub Discussions** — Q&R approfondies, RFC, « montrez votre travail » → [**Discussions**](https://github.com/LJYSkywalker/Z-design/discussions)
- 🐛 **GitHub Issues** — rapports de bugs, demandes de fonctionnalités → [**Issues**](https://github.com/LJYSkywalker/Z-design/issues)

Les labels [`good-first-issue`](https://github.com/LJYSkywalker/Z-design/issues?q=is%3Aissue+is%3Aopen+label%3A%22good+first+issue%22) et [`help-wanted`](https://github.com/LJYSkywalker/Z-design/issues?q=is%3Aissue+is%3Aopen+label%3A%22help+wanted%22) sont la façon la plus simple de se lancer.

---

## Contribuer

Z-design continue d'avancer parce que des contributeurs — designers, ingénieurs, auteurs de prompts — continuent de se présenter. Beaucoup des skills, systèmes de design et plugins les plus utilisés ont été écrits par des personnes extérieures à l'équipe principale.
...
| Un nouveau **plugin** | Déposez `z-design.json` + manifeste sous un dossier de catégorie | [`plugins/community/`](../../plugins/community/) · spec dans [`plugins/spec/SPEC.md`](../../plugins/spec/SPEC.md) · guide de dev agent dans [`plugins/spec/AGENT-DEVELOPMENT.md`](../../plugins/spec/AGENT-DEVELOPMENT.md) |
...
| Corriger un bug ou peaufiner l'UI | Parcourez le label [`good-first-issue`](https://github.com/LJYSkywalker/Z-design/issues?q=is%3Aissue+is%3Aopen+label%3A%22good+first+issue%22) | [Issues →](https://github.com/LJYSkywalker/Z-design/issues) |
...
### 🏅 Programme Z-design Fellow

Nous recrutons des **Z-design Fellows** dans le monde entier — les Fellows façonnent le produit aux côtés de l'équipe principale, représentent officiellement Z-design dans leur région et font grandir la communauté localement, avec le soutien d'un financement ($1,000 / MR), de crédits LLM gratuits et d'une voie de revue directe. Détails → [`MAINTAINERS.md`](../../MAINTAINERS.md) et l'annonce sur [Discord](https://discord.gg/9ptkbbqRu).
...
<a href="https://github.com/LJYSkywalker/Z-design/graphs/contributors">
  <img src="https://contrib.rocks/image?repo=nexu-io/open-design&max=500&columns=20&anon=1&cache_bust=2026-05-30" alt="Contributeurs d'Z-design" />
</a>
...
<picture>
  <img alt="Z-design — métriques du dépôt" src="https://repo-assets.open-design.ai/resources/images/github-metrics.svg" />
</picture>
...
<p align="center">
  <a href="https://github.com/LJYSkywalker/Z-design"><img src="https://repo-assets.open-design.ai/resources/images/star-us.png" alt="Mettez une étoile à Z-design sur GitHub — github.com/LJYSkywalker/Z-design" width="100%" /></a>
</p>
...
<a href="https://star-history.com/#nexu-io/open-design&Date">
  <picture>
    ...
    <img alt="Historique des étoiles d'Z-design" src="https://api.star-history.com/svg?repos=nexu-io/open-design&type=Date&cache_bust=2026-05-28" />
  </picture>
</a>
...
| [`heygen-com/hyperframes`](https://github.com/heygen-com/hyperframes) | Le framework de motion graphics HTML→MP4, intégré comme `hyperframes-html` de première classe dans Z-design. |
