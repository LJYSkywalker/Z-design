...
3. Abra o `.env` no seu editor, encontre `ZD_API_TOKEN=` e cole o token gerado.
...
- `ZD_BIN` — caminho absoluto para `apps/daemon/dist/cli.js`.
- `ZD_DAEMON_URL` — URL do daemon em execução.
- `ZD_PROJECT_ID` — id do projeto ativo.
- `ZD_PROJECT_DIR` — diretório de arquivos do projeto ativo.

Se a geração de mídia falhar com `ZD_BIN: parameter not set`, com `apps/daemon/dist/cli.js` ausente ou com `failed to reach daemon at http://127.0.0.1:0`, recompile o CLI do daemon e reinicie o runtime gerenciado:
...
`ZD_DAEMON_URL` precisa ser uma porta de daemon real, como `http://127.0.0.1:7457`, e não `http://127.0.0.1:0`. O `:0` é apenas uma dica interna de "escolha uma porta livre" no launch e não deveria vazar para sessões de agente.
...
- **geração de mídia diz que `ZD_BIN` está faltando ou que a URL do daemon é `:0`** — rode as verificações do dispatcher de mídia acima. Não retome a sessão antiga do CLI; reabra o projeto pelo app Open Design para o daemon injetar variáveis `ZD_*` novas.
- **Codex carrega muito contexto de plugin** — suba o Open Design com `ZD_CODEX_DISABLE_PLUGINS=1 pnpm tools-dev` para que processos Codex spawnados pelo daemon rodem com `--disable plugins`.
