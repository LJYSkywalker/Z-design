...
3. エディタで `.env` を開き、`ZD_API_TOKEN=` を見つけて、生成したトークンを貼り付けます。
...
- `ZD_BIN` — `apps/daemon/dist/cli.js` への絶対パス。
- `ZD_DAEMON_URL` — 実行中の daemon URL。
- `ZD_PROJECT_ID` — アクティブなプロジェクト ID。
- `ZD_PROJECT_DIR` — アクティブなプロジェクトのファイルディレクトリ。

メディア生成が `ZD_BIN: parameter not set`、`apps/daemon/dist/cli.js` の欠落、または `failed to reach daemon at http://127.0.0.1:0` で失敗する場合は、daemon CLI を再ビルドして管理対象ランタイムを再起動してください：
...
`ZD_DAEMON_URL` は `http://127.0.0.1:0` ではなく、`http://127.0.0.1:7457` のような実際の daemon ポートでなければなりません。`:0` という値は内部的な「空きポートを選択する」起動ヒントにすぎず、エージェントセッションに漏れてはなりません。
...
- **メディア生成で `ZD_BIN` が欠落、または daemon URL が `:0`** — 上記のメディアディスパッチャーチェックを実行してください。古い CLI セッションを再開せず、Open Design アプリからプロジェクトを再度開いて、daemon が新しい `ZD_*` 変数を注入できるようにしてください。
- **Codex がプラグインコンテキストを多く読み込みすぎる** — `ZD_CODEX_DISABLE_PLUGINS=1 pnpm tools-dev` で Open Design を起動すると、daemon から起動された Codex プロセスが `--disable plugins` で実行されます。
