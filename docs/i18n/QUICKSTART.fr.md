...
3. Ouvrez `.env` dans votre éditeur, trouvez `ZD_API_TOKEN=` et collez le token généré.
...
- `ZD_BIN` — chemin absolu vers `apps/daemon/dist/cli.js`.
- `ZD_DAEMON_URL` — URL du daemon en cours d’exécution.
- `ZD_PROJECT_ID` — id du projet actif.
- `ZD_PROJECT_DIR` — dossier de fichiers du projet actif.

Si la génération média échoue avec `ZD_BIN: parameter not set`, `apps/daemon/dist/cli.js` manquant, ou `failed to reach daemon at http://127.0.0.1:0`, rebuildez la CLI daemon et redémarrez le runtime géré :
...
`ZD_DAEMON_URL` doit être un vrai port daemon comme `http://127.0.0.1:7457`, pas `http://127.0.0.1:0`. La valeur `:0` est seulement une indication interne "choisir un port libre" au lancement et ne doit pas se retrouver dans les sessions agent.
...
- **la génération média dit que `ZD_BIN` manque ou que l’URL daemon vaut `:0`** — exécutez les checks du dispatcher média ci-dessus. Ne reprenez pas l’ancienne session CLI ; rouvrez le projet depuis l’app Open Design pour que le daemon injecte des variables `ZD_*` fraîches.
- **Codex charge trop de contexte plugin** — démarrez Open Design avec `ZD_CODEX_DISABLE_PLUGINS=1 pnpm tools-dev` pour que les processus Codex lancés par le daemon tournent avec `--disable plugins`.
