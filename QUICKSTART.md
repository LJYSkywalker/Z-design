...
3. Open `.env` in your editor, find `ZD_API_TOKEN=`, and paste the generated token there.
...
pnpm --filter @z-design/daemon build  # build apps/daemon/dist/cli.js for `od`
pnpm --filter @z-design/web build     # build the web package when needed
...
- `ZD_BIN` — absolute path to `apps/daemon/dist/cli.js`.
- `ZD_DAEMON_URL` — the running daemon URL.
- `ZD_PROJECT_ID` — the active project id.
- `ZD_PROJECT_DIR` — the active project's file directory.

If media generation fails with `ZD_BIN: parameter not set`, `apps/daemon/dist/cli.js` missing, or `failed to reach daemon at http://127.0.0.1:0`, rebuild the daemon CLI and restart the managed runtime:
...
```bash
echo "ZD_BIN=$ZD_BIN"
echo "ZD_PROJECT_ID=$ZD_PROJECT_ID"
echo "ZD_PROJECT_DIR=$ZD_PROJECT_DIR"
echo "ZD_DAEMON_URL=$ZD_DAEMON_URL"
ls -la "$ZD_BIN"
```

`ZD_DAEMON_URL` must be a real daemon port such as `http://127.0.0.1:7457`, not `http://127.0.0.1:0`. The `:0` value is only an internal "pick a free port" launch hint and should not leak into agent sessions.
...
- **`better-sqlite3` fails to load / ABI mismatch after a Node.js version change** — `pnpm install` re-runs `postinstall` automatically and rebuilds the native addon for the current Node.js. To rebuild manually or verify the fix: `pnpm --filter @z-design/daemon rebuild better-sqlite3` then `pnpm --filter @z-design/daemon exec node -e "require('better-sqlite3')"`. Requires build tools: `python3`, `make`, `g++` (or `clang++`). If you have `ignore-scripts=true` in your `.npmrc`, run `node scripts/postinstall.mjs` after `pnpm install`.
...
- **media generation says `ZD_BIN` is missing or daemon URL is `:0`** — run the media dispatcher checks above. Do not resume the old CLI session; reopen the project from the Open Design app so the daemon can inject fresh `ZD_*` variables.
- **Codex loads too much plugin context** — start Open Design with `ZD_CODEX_DISABLE_PLUGINS=1 pnpm tools-dev` to make daemon-spawned Codex processes run with `--disable plugins`.
...
- **`Authorization: Bearer <ZD_API_TOKEN>` required on macOS** — Docker Desktop bridge networking makes the daemon see requests as non-loopback. Enable host networking in Docker Desktop and use `network_mode: host`. See [`deploy/README.md` — Docker Desktop on macOS](deploy/README.md#docker-desktop-on-macos).
