...
async function main(): Promise<void> {
  ...
  const namespace = process.env.ZD_PACKAGED_E2E_NAMESPACE ?? defaultNamespace(platform);
  const reportRoot = resolveFromWorkspace(
    process.env.ZD_PACKAGED_E2E_REPORT_DIR ?? join('.tmp', 'release-report', platform),
  );
  const report = await createReport(reportRoot);

  process.env.ZD_PACKAGED_E2E_REPORT_DIR = report.root;

  await report.json('manifest.json', {
    ...(process.env.ZD_PACKAGED_E2E_RELEASE_CHANNEL == null
      ? {}
      : { channel: process.env.ZD_PACKAGED_E2E_RELEASE_CHANNEL }),
    ...(process.env.ZD_PACKAGED_E2E_RELEASE_VERSION == null
      ? {}
      : { releaseVersion: process.env.ZD_PACKAGED_E2E_RELEASE_VERSION }),
  ...
  await saveRequiredSource(report, 'tools-pack.json', process.env.ZD_PACKAGED_E2E_BUILD_JSON_PATH);
  await saveOptionalSource(report, 'tools-pack.log', process.env.ZD_PACKAGED_E2E_BUILD_LOG_PATH);
