...
async function emitPluginAuthoringRun() {
  ...
  const summary = [
    'Created generated-plugin with z-design.json, SKILL.md, and examples/demo.md.',
...
async function createOrbitLiveArtifact() {
  const baseUrl = process.env.ZD_DAEMON_URL;
  const token = process.env.ZD_TOOL_TOKEN;
  if (!baseUrl || !token) {
    throw new Error('Orbit fake run requires ZD_DAEMON_URL and ZD_TOOL_TOKEN');
  }
