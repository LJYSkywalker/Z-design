...
export async function runToolsDevJson<T>(
  ...
  const useNpmExecPathWithNode = process.env.ZD_E2E_PNPM_COMMAND == null
    && pnpmExecPath != null
    && nodeLoadablePackageManagerExtensions.has(extname(pnpmExecPath).toLowerCase());
  const command = useNpmExecPathWithNode
    ? process.execPath
    : (process.env.ZD_E2E_PNPM_COMMAND == null && pnpmExecPath ? pnpmExecPath : pnpmCommand);
  ...
  const { stdout } = await execFileAsync(command, commandArgs, {
    ...
    env: {
      ...
      ZD_MEDIA_CONFIG_DIR: suite.dataDir,
    },
