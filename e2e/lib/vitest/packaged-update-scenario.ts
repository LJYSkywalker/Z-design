...
export function resolvePackagedUpdateScenario(input: {
  ...
  if (releaseChannel == null || releaseChannel === '' || releaseVersion == null || releaseVersion === '') {
    throw new Error('ZD_PACKAGED_E2E_RELEASE_CHANNEL and ZD_PACKAGED_E2E_RELEASE_VERSION must be set together');
  }
...
export function applyPackagedUpdateEnv(
  ...
  env.ZD_UPDATE_ENABLED = '1';
  env.ZD_UPDATE_METADATA_URL = metadataUrl;
  env.ZD_UPDATE_OPEN_DRY_RUN = options.openDryRun === false ? '0' : '1';
  env.ZD_UPDATE_AUTO_CHECK = '1';
  if (scenario.currentVersionOverride == null) {
    delete env.ZD_UPDATE_CURRENT_VERSION;
  } else {
    env.ZD_UPDATE_CURRENT_VERSION = scenario.currentVersionOverride;
  }
}
