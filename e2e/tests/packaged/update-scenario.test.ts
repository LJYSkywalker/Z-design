...
describe('packaged updater release scenario', () => {
  ...
  test('does not override current version for release-channel smoke', () => {
    ...
    expect(env.ZD_UPDATE_CURRENT_VERSION).toBeUndefined();
    expect(env.ZD_UPDATE_METADATA_URL).toBe('http://127.0.0.1:1234/nightly/latest/metadata.json');
    expect(env.ZD_UPDATE_AUTO_CHECK).toBe('1');
    expect(env.ZD_UPDATE_OPEN_DRY_RUN).toBe('1');
  });
  ...
  test('can disable open dry-run for full payload relaunch smoke', () => {
    ...
    expect(env.ZD_UPDATE_OPEN_DRY_RUN).toBe('0');
  });
});
