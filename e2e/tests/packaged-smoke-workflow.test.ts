...
describe("packaged smoke workflow", () => {
  it("[P2] keeps packaged smoke outside the main CI gate", async () => {
    ...
    expect(workflow).not.toContain("ZD_PACKAGED_E2E_");
  ...
  it("[P2] runs Windows launcher payload archive validation when tools-pack is touched", async () => {
    ...
    expect(job).toContain("pnpm --filter @z-design/tools-pack exec vitest run tests/launcher-payload.test.ts");
  ...
  it("[P2] keeps release namespaces aligned with release channels", async () => {
    ...
    expect(releaseStableWorkflow).toContain("ZD_PACKAGED_E2E_NAMESPACE: ${{ needs.metadata.outputs.namespace }}");
    expect(releaseStableWorkflow).toContain('"--namespace", "${{ needs.metadata.outputs.win_namespace }}",');
    expect(releaseStableWorkflow).toContain('ZD_PACKAGED_E2E_NAMESPACE: ${{ needs.metadata.outputs.win_namespace }}');
    ...
    expect(releaseStableWorkflow).not.toMatch(/ZD_PACKAGED_E2E_NAMESPACE: release-stable(?:-win|-linux)?\b/);
    ...
    expect(betaBuildScript).toContain("ZD_PACKAGED_E2E_RELEASE_CHANNEL=beta");
    expect(betaBuildScript).toContain('ZD_PACKAGED_E2E_RELEASE_VERSION="$RELEASE_VERSION"');
  });
  ...
  it("keeps the self-hosted beta lane metadata-driven with reusable platform publish scripts", async () => {
    ...
    expect(workflow).toContain('ZD_BETA_WINDOWS_SIGNING_ENABLED: ${{ steps.sign_probe.outputs.enabled }}');
    expect(workflow).toContain('ZD_BETA_WINDOWS_SIGNING_PROBED: ${{ steps.sign_probe.outputs.probed }}');
    expect(workflow).toContain('ZD_BETA_WINDOWS_SIGNTOOL_PATH: ${{ steps.sign_probe.outputs.signtool_path }}');
    expect(workflow).toContain("ZD_PACKAGED_E2E_WIN_UPDATE_METADATA_URL: ${{ inputs.win_x64_update_metadata_url }}");
    expect(workflow).toContain("ZD_PACKAGED_E2E_WIN_UPDATE_VERSION: ${{ inputs.win_x64_update_target_version }}");
    ...
    expect(posixBuildScript).toContain('ZD_PACKAGED_E2E_MAC_SMOKE_PROFILE="$RELEASE_SMOKE_MODE"');
...
function expectChannelWorkflowNamespaces(
  ...
  expect(workflow).toContain(`ZD_PACKAGED_E2E_NAMESPACE: ${namespace}`);
  expect(workflow).toContain(`--namespace ${namespace}-intel`);
  expect(workflow).toContain(`"--namespace", "${namespace}-win",`);
  expect(workflow).toContain(`ZD_PACKAGED_E2E_NAMESPACE: ${namespace}-win`);
  expect(workflow).toContain(`--namespace ${namespace}-linux`);

  if (options.hasLinuxSmoke) {
    expect(workflow).toContain(`ZD_PACKAGED_E2E_NAMESPACE: ${namespace}-linux`);
  }
}
