...
describe('tools-dev sandbox mode smoke', () => {
  test('launches web/daemon with ZD_SANDBOX_MODE=1 and a sandboxed agent spawn', async () => {
    ...
    await suite.with.toolsDev(
      async ({ status, webUrl }) => {
        ...
        expect(capture.env.ZD_SANDBOX_MODE).toBe('1');
        expect(capture.env.ZD_DATA_DIR).toBe(suite.dataDir);
        expect(capture.env.ZD_PROJECT_ID).toBe(project.project.id);
        expectPathUnder(capture.env.ZD_AGENT_HOME, roots.agentHomeDir, 'ZD_AGENT_HOME');
      ...
      {
        env: {
          ZD_E2E_SANDBOX_CAPTURE_PATH: capturePath,
          ZD_SANDBOX_MODE: '1',
        },
      },
    );
...
function emitRun(promptText) {
  if (emitted) return;
  emitted = true;
  const capturePath = process.env.ZD_E2E_SANDBOX_CAPTURE_PATH;
  if (capturePath && process.env.ZD_TOOL_TOKEN) {
    const env = {};
    ...
    writeFileSync(
      ...
      JSON.stringify({
        ...
        cwd: process.cwd(),
        env,
        hasToolToken: Boolean(process.env.ZD_TOOL_TOKEN),
