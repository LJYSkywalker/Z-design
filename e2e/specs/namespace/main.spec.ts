...
describe('namespace isolation spec', () => {
  test('keeps namespace in lifecycle infrastructure while daemon clients use URL or concrete IPC transport', async () => {
    ...
    await suite.with.toolsDev(async ({ runtime, status, webUrl }) => {
      ...
      const ipcPath = installInfo.env.ZD_SIDECAR_IPC_PATH;
      if (typeof ipcPath !== 'string' || ipcPath.length === 0) {
        throw new Error('MCP install-info did not include ZD_SIDECAR_IPC_PATH');
      }
      expect(ipcPath).toEqual(expect.any(String));
      expect(installInfo.args).not.toContain('--daemon-url');
      expect(installInfo.env.ZD_DATA_DIR).toBe(suite.dataDir);
      expect(installInfo.env).not.toHaveProperty('ZD_NAMESPACE');
      expect(installInfo.env).not.toHaveProperty('ZD_SIDECAR_NAMESPACE');
      expect(installInfo.env).not.toHaveProperty('ZD_SIDECAR_IPC_BASE');

      const cliStatus = await runDaemonCliJson<DaemonStatusResponse>(
        ['daemon', 'status', '--json'],
        {
          ZD_DATA_DIR: suite.dataDir,
          ZD_NAMESPACE: 'wrong-daemon-namespace',
          ZD_SIDECAR_IPC_BASE: path.join(suite.scratchDir, 'wrong-ipc-base'),
          ZD_SIDECAR_IPC_PATH: ipcPath,
          ZD_SIDECAR_NAMESPACE: 'wrong-sidecar-namespace',
        },
      );
      ...
      await access(path.join(expectedPluginRoot, 'z-design.json'));
...
async function writeLocalPluginFixture(root: string): Promise<string> {
  ...
  await writeFile(
    path.join(pluginRoot, 'z-design.json'),
    JSON.stringify(
      {
...
async function runDaemonCli(
  ...
  delete mergedEnv.ZD_DAEMON_URL;
  delete mergedEnv.ZD_PORT;
