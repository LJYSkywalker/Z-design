...
const toolsPackDir = resolveFromWorkspace(process.env.ZD_PACKAGED_E2E_TOOLS_PACK_DIR ?? '.tmp/tools-pack');
const namespace = resolvePackagedSmokeNamespace('win');
const toolsPackBin = join(workspaceRoot, 'tools', 'pack', 'bin', 'tools-pack.mjs');
const maxInstallDurationMs = Number.parseInt(process.env.ZD_PACKAGED_E2E_WIN_MAX_INSTALL_MS ?? '120000', 10);
const smokeProfile = process.env.ZD_PACKAGED_E2E_WIN_SMOKE_PROFILE ?? 'core';
const verifyCoreOnly = smokeProfile === 'core';
const verifyReinstallWhileRunning = !verifyCoreOnly && process.env.ZD_PACKAGED_E2E_WIN_VERIFY_REINSTALL !== '0';
const updateMetadataUrl = normalizeOptionalEnv(process.env.ZD_PACKAGED_E2E_WIN_UPDATE_METADATA_URL);
const updateVersion = normalizeOptionalEnv(process.env.ZD_PACKAGED_E2E_WIN_UPDATE_VERSION);
const updateBuildJsonPath = normalizeOptionalEnv(process.env.ZD_PACKAGED_E2E_WIN_UPDATE_BUILD_JSON_PATH);
const releaseChannel = process.env.ZD_PACKAGED_E2E_RELEASE_CHANNEL;
const releaseVersion = process.env.ZD_PACKAGED_E2E_RELEASE_VERSION;
...
const shouldRunPackagedWinSmoke = process.platform === 'win32' && process.env.ZD_PACKAGED_E2E_WIN === '1';
const winDescribe = shouldRunPackagedWinSmoke ? describe : describe.skip;
const shouldRunPackagedWinOnboardingSmoke =
  shouldRunPackagedWinSmoke && process.env.ZD_PACKAGED_E2E_WIN_ONBOARDING_SMOKE === '1';
    ...
    process.platform === 'win32'
      ? execFileAsync(
          ...
          [
            ...
            'Bypass',
            '-Command',
            "& { $process = Start-Process -FilePath $env:ZD_TEST_INSTALLER_PATH -ArgumentList '/S', $env:ZD_TEST_INSTALL_DIR_ARG -Wait -PassThru; exit $process.ExitCode }",
          ],
          {
            cwd: dirname(installerPath),
            env: {
              ...process.env,
              ZD_TEST_INSTALL_DIR_ARG: `/D=${installDir}`,
              ZD_TEST_INSTALLER_PATH: installerPath,
            },
            maxBuffer: 20 * 1024 * 1024,
          },
        )
...
async function resolveLocalPayloadUpdateFixture(): Promise<{ payloadPath: string; targetVersion: string }> {
  ...
  if (fallbackBuildJsonPath == null) {
    throw new Error(
      'full packaged windows payload smoke requires update payload metadata; set ZD_PACKAGED_E2E_WIN_UPDATE_METADATA_URL or provide windows-tools-pack-update-build.json next to ZD_PACKAGED_E2E_BUILD_JSON_PATH',
    );
  }
...
function resolveFallbackUpdateBuildJsonPath(): string | null {
  ...
  const mainBuildJsonPath = normalizeOptionalEnv(process.env.ZD_PACKAGED_E2E_BUILD_JSON_PATH);
...
const UPDATE_ENV_KEYS = [
  'ZD_UPDATE_AUTO_CHECK',
  'ZD_UPDATE_ENABLED',
  'ZD_UPDATE_METADATA_URL',
  'ZD_UPDATE_CURRENT_VERSION',
  'ZD_UPDATE_OPEN_DRY_RUN',
] as const;
...
async function seedPackagedOnboardingComplete(): Promise<void> {
  ...
  // hands it to the runtime via ZD_PACKAGED_CONFIG_PATH, so the live daemon's
