...
const screenshotPath = resolveFromWorkspace(
  process.env.ZD_PACKAGED_E2E_SCREENSHOT_PATH ?? join(toolsPackDir, 'screenshots', `${namespace}.png`),
);
...
const shouldRunLinuxHeadlessSmoke =
  process.platform === 'linux' && process.env.ZD_PACKAGED_E2E_LINUX_HEADLESS === '1';
const linuxHeadlessDescribe = shouldRunLinuxHeadlessSmoke ? describe : describe.skip;
const shouldRunLinuxAppImageSmoke =
  process.platform === 'linux' && process.env.ZD_PACKAGED_E2E_LINUX_APPIMAGE === '1';
