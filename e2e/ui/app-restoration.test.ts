...
async function runDeckPaginationNextPrevCorrectnessFlow(page: Page) {
  ...
  await page.reload();
  await zDesignFile(page, 'pagination.html');
...
async function runDeckPaginationPerFileIsolatedFlow(page: Page) {
  ...
  await zDesignFile(page, 'deck-alpha.html');
  const frame = artifactPreviewFrame(page);
  ...
  await page.getByTestId('design-files-tab').click();
  await zDesignFile(page, 'deck-beta.html');
...
async function runUploadedImageRendersInPreviewFlow(page: Page) {
  ...
  await page.reload();
  await zDesignFile(page, 'image-preview.html');
