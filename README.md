...
> **macOS users:** If the web UI shows `Authorization: Bearer <ZD_API_TOKEN> required`, Docker Desktop bridge networking is the cause. See [Docker Desktop on macOS](deploy/README.md#docker-desktop-on-macos) for the fix.
...
**Security model.** Read-only by default, the daemon binds to `127.0.0.1`, and SSRF is blocked at the proxy edge. LAN exposure requires an explicit `ZD_BIND_HOST` plus `ZD_ALLOWED_ORIGINS`. Connector credentials and live-artifact preview routes stay loopback-only regardless.
...
| [`OpenCoworkAI/open-codesign`](https://github.com/OpenCoworkAI/open-codesign) | The first GLM-native agentic design workspace; UX patterns we borrow (streaming-artifact loop, sandboxed iframe, live agent panel). |
