...
describe("tools-dev diagnostics", () => {
  it("detects native addon ABI mismatches", () => {
    ...
    assert.match(diagnostics[0].recommendation, /corepack pnpm --filter @z-design\/daemon rebuild better-sqlite3 --pending/);
  ...
  it("appends log tails and recommendations to startup timeout errors", () => {
    ...
    assert.match(error.message, /corepack pnpm --filter @z-design\/daemon rebuild better-sqlite3 --pending/);
  });
});
