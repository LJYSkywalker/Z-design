...
import { createSidecarLaunchEnv, requestJsonIpc } from "@z-design/sidecar";
import {
  ...
  try {
    ...
    return await spawnSidecarRuntime({
      ...
      env: {
        ...
        ...(spawnOptions.requireDesktopAuth ? { ZD_REQUIRE_DESKTOP_AUTH: "1" } : {}),
      },
      logHandle,
    });
  } finally {
  ...
  try {
    ...
    return await spawnSidecarRuntime({
      ...
      env: {
        ...
        ...(options.parentPid == null ? {} : { [TOOLS_DEV_PARENT_PID_ENV]: String(options.parentPid) }),
        ...(options.prod === true
          ? { NODE_ENV: "production", ZD_WEB_OUTPUT_MODE: "server", ZD_WEB_PROD: "1" }
          : {}),
      },
      logHandle,
    });
...
async function buildDesktop(config: ToolDevConfig, logHandle: FileHandle): Promise<void> {
  await logHandle.write(`\n[tools-dev] building @z-design/desktop at ${new Date().toISOString()}\n`);
  const invocation = createPackageManagerInvocation(["--filter", "@z-design/desktop", "build"], process.env);
...
async function ensureDaemonCliBuild(config: ToolDevConfig, logHandle: FileHandle): Promise<void> {
  ...
  await logHandle.write(`\n[tools-dev] building @z-design/daemon because ${reason} at ${new Date().toISOString()}\n`);
  const invocation = createPackageManagerInvocation(["--filter", "@z-design/daemon", "build"], process.env);
  await runLoggedCommand({
    args: invocation.args,
    command: invocation.command,
    cwd: config.workspaceRoot,
...
function addPortOptions(command: ReturnType<typeof cli.command>) {
  return command
    ...
    .option("--prod", "use production build (requires pnpm --filter @z-design/web build first)");
}
