...
- `tools/dev` provides `@z-design/tools-dev` and the `tools-dev` bin. It is the only currently active local development lifecycle control plane.
...
- `tools/pack` provides `@z-design/tools-pack` and the `tools-pack` bin. The active slice is packaged artifact build/install/start/stop/logs/uninstall/cleanup/list/reset plus beta release artifact preparation for mac and Windows lanes, plus a Linux AppImage lane with optional containerized builds.
- `tools/serve` provides `@z-design/tools-serve` and the `tools-serve` bin. It owns local fixture services such as `tools-serve start updater`.

## Retired tools

- `tools/pr` / `@z-design/tools-pr` / `pnpm tools-pr` has been retired from this repository. Maintainer PR-duty workflows now live outside the product workspace in `PerishCode/duty`; do not restore an Open Design-local PR-duty tool without a new explicit maintainer decision.
...
- Orchestration layers must consume primitives from `@z-design/sidecar-proto`, `@z-design/sidecar`, and `@z-design/platform`.
...
- Port flags are authoritative inputs: `--daemon-port` and `--web-port`. Internal env vars are `ZD_PORT` and `ZD_WEB_PORT`; do not introduce `NEXT_PORT`.

## Common tools commands

```bash
pnpm --filter @z-design/tools-dev typecheck
pnpm --filter @z-design/tools-dev build
pnpm --filter @z-design/tools-pack typecheck
pnpm --filter @z-design/tools-pack build
pnpm --filter @z-design/tools-serve typecheck
pnpm --filter @z-design/tools-serve build
