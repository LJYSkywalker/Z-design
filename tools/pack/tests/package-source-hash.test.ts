...
describe("hashPackageSourcePath", () => {
  it("includes package versions in the source hash", async () => {
    ...
    try {
      ...
      await writeFile(join(packageRoot, "package.json"), `${JSON.stringify({ name: "@z-design/packaged", version: "1.0.0" }, null, 2)}\n`, "utf8");
      const firstHash = await hashPackageSourcePath(packageRoot);

      await writeFile(join(packageRoot, "package.json"), `${JSON.stringify({ name: "@z-design/packaged", version: "1.0.1" }, null, 2)}\n`, "utf8");
