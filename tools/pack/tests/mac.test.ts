...
const envState = { odDataDir: process.env.ZD_DATA_DIR };

afterEach(() => {
  if (envState.odDataDir == null) {
    delete process.env.ZD_DATA_DIR;
  } else {
    process.env.ZD_DATA_DIR = envState.odDataDir;
  }
});
...
describe("resolveSeededAppConfigPaths", () => {
  ...
  it("prefers ZD_DATA_DIR when provided", () => {
    process.env.ZD_DATA_DIR = "/custom/data";
    const config = makeConfig("/work");
    expect(resolveSeededAppConfigPaths(config)).toEqual({
  ...
  it("resolves relative ZD_DATA_DIR against the workspace root", () => {
    process.env.ZD_DATA_DIR = "e2e/ui/.od-data";
    const config = makeConfig("/work");
    expect(resolveSeededAppConfigPaths(config)).toEqual({
  ...
  it("expands $HOME-style ZD_DATA_DIR values", () => {
    process.env.ZD_DATA_DIR = "$HOME/.open-design";
    const config = makeConfig("/work");
    expect(resolveSeededAppConfigPaths(config)).toEqual({
