...
const CONFIG_ENV = "ZD_TOOLS_PACK_WEB_STANDALONE_HOOK_CONFIG";
...
async function writeRootWebPackage(resourcesRoot: string): Promise<void> {
  const webPackageRoot = join(resourcesRoot, "app", "node_modules", "@z-design", "web");
  await mkdir(join(webPackageRoot, "dist", "sidecar"), { recursive: true });
  await writeFile(join(webPackageRoot, "package.json"), "{\"name\":\"@z-design/web\"}\n", "utf8");
  ...
  darwinSymlinkIt("signs versioned mac frameworks at their Current version path", async () => {
    ...
    try {
      ...
      if (oldCodesignLog == null) {
        delete process.env.ZD_FAKE_CODESIGN_LOG;
      } else {
        process.env.ZD_FAKE_CODESIGN_LOG = oldCodesignLog;
      }
    }
  });
});
