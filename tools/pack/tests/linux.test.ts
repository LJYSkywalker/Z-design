...
import { requestJsonIpc, resolveAppIpcPath } from "@z-design/sidecar";
import {
  APP_KEYS,
...
vi.mock("@z-design/sidecar", async (importOriginal) => {
  const actual = await importOriginal<typeof import("@z-design/sidecar")>();
  return {
    ...actual,
...
describe("buildDockerArgs", () => {
  ...
  it("exports ZD_TOOLS_PACK_PNPM_BIN=/tmp/pnpm so the inner build's production install skips npm", () => {
    const args = buildDockerArgs(makeConfig(), { uid: 1000, gid: 1000 });
    const envFlagIndex = args.findIndex(
      (arg, i) => arg === "-e" && args[i + 1] === "ZD_TOOLS_PACK_PNPM_BIN=/tmp/pnpm",
    );
...
describe("resolveProductionInstallCommand", () => {
  it("defaults to npm install --omit=dev --no-package-lock when ZD_TOOLS_PACK_PNPM_BIN is unset", () => {
  ...
  it("treats an empty ZD_TOOLS_PACK_PNPM_BIN as unset and keeps the npm host default", () => {
    expect(resolveProductionInstallCommand({ ZD_TOOLS_PACK_PNPM_BIN: "" })).toEqual({
      command: "npm",
      args: ["install", "--omit=dev", "--no-package-lock"],
    });
  });

  it("uses ZD_TOOLS_PACK_PNPM_BIN with hoisted-layout pnpm flags when set", () => {
    ...
    expect(
      resolveProductionInstallCommand({ ZD_TOOLS_PACK_PNPM_BIN: "/tmp/pnpm" }),
    ).toEqual({
      command: "/tmp/pnpm",
  ...
  it("chains end-to-end with buildDockerArgs: docker exports ZD_TOOLS_PACK_PNPM_BIN and the resolver returns the standalone pnpm install for that value", () => {
    const dockerArgs = buildDockerArgs(makeConfig(), { uid: 1000, gid: 1000 });
    const envFlagIndex = dockerArgs.findIndex(
      (arg, i) => arg === "-e" && dockerArgs[i + 1]?.startsWith("ZD_TOOLS_PACK_PNPM_BIN="),
    );
    expect(envFlagIndex).toBeGreaterThan(-1);
    const envValue = dockerArgs[envFlagIndex + 1]?.split("=")[1];
    expect(envValue).toBe("/tmp/pnpm");

    const resolved = resolveProductionInstallCommand({ ZD_TOOLS_PACK_PNPM_BIN: envValue });
...
Exec=env ZD_PACKAGED_NAMESPACE=@@NAMESPACE@@ @@EXEC_PATH@@ --appimage-extract-and-run %U
Icon=@@ICON_PATH@@
...
`;
  ...
  it("substitutes all @@TOKEN@@ placeholders", () => {
    ...
    expect(out).toContain(
      "Exec=env ZD_PACKAGED_NAMESPACE=default /home/u/.local/bin/Open-Design.default.AppImage --appimage-extract-and-run %U",
    );
    expect(out).toContain("Icon=open-design-default");
  });

  it("uses ZD_PACKAGED_NAMESPACE (not ZD_NAMESPACE) so apps/packaged actually picks up the namespace override", () => {
    const out = renderDesktopTemplate(template, {
      namespace: "ns",
      execPath: "/x",
      iconName: "open-design-ns",
    });
    expect(out).toMatch(/^Exec=env ZD_PACKAGED_NAMESPACE=ns /m);
    expect(out).not.toMatch(/ZD_NAMESPACE=/);
  });
