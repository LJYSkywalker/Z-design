...
describe("release workflows", () => {
  it("requires Vela CLI only for beta mac arm64 packaging", async () => {
    ...
    expect(buildMac).toContain('ZD_PACKAGED_E2E_MAC_UPDATE_BUILD_JSON_PATH="$update_build_json_path"');
    expect(buildMac).toContain('ZD_PACKAGED_E2E_MAC_UPDATE_VERSION="${ZD_PACKAGED_E2E_MAC_UPDATE_VERSION:-$update_version}"');
    ...
    expect(beta).toContain("ZD_PACKAGED_E2E_MAC_UPDATE_METADATA_URL: ${{ inputs.mac_arm64_update_metadata_url }}");
    expect(beta).toContain("ZD_PACKAGED_E2E_WIN_UPDATE_METADATA_URL: ${{ inputs.win_x64_update_metadata_url }}");
    ...
    expect(selfHostedMac).toContain("ZD_UPDATE_METADATA_URL: ${{ inputs.release_public_origin }}/beta/latest/metadata.json");
    ...
    expect(selfHostedMac).toContain("ZD_PACKAGED_E2E_MAC_UPDATE_METADATA_URL: ${{ inputs.mac_arm64_update_metadata_url }}");
    ...
    for (const workspaceBuild of [winApp, macWorkspace, linuxPack]) {
      const sidecarProtoBuild = 'await runPnpm(config, ["--filter", "@z-design/sidecar-proto", "build"])';
      const launcherProtoBuild = 'await runPnpm(config, ["--filter", "@z-design/launcher-proto", "build"])';
      const sidecarBuild = 'await runPnpm(config, ["--filter", "@z-design/sidecar", "build"])';
