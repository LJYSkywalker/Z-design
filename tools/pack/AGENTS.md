...
- Consuming sidecar/process/path primitives from `@z-design/sidecar-proto`, `@z-design/sidecar`, and `@z-design/platform`.
...
The runtime updater reads `https://releases.open-design.ai/<channel>/latest/metadata.json` unless `ZD_UPDATE_METADATA_URL` overrides it. For package-launcher updates:
...
- `ZD_UPDATE_CURRENT_VERSION` may override the packaged version for tests, but user-flow package validation should prefer building the package with the intended `--app-version`.
...
```bash
ZD_UPDATE_ENABLED=1
ZD_UPDATE_METADATA_URL=<fixture metadataUrl>
ZD_UPDATE_CURRENT_VERSION=99.0.0-beta.0
ZD_UPDATE_OPEN_DRY_RUN=1
ZD_UPDATE_AUTO_CHECK=1
```
...
```bash
pnpm --filter @z-design/desktop test -- tests/main/updater.test.ts tests/main/updater-host-boundary.test.ts tests/main/preload-host-boundary.test.ts
pnpm --filter @z-design/web test -- tests/components/UpdaterPopup.test.tsx tests/lib/updater.test.ts
pnpm --filter @z-design/tools-serve test
pnpm --filter @z-design/tools-pack test -- tests/win-identity.test.ts tests/win-app.test.ts tests/win-builder.test.ts
pnpm --filter @z-design/desktop typecheck
pnpm --filter @z-design/web typecheck
