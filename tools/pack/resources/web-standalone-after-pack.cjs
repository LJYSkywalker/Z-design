...
async function pruneRootNext(appNodeModulesRoot, platformName) {
  ...
  await removePathAndRecord(
    path.join(appNodeModulesRoot, "@z-design", "web", ".next", "standalone"),
    "root @z-design/web standalone output",
    removedPaths,
  );
...
async function pruneRootWebPackage(appNodeModulesRoot, platformName) {
  ...
  const webPackageRoot = path.join(appNodeModulesRoot, "@z-design", "web");
  const removedPaths = [];
  for (const entry of [".next", "app", "next.config.ts", "public", "src"]) {
    await removePathAndRecord(
      path.join(webPackageRoot, entry),
      "root @z-design/web standalone-safe package residue",
      removedPaths,
    );
  }
  return removedPaths;
...
async function auditRootWebPackage(appNodeModulesRoot) {
  ...
  for (const requiredPath of [packageJsonPath, sidecarEntryPath]) {
    if (!(await pathExists(requiredPath))) {
      throw new Error(`[tools-pack web-standalone] root @z-design/web audit missing: ${requiredPath}`);
    }
  }
  return {
