...
import { resolveNamespace } from "@z-design/sidecar";
...
function resolveToolPackWebOutputMode(platform: ToolPackPlatform, value: string | undefined): ToolPackWebOutputMode {
  ...
  throw new Error(`unsupported ZD_WEB_OUTPUT_MODE value: ${value}`);
}
...
function resolveToolPackUpdateMetadataUrl(value: string | undefined): string | undefined {
  ...
  try {
    ...
    throw new Error(`ZD_UPDATE_METADATA_URL must be an absolute URL: ${value}`);
  }
  if (parsed.protocol !== "https:" && parsed.protocol !== "http:") {
    throw new Error(`ZD_UPDATE_METADATA_URL must use http(s): ${value}`);
  }
  return normalized;
}
...
export function resolveToolPackConfig(
  ...
  return {
    ...
    updateMetadataUrl: resolveToolPackUpdateMetadataUrl(process.env.ZD_UPDATE_METADATA_URL),
    ...
    webOutputMode: resolveToolPackWebOutputMode(platform, process.env.ZD_WEB_OUTPUT_MODE),
