...
export function resolveWinSigningConfig(): WinSigningConfig {
  const certificateSha1 = normalizeSha1(process.env.ZD_WIN_SIGN_CERT_SHA1 ?? process.env.WIN_SIGN_CERT_SHA1);
  if (certificateSha1 == null) {
    throw new Error("signed Windows builds require ZD_WIN_SIGN_CERT_SHA1");
  }
  return {
    certificateSha1,
    digestAlgorithm: "sha256",
    signtoolPath: process.env.ZD_WIN_SIGNTOOL_PATH ?? process.env.WIN_SIGNTOOL_PATH ?? DEFAULT_SIGNTOOL_CANDIDATES[0],
    timestampAlgorithm: "sha256",
    timestampUrl: process.env.ZD_WIN_SIGN_TIMESTAMP_URL ?? process.env.WIN_SIGN_TIMESTAMP_URL ?? DEFAULT_TIMESTAMP_URL,
  };
}
...
function normalizeSha1(value: string | undefined): string | null {
  ...
  if (!/^[0-9A-F]{40}$/.test(normalized)) {
    throw new Error("ZD_WIN_SIGN_CERT_SHA1 must be a 40-character SHA1 thumbprint");
  }
  return normalized;
}
