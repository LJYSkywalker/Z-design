...
import { createCommandInvocation, createPackageManagerInvocation } from "@z-design/platform";
...
async function runEsbuild(config: ToolPackConfig, args: string[]): Promise<void> {
  await runPnpm(config, ["--filter", "@z-design/packaged", "exec", "esbuild", ...args]);
}
...
async function buildWorkspaceArtifacts(config: ToolPackConfig): Promise<void> {
  ...
  try {
    await runPnpm(config, ["--filter", "@z-design/web", "build"], { ZD_WEB_OUTPUT_MODE: config.webOutputMode });
...
async function buildPrebundledStandaloneRuntime(
  ...
  await writeFile(
    ...
    [
      ...
      "process.env.ZD_BIN ??= selfPath;",
      "process.env.ZD_DAEMON_CLI_PATH ??= selfPath;",
