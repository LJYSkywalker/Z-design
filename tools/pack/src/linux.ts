...
import {
  ...
  type SidecarStamp,
} from "@z-design/sidecar-proto";
import { createSidecarLaunchEnv, requestJsonIpc, resolveAppIpcPath } from "@z-design/sidecar";
import {
...
const DESKTOP_LOG_ECHO_ENV = "ZD_DESKTOP_LOG_ECHO";
...
const PRODUCTION_INSTALL_PNPM_BIN_ENV = "ZD_TOOLS_PACK_PNPM_BIN";
...
export function buildDockerArgs(
  ...
  //     @z-design/sidecar-proto (restricted to namespace charset)
...
// buildDockerArgs sets ZD_TOOLS_PACK_PNPM_BIN to the standalone pnpm binary it
...
async function buildWorkspaceArtifacts(config: ToolPackConfig): Promise<void> {
  ...
  await runPnpm(config, ["--filter", "@z-design/contracts", "build"]);
  await runPnpm(config, ["--filter", "@z-design/registry-protocol", "build"]);
  await runPnpm(config, ["--filter", "@z-design/sidecar-proto", "build"]);
  await runPnpm(config, ["--filter", "@z-design/launcher-proto", "build"]);
  await runPnpm(config, ["--filter", "@z-design/sidecar", "build"]);
  await runPnpm(config, ["--filter", "@z-design/platform", "build"]);
  await runPnpm(config, ["--filter", "@z-design/host", "build"]);
  await runPnpm(config, ["--filter", "@z-design/download", "build"]);
  await runPnpm(config, ["--filter", "@z-design/agui-adapter", "build"]);
  await runPnpm(config, ["--filter", "@z-design/plugin-runtime", "build"]);
  await runPnpm(config, ["--filter", "@z-design/download", "build"]);
  await runPnpm(config, ["--filter", "@z-design/host", "build"]);
  await runPnpm(config, ["--filter", "@z-design/diagnostics", "build"]);
  await runPnpm(config, ["--filter", "@z-design/components", "build"]);
  await runPnpm(config, ["--filter", "@z-design/daemon", "build"]);
  try {
    await runPnpm(config, ["--filter", "@z-design/web", "build"], { ZD_WEB_OUTPUT_MODE: "server" });
    await runPnpm(config, ["--filter", "@z-design/web", "build:sidecar"]);
  ...
  await runPnpm(config, ["--filter", "@z-design/desktop", "build"]);
  await runPnpm(config, ["--filter", "@z-design/packaged", "build"]);
}
...
async function validateDesktopAppImageMarker(
  ...
  // ZD_PACKAGED_NAMESPACE -- so apps/packaged falls back to a SIDECAR_SOURCES.PACKAGED
...
//   <assembledAppRoot>/node_modules/@z-design/packaged/dist/headless.mjs
...
function resolveHeadlessEntryPath(paths: LinuxPaths): string {
  return join(paths.assembledAppRoot, "node_modules", "@z-design", "packaged", "dist", "headless.mjs");
}
...
export async function installPackedLinuxHeadless(config: ToolPackConfig): Promise<LinuxHeadlessInstallResult> {
  ...
  // pre-sanitized by sidecar-proto to [A-Za-z0-9._-]. ZD_DATA_DIR is baked
  ...
  const script = [
    ...
    `ZD_PACKAGED_NAMESPACE=${JSON.stringify(config.namespace)} ZD_DATA_DIR=${JSON.stringify(dataDir)} ZD_RESOURCE_ROOT=${JSON.stringify(paths.resourceRoot)} exec ${JSON.stringify(nodePath)} ${JSON.stringify(entryPath)} "$@"`,
  ].join("\n") + "\n";
  ...
  try {
    child = await spawnBackgroundProcess({
      ...
      env: {
        ...
        ZD_PACKAGED_NAMESPACE: config.namespace,
        ...
        // headless.ts computes: join(ZD_DATA_DIR, "namespaces") which must
        // equal config.roots.runtime.namespaceBaseRoot.
        ZD_DATA_DIR: dirname(config.roots.runtime.namespaceBaseRoot),
        ZD_RESOURCE_ROOT: paths.resourceRoot,
      },
      logFd: logHandle.fd,
    });
  } finally {
