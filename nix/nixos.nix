...
#   };
{
  ...
  daemonEnvironment =
    {
      ZD_PORT = toString cfg.port;
      ZD_DATA_DIR = toString cfg.dataDir;
    ...
    // lib.optionalAttrs cfg.webFrontend.enable {
      ...
      ZD_WEB_PORT = toString cfg.webFrontend.port;
    }
    // lib.optionalAttrs (cfg.webFrontend.allowedOrigins != []) {
      ...
      ZD_ALLOWED_ORIGINS = lib.concatStringsSep "," cfg.webFrontend.allowedOrigins;
    }
    // cfg.extraEnv;
in {
    ...
    // {
      ...
      openFirewall = lib.mkOption {
        ...
        description = ''
          ...
          `extraEnv.ZD_BIND_HOST` to the externally reachable address
          ...
          `ZD_BIND_HOST = "0.0.0.0"` and add the externally reachable
          origin (e.g. `http://laptop.local:7456`) to
          `webFrontend.allowedOrigins`, which feeds `ZD_ALLOWED_ORIGINS`.
        '';
      };
    };
