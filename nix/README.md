...
> (which feeds `ZD_ALLOWED_ORIGINS`) or, for the loopback-only
> split-port case, set `extraEnv.ZD_WEB_PORT = "<proxy-port>"`. See
...
`ZD_ALLOWED_ORIGINS` environment variable (comma-separated). If you
run the daemon outside the modules — for example, behind your own
nginx/caddy — set `ZD_ALLOWED_ORIGINS` directly in the daemon's
environment with the same shape:

```
ZD_ALLOWED_ORIGINS=http://host1:port,https://host1:port,http://host2:port
```
...
The web package is built with `ZD_DAEMON_URL = ""` so the bundled JS
issues **relative** requests — `/api/*`, `/artifacts/*`, `/frames/*` —
instead of baking a daemon URL into the export. There is no runtime
config endpoint; the SPA does not read `ZD_DAEMON_URL` from the
serving environment.
...
same-origin gate is built from `ZD_BIND_HOST:port` (loopback hosts
...
| Proxy at a loopback host but different port (e.g. `http://127.0.0.1:8080` while daemon is on `:7457`)                                       | Either `extraEnv.ZD_WEB_PORT = "8080"` (whitelists `8080` on every loopback host) or `services.open-design.webFrontend.allowedOrigins`.           |
...
`ZD_ALLOWED_ORIGINS`; if you run the daemon outside the modules,
export `ZD_ALLOWED_ORIGINS` directly with the same shape (see
