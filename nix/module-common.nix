...
in {
  ...
  port = lib.mkOption {
    ...
    description = ''
      ...
      `ZD_DAEMON_URL` injection — see section (4) of `nix/README.md`.
    '';
  };
  ...
  extraEnv = lib.mkOption {
    ...
    description = ''
      ...
      service (e.g. `ZD_CODEX_DISABLE_PLUGINS = "1"`). Secrets belong
      in `environmentFile`, not here.
    '';
    example = lib.literalExpression ''
      {
        ZD_CODEX_DISABLE_PLUGINS = "1";
      }
    '';
  };
  ...
  webFrontend = {
    ...
    # with `ZD_DAEMON_URL=""`, so the bundled JS issues relative
    ...
    allowedOrigins = lib.mkOption {
      ...
      description = ''
        ...
        to set `extraEnv.ZD_WEB_PORT = "<proxy-port>"`, which whitelists
        that port on every loopback host without enumerating origins.

        Each entry is forwarded to the daemon via the `ZD_ALLOWED_ORIGINS`
