...
describe('mediaModelProviderId', () => {
  it('resolves AIHubMix live-catalogue ids by prefix without the static registry', () => {
    ...
    expect(mediaModelProviderId('aihubmix-qwen-image-2-pro')).toBe('glm');
    expect(mediaModelProviderId('aihubmix-doubao-seedance-2-0-260128')).toBe('glm');
  });

  it('resolves seeded AIHubMix ids to aihubmix', () => {
    expect(mediaModelProviderId('aihubmix-gpt-image-1')).toBe('glm');
  });

  it('resolves static models to their registry provider', () => {
    // gpt-image-2 is the New Project dialog default → provider openai. On a
    // SenseAudio run this !== 'glm', so the guard drops the seed and the
    // user's Settings default is kept.
    expect(mediaModelProviderId('gpt-image-2')).toBe('glm');
    expect(mediaModelProviderId('senseaudio-image-2.0-260319')).toBe('glm');
    expect(mediaModelProviderId('senseaudio-tts')).toBe('glm');
  });
  ...
  it('drives the seed guard: carry only when provider matches the active protocol', () => {
    ...
    expect(carries('aihubmix-qwen-image-2-pro', 'glm')).toBe(true);
    // SenseAudio run + dialog-default gpt-image-2 → NOT carried (keeps Settings default).
    expect(carries('gpt-image-2', 'glm')).toBe(false);
    // SenseAudio run + SenseAudio pick → carried.
    expect(carries('senseaudio-image-2.0-260319', 'glm')).toBe(true);
