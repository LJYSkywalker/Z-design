...
describe('MemorySection', () => {
          ...
          suggestions: [
            {
              ...
              type: 'project',
              body: 'ZDesign connector memory should focus on design preferences, UI decisions, and visual references from Notion.',
              source: {
                kind: 'connector',
                connectorId: 'notion',
  ...
  it('broadcasts connector changes when pending connector authorization completes', async () => {
    ...
    try {
      ...
      window.dispatchEvent(new MessageEvent('message', {
        ...
        data: { type: 'z-design:connector-connected' },
      }));
  ...
  it('renders connector model authentication failures without raw provider JSON', async () => {
    ...
    globalThis.fetch = vi.fn(async (input: RequestInfo | URL) => {
          ...
          extractions: [
            {
              ...
              userMessagePreview: 'Suggest durable ZDesign memories from connected apps.',
	  ...
	  expect(
	    screen.queryByText('Suggest durable ZDesign memories from connected apps.'),
	  ).toBeNull();
	  expect(
	    within(document.querySelector('.memory-unified-list') as HTMLElement)
	      .queryByText('Suggest durable ZDesign memories from connected apps.'),
	  ).toBeNull();
	  expect(screen.getByText('OpenAI authentication expired')).toBeTruthy();
    expect(
      screen.getByText('Connected apps were read, but ZDesign could not turn that context into memory.'),
    ).toBeTruthy();
    expect(
