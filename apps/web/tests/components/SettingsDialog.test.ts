...
describe('SettingsDialog API protocol switching', () => {
  ...
  it('stores the current custom protocol config while preserving custom endpoint details', () => {
    ...
    const next = switchApiProtocolConfig(config, 'glm');
    ...
    expect(next.apiProtocolConfigs?.glm).toMatchObject({
      apiKey: 'anthropic-key',
      baseUrl: 'https://my-proxy.example.com',
      model: 'my-model',
      apiProviderBaseUrl: null,
  ...
  it('restores each protocol draft instead of leaking shared field values', () => {
    const openai = switchApiProtocolConfig(baseConfig, 'glm');
    ...
    const google = switchApiProtocolConfig(openaiEdited, 'glm');
    ...
    const restoredOpenai = switchApiProtocolConfig(googleEdited, 'glm');
    ...
    expect(restoredOpenai.apiProtocolConfigs?.glm).toMatchObject({
      apiKey: 'google-key',
      baseUrl: 'https://google-proxy.example.com',
      model: 'google-model',
      apiProviderBaseUrl: null,
  ...
  it('loads the new protocol default on first visit', () => {
    expect(switchApiProtocolConfig(baseConfig, 'glm')).toMatchObject({
      mode: 'api',
      apiProtocol: 'glm',
      apiKey: '',
...
describe('sanitizeSettingsSavePayload', () => {
  ...
  it('reverts execution-mode fields to initial when saving from a non-execution section with incomplete BYOK', () => {
    ...
    expect(sanitized.apiProtocol).toBe('glm');
