...
describe('SettingsDialog execution settings BYOK interactions', () => {
  ...
  it('updates model and base URL when quick fill provider changes', () => {
    renderSettingsDialog({ apiProtocol: 'glm', baseUrl: 'https://api.openai.com/v1', model: 'gpt-4o', apiProviderBaseUrl: 'https://api.openai.com/v1' });
  ...
  it('supports custom model entry in BYOK mode', async () => {
    const { onPersist } = renderSettingsDialog({ apiProtocol: 'glm', baseUrl: 'https://api.openai.com/v1', model: 'gpt-4o', apiProviderBaseUrl: 'https://api.openai.com/v1' });
...
describe('SettingsDialog MCP server interactions', () => {
  const installInfo = {
    ...
    args: [
      '/Applications/Open Design.app/Contents/Resources/app/node_modules/@z-design/daemon/dist/cli.js',
      'mcp',
      '--daemon-url',
...
describe('SettingsDialog language interactions', () => {
  afterEach(() => {
    cleanup();
    window.localStorage.removeItem('z-design:locale');
  ...
  it('switches locale immediately and updates localStorage', async () => {
    ...
    expect(window.localStorage.getItem('z-design:locale')).toBe('zh-CN');
  ...
  it('sets rtl direction for rtl locales', async () => {
    ...
    expect(window.localStorage.getItem('z-design:locale')).toBe('fa');
  ...
  it('does not route language changes through autosave and closing does not revert an applied locale', async () => {
    ...
    expect(window.localStorage.getItem('z-design:locale')).toBe('de');
...
describe('SettingsDialog about interactions', () => {
  ...
  it('opens the releases page when the latest release info is stale', async () => {
    ...
    fetchLatestGithubReleaseInfoMock.mockResolvedValue({
      tagName: 'v0.4.1',
      htmlUrl: 'https://github.com/Aqu-210L/Z-design/releases/tag/v0.4.1',
      stale: true,
    });
    ...
    await waitFor(() => {
      expect(openSpy).toHaveBeenCalledWith(
        'https://github.com/Aqu-210L/Z-design/releases',
        '_blank',
        'noopener,noreferrer',
      );
    });
