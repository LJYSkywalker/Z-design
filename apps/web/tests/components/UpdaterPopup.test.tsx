...
import type { ZDesignHostUpdaterStatusListener, ZDesignHostUpdaterStatusSnapshot } from '@z-design/host';
import { installMockZDesignHost } from '@z-design/host/testing';

import { UpdaterPopup } from '../../src/components/UpdaterPopup';
import { I18nProvider } from '../../src/i18n';

function idleStatus(): ZDesignHostUpdaterStatusSnapshot {
  return {
    arch: 'arm64',
    capabilities: {
...
function downloadedStatus(overrides: Partial<ZDesignHostUpdaterStatusSnapshot> = {}): ZDesignHostUpdaterStatusSnapshot {
  return {
    ...idleStatus(),
...
function payloadDownloadedStatus(overrides: Partial<ZDesignHostUpdaterStatusSnapshot> = {}): ZDesignHostUpdaterStatusSnapshot {
...
describe('UpdaterPopup', () => {
  ...
  it('stays hidden for non-installable updater states', async () => {
    for (const status of [
      ...
      restoreHost = installMockZDesignHost({
        host: {
          updater: {
  ...
  it('keeps the prompt in handoff loading after opening the installer', async () => {
    ...
    let resolveInstall: (status: ZDesignHostUpdaterStatusSnapshot) => void = () => undefined;
    const install = vi.fn(() => new Promise<ZDesignHostUpdaterStatusSnapshot>((resolve) => {
  ...
  it('reacts to updater subscription events by showing the ready indicator only', async () => {
    const listeners = new Set<ZDesignHostUpdaterStatusListener>();
    restoreHost = installMockZDesignHost({
      host: {
        updater: {
