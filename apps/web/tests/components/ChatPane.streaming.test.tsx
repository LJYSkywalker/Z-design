    ...
    <>
      ...
      {onShareToZDesign ? (
        <button
          ...
          onClick={onShareToZDesign}
        >
          {shareToZDesignBusy ? 'Preparing package…' : 'Share to Open Design'}
        </button>
      ) : null}
    </>
  ),
  ...
  it('keeps Share to Open Design busy on the assistant turn that started packaging', () => {
    ...
    const { rerender } = render(
      <ChatPane
        ...
        shareToZDesignBusyMessageId={null}
      />,
    );

    fireEvent.click(screen.getByTestId('share-to-od-assistant-1'));
    expect(onShareToZDesign).toHaveBeenCalledWith('assistant-1');
    ...
    rerender(
      <ChatPane
        ...
        shareToZDesignBusyMessageId="assistant-1"
      />,
    );
