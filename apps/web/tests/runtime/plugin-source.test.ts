...
describe('derivePluginSourceLinks · url + local + bundled sources', () => {
  ...
  it('routes bundled official sources to the Open Design repo', () => {
    ...
    expect(out.sourceUrl).toBe('https://github.com/Aqu-210L/Z-design');
    expect(out.sourceKindLabel).toBe('Official');
    expect(out.sourceLabel).toBe('nexu-io/open-design');
    expect(out.authorProfileUrl).toBe('https://github.com/Aqu-210L/Z-design');
    expect(out.homepageUrl).toBe('https://github.com/Aqu-210L/Z-design');
  });
});
    ...
    const out = derivePluginSourceLinks(
      makeRecord({
        ...
        manifest: {
          name:    'p',
          version: '1.0.0',
          homepage: 'https://github.com/Aqu-210L/Z-design',
        } as InstalledPluginRecord['manifest'],
      }),
    );
    expect(out.contributeUrl).toBe('https://github.com/Aqu-210L/Z-design/issues/new');
    expect(out.contributeOnGithub).toBe(true);
    expect(out.homepageUrl).toBe('https://github.com/Aqu-210L/Z-design');
  });
