...
describe('resolveStandaloneServerEntry', () => {
  it('resolves the traced monorepo standalone server entry', async () => {
    const previousDistDir = process.env.ZD_WEB_DIST_DIR;
    delete process.env.ZD_WEB_DIST_DIR;
    ...
    try {
      ...
      if (previousDistDir == null) {
        ...
        process.env.ZD_WEB_DIST_DIR = previousDistDir;
      }
...
describe('createStandaloneServerArgs', () => {
  ...
  it('uses a data import that exits when the recorded parent disappears', () => {
    const importSpecifier = createStandaloneParentMonitorImport('ZD_TEST_PARENT_PID');
    ...
    expect(source).toContain('process.env["ZD_TEST_PARENT_PID"]');
...
describe('standalone backend binding', () => {
  it('keeps the hidden standalone backend on loopback even when the public sidecar host is wider', () => {
    const env = createStandaloneBackendEnv({
      baseEnv: { ...process.env, ZD_HOST: '0.0.0.0' },
      parentPid: 1234,
      port: 5876,
    });
    ...
    expect(env.ZD_STANDALONE_PARENT_PID).toBe('1234');
  });

  it('accepts a listening standalone backend before the app root responds to HTTP', async () => {
    const previousOutputMode = process.env.ZD_WEB_OUTPUT_MODE;
    const previousStandaloneRoot = process.env.ZD_WEB_STANDALONE_ROOT;
    const previousStartupTimeout = process.env.ZD_STANDALONE_STARTUP_TIMEOUT_MS;
...
`,
      ...
      process.env.ZD_WEB_OUTPUT_MODE = 'standalone';
      process.env.ZD_WEB_STANDALONE_ROOT = standaloneRoot;
      process.env.ZD_STANDALONE_STARTUP_TIMEOUT_MS = '500';
      ...
      if (previousOutputMode == null) delete process.env.ZD_WEB_OUTPUT_MODE;
      else process.env.ZD_WEB_OUTPUT_MODE = previousOutputMode;
      if (previousStandaloneRoot == null) delete process.env.ZD_WEB_STANDALONE_ROOT;
      else process.env.ZD_WEB_STANDALONE_ROOT = previousStandaloneRoot;
      if (previousStartupTimeout == null) delete process.env.ZD_STANDALONE_STARTUP_TIMEOUT_MS;
      else process.env.ZD_STANDALONE_STARTUP_TIMEOUT_MS = previousStartupTimeout;
...
`,
      ...
      process.env.ZD_WEB_OUTPUT_MODE = 'standalone';
      process.env.ZD_WEB_STANDALONE_ROOT = standaloneRoot;
      process.env.ZD_STANDALONE_STARTUP_TIMEOUT_MS = '1000';
...
describe('resolveNextBundlerOptions', () => {
  it('uses webpack for local dev by default to avoid stale Turbopack chunk graphs', () => {
    const previous = process.env.ZD_WEB_DEV_BUNDLER;
    delete process.env.ZD_WEB_DEV_BUNDLER;

    try {
      expect(resolveNextBundlerOptions(true)).toEqual({ webpack: true });
    } finally {
      if (previous == null) delete process.env.ZD_WEB_DEV_BUNDLER;
      else process.env.ZD_WEB_DEV_BUNDLER = previous;
    }
  });

  it('lets local developers explicitly opt back into Turbopack', () => {
    const previous = process.env.ZD_WEB_DEV_BUNDLER;
    process.env.ZD_WEB_DEV_BUNDLER = 'turbopack';
...
describe('normalizeDaemonProxyOriginHeader', () => {
  ...
  it('normalizes matching wildcard configured dev origins to the daemon origin', () => {
    const previous = process.env.ZD_ALLOWED_DEV_ORIGINS;
    process.env.ZD_ALLOWED_DEV_ORIGINS = '*.local-origin.dev';
    try {
      expect(
        normalizeDaemonProxyOriginHeader({
          daemonOrigin: 'http://127.0.0.1:7456',
      ...
      if (previous == null) delete process.env.ZD_ALLOWED_DEV_ORIGINS;
      else process.env.ZD_ALLOWED_DEV_ORIGINS = previous;
    }
  });
