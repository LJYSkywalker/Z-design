...
const HOST = process.env.ZD_HOST || "127.0.0.1";
if (process.env.ZD_HOST != null && !/^[a-zA-Z0-9._\-:[\]@]+$/.test(process.env.ZD_HOST)) {
  throw new Error(`ZD_HOST contains invalid characters: ${process.env.ZD_HOST}`);
}
...
const WEB_OUTPUT_MODE_ENV = "ZD_WEB_OUTPUT_MODE";
const WEB_STANDALONE_ROOT_ENV = "ZD_WEB_STANDALONE_ROOT";
const STANDALONE_PARENT_PID_ENV = "ZD_STANDALONE_PARENT_PID";
const STANDALONE_STARTUP_TIMEOUT_ENV = "ZD_STANDALONE_STARTUP_TIMEOUT_MS";
...
export function resolveNextBundlerOptions(isDev: boolean): NextBundlerOptions {
  if (!isDev) return {};
  const configured = (process.env.ZD_WEB_DEV_BUNDLER ?? "webpack").trim().toLowerCase();
...
function resolveWebRoot(): string {
  ...
  for (let depth = 0; depth < 8; depth += 1) {
    try {
      ...
      if (packageJson.name === "@z-design/web") return current;
    } catch {
  ...
  throw new Error("failed to resolve @z-design/web package root");
}
...
function configuredAllowedDevHosts(): Set<string> {
  return new Set(
    (process.env.ZD_ALLOWED_DEV_ORIGINS ?? "")
      .split(",")
...
async function startStandaloneBackend(webRoot: string | null): Promise<StandaloneBackend> {
  ...
  if (entryPath == null) {
    throw new Error(
      webRoot == null
        ? `missing Next.js standalone server under ${WEB_STANDALONE_ROOT_ENV}; configure ${WEB_STANDALONE_ROOT_ENV} or install @z-design/web`
...
async function startRegularNextSidecar(
  ...
  const dev = process.env.ZD_WEB_PROD !== "1" && runtime.mode === "dev";
