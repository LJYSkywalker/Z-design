import {
  ...
  type ZDesignHostActionResult,
  type ZDesignHostFailure,
  type ZDesignHostUpdaterActionOptions,
  type ZDesignHostUpdaterResult,
  type ZDesignHostUpdaterStatusListener,
  type ZDesignHostUpdaterStatusSnapshot,
} from '@z-design/host';
...
export type UpdaterActionResult =
  | { ok: true; model: UpdaterModel; status: ZDesignHostUpdaterStatusSnapshot }
  | ZDesignHostFailure;
...
export type UpdaterModel = {
  ...
  status: ZDesignHostUpdaterStatusSnapshot | null;
  supported: boolean;
};

function modelFromHostResult(result: ZDesignHostUpdaterResult): UpdaterActionResult {
...
export function deriveUpdaterModel(
  ...
  const hostAvailable = options.hostAvailable ?? isZDesignHostAvailable();
...
export async function readUpdaterStatus(options?: ZDesignHostUpdaterActionOptions): Promise<UpdaterActionResult> {
  return modelFromHostResult(await getHostUpdaterStatus(options));
}

export async function checkForUpdaterUpdate(options?: ZDesignHostUpdaterActionOptions): Promise<UpdaterActionResult> {
  return modelFromHostResult(await checkHostUpdater(options));
}

export async function downloadUpdaterUpdate(options?: ZDesignHostUpdaterActionOptions): Promise<UpdaterActionResult> {
  return modelFromHostResult(await downloadHostUpdater(options));
}

export async function openUpdaterInstaller(options?: ZDesignHostUpdaterActionOptions): Promise<UpdaterActionResult> {
...
export function subscribeToUpdaterStatus(listener: ZDesignHostUpdaterStatusListener): () => void {
