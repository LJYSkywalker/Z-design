...
export type { PluginInstallOutcome } from '@z-design/contracts';
export type { PluginShareAction } from '@z-design/contracts';
...
export async function createProject(input: {
  ...
  try {
    ...
    // when `crypto.randomUUID` is unavailable. Z-Design served over
...
const PROJECT_TABS_CACHE_PREFIX = 'z-design:project-tabs:v1:';
...
export interface PluginShareTaskStart {
  taskId: string;
  action: 'publish-github' | 'contribute-z-design';
  path: string;
...
export async function contributeGeneratedPluginToZDesign(
  projectId: string,
  relativePath: string,
): Promise<PluginShareOutcome> {
  return postGeneratedPluginShareAction(projectId, relativePath, 'contribute-z-design');
}
