...
import type { AmrModelsResponse, ChatSessionMode } from '@z-design/contracts';
...
const APP_CONFIG_CHANGED_EVENT = 'z-design:app-config-changed';
...
function AppInner() {
  ...
  // Loading Z-Design…</div>`) is itself >MIN_VISIBLE_TEXT and would
  ...
  const handleImportFolderResponse = useCallback(async (result: ZDesignHostProjectImportSuccess) => {
  ...
  if (pendingFirstRunOnboardingRoute) {
    ...
    appMain = (
      <EntryView
        ...
        onZDesignSystem={(id: string) => navigate({ kind: 'design-system-detail', designSystemId: id })}
        onDesignSystemsRefresh={refreshDesignSystems}
