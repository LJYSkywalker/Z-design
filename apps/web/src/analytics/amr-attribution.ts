...
const AMR_ATTRIBUTION_STORAGE_KEY = 'z-design:amr-entry-attribution:v1';
...
// Builds the AMR handoff URL with Z-Design attribution params. When
// `deviceId` is provided it is added as `od_device_id`, so AMR can link the
// landing/registration directly back to this Z-Design install instead of
// only through the one-shot entry id. The caller passes it ONLY when the user
// has consented to metrics: AMR is Z-Design's official model service, so
...
async function mirrorAmrEntryToAmrAnalytics(
  ...
  try {
    ...
    // AMR analytics mirroring must never block the primary Z-Design action.
  }
}
