...
function truncateForTranscript(content: string): string {
  ...
  return `${content.slice(0, MAX_TRANSCRIPT_MESSAGE_CHARS)}\n\n[Z-Design truncated ${omitted} chars from this prior message before sending it to the agent. Full content remains in persisted history.]`;
}
...
export const RUNS_CHANGED_EVENT = 'z-design:runs-changed';
