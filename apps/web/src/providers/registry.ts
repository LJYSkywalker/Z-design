import type {
  ...
  ZDesignGithubLatestReleaseResponse,
...
function popupBlockedMessage(): string {
  return 'Popup blocked. Allow popups for Z-Design and try again.';
}
...
export async function connectConnector(connectorId: string): Promise<ConnectorActionResult> {
  ...
  const useExternalBrowser = isZDesignHostAvailable();
  try {
...
export async function fetchLatestGithubReleaseInfo(): Promise<LatestGithubReleaseInfo | null> {
  try {
    ...
    if (!resp.ok) return null;
    const json = (await resp.json()) as Partial<ZDesignGithubLatestReleaseResponse>;
...
export async function fetchHostEditors(): Promise<
  import('@z-design/contracts').HostEditorsResponse
> {
  const resp = await fetch('/api/editors');
  if (!resp.ok) throw new Error(`GET /api/editors failed: ${resp.status}`);
  return (await resp.json()) as import('@z-design/contracts').HostEditorsResponse;
}

export async function openProjectInEditor(
  projectId: string,
  editorId: import('@z-design/contracts').HostEditorId,
): Promise<import('@z-design/contracts').OpenProjectInEditorResponse> {
  const resp = await fetch(
  ...
  return (await resp.json()) as import('@z-design/contracts').OpenProjectInEditorResponse;
}
