...
function inferProviderFromModel(_modelId: string): MemoryExtractionProvider {
  // GLM is the sole memory-extraction provider for Z-Design. The legacy
  // model-id heuristics (claude→anthropic, gemini→google, else→openai) no
  // longer apply.
  return 'glm';
}

// Map a CLI agent id to the API protocol family it speaks. GLM is the sole
// memory-extraction provider, so any active agent routes to 'glm'. Kept as
// a function because the call site branches on a non-null result to label
// the "Same as chat" memory default.
...
export function MemoryModelInline({
    ...
    (modelId: string): MemoryExtractionConfigShape => {
      ...
      if (mode === 'api') {
        return {
          ...
          apiKey: chatApiKey,
          // GLM has no Azure-style api-version.
          apiVersion: '',
        };
      }
      const provider =
  ...
  useEffect(() => {
    ...
    // GLM has no Azure-style api-version; the legacy azure branch was always ''.
    const azureVersion = '';
    const drift =
