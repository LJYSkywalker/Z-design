...
export function buildPluginFolderAgentActionPrompt(
  ...
  return [
    ...
    `Manifest: \`${folderPath}/z-design.json\``,
    '',
...
function buildContributePrompt(folderPath: string): string {
  return [
    'Open a draft Pull Request that adds this generated plugin to the Z-Design community catalog at `nexu-io/open-design`.',
    ...
    `Manifest: \`${folderPath}/z-design.json\``,
    '',
    'Run this deterministic Z-Design CLI workflow from the current project workspace:',
    '',
    `\`"$ZD_NODE_BIN" "$ZD_BIN" plugin open-design-pr ${folderPath}\``,
    '',
...
// Z-Design issue URL and belongs to the "Z-Design PR" button. Before
...
function buildPublishPrompt(folderPath: string): string {
  return [
    ...
    `Manifest: \`${folderPath}/z-design.json\``,
    '',
    'This is the **repository publish** action, NOT the registry-submission action — do NOT route through `od plugin publish --to open-design`. That command emits an Z-Design issue URL and belongs to the "Z-Design PR" button.',
    '',
    'Run this deterministic Z-Design CLI workflow from the current project workspace:',
    '',
    `\`"$ZD_NODE_BIN" "$ZD_BIN" plugin publish-repo ${folderPath}\``,
    '',
