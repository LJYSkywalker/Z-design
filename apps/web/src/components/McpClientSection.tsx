...
//
// Z-Design connects to the configured servers as a CLIENT and surfaces
...
import type { TrackingExternalMcpElement } from '@z-design/contracts/analytics';
import {
  disconnectMcpOAuth,
  fetchMcpOAuthStatus,
  fetchMcpServers,
...
// so the most useful categories for Z-Design (visual generation, then
      ...
      {expanded ? (
        <>
          ...
          {isHttpLike && !row._isNew && row.id ? (
            usesManagedOAuth ? (
              ...
              <div className="mcp-oauth-hint hint">
                <strong>No managed OAuth.</strong> Z-Design will use this
          ...
          {isHttpLike && row._isNew && usesManagedOAuth ? (
            <div className="mcp-oauth-hint hint">
              Save first, then click <strong>Connect</strong> to grant Z-Design
              access via the provider's OAuth flow.
            </div>
          ) : null}
          {isHttpLike && row._isNew && !usesManagedOAuth ? (
            <div className="mcp-oauth-hint hint">
              <strong>No managed OAuth.</strong> Save this server and Z-Design
        ...
        {connected ? (
          ...
          <>
            ...
            <span>
              ...
              <span className="hint">
                Click Connect to grant Z-Design access via the provider's OAuth flow.
              </span>
            </span>
          </>
        )}
