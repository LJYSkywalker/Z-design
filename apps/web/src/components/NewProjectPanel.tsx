...
import { Dialog, DialogDescription, DialogFooter, DialogTitle } from '@z-design/components';
import { createTabToTracking } from '@z-design/contracts/analytics';
import { isZDesignHostAvailable, pickHostWorkingDir } from '@z-design/host';
import type { ZDesignHostProjectImportSuccess } from '@z-design/host';
...
import type { ConnectorDetail } from '@z-design/contracts';
import type {
...
interface Props {
  ...
  onImportFolderResponse?: (response: ZDesignHostProjectImportSuccess) => Promise<void> | void;
  ...
  async function handlePickWorkingDir() {
    ...
    try {
      if (isZDesignHostAvailable()) {
        ...
        setWorkingDirError({
          message: `Couldn't open the folder picker (${'reason' in result ? result.reason : 'host unavailable'}). Please update Z-Design and try again.`,
        });
        return;
      }
...
function HighFidelityArt() {
  return (
    <svg viewBox="0 0 120 70" width="100%" height="100%" aria-hidden>
      <rect x="6" y="8" width="34" height="6" rx="2" fill="var(--text-strong)" />
      <rect x="6" y="20" width="46" height="4" rx="2" fill="var(--text-muted)" />
      <rect x="6" y="28" width="42" height="4" rx="2" fill="var(--text-faint)" />
      <rect x="6" y="40" width="22" height="9" rx="2" fill="var(--accent)" />
      <rect x="64" y="8" width="50" height="54" rx="4" fill="var(--accent-tint)" />
      <rect x="70" y="14" width="38" height="4" rx="2" fill="var(--accent)" />
      <rect x="70" y="22" width="32" height="3" rx="1.5" fill="var(--text-muted)" />
      <rect x="70" y="29" width="36" height="3" rx="1.5" fill="var(--text-faint)" />
