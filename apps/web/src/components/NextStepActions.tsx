...
interface Props {
  ...
  // Contribute the artifact to the Z-Design community gallery.
...
export function NextStepActions({
  ...
  const handleContribute = useCallback(() => {
    if (!onShareToZDesign || shareToZDesignBusy) return;
    track('share_to_open_design');
    onShareToZDesign();
    closeAll();
  }, [closeAll, onShareToZDesign, shareToZDesignBusy, track]);
  ...
  const canContribute = !!onShareToZDesign;
            ...
            >
              ...
              {canContribute ? (
                <button
                  ...
                  disabled={shareToZDesignBusy}
                  onClick={handleContribute}
                >
                  <Icon
                    name={shareToZDesignBusy ? 'spinner' : 'globe'}
                    size={14}
                    className={shareToZDesignBusy ? 'icon-spin' : styles.toolboxRowIcon}
                  />
