...
import { VisuallyHidden } from '@z-design/components';
          ...
          {onShareAction ? (
            ...
            >
              ...
              <button
                type="button"
                className="plugins-home__action plugins-home__action--secondary plugins-home__action--compact"
                onClick={() => onShareAction(record, 'contribute-z-design')}
                disabled={pendingAny || shareBusy}
                aria-busy={sharePendingAction === 'contribute-z-design' ? 'true' : undefined}
                aria-label={`Contribute ${title} to Z-Design`}
                title="Contribute plugin to Z-Design with a pull request"
                data-testid={`plugins-home-contribute-z-design-${record.id}`}
              >
                <Icon
                  name={sharePendingAction === 'contribute-z-design' ? 'spinner' : 'share'}
                  size={12}
                />
                <span>{sharePendingAction === 'contribute-z-design' ? 'Starting…' : 'Contribute'}</span>
              </button>
            </div>
          ) : null}
        </div>
