...
interface ZDesignDesktopApi {
  exportDiagnostics(): Promise<DesktopExportResult>;
}

declare global {
  interface Window {
    zDesignDesktop?: ZDesignDesktopApi;
  }
}
 ...
 * save dialog via window.zDesignDesktop) and the browser (triggers a
...
export function ExportDiagnosticsRow() {
  ...
  const handleClick = async () => {
    ...
    setStatus({ kind: 'busy' });
    try {
      if (window.zDesignDesktop != null) {
        const result = await window.zDesignDesktop.exportDiagnostics();
        if (result.ok) {
