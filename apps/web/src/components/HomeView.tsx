...
import { Dialog, DialogFooter, DialogTitle } from '@z-design/components';
import type {
  ApplyResult,
  ChatSessionMode,
  ConnectorDetail,
...
import { DEFAULT_UNSELECTED_SCENARIO_PLUGIN_ID } from '@z-design/contracts';
import { projectKindToTracking } from '@z-design/contracts/analytics';
...
const AUTHORING_DEFAULT_SCENARIO_INPUTS = {
  artifactKind: 'Z-Design plugin',
  audience: 'Z-Design plugin authors',
  topic: 'packaging a reusable workflow as an Z-Design plugin',
};
...
export function HomeView({
  ...
  useEffect(() => {
    ...
    load();
    window.addEventListener('z-design:plugins-changed', load);
    return () => {
      cancelled = true;
      window.removeEventListener('z-design:plugins-changed', load);
    };
  }, []);
  ...
  async function handlePickWorkingDir() {
    ...
    if (isZDesignHostAvailable()) {
      ...
      setError(
        `Couldn't open the folder picker (${'reason' in result ? result.reason : 'host unavailable'}). Please update Z-Design and try again.`,
      );
      return;
    }
