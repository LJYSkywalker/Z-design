...
function SkillPluginCandidateCard({
  ...
  async function createDraft() {
    ...
    try {
      ...
      if (data?.validation?.ok === false) {
        ...
        if (install?.ok === false) {
          ...
          if (typeof window !== "undefined") {
            window.dispatchEvent(new CustomEvent("z-design:plugins-changed"));
          }
      ...
      if (draftPath && onRequestOpenFile) onRequestOpenFile(`${draftPath}/z-design.json`);
    } catch (err) {
  ...
  async function share(action: "contribute-z-design") {
    if (!projectId) return;
    setBusy("contribute");
    setNotice(null);
    try {
      ...
      setNotice({
        message: `Z-Design contribution task started for ${data?.path ?? "the draft"}.`,
      });
    } catch (err) {
      setNotice({ message: err instanceof Error ? err.message : String(err) });
    } finally {
        ...
        <div className="plugin-action-card__body">
          ...
          <div className="plugin-action-card__actions">
            <button
              ...
              disabled={disabled}
              onClick={() => void share("contribute-z-design")}
            >
...
interface Props {
  ...
  // Click handler for the post-completion "Share to Z-Design" submission
  // action. ProjectView wires this to handleSend with the bundled
  // `od-share-to-community` trigger prompt.
  onShareToZDesign?: () => void;
  shareToZDesignBusy?: boolean;
...
function AssistantMessageImpl({
  ...
  const showCompletionRow =
    ...
    !!copyMarkdown ||
    canFork;
  const canShowZDesignSubmission = !!onShareToZDesign && showFeedback && runSucceeded;
  const showZDesignSubmission =
    canShowZDesignSubmission && (!!isLast || shareToZDesignBusy);
  ...
  const showNextStepActions =
    ...
    ((!!isLast && !!onToolboxAction) || showZDesignSubmission);
      ...
      <div className="assistant-flow">
        ...
        {showNextStepActions ? (
          <NextStepActions
            ...
            onShareToZDesign={showZDesignSubmission ? onShareToZDesign : undefined}
            shareToZDesignBusy={shareToZDesignBusy}
          />
        ) : null}
      </div>
    </div>
...
function hasPluginFinalActionHint(content: string): boolean {
  return /\b(Add to My plugins|Z-Design PR|Publish repo|plugin publish|ready to publish|ready to add)\b/i.test(
    content,
  );
}
