...
import type { AppliedPluginSnapshot, ChatSessionMode, WorkspaceContextItem } from '@z-design/contracts';
import type { TrackingProjectKind } from '@z-design/contracts/analytics';
import {
...
interface Props {
  ...
  // "Share to Z-Design" button on each completed assistant message —
  // wired by ProjectView to handleSend with the bundled
  // `od-share-to-community` scenario's trigger prompt.
  onShareToZDesign?: (assistantMessageId: string) => void;
  shareToZDesignBusyMessageId?: string | null;
          ...
          <div className="chat-log-wrap">
            ...
            >
              ...
              <ChatRows
                ...
                onShareToZDesign={onShareToZDesign}
...
interface AssistantCallbacks {
  ...
  onShareToZDesign: ((assistantMessageId: string) => void) | undefined;
}
  ...
  const renderItem = (item: ChatRenderItem) => {
    ...
    return (
      <AssistantMessage
        ...
        onShareToZDesign={
          onShareToZDesign
            ? () => assistantCallbacksRef.current.onShareToZDesign?.(m.id)
            : undefined
        }
        shareToZDesignBusy={shareToZDesignBusyMessageId === m.id}
...
const QUEUED_SEND_DRAG_MIME = 'application/x-z-design-queued-send';
...
export function buildRunErrorDiagnosticText(input: RunErrorDiagnosticInput): string {
  const lines = [
    'Z-Design run error diagnostics',
