...
import { normalizeCustomReason } from '@z-design/contracts/analytics';
import {
...
import type { AppliedPluginSnapshot, ChatAnalyticsEntryFrom, ChatSessionMode, InstalledPluginRecord, WorkspaceContextItem } from '@z-design/contracts';
import type {
  AgentEvent,
  AgentInfo,
  AppConfig,
...
function historyWithWorkspaceContext(
  ...
  const block = [
    ...
    'Z-Design selected the currently focused workspace tab as the default context for this turn.',
...
export function ProjectView({
  ...
  const handlePluginFolderAgentAction = useCallback(
    async (relativePath: string, action: PluginFolderAgentAction) => {
      ...
      const shareAction = action === 'publish' ? 'publish-github' : 'contribute-z-design';
      setActivePluginActionPaths((prev) => new Set(prev).add(relativePath));
      let taskStart;
      try {
  ...
  // "Share to Z-Design" — kicks off the bundled `od-share-to-community`
  ...
  const [shareToZDesignBusyMessageId, setShareToZDesignBusyMessageId] = useState<string | null>(null);
  const shareToZDesignBusyMessageIdRef = useRef<string | null>(null);
  useEffect(() => {
    if (!shareToZDesignBusyMessageIdRef.current || currentConversationBusy) return;
    shareToZDesignBusyMessageIdRef.current = null;
    setShareToZDesignBusyMessageId(null);
  }, [currentConversationBusy]);
  const handleShareToZDesign = useCallback((assistantMessageId: string) => {
    if (currentConversationActionDisabled || shareToZDesignBusyMessageIdRef.current) return;
    shareToZDesignBusyMessageIdRef.current = assistantMessageId;
    setShareToZDesignBusyMessageId(assistantMessageId);
  ...
  // `z-design:config` localStorage blob and stays in sync with the
  // platform `storage` event (cross-tab) plus the same-tab
  // `z-design:critique-theater-toggle` CustomEvent. The mount itself
        ...
        <div className="split-chat-slot" hidden={workspaceFocused}>
          {commentInspectorActive ? (
            ...
            <ChatPane
              ...
              onShareToZDesign={handleShareToZDesign}
              shareToZDesignBusyMessageId={shareToZDesignBusyMessageId}
...
function pluginWorkflowTitle(action: PluginFolderAgentAction): string {
  return action === 'publish' ? 'Publish repo' : 'Z-Design PR';
}
...
function pluginWorkflowPlannedSteps(action: PluginFolderAgentAction): string[] {
  ...
  return [
    'Ensure the Z-Design fork exists',
