// Content fixtures for the "Use Z-Design everywhere" guide modal.
//
...
export const GUIDE_SECTIONS: GuideSection[] = [
  {
    id: 'overview',
    tabLabel: 'Overview',
    heading: 'Z-Design works wherever your agent works',
    intro:
      'Z-Design is more than a window — it is a local privileged daemon ' +
    ...
    bullets: [
      ...
      'MCP server — wires Z-Design as a Model Context Protocol server so any MCP-capable agent can list skills, run scenarios, and read artifacts.',
      'HTTP API — `http://127.0.0.1:7456/api/*` REST + SSE endpoints; the same surface the web UI uses.',
      'Skills — drop-in `SKILL.md` packs (Claude-compatible) that any agent already on your PATH can invoke without Z-Design at all.',
    ...
    snippets: [
      ...
      },
      {
        ...
        body:
          ...
          '# Then start Z-Design in the shell you normally use for dev:\n' +
          'pnpm tools-dev',
      },
    ],
    footer:
      'The daemon writes to `./.od/` (project-local) by default. Set ' +
      '`ZD_DATA_DIR=~/.open-design` to share data across projects.',
  },
  {
    id: 'cli',
    tabLabel: 'CLI · od',
    heading: 'Drive Z-Design from any shell',
    intro:
    ...
    snippets: [
      ...
      },
      {
        ...
        body:
          ...
          '\n' +
          'export ZD_NODE_BIN="${ZD_NODE_BIN:-/opt/homebrew/opt/node@24/bin/node}"\n' +
          'export ZD_BIN="$PWD/apps/daemon/dist/cli.js"\n' +
          '"$ZD_NODE_BIN" "$ZD_BIN" daemon start --headless --serve-web --port 7456',
      },
      {
    ...
    footer:
      ...
      '`"$ZD_NODE_BIN" "$ZD_BIN"` after exporting those variables.',
  },
  {
    id: 'mcp',
    tabLabel: 'MCP server',
    heading: 'Expose Z-Design as an MCP server to any coding agent',
    intro:
      'Z-Design ships with a Model Context Protocol server (`od mcp`) ' +
      'that lets any MCP-capable client — Cursor, Claude Code, Antigravity, ' +
      'VS Code Copilot Chat, openclaw, hermes — discover Z-Design tools ' +
    ...
    bullets: [
      ...
      'Pins `ZD_DATA_DIR` so the spawned MCP process writes to the same place the daemon already uses (avoids EPERM in packaged macOS app bundles).',
    ],
    snippets: [
      {
    ...
    footer:
      'In the Z-Design app, open Settings → Integrations to copy a ' +
  ...
  },
  {
    ...
    intro:
      ...
      'Server-Sent Events with the contract types in `@z-design/contracts`.',
    bullets: [
    ...
    footer:
      ...
      '`@z-design/contracts` — import them in your script for full ' +
      'autocomplete without wiring a generator.',
  },
  {
    id: 'skills',
    tabLabel: 'Skills & headless',
    heading: 'Drop-in Skills for any agent — even without Z-Design running',
    intro:
      'A Skill is a directory with a Claude-compatible `SKILL.md` ' +
      '(YAML front-matter + body). Z-Design extends the format with the ' +
      '`od:` namespace (`mode`, `preview`, `design_system`, `inputs`, …) so ' +
      'the same artifact can be used both inside Z-Design and by a vanilla ' +
    ...
    snippets: [
      {
        label: 'Minimal SKILL.md (Claude-compatible front matter + Z-Design extras)',
        language: 'yaml',
        body:
          '---\n' +
      ...
      },
      {
        ...
        body:
          ...
          '# Shell 1: start Z-Design after ingesting.\n' +
          'pnpm tools-dev\n' +
