...
import { Button, Textarea } from '@z-design/components';
import type { ConnectorConnectResponse, ConnectorDetail, ConnectorStatusResponse } from '@z-design/contracts';
...
const CONNECTOR_CALLBACK_MESSAGE_TYPE = 'z-design:connector-connected';
          ...
          <Button
            ...
            onClick={() => {
              ...
              if (!state.company.trim()) {
                setError('Tell Z-Design about the company or design system first.');
                return;
              }
              setStep('confirm');
            }}
  ...
  const sendProjectChatMessage = useCallback(
    async (
      ...
      if (config.mode !== 'daemon' || !config.agentId) {
        setChatError('Pick a local agent first, then ask Z-Design to update this design system.');
        return;
      }
          ...
          <div className="ds-review-column">
              ...
              <p>
                {generationActive
                  ? activeJob?.kind === 'token-contract-rebuild'
                    ? 'Z-Design is preparing a token contract rebuild for review. The active contract stays unchanged until you accept it.'
                    : activeJob?.kind === 'revision'
                      ? 'Z-Design is applying your feedback. You can keep reviewing while the updated draft is prepared.'
                      : 'Z-Design is still working, but you can start giving feedback on the work so far.'
                  : 'Z-Design is ready for review. Give feedback on the work so far, then publish when it is useful for future projects.'}
              </p>
              <label>
                <input
                  type="checkbox"
            ...
            <div className="ds-warning-card">
              ...
              <span>
                ...
                Z-Design is rendering typography with substitute web fonts.
              </span>
    ...
    <section className={`ds-workspace-activity-card is-${status}`}>
      <div className="ds-workspace-activity-head">
        ...
        <span>
          <strong>
            {status === 'running'
              ? 'Z-Design is updating this system'
              : status === 'failed'
          ...
          <strong>
            {active
              ? job.kind === 'token-contract-rebuild'
                ? 'Z-Design is rebuilding tokens'
                : job.kind === 'revision'
                  ? 'Z-Design is revising'
                  : 'Z-Design is still working'
              : job.status === 'failed'
...
function GitHubRepositoryAccessPanel({
  ...
  const methods: GitHubAccessMethod[] = [
    ...
    },
    {
      id: 'native-oauth',
      icon: 'link',
      title: 'Z-Design account',
      badge: 'Coming soon',
      tone: 'muted',
      description: 'Native GitHub sign-in managed by Z-Design; this build does not use an OD-managed GitHub token yet.',
    },
    {
      id: 'composio',
      icon: composioIcon,
    ...
    >
      <div className="ds-github-access-header">
        <span>
          ...
          <p>Paste a GitHub URL. Z-Design will use the first working access method.</p>
        </span>
        <button
          type="button"
...
function buildCreationAgentPrompt(
  ...
  const title = inferDesignSystemTitle(state);
  return [
    'Create this project as a complete Z-Design design system workspace.',
    '',
    ...
    state.codeFolders.length
      ? `Read the linked local code folders that Z-Design attached to this project: ${state.codeFolders.join(', ')}. Treat them as source context only unless the user asks you to edit them.\n\n${localFolderRunbook}`
      : '',
...
function buildSourceContextManifest(
  ...
  sections.push(
    ...
    '- README.md and SKILL.md should make the extracted system reusable as a real Z-Design design-system package.',
    ...
    '- Before marking the design system ready, run `"$ZD_NODE_BIN" "$ZD_BIN" tools connectors design-system-package-audit --path . --fail-on-warnings` and fix every reported error or warning.',
...
function buildLocalFolderRunbook(folders: string[]): string {
  ...
  const intakeCommands = folders
    .map((folder, index) => `   - \`"$ZD_NODE_BIN" "$ZD_BIN" tools connectors local-design-context --path ${shellQuote(folder)} --output context/local-code/${localEvidenceFileName(folder, index)}\``)
    .join('\n');
  return [
...
function buildGithubConnectorRunbook(githubUrls: string[]): string {
  ...
  const intakeCommands = githubUrls
    .map((url) => `   - \`"$ZD_NODE_BIN" "$ZD_BIN" tools connectors github-design-context --repo ${shellQuote(url)} --output context/github/${githubEvidenceFileName(url)}\``)
    .join('\n');
  return [
