...
(() => {
  const cancel = window.__zDesignBrowserPickerCancel;
...
export function browserElementPickerScript(filePath: string): string {
  return `
(() => new Promise((resolve) => {
  const previousCancel = window.__zDesignBrowserPickerCancel;
  ...
  function cleanup(result) {
    ...
    if (hovered) hovered.classList.remove('__open_design_browser_pick_hover__');
    style.remove();
    window.__zDesignBrowserPickerCancel = null;
    resolve(result || null);
  }
  function onMove(ev) {
  ...
  window.__zDesignBrowserPickerCancel = () => cleanup(null);
