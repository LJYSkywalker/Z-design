...
import { sessionModeToTracking } from '@z-design/contracts/analytics';
import type {
...
export const ChatComposer = forwardRef<ChatComposerHandle, Props>(
  function ChatComposer(
    ...
    // behind `zDesignToolbox` until the panel subsystem is removed wholesale.
    ...
    function expandSearchCommand(input: string): { prompt: string; query: string } | null {
      ...
      return {
        ...
        prompt: [
          ...
          'POSIX: "$ZD_NODE_BIN" "$ZD_BIN" research search --query "<search query>" --max-sources 5',
          'PowerShell: & $env:ZD_NODE_BIN $env:ZD_BIN research search --query "<search query>" --max-sources 5',
          'cmd.exe: "%ZD_NODE_BIN%" "%ZD_BIN%" research search --query "<search query>" --max-sources 5',
