...
import { agentIdToTracking } from '@z-design/contracts/analytics';
...
const ONBOARDING_DROPDOWN_OPEN_EVENT = 'z-design:onboarding-dropdown-open';
...
// lives in `@z-design/contracts` so the daemon's `/api/projects`
...
interface Props {
  ...
  onZDesignSystem?: (id: string) => void;
...
function OnboardingView({
  ...
  const apiProtocol = config.apiProtocol ?? 'glm';
  ...
  const canFetchProviderModels =
    // GLM is the sole protocol; the legacy azure/ollama guards (no public
    // model list) always held, so they're dropped.
    Boolean(config.apiKey.trim()) &&
    Boolean(config.baseUrl.trim()) &&
  ...
  // Z-Design Cloud sign-in in one gesture. Mirrors the AMR card's
  ...
  // Connect step, default face: a minimal, centered Z-Design Cloud sign-in
    ...
    return (
      ...
      >
        ...
        <footer className="onboarding-cloud__footer">
          © {new Date().getFullYear()} Z-Design · {t('settings.onboardingCloudRights')}
        </footer>
      </section>
    );
  }
