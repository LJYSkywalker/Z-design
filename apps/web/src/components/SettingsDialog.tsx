...
import { Button, VisuallyHidden } from '@z-design/components';
...
// Fixed-origin gateway protocols: managed single-endpoint providers where the
// user only supplies an API key and the Base URL is implied. GLM is one — its
// origin is always open.bigmodel.cn/api/paas/v4, so the Settings form hides the
// Base URL field for it. (The canonical source is FIXED_ORIGIN_GATEWAYS in
// ../state/apiProtocols; this set mirrors it for the Settings render path.)
...
// GLM's /models endpoint returns its full catalogue, so it joins this set.
...
export function canFetchProviderModels(
  ...
  // GLM is the sole protocol; the legacy azure/ollama guards (which disabled
  // connect for protocols without a public /v1/models) no longer apply.
  void protocol;
  return (
...
function missingByokModelFetchFields(
  ...
  const missing: ByokRequiredField[] = [];
  // GLM fetches its model list behind the API key (unlike the former
  // AIHubMix public catalogue). The legacy `protocol !== 'aihubmix'` guard
  // always held for GLM, so a missing key is always blocking.
  void protocol;
...
function providerConnectionTestKey(
  ...
  return [
    ...
    config.apiKey.trim(),
    config.model.trim(),
    // GLM has no Azure-style api-version stamp; the legacy azure branch is
    // always the empty string here.
    '',
  ].join('\n');
}
...
function byokProviderRequiresApiKey(
  ...
  // GLM has no local/self-hosted no-auth mode; the legacy local-Ollama bypass
  // never applied to it.
  void protocol;
  void baseUrl;
  return true;
...
function byokFirstPartyBaseUrlHint(
  ...
  // GLM is the sole protocol; it has no first-party base-URL typo hint (the
  // legacy anthropic/openai/google hints pointed at vendor-specific hosts).
  void protocol;
  void protocolProviders;
  return undefined;
}
...
function displayAgentName(agent: Pick<AgentInfo, 'id' | 'name'>): string {
  return agent.id === 'amr' ? 'Z-Design AMR' : agent.name;
}
...
function nextApiProtocolConfig(
  ...
  if (currentConfig.apiProviderBaseUrl === null) {
    return {
      ...currentConfig,
      apiKey: '',
      // GLM has no Azure-style api-version stamp.
      apiVersion: '',
...
function applyApiProtocolConfig(
  ...
  return {
    ...
    // GLM has no Azure-style api-version stamp, and no separate BYOK media-
    // model pickers (the legacy aihubmix/senseaudio/azure branches were the
    // only consumers). All collapse to empty under the GLM-only model.
    apiVersion: '',
    byokImageModel: '',
    byokVideoModel: '',
    byokSpeechModel: '',
...
export function switchApiProtocolConfig(
  ...
  const currentProtocol = config.apiProtocol ?? 'glm';
...
export function SettingsDialog({
  ...
  const [cfg, setCfg] = useState<AppConfig>(() => ({
    ...initial,
    baseUrl: resolveFixedOriginBaseUrl(initial.apiProtocol ?? 'glm', initial.baseUrl),
  }));
  ...
  const [providerModelsCommittedKey, setProviderModelsCommittedKey] =
    useState<string | null>(() => {
      const protocol = initial.apiProtocol ?? 'glm';
      if (
        initial.mode !== 'api' ||
        // GLM is the sole protocol; the legacy azure/ollama fetch guards (no
        // public model list) are dropped — GLM fetches its list behind the key.
  ...
  const handleInstallLatest = useCallback(async () => {
    ...
    window.open('https://github.com/Aqu-210L/Z-design/releases', '_blank', 'noopener,noreferrer');
  ...
  const handleFetchProviderModels = async (
    ...
    // GLM is the sole protocol; the legacy azure/ollama unsupported-fetch
    // guards (those protocols had no public model list) are unreachable.
  ...
  const apiProtocol = cfg.apiProtocol ?? 'glm';
  ...
  useEffect(() => {
    ...
    if (visualStabilityMode) return;
    // GLM is the sole protocol; the legacy azure/ollama fetch guards (no
    // public model list) are unreachable.
    if (byokFirstPartyBaseUrl?.hostTypo) return;
    if (blockingByokDraftIssues(byokModelFetchDraftValidation).length > 0) return;
    // GLM needs a key and fetches its model list on input commit (blur), so we
    // wait until the committed key matches the live one before refetching — we
    // don't fire on each keystroke. (The legacy AIHubMix key-less immediate
    // fetch no longer applies.)
  ...
  // GLM is a fixed-origin gateway (open.bigmodel.cn), so the Base URL field is
  // read-only once a known GLM base is present. The legacy anthropic/google
  // first-party check collapses to "GLM first-party".
  const baseUrlReadOnly =
    cfg.apiProviderBaseUrl !== null &&
    Boolean(cfg.baseUrl.trim()) &&
    !baseUrlInvalid;
  // GLM has no vendor-specific placeholder; the canonical base is shown as the
  // field value, so an undefined placeholder suffices.
  const baseUrlPlaceholder = undefined;
  useEffect(() => {
            ...
            <section className="settings-section settings-section-card settings-section-byok">
              ...
              <ByokModelField
                ...
                labels={{
                  ...
                  // GLM has no Azure-style deployment-name concept; the legacy
                  // azure branches collapse to the generic model labels.
                ...
                // GLM is neither Azure (deployment-name fetch hint) nor the
                // legacy Ollama (unsupported fetch); both hints are always off.
                showAzureModelFetchHint={false}
                showFetchModelsUnsupportedHint={false}
                // GLM shows the suggested-models hint whenever no preset
                // provider is selected (the legacy azure guard always held).
              ...
              {/* GLM has no Azure api-version field, no BYOK image/video/speech
                  model pickers (those were AIHubMix/SenseAudio-only). All four
                  legacy conditional blocks collapse to null under GLM-only. */}
              {null}
              {null}
              {null}
              {null}
...
// `od` we might add to PATH, and most Z-Design users run from
 ...
 *     `z-design:config` localStorage blob; this is what gates
