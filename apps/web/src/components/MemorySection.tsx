...
function providerDisplayName(provider: MemoryExtractionRecord['provider'] | undefined): string {
  if (provider?.credentialSource === 'chat-cli') {
    // GLM is the sole extraction provider; a chat-cli sourced provider still
    // runs as GLM. Kept as a branch for credentialSource distinction.
    return 'Local CLI';
  }
  // GLM is the sole kind; the legacy per-vendor display names collapse to GLM.
...
function describeExtractionFailure(record: MemoryExtractionRecord): FriendlyExtractionFailure | null {
  ...
  const source =
    record.kind === 'connector'
      ? 'Connected apps were read, but ZDesign could not turn that context into memory.'
      : 'ZDesign could not run memory extraction for this chat.';
    ...
    try {
      ...
      if (friendlyFailure) {
        ...
        setConnectorError(
          ...
          ?? 'No memory suggestions found. ZDesign could not read useful content from the selected app yet.',
        );
      } else {
        setConnectorStatus(
        ...
        <div className="memory-tab-panel">
          <div className="memory-source-summary">
            ...
            <div>
              ...
              <p className="hint">
                ZDesign can learn preferences and project facts from future
                chat turns.
              </p>
            </div>
            <label
          ...
          <div className="memory-connector-workbench">
            <div className="memory-connector-picker-head">
              <div>
                ...
                <p className="hint">
                  Select connected apps first. ZDesign only scans the apps you choose.
                </p>
              </div>
              <span className="memory-source-badge">
