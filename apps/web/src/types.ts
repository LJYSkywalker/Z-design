...
// GLM is the sole BYOK provider for Z-Design. The union previously carried
// seven vendor protocols (anthropic/openai/azure/google/ollama/senseaudio/
// aihubmix); it now carries only 'glm' so every Record<ApiProtocol, X> table
// and every provider branch collapses to a single entry. Saved configs with
// an old protocol literal are coerced to 'glm' by config migration v2 in
// state/config.ts (see loadConfig).
export type ApiProtocol = 'glm';
