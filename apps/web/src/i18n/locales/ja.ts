...
export const ja: Dict = {
  ...
  'chat.amrCard.switchBody': 'Z-Design 公式の AMR モデルサービスに切り替えてください — API キーの設定は不要です。サインイン・認可・チャージが完了すると、このタスクは自動で再試行されます。',
  ...
  'settings.onboardingCreateBody': '作成したいサイト、アプリ、スライド、画像、動画を記述してください。Z-Design がプロジェクトを作成し、編集可能な状態で作業を保持します。',
  ...
  'settings.onboardingCloudTitle': 'Z-Design にサインイン',
  'settings.onboardingCloudBody': 'サインインすれば、クラウド AI ですぐにデザインを始められます。複雑な設定は一切不要です。',
  'settings.onboardingCloudSignIn': 'Z-Design Cloud にサインイン',
  ...
  'settings.onboardingGateTooltipAmr': 'この先のステップは AI で実行されます。続けるには Z-Design AMR にサインインしてください。',
  ...
  'settings.agentInstall.pathHint': 'npm または Homebrew で CLI をインストールしたのに未インストールのまま表示される場合は、Z-Design daemon が引き継ぐ PATH にツールの bin ディレクトリが含まれていることを確認してください（macOS では Terminal と GUI アプリで PATH が異なる場合があります）。QUICKSTART.md（"Local agent CLI and PATH" セクション）を参照してください。',
  'settings.agentInstall.stepOpenLinks': '利用したいエージェントの「インストール」または「ドキュメント」を開きます。',
  'settings.agentInstall.stepAuth': 'Z-Design に戻る前に、ベンダー CLI で認証（サインインまたは API 資格情報の追加）を行います。',
  ...
  'settings.mcpServerHint': 'Z-Design を MCP サーバーとしてコーディングエージェントに公開します。',
  ...
  'settings.privacyHint': 'Z-Design チームと共有されるデータ',
  'settings.privacyConsentKicker': 'Z-Design の改善にご協力ください',
  'settings.privacyConsentLead': 'Z-Design は、改善のために使用状況データをチームと共有できます。これには次の内容が含まれます:',
  ...
  'settings.mcpHint': 'コーディング Agent（Cursor、Claude Code、VS Code など）が Z-Design プロジェクトを直接読み取れるようにします。',
  'settings.mcpDaemonError': 'インストールパスを解決するためのローカルデーモンに接続できませんでした（{error}）。Z-Designが実行中であることを確認してから、このパネルを再度開いてください。',
  'settings.mcpBuildDaemon': '先にデーモンをビルドしてください。',
  'settings.mcpNodeMissing': 'Nodeバイナリが見つかりません。',
  'settings.mcpBuildHint': 'apps/daemon/dist/cli.js が見つかりません。`pnpm --filter @z-design/daemon build` を実行して更新してください。',
  ...
  'settings.mcpCapabilityDefault': 'Z-Designで開いているプロジェクトとファイルをデフォルトにするため、どのデザインかを言い直すことなく「これを私のアプリで作って」と言えます。',
  'settings.mcpRunningNote': 'MCPツールの呼び出しを成功させるには、Z-Designが実行中である必要があります。Z-Designを開く前にコーディングAgentを起動した場合は、ライブデーモンに接続できるようAgentを再起動してください。',
  ...
  'entry.discordAria': 'Z-Design の Discord に参加',
  'entry.discordAriaWithOnline': 'Z-Design の Discord に参加 - {online}',
  ...
  'homeHero.subtitlePrefix': 'GLM ネイティブのエージェント式デザインワークスペース。',
  ...
  'homeHero.chip.createPluginHint': '再利用可能な Z-Design プラグインを作成し、マイプラグインに追加します。',
  ...
  'handoff.promptIntro': 'このローカルのZ-Designプロジェクトフォルダから続行してください:',
  ...
  'pluginsHome.subtitle': 'このランタイムに同梱された、すぐに使える Z-Design ワークフロー。1つ選んでスタータープロンプトを読み込むか、レジストリでさらに探せます。',
  'pluginsHome.browseRegistry': 'レジストリを見る',
  'pluginsHome.count': '{total}件中{filtered}件',
  'pluginsHome.loadingCatalog': 'カタログを読み込み中…',
  'pluginsHome.emptyCatalog': 'カタログは空です。バンドルされたプラグインは Z-Design に同梱されており、ここに自動的に表示されるはずです — 解消しない場合はデーモンを再起動してみてください。',
  ...
  'pluginCard.contributeAria': '{title} を Z-Design に貢献',
  'pluginCard.contributeTitle': 'プラグインを pull request で Z-Design に貢献',
  ...
  'integrations.lede': '外部システムを接続し、MCP ツールをエージェントループに取り込み、他の IDE やスクリプト、自動化から Z-Design を利用できます。',
  ...
  'mcpClient.daemonError': 'ローカルデーモンに接続できませんでした。Z-Design が実行中であることを確認してから、このパネルを再度開いてください。',
  'mcpClient.saveFailed': '保存に失敗しました。デーモンが実行中であることを確認して、もう一度お試しください。',
  'useEverywhere.modalAria': 'Z-Design をあらゆる場所で使う',
  'useEverywhere.modalTitle': 'Z-Design をあらゆる場所で使う',
  'useEverywhere.modalSubtitle': 'Z-Design をあらゆる IDE、Agent、スクリプトに組み込めます — CLI、HTTP、MCP、Skills 対応。「Agent 用のガイドをコピー」を使って Claude Code、Codex、Cursor、openclaw、hermes に貼り付ければ、すべてをセットアップできます。',
  ...
  'useEverywhere.section.overview.heading': 'Z-Design は、お使いの Agent が動く場所ならどこでも動作します',
  'useEverywhere.section.overview.intro': 'Z-Design は単なるウィンドウ以上のものです — ローカルの特権デーモン（`od`）に加え、Skills + Design-Systems + Atoms のレジストリを備えています。マシン上で起動すれば、あらゆるコード Agent（Claude Code、Codex、Cursor、OpenCode/openclaw、Hermes、独自のスクリプト）が、4 つの相互に交換可能なインターフェースを通じて、生成の実行、プロジェクトの検査、デザイン成果物の作成を行えます。',
  'useEverywhere.section.overview.bullet1': 'CLI — ヘッドレスなスクリプト、CI、シェル自動化向けの `od <command>`。',
  'useEverywhere.section.overview.bullet2': 'MCP サーバー — Z-Design を Model Context Protocol サーバーとして接続し、MCP 対応のあらゆる Agent がスキルの一覧表示、シナリオの実行、成果物の読み取りを行えるようにします。',
  'useEverywhere.section.overview.bullet3': 'HTTP API — `http://127.0.0.1:7456/api/*` の REST + SSE エンドポイント。Web UI が使うのと同じインターフェースです。',
  'useEverywhere.section.overview.bullet4': 'Skills — ドロップインで使える `SKILL.md` パック（Claude 互換）。PATH 上にあるあらゆる Agent が、Z-Design なしでも呼び出せます。',
  ...
  'useEverywhere.section.overview.footer': 'デーモンはデフォルトで `./.od/`（プロジェクトローカル）に書き込みます。プロジェクト間でデータを共有するには `ZD_DATA_DIR=~/.open-design` を設定してください。',
  'useEverywhere.section.cli.tab': 'CLI · od',
  'useEverywhere.section.cli.heading': '任意のシェルから Z-Design を操作する',
  ...
  'useEverywhere.section.mcp.heading': 'Z-Design を MCP サーバーとして任意のコーディング Agent に公開する',
  'useEverywhere.section.mcp.intro': 'Z-Design には Model Context Protocol サーバー（`od mcp`）が同梱されており、MCP 対応のあらゆるクライアント — Cursor、Claude Code、Antigravity、VS Code Copilot Chat、openclaw、hermes — が、手動でシェルを呼び出すことなく Z-Design のツール（スキルの一覧表示、プレビューのレンダリング、メディアの生成、プラグインの実行）を検出できます。デーモンは主要な各クライアント向けに、貼り付けるだけで使えるインストール用スニペットを `GET /api/mcp/install-info` で公開します。',
  ...
  'useEverywhere.section.mcp.bullet4': '`ZD_DATA_DIR` を固定することで、起動された MCP プロセスがデーモンが既に使用しているのと同じ場所に書き込むようにします（パッケージ化された macOS アプリバンドルでの EPERM を回避）。',
  ...
  'useEverywhere.section.mcp.footer': 'Z-Design アプリで「設定 → 連携」を開くと、JSON を手作業で編集する代わりに、クライアント別のインストールコマンド（Cursor、Claude Code、Antigravity、VS Code）をコピーできます。',
  'useEverywhere.section.http.tab': 'HTTP API',
  'useEverywhere.section.http.heading': 'web UI が使用するものと同じ REST + SSE サーフェス',
  'useEverywhere.section.http.intro': 'ローカルデーモンは `http://127.0.0.1:7456`（ポートは設定可能）で HTTP API を提供します。web UI が呼び出すすべてのエンドポイントは、あなたのスクリプトからも自由に利用できます。ストリーミングエンドポイント（チャットターン、プロジェクト実行）は、`@z-design/contracts` のコントラクト型を伴う Server-Sent Events を発行します。',
  ...
  'useEverywhere.section.http.footer': 'すべてのリクエスト/レスポンスの純粋な TypeScript 型は `@z-design/contracts` にあります — ジェネレーターを組み込まなくても、スクリプトにインポートするだけで完全な補完が得られます。',
  'useEverywhere.section.skills.tab': 'スキルとヘッドレス',
  'useEverywhere.section.skills.heading': 'あらゆる Agent 向けのドロップイン Skills — Z-Design が起動していなくても利用可能',
  'useEverywhere.section.skills.intro': 'Skill は、Claude 互換の `SKILL.md`（YAML フロントマター + 本文）を含むディレクトリです。Z-Design はこのフォーマットを `od:` 名前空間（`mode`、`preview`、`design_system`、`inputs`、…）で拡張し、同じ成果物を Z-Design 内でも、Claude Code、Codex、openclaw、hermes のような素の Agent でも利用できるようにしています。検出は優先順位のチェーンに従うため、プロジェクトは自身のスキルを上書きできます。',
  ...
  'useEverywhere.section.skills.snippet1': '最小限の SKILL.md（Claude 互換のフロントマター + Z-Design の追加項目）',
  ...
  'entry.githubStarAria': 'GitHub で Z-Design にスターを付ける',
  ...
  'newproj.locationDefault': 'Z-Design プロジェクト',
  ...
  'socialShare.zDesignSection': 'Z-Design を共有',
  'socialShare.zDesignTitle': 'GitHub の Z-Design',
  'socialShare.zDesignText': 'Z-Design は、デザイン成果物の作成・編集・デプロイ・引き継ぎを行うためのオープンソースのワークスペースです。',
  ...
  'socialShare.projectText': 'Z-Design で作成: {title}。デプロイされた成果物を見て、リポジトリにスターを付けてください: {repo}',
  'socialShare.projectCopyText': 'Z-Design で作成: {title}\n{url}\nZ-Design リポジトリ: {repo}',
  ...
  'chat.activeFilePlaceholder': 'Z-Design に {file} の変更を依頼...',
  ...
  'chat.mode.design.solves': 'Z-Design にファイル生成、workspace 更新、brief からマルチモーダルな成果物への変換を任せたいときに使います。',
  ...
  'preview.shareTextDefault': 'Z-Design テンプレート: {title}',
  ...
  'preview.errorBody': '例の HTML を取得できませんでした。Z-Design が起動していることを確認して再試行してください。',
  'preview.retry': '再試行',
  ...
  'assistant.shareToZDesign': 'Z-Design に共有',
  'assistant.shareToZDesignBusy': 'パッケージを準備中…',
  ...
  'pet.tabBuiltIn': '組み込み',
  'pet.tabBuiltInHint': 'Z-Design に同梱された厳選コンパニオン — 選んで迎え入れよう。',
  ...
  'pet.codexBundled': '同梱',
  'pet.codexBundledTitle': 'Z-Design に同梱 — ダウンロード不要。',
  ...
  'settings.projectLocationsDescription': '複数の Z-Design プロジェクトフォルダを含められる作業ベースを追加します。新しいプロジェクトは選択したベース内の 1 つのフォルダとして保存されます。',
  ...
  'settings.customInstructionsHint': 'ZDesignがすべてのチャットで従うべき固定の指示。事実、設定、プロジェクトのコンテキストをメモリとして保存します。',
  ...
  'settings.memoryStarterProjectDesc': 'Z-Design v0.5 — チャット駆動のデザインエディター',
  ...
  'updater.availableBody': 'Z-Design {version} を利用できます。インストーラーを開く前にダウンロードします。',
  ...
  'updater.installerOpenBody': 'インストーラーが開いています。更新を完了できるよう Z-Design を終了しています。',
  ...
  'updater.quitButton': 'Z-Design を終了',
  'updater.quitFailedBody': 'インストーラーは開いていますが、Z-Design を終了できませんでした。アプリを置き換える前に Z-Design を終了してください。',
  'updater.quitFailedTitle': '終了できませんでした',
  'updater.quitting': '終了しています...',
  'updater.ready': 'アップデート準備完了',
  'updater.readyGeneric': '新しいバージョンの準備ができました。Z-Design を閉じてインストーラーを開きます。',
  'updater.readyVersion': 'Z-Design {version} の準備ができました。Z-Design を閉じてインストーラーを開きます。',
  ...
  'dsManager.createBody': 'ブランド・プロダクト・コード・アセット・デザインの参考資料を Z-Design に学習させます。',
  ...
  'settings.customInstructionsDesc': 'Z-Design がすべてのチャットで従う固定の指示です。これらは保存されたメモリではありません。事実・好み・プロジェクトのコンテキストには「メモリ」を使用してください。',
