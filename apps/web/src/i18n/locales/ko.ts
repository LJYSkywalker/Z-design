...
export const ko: Dict = {
  ...
  'chat.amrCard.switchBody': 'Z-Design 공식 AMR 모델 서비스로 전환하세요 — API 키 설정이 필요 없습니다. 로그인・인증・충전이 완료되면 이 작업이 자동으로 재시도됩니다.',
  ...
  'settings.onboardingCreateBody': '원하는 사이트, 앱, 덱, 이미지 또는 동영상을 설명하세요. Z-Design이 프로젝트를 생성하고 작업물을 편집 가능한 상태로 유지합니다.',
  ...
  'settings.onboardingCloudTitle': 'Z-Design에 로그인',
  'settings.onboardingCloudBody': '로그인하면 클라우드 AI로 바로 디자인을 시작할 수 있습니다. 복잡한 설정이 전혀 필요 없습니다.',
  'settings.onboardingCloudSignIn': 'Z-Design Cloud에 로그인',
  ...
  'settings.onboardingGateTooltipAmr': '다음 단계는 AI로 실행됩니다 — 계속하려면 Z-Design AMR에 로그인하세요.',
  ...
  'settings.agentInstall.pathHint': 'npm 또는 Homebrew로 CLI를 설치했는데도 아직 미설치로 표시된다면, Z-Design daemon이 상속받는 PATH에 도구의 bin 디렉터리가 포함되어 있는지 확인하세요(macOS에서는 Terminal과 GUI 앱의 PATH가 다를 수 있습니다). QUICKSTART.md("Local agent CLI and PATH" 섹션)를 참고하세요.',
  'settings.agentInstall.stepOpenLinks': '원하는 에이전트 카드에서 설치 또는 문서 링크를 여세요.',
  'settings.agentInstall.stepAuth': 'Z-Design으로 돌아오기 전에 공급자 CLI에서 인증(로그인 또는 API 자격 증명 추가)을 완료하세요.',
  ...
  'settings.mcpServerHint': 'Z-Design을 코딩 에이전트용 MCP 서버로 노출합니다.',
  ...
  'settings.privacyHint': 'Z-Design 팀과 공유되는 데이터',
  'settings.privacyConsentKicker': 'Z-Design 개선에 참여하기',
  'settings.privacyConsentLead': 'Z-Design은 제품 개선을 위해 사용 데이터를 팀과 공유할 수 있습니다. 여기에는 다음이 포함됩니다:',
  ...
  'settings.mcpHint': '코딩 Agent(Cursor, Claude Code, VS Code…)가 Z-Design 프로젝트를 직접 읽을 수 있도록 합니다.',
  'settings.mcpDaemonError': '설치 경로를 확인하기 위해 로컬 데몬에 연결하지 못했습니다 ({error}). Z-Design이 실행 중인지 확인한 후 이 패널을 다시 열어 주세요.',
  'settings.mcpBuildDaemon': '먼저 데몬을 빌드하세요.',
  'settings.mcpNodeMissing': 'Node 바이너리가 없습니다.',
  'settings.mcpBuildHint': 'apps/daemon/dist/cli.js가 없습니다. `pnpm --filter @z-design/daemon build`를 실행한 후 새로고침하세요.',
  ...
  'settings.mcpCapabilityDefault': 'Z-Design에서 열려 있는 프로젝트와 파일을 기본값으로 사용하므로, 어떤 디자인인지 다시 말하지 않고도 "이걸 내 앱에 만들어 줘"라고 말할 수 있습니다.',
  'settings.mcpRunningNote': 'MCP 도구 호출이 성공하려면 Z-Design이 실행 중이어야 합니다. Z-Design을 열기 전에 코딩 Agent를 시작했다면, 실행 중인 데몬에 연결할 수 있도록 Agent를 다시 시작하세요.',
  ...
  'entry.discordAria': 'Z-Design Discord 참여하기',
  'entry.discordAriaWithOnline': 'Z-Design Discord 참여하기 - {online}',
  ...
  'homeHero.subtitlePrefix': 'GLM 네이티브 에이전트 기반 디자인 워크스페이스.',
  ...
  'homeHero.chip.createPluginHint': '재사용 가능한 Z-Design 플러그인을 제작하여 내 플러그인에 추가하세요.',
  ...
  'handoff.promptIntro': '이 로컬 Z-Design 프로젝트 폴더에서 계속 진행하세요:',
  ...
  'pluginsHome.subtitle': '이 런타임에 번들로 제공되는 바로 사용 가능한 Z-Design 워크플로입니다. 하나를 선택해 시작용 프롬프트를 불러오거나, 레지스트리에서 더 둘러보세요.',
  'pluginsHome.browseRegistry': '레지스트리 둘러보기',
  'pluginsHome.count': '{total}개 중 {filtered}개',
  'pluginsHome.loadingCatalog': '카탈로그 불러오는 중…',
  'pluginsHome.emptyCatalog': '카탈로그가 비어 있습니다. 번들 플러그인은 Z-Design에 포함되어 있으며 여기에 자동으로 표시되어야 합니다 — 문제가 계속되면 데몬을 다시 시작해 보세요.',
  ...
  'pluginCard.contributeAria': '{title}을(를) Z-Design에 기여',
  'pluginCard.contributeTitle': 'pull request로 플러그인을 Z-Design에 기여',
  ...
  'integrations.lede': '외부 시스템을 연결하고, MCP 도구를 Agent 루프에 가져오고, 다른 IDE, 스크립트, 자동화에서 Z-Design을 사용하세요.',
  ...
  'mcpClient.daemonError': '로컬 데몬에 연결할 수 없습니다. Z-Design이 실행 중인지 확인한 후 이 패널을 다시 여세요.',
  'mcpClient.saveFailed': '저장에 실패했습니다. 데몬이 실행 중인지 확인하고 다시 시도하세요.',
  'useEverywhere.modalAria': '어디서나 Z-Design 사용',
  'useEverywhere.modalTitle': '어디서나 Z-Design 사용',
  'useEverywhere.modalSubtitle': 'Z-Design을 어떤 IDE, Agent, 스크립트에든 적용하세요 — CLI, HTTP, MCP, Skills. “Agent용 가이드 복사”를 사용해 Claude Code, Codex, Cursor, openclaw, hermes에 붙여 넣어 모든 것을 설정하세요.',
  ...
  'useEverywhere.section.overview.heading': 'Z-Design는 Agent가 작동하는 곳이라면 어디서든 작동합니다',
  'useEverywhere.section.overview.intro': 'Z-Design은 단순한 창 이상입니다 — 로컬 권한 데몬(`od`)에 Skills + Design-Systems + Atoms 레지스트리를 더한 것입니다. 머신에서 실행되면 어떤 코드 Agent(Claude Code, Codex, Cursor, OpenCode/openclaw, Hermes, 직접 만든 스크립트)든 네 가지 호환 가능한 인터페이스를 통해 생성을 구동하고, 프로젝트를 검사하고, 디자인 아티팩트를 만들 수 있습니다.',
  'useEverywhere.section.overview.bullet1': 'CLI — 헤드리스 스크립트, CI, 셸 자동화를 위한 `od <command>`.',
  'useEverywhere.section.overview.bullet2': 'MCP 서버 — Z-Design을 Model Context Protocol 서버로 연결해 MCP 지원 Agent가 skills를 나열하고, 시나리오를 실행하고, 아티팩트를 읽을 수 있게 합니다.',
  'useEverywhere.section.overview.bullet3': 'HTTP API — `http://127.0.0.1:7456/api/*` REST + SSE 엔드포인트, 웹 UI가 사용하는 것과 동일한 인터페이스입니다.',
  'useEverywhere.section.overview.bullet4': 'Skills — PATH에 이미 있는 어떤 Agent든 Z-Design 없이도 호출할 수 있는 드롭인 `SKILL.md` 팩(Claude 호환).',
  ...
  'useEverywhere.section.overview.footer': '데몬은 기본적으로 `./.od/`(프로젝트 로컬)에 기록합니다. 프로젝트 간에 데이터를 공유하려면 `ZD_DATA_DIR=~/.open-design`을 설정하세요.',
  'useEverywhere.section.cli.tab': 'CLI · od',
  'useEverywhere.section.cli.heading': '모든 셸에서 Z-Design 구동',
  ...
  'useEverywhere.section.mcp.heading': 'Z-Design를 MCP 서버로 모든 코딩 Agent에 노출',
  'useEverywhere.section.mcp.intro': 'Z-Design에는 Model Context Protocol 서버(`od mcp`)가 포함되어 있어, MCP 지원 클라이언트 — Cursor, Claude Code, Antigravity, VS Code Copilot Chat, openclaw, hermes — 가 수동으로 셸을 거치지 않고도 Z-Design 도구(skills 나열, 미리 보기 렌더링, 미디어 생성, 플러그인 실행)를 발견할 수 있습니다. 데몬은 각 주요 클라이언트에 대해 바로 붙여 넣을 수 있는 설치 스니펫을 `GET /api/mcp/install-info`를 통해 게시합니다.',
  ...
  'useEverywhere.section.mcp.bullet4': '`ZD_DATA_DIR`을 고정하여 생성된 MCP 프로세스가 데몬이 이미 사용 중인 위치와 동일한 곳에 기록하도록 합니다(패키징된 macOS 앱 번들에서 EPERM 방지).',
  ...
  'useEverywhere.section.mcp.footer': 'Z-Design 앱에서 설정 → 통합을 열어 JSON을 직접 편집하는 대신 클라이언트별 설치 명령(Cursor, Claude Code, Antigravity, VS Code)을 복사하세요.',
  'useEverywhere.section.http.tab': 'HTTP API',
  'useEverywhere.section.http.heading': '웹 UI가 사용하는 것과 동일한 REST + SSE 표면',
  'useEverywhere.section.http.intro': '로컬 데몬은 `http://127.0.0.1:7456`에서 HTTP API를 제공합니다(포트 구성 가능). 웹 UI가 호출하는 모든 엔드포인트는 스크립트에서도 마찬가지로 사용할 수 있습니다. 스트리밍 엔드포인트(채팅 턴, 프로젝트 실행)는 `@z-design/contracts`의 계약 타입과 함께 Server-Sent Events를 내보냅니다.',
  ...
  'useEverywhere.section.http.footer': '모든 요청/응답에 대한 순수 TypeScript 타입은 `@z-design/contracts`에 있습니다 — 생성기를 연결하지 않고도 완전한 자동 완성을 위해 스크립트에서 가져오세요.',
  'useEverywhere.section.skills.tab': '스킬 & 헤드리스',
  'useEverywhere.section.skills.heading': '어떤 Agent를 위한 드롭인 Skills — Z-Design이 실행되고 있지 않아도',
  'useEverywhere.section.skills.intro': 'Skill은 Claude 호환 `SKILL.md`(YAML 프런트 매터 + 본문)를 포함하는 디렉터리입니다. Z-Design은 `od:` 네임스페이스(`mode`, `preview`, `design_system`, `inputs`, …)로 형식을 확장하므로 동일한 아티팩트를 Z-Design 내부와 Claude Code, Codex, openclaw, hermes 같은 일반 Agent 양쪽에서 사용할 수 있습니다. 발견은 우선순위 체인을 따르므로 프로젝트가 자체 skills를 재정의할 수 있습니다.',
  ...
  'useEverywhere.section.skills.snippet1': '최소 SKILL.md (Claude 호환 front matter + Z-Design 추가 항목)',
  ...
  'entry.githubStarAria': 'GitHub 에서 Z-Design 에 별 누르기',
  ...
  'newproj.locationDefault': 'Z-Design 프로젝트',
  ...
  'socialShare.zDesignSection': 'Z-Design 공유',
  'socialShare.zDesignTitle': 'GitHub의 Z-Design',
  'socialShare.zDesignText': 'Z-Design은 디자인 아티팩트를 생성, 편집, 배포 및 인수인계하기 위한 오픈소스 워크스페이스입니다.',
  ...
  'socialShare.projectText': 'Z-Design으로 제작: {title}. 배포된 아티팩트를 확인하고 저장소에 스타를 남겨주세요: {repo}',
  'socialShare.projectCopyText': 'Z-Design으로 제작: {title}\n{url}\nZ-Design 저장소: {repo}',
  ...
  'chat.activeFilePlaceholder': 'Z-Design에 {file} 변경 요청...',
  ...
  'chat.mode.design.solves': 'Z-Design이 파일을 생성하고 workspace를 업데이트하거나 brief를 멀티모달 deliverable로 바꿔야 할 때 사용합니다.',
  ...
  'preview.shareTextDefault': 'Z-Design 템플릿: {title}',
  ...
  'preview.errorBody': '예제 HTML을 가져오지 못했습니다. Z-Design이 실행 중인지 확인하고 다시 시도하세요.',
  'preview.retry': '다시 시도',
  ...
  'assistant.shareToZDesign': 'Z-Design에 공유',
  'assistant.shareToZDesignBusy': '패키지 준비 중…',
  ...
  'pet.tabBuiltIn': '내장',
  'pet.tabBuiltInHint': 'Z-Design에 기본 포함된 친구들 — 골라서 입양하세요.',
  ...
  'pet.codexBundled': '내장',
  'pet.codexBundledTitle': 'Z-Design 에 내장 — 다운로드 불필요.',
  ...
  'settings.projectLocationsDescription': '여러 Z-Design 프로젝트 폴더를 포함할 수 있는 작업 베이스를 추가합니다. 새 프로젝트는 선택한 베이스 안의 폴더로 저장됩니다.',
  ...
  'settings.customInstructionsHint': '모든 채팅에서 Z-Design이 따라야 할 고정 지침입니다. 사실, 선호 사항, 프로젝트 컨텍스트를 메모리로 저장하세요.',
  ...
  'settings.memoryStarterProjectDesc': 'Z-Design v0.5 — 채팅 기반 디자인 편집기',
  ...
  'updater.availableBody': 'Z-Design {version}을 사용할 수 있습니다. 설치 프로그램을 열기 전에 다운로드됩니다.',
  ...
  'updater.installerOpenBody': '설치 프로그램이 열려 있습니다. 업데이트를 완료할 수 있도록 Z-Design을 종료하는 중입니다.',
  ...
  'updater.quitButton': 'Z-Design 종료',
  'updater.quitFailedBody': '설치 프로그램이 열려 있지만 Z-Design을 종료할 수 없습니다. 앱을 교체하기 전에 Z-Design을 종료하세요.',
  'updater.quitFailedTitle': '종료할 수 없음',
  'updater.quitting': '종료 중...',
  'updater.ready': '업데이트 준비됨',
  'updater.readyGeneric': '새 버전이 준비되었습니다. Z-Design을 닫고 설치 프로그램을 엽니다.',
  'updater.readyVersion': 'Z-Design {version}이 준비되었습니다. Z-Design을 닫고 설치 프로그램을 엽니다.',
  ...
  'dsManager.createBody': 'Z-Design에 브랜드, 제품, 코드, 에셋, 디자인 레퍼런스를 학습시키세요.',
  ...
  'settings.customInstructionsDesc': 'Z-Design이 모든 채팅에서 따르는 고정 지침입니다. 저장된 메모리가 아니며, 사실, 선호 사항, 프로젝트 컨텍스트에는 메모리를 사용하세요.',
