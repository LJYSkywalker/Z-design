...
export const fa: Dict = {
  ...
  'chat.amrCard.switchBody': 'به سرویس رسمی مدل AMR از Z-Design سوئیچ کنید — بدون نیاز به تنظیم کلید API. پس از ورود، اعطای دسترسی و شارژ، این اجرا به‌طور خودکار دوباره انجام می‌شود.',
  ...
  'settings.onboardingCreateBody': 'سایت، اپلیکیشن، ارائه، تصویر یا ویدیویی را که می‌خواهید توصیف کنید. Z-Design یک پروژه می‌سازد و کار را قابل ویرایش نگه می‌دارد.',
  ...
  'settings.onboardingGateTooltipAmr': 'مراحل بعدی با هوش مصنوعی اجرا می‌شوند — برای ادامه وارد Z-Design AMR شوید.',
  ...
  'settings.agentInstall.stepAuth': 'قبل از بازگشت به Z-Design، در CLI ارائه‌دهنده احراز هویت کنید (ورود یا افزودن اطلاعات API).',
  ...
  'settings.mcpServerHint': 'Z-Design را به‌عنوان سرور MCP برای عامل برنامه‌نویسی خود در دسترس قرار دهید.',
  ...
  'settings.privacyHint': 'چه داده‌هایی با تیم Z-Design به اشتراک گذاشته می‌شود',
  'settings.privacyConsentKicker': 'به ما در بهبود Z-Design کمک کنید',
  'settings.privacyConsentLead': 'Z-Design می‌تواند داده‌های استفاده را با تیم ما به اشتراک بگذارد تا به بهبود کمک کند. این شامل موارد زیر است:',
  ...
  'settings.mcpHint': 'به Agentهای کدنویسی (Cursor، Claude Code، VS Code…) اجازه دهید پروژه‌های Z-Design شما را مستقیماً بخوانند.',
  'settings.mcpDaemonError': 'امکان دسترسی به daemon محلی برای تعیین مسیرهای نصب وجود نداشت ({error}). مطمئن شوید که Z-Design در حال اجراست، سپس این پنل را دوباره باز کنید.',
  'settings.mcpBuildDaemon': 'ابتدا daemon را بسازید.',
  'settings.mcpNodeMissing': 'باینری Node یافت نشد.',
  'settings.mcpBuildHint': 'apps/daemon/dist/cli.js یافت نشد. دستور `pnpm --filter @z-design/daemon build` را اجرا کنید و صفحه را تازه‌سازی کنید.',
  ...
  'settings.mcpCapabilityDefault': 'به‌صورت پیش‌فرض از پروژه و فایلی که در Z-Design باز کرده‌اید استفاده می‌کند، بنابراین می‌توانید بگویید «این را در اپ من بساز» بدون اینکه دوباره مشخص کنید کدام طراحی.',
  'settings.mcpRunningNote': 'برای موفقیت فراخوانی‌های ابزار MCP، Z-Design باید در حال اجرا باشد. اگر Agent کدنویسی خود را پیش از باز کردن Z-Design راه‌اندازی کرده‌اید، Agent را راه‌اندازی مجدد کنید تا بتواند به daemon زنده دسترسی پیدا کند.',
  ...
  'homeHero.subtitlePrefix': 'فضای کاری طراحی عاملی بومی GLM.',
  ...
  'homeHero.chip.createPluginHint': 'یک افزونه قابل استفاده مجدد Z-Design بسازید و آن را به افزونه‌های من اضافه کنید.',
  ...
  'handoff.promptIntro': 'ادامه از این پوشه محلی پروژه Z-Design:',
  ...
  'pluginsHome.subtitle': 'گردش‌کارهای آماده Z-Design که همراه این runtime ارائه شده‌اند. یکی را برای بارگذاری یک پرامپت شروع انتخاب کنید، یا برای موارد بیشتر در رجیستری بگردید.',
  ...
  'pluginsHome.emptyCatalog': 'کاتالوگ خالی است. پلاگین‌های همراه با Z-Design ارائه می‌شوند و باید به‌صورت خودکار اینجا نمایش داده شوند — اگر این مشکل ادامه داشت، daemon را مجدداً راه‌اندازی کنید.',
  ...
  'pluginCard.contributeAria': 'مشارکت {title} در Z-Design',
  'pluginCard.contributeTitle': 'مشارکت افزونه در Z-Design با یک PR',
  ...
  'integrations.lede': 'سیستم‌های خارجی را متصل کنید، ابزارهای MCP را به حلقه Agent خود بیاورید و از Z-Design در سایر IDEها، اسکریپت‌ها و خودکارسازی‌ها استفاده کنید.',
  ...
  'mcpClient.daemonError': 'دسترسی به daemon محلی ممکن نشد. مطمئن شوید Z-Design در حال اجراست، سپس این پنل را دوباره باز کنید.',
  'mcpClient.saveFailed': 'ذخیره ناموفق بود. بررسی کنید که daemon در حال اجرا باشد و دوباره تلاش کنید.',
  'useEverywhere.modalAria': 'استفاده از Z-Design در همه‌جا',
  'useEverywhere.modalTitle': 'استفاده از Z-Design در همه‌جا',
  'useEverywhere.modalSubtitle': 'Z-Design را در هر IDE، Agent یا اسکریپتی بگنجانید — CLI، HTTP، MCP و Skills. از «Copy guide for an agent» استفاده کنید و آن را در Claude Code، Codex، Cursor، openclaw یا hermes جای‌گذاری کنید تا همه‌چیز راه‌اندازی شود.',
  ...
  'useEverywhere.section.overview.heading': 'Z-Design هر جا که Agent شما کار کند، کار می‌کند',
  'useEverywhere.section.overview.intro': 'Z-Design چیزی فراتر از یک پنجره است — یک daemon محلیِ دارای دسترسی ویژه (`od`) به‌علاوه یک رجیستری از Skills + Design-Systems + Atoms است. به‌محض اینکه روی دستگاه شما در حال اجرا باشد، هر code agent (Claude Code، Codex، Cursor، OpenCode/openclaw، Hermes، یا اسکریپت خودتان) می‌تواند تولیدها را هدایت کند، پروژه‌ها را بررسی کند و از طریق چهار سطح قابل‌تعویض، آثار طراحی تولید کند.',
  'useEverywhere.section.overview.bullet1': 'CLI — `od <command>` برای اسکریپت‌های headless، CI و اتوماسیون shell.',
  'useEverywhere.section.overview.bullet2': 'سرور MCP — Z-Design را به‌عنوان یک سرور Model Context Protocol سیم‌کشی می‌کند تا هر Agent سازگار با MCP بتواند skillها را فهرست کند، سناریوها را اجرا کند و آثار را بخواند.',
  ...
  'useEverywhere.section.overview.bullet4': 'Skills — بسته‌های آمادهٔ `SKILL.md` (سازگار با Claude) که هر Agent موجود در PATH شما می‌تواند بدون نیاز به Z-Design اصلاً فراخوانی‌شان کند.',
  ...
  'useEverywhere.section.overview.footer': 'daemon به صورت پیش‌فرض در `./.od/` (محلی پروژه) می‌نویسد. برای اشتراک‌گذاری داده‌ها میان پروژه‌ها، `ZD_DATA_DIR=~/.open-design` را تنظیم کنید.',
  'useEverywhere.section.cli.tab': 'CLI · od',
  'useEverywhere.section.cli.heading': 'کنترل Z-Design از هر shell',
  ...
  'useEverywhere.section.mcp.heading': 'در معرض قرار دادن Z-Design به‌عنوان یک سرور MCP برای هر Agent کدنویسی',
  'useEverywhere.section.mcp.intro': 'Z-Design با یک سرور Model Context Protocol (`od mcp`) ارائه می‌شود که به هر کلاینت سازگار با MCP — Cursor، Claude Code، Antigravity، VS Code Copilot Chat، openclaw، hermes — اجازه می‌دهد ابزارهای Z-Design را کشف کند (فهرست‌کردن skillها، رندر پیش‌نمایش‌ها، تولید رسانه، اجرای پلاگین‌ها) بدون نیاز به اجرای دستی در shell. این daemon برای هر کلاینت اصلی یک قطعهٔ نصب آمادهٔ جای‌گذاری را از طریق `GET /api/mcp/install-info` منتشر می‌کند.',
  ...
  'useEverywhere.section.mcp.bullet4': '`ZD_DATA_DIR` را پین می‌کند تا فرایند MCP ایجادشده در همان مکانی بنویسد که daemon از پیش استفاده می‌کند (از EPERM در باندل‌های اپلیکیشن بسته‌بندی‌شده macOS جلوگیری می‌کند).',
  ...
  'useEverywhere.section.mcp.footer': 'در اپلیکیشن Z-Design، به Settings ← Integrations بروید تا یک دستور نصب مخصوص کلاینت (Cursor، Claude Code، Antigravity، VS Code) را کپی کنید، به‌جای ویرایش دستی JSON.',
  ...
  'useEverywhere.section.http.footer': 'تایپ‌های خالص TypeScript برای هر درخواست/پاسخ در `@z-design/contracts` قرار دارند — آن‌ها را در اسکریپت خود import کنید تا بدون سیم‌کشی یک generator، تکمیل خودکار کامل داشته باشید.',
  'useEverywhere.section.skills.tab': 'مهارت‌ها و حالت بدون رابط',
  'useEverywhere.section.skills.heading': 'Skillهای آمادهٔ استفاده برای هر Agent — حتی بدون اجرای Z-Design',
  'useEverywhere.section.skills.intro': 'یک Skill دایرکتوری‌ای است با یک `SKILL.md` سازگار با Claude (فرانت‌متر YAML + بدنه). Z-Design این فرمت را با namespace به نام `od:` گسترش می‌دهد (`mode`، `preview`، `design_system`، `inputs`، …) تا همان اثر هم درون Z-Design و هم توسط یک Agent معمولی مانند Claude Code، Codex، openclaw یا hermes قابل‌استفاده باشد. کشف از یک زنجیرهٔ اولویت پیروی می‌کند تا پروژه‌ها بتوانند skillهای خود را بازنویسی کنند.',
  ...
  'useEverywhere.section.skills.snippet1': 'حداقل SKILL.md (front matter سازگار با Claude + افزوده‌های Z-Design)',
  ...
  'entry.githubStarAria': 'به Z-Design در GitHub ستاره بدهید',
  ...
  'newproj.locationDefault': 'پروژه‌های Z-Design',
  ...
  'socialShare.zDesignSection': 'اشتراک‌گذاری Z-Design',
  'socialShare.zDesignTitle': 'Z-Design در GitHub',
  'socialShare.zDesignText': 'Z-Design یک فضای کاری متن‌باز برای ساخت، ویرایش، استقرار و تحویل آرتیفکت‌های طراحی است.',
  ...
  'socialShare.projectText': 'ساخته‌شده با Z-Design: {title}. آرتیفکت مستقرشده را ببینید و به مخزن ستاره بدهید: {repo}',
  'socialShare.projectCopyText': 'ساخته‌شده با Z-Design: {title}\n{url}\nمخزن Z-Design: {repo}',
  ...
  'chat.activeFilePlaceholder': 'از Z-Design بخواهید {file} را تغییر دهد...',
  ...
  'chat.mode.design.solves': 'وقتی می خواهید Z-Design فایل بسازد، workspace را به روز کند یا brief را به خروجی های چندرسانه ای قابل تحویل تبدیل کند استفاده کنید.',
  ...
  'preview.shareTextDefault': 'قالب Z-Design: {title}',
  ...
  'preview.errorBody': 'دریافت HTML نمونه با خطا مواجه شد. مطمئن شوید Z-Design در حال اجراست و دوباره تلاش کنید.',
  ...
  'designFiles.usefulInfoTip7': 'از Z-Design راضی هستید؟ در GitHub به ما ستاره بدهید.',
  ...
  'assistant.shareToZDesign': 'اشتراک‌گذاری در Z-Design',
  'assistant.shareToZDesignBusy': 'در حال آماده‌سازی بسته…',
  ...
  'pet.tabBuiltInHint': 'همراهان منتخب همراه Z-Design — یکی را انتخاب و فرزندخوانده کن.',
  ...
  'pet.codexBundledTitle': 'همراه Z-Design ارائه می‌شود — نیازی به دانلود نیست.',
  ...
  'settings.projectLocationsDescription': 'پایه‌های کاری اضافه کنید که می‌توانند چند پوشهٔ پروژهٔ Z-Design را در خود داشته باشند. پروژه‌های جدید به‌صورت یک پوشه داخل پایهٔ انتخاب‌شده ذخیره می‌شوند.',
  ...
  'settings.customInstructionsHint': 'دستورالعمل‌های ثابتی که Z-Design باید در هر گفت‌وگو دنبال کند. حقایق، ترجیحات و زمینهٔ پروژه را به‌عنوان حافظه ذخیره کنید.',
  ...
  'settings.memoryStarterProjectDesc': 'Z-Design v0.5 — ویرایشگر طراحی مبتنی بر گفتگو',
  ...
  'updater.availableBody': 'Z-Design {version} موجود است. پیش از باز شدن نصب‌کننده دانلود می‌شود.',
  ...
  'updater.installerOpenBody': 'نصب‌کننده باز است. Z-Design در حال بسته شدن است تا بتوانید به‌روزرسانی را کامل کنید.',
  ...
  'updater.quitButton': 'بستن Z-Design',
  'updater.quitFailedBody': 'نصب‌کننده باز است، اما Z-Design بسته نشد. پیش از جایگزینی برنامه، Z-Design را ببندید.',
  'updater.quitFailedTitle': 'امکان بستن نیست',
  'updater.quitting': 'در حال بستن...',
  'updater.ready': 'به‌روزرسانی آماده است',
  'updater.readyGeneric': 'نسخه جدید آماده است. Z-Design بسته می‌شود و نصب‌کننده را باز می‌کند.',
  'updater.readyVersion': 'Z-Design {version} آماده است. Z-Design بسته می‌شود و نصب‌کننده را باز می‌کند.',
  ...
  'dsManager.createBody': 'برند، محصول، کد، دارایی‌ها و مراجع طراحی خود را به Z-Design بیاموزید.',
  ...
  'settings.customInstructionsDesc': 'دستورالعمل‌های ثابتی که Z-Design در هر گفت‌وگو از آن‌ها پیروی می‌کند. این‌ها حافظه ذخیره‌شده نیستند؛ برای واقعیت‌ها، ترجیحات و بافت پروژه از حافظه استفاده کنید.',
