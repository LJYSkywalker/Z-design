...
export const en: Dict = {
  ...
  'chat.amrCard.switchBody': 'Switch to Z-Design\'s official AMR model service — no API key setup needed. After you sign in, authorize, and top up, this run retries automatically.',
  ...
  'settings.onboardingCreateBody': 'Describe the site, app, deck, image, or video you want. Z-Design will create a project and keep the work editable.',
  ...
  'settings.onboardingGateTooltipAmr': 'The next steps run on AI — sign in to Z-Design AMR to continue.',
  ...
  'settings.agentInstall.pathHint': 'If you installed a CLI with npm or Homebrew and it still shows as not installed, ensure the tool\'s bin directory is on the PATH the Z-Design daemon inherits (Terminal vs GUI apps can differ on macOS). See QUICKSTART.md (section "Local agent CLI and PATH").',
  'settings.agentInstall.stepOpenLinks': 'Open Install or Docs for your preferred agent.',
  'settings.agentInstall.stepAuth': 'Authenticate with the vendor CLI (sign in or add API credentials) before returning to Z-Design.',
  ...
  'settings.mcpServerHint': 'Expose Z-Design as an MCP server for your coding agent.',
  ...
  'settings.privacyHint': 'What data is shared with the Z-Design team',
  'settings.privacyConsentKicker': 'Help us improve Z-Design',
  'settings.privacyConsentLead': 'Z-Design can share usage data with our team to help us improve. This includes:',
  ...
  'settings.mcpHint': 'Let coding agents (Cursor, Claude Code, VS Code…) read your Z-Design projects directly.',
  'settings.mcpDaemonError': 'Couldn\'t reach the local daemon to resolve install paths ({error}). Make sure Z-Design is running, then reopen this panel.',
  'settings.mcpBuildDaemon': 'Build the daemon first.',
  'settings.mcpNodeMissing': 'Node binary is missing.',
  'settings.mcpBuildHint': 'apps/daemon/dist/cli.js is missing. Run `pnpm --filter @z-design/daemon build` and refresh.',
  ...
  'settings.mcpCapabilityDefault': 'Default to the project and file you have open in Z-Design, so you can say "build this in my app" without re-stating which design.',
  'settings.mcpRunningNote': 'Z-Design must be running for MCP tool calls to succeed. If you started your coding agent before opening Z-Design, restart the agent so it can reach the live daemon.',
  ...
  'entry.discordAria': 'Join the Z-Design Discord',
  'entry.discordAriaWithOnline': 'Join the Z-Design Discord - {online}',
  ...
  'homeHero.subtitlePrefix': 'The GLM-native agentic design workspace.',
  ...
  'homeHero.chip.createPluginHint': 'Author a reusable Z-Design plugin and add it to My plugins.',
  ...
  'handoff.promptIntro': 'Continue from this local Z-Design project folder:',
  ...
  'designSystemPicker.noPreview': 'No preview page. Z-Design Systems to view the full preview.',
  ...
  'pluginsHome.subtitle': 'Ready-to-use Z-Design workflows bundled with this runtime. Pick one to load a starter prompt, or browse the registry for more.',
  'pluginsHome.browseRegistry': 'Browse registry',
  'pluginsHome.count': '{filtered} of {total}',
  'pluginsHome.loadingCatalog': 'Loading catalog…',
  'pluginsHome.emptyCatalog': 'Catalog is empty. Bundled plugins ship with Z-Design and should appear here automatically — try restarting the daemon if this persists.',
  ...
  'pluginCard.contributeAria': 'Contribute {title} to Z-Design',
  'pluginCard.contributeTitle': 'Contribute plugin to Z-Design with a pull request',
  ...
  'integrations.lede': 'Connect external systems, bring MCP tools into your agent loop, and use Z-Design from other IDEs, scripts, and automations.',
  ...
  'mcpClient.daemonError': 'Could not reach the local daemon. Make sure Z-Design is running, then reopen this panel.',
  'mcpClient.saveFailed': 'Save failed. Check that the daemon is running and try again.',
  'useEverywhere.modalAria': 'Use Z-Design everywhere',
  'useEverywhere.modalTitle': 'Use Z-Design everywhere',
  'useEverywhere.modalSubtitle': 'Drop Z-Design into any IDE, agent, or script — CLI, HTTP, MCP, and Skills. Use “Copy guide for an agent” and paste into Claude Code, Codex, Cursor, openclaw, or hermes to set up everything.',
  ...
  'useEverywhere.section.overview.heading': 'Z-Design works wherever your agent works',
  'useEverywhere.section.overview.intro': 'Z-Design is more than a window — it is a local privileged daemon (`od`) plus a Skills + Design-Systems + Atoms registry. Once it is running on your machine, any code agent (Claude Code, Codex, Cursor, OpenCode/openclaw, Hermes, your own script) can drive generations, inspect projects, and produce design artifacts through four interchangeable surfaces.',
  'useEverywhere.section.overview.bullet1': 'CLI — `od <command>` for headless scripts, CI, and shell automation.',
  'useEverywhere.section.overview.bullet2': 'MCP server — wires Z-Design as a Model Context Protocol server so any MCP-capable agent can list skills, run scenarios, and read artifacts.',
  ...
  'useEverywhere.section.overview.bullet4': 'Skills — drop-in `SKILL.md` packs (Claude-compatible) that any agent already on your PATH can invoke without Z-Design at all.',
  ...
  'useEverywhere.section.overview.footer': 'The daemon writes to `./.od/` (project-local) by default. Set `ZD_DATA_DIR=~/.open-design` to share data across projects.',
  'useEverywhere.section.cli.tab': 'CLI · od',
  'useEverywhere.section.cli.heading': 'Drive Z-Design from any shell',
  ...
  'useEverywhere.section.mcp.heading': 'Expose Z-Design as an MCP server to any coding agent',
  'useEverywhere.section.mcp.intro': 'Z-Design ships with a Model Context Protocol server (`od mcp`) that lets any MCP-capable client — Cursor, Claude Code, Antigravity, VS Code Copilot Chat, openclaw, hermes — discover Z-Design tools (list skills, render previews, generate media, run plugins) without shelling out manually. The daemon publishes a ready-to-paste install snippet via `GET /api/mcp/install-info` for each major client.',
  ...
  'useEverywhere.section.mcp.bullet4': 'Pins `ZD_DATA_DIR` so the spawned MCP process writes to the same place the daemon already uses (avoids EPERM in packaged macOS app bundles).',
  ...
  'useEverywhere.section.mcp.footer': 'In the Z-Design app, open Settings → Integrations to copy a client-specific install command (Cursor, Claude Code, Antigravity, VS Code) instead of editing JSON by hand.',
  'useEverywhere.section.http.tab': 'HTTP API',
  'useEverywhere.section.http.heading': 'Same REST + SSE surface the web UI uses',
  'useEverywhere.section.http.intro': 'The local daemon serves an HTTP API at `http://127.0.0.1:7456` (port configurable). Every endpoint the web UI calls is also fair game for your scripts. Streaming endpoints (chat turns, project runs) emit Server-Sent Events with the contract types in `@z-design/contracts`.',
  ...
  'useEverywhere.section.http.footer': 'Pure TypeScript types for every request/response live in `@z-design/contracts` — import them in your script for full autocomplete without wiring a generator.',
  'useEverywhere.section.skills.tab': 'Skills & headless',
  'useEverywhere.section.skills.heading': 'Drop-in Skills for any agent — even without Z-Design running',
  'useEverywhere.section.skills.intro': 'A Skill is a directory with a Claude-compatible `SKILL.md` (YAML front-matter + body). Z-Design extends the format with the `od:` namespace (`mode`, `preview`, `design_system`, `inputs`, …) so the same artifact can be used both inside Z-Design and by a vanilla agent like Claude Code, Codex, openclaw, or hermes. Discovery follows a precedence chain so projects can override their own skills.',
  ...
  'useEverywhere.section.skills.snippet1': 'Minimal SKILL.md (Claude-compatible front matter + Z-Design extras)',
  ...
  'entry.githubStarAria': 'Star Z-Design on GitHub',
  ...
  'newproj.locationDefault': 'Z-Design projects',
  ...
  'socialShare.zDesignSection': 'Share Z-Design',
  'socialShare.zDesignTitle': 'Z-Design on GitHub',
  'socialShare.zDesignText': 'Z-Design is an open-source workspace for creating, editing, deploying, and handing off design artifacts.',
  ...
  'socialShare.projectText': 'Built with Z-Design: {title}. View the deployed artifact and star the repo: {repo}',
  'socialShare.projectCopyText': 'Built with Z-Design: {title}\n{url}\nZ-Design repo: {repo}',
  ...
  'chat.activeFilePlaceholder': 'Ask Z-Design to change {file}...',
  ...
  'chat.mode.design.solves': 'Use it when you want Z-Design to generate files, update the workspace, or turn a brief into multimodal deliverables.',
  ...
  'preview.errorBody': 'The example HTML failed to fetch. Make sure Z-Design is running and try again.',
  'preview.retry': 'Retry',
  ...
  'designFiles.usefulInfoTip7': 'Enjoying Z-Design? Star us on GitHub.',
  ...
  'assistant.shareToZDesign': 'Share to Z-Design',
  'assistant.shareToZDesignBusy': 'Preparing package…',
  ...
  'pet.tabBuiltInHint': 'Curated companions bundled with Z-Design — pick one and adopt.',
  ...
  'pet.codexBundledTitle': 'Ships with Z-Design — no download needed.',
  ...
  'settings.projectLocationsDescription': 'Add work bases that can contain multiple Z-Design project folders. New projects are saved as one folder inside the selected base.',
  ...
  'settings.customInstructionsHint': 'Fixed instructions ZDesign should follow in every chat. Save facts, preferences, and project context as memories.',
  ...
  'updater.availableBody': 'Z-Design {version} is available. It will download before the installer can open.',
  ...
  'updater.installerOpenBody': 'The installer is open. Z-Design is quitting so you can finish the update.',
  ...
  'updater.quitButton': 'Quit Z-Design',
  'updater.quitFailedBody': 'The installer is open, but Z-Design could not quit. Quit Z-Design before replacing the app.',
  'updater.quitFailedTitle': 'Could not quit',
  'updater.quitting': 'Quitting...',
  'updater.ready': 'Update ready',
  'updater.readyGeneric': 'A new version is ready. Z-Design will close and open the installer.',
  'updater.readyVersion': 'Z-Design {version} is ready. Z-Design will close and open the installer.',
  ...
  'dsManager.createBody': 'Teach Z-Design your brand, product, code, assets, and design references.',
  ...
  'settings.customInstructionsDesc': 'Fixed instructions Z-Design follows in every chat. These are not saved memories; use Memory for facts, preferences, and project context.',
