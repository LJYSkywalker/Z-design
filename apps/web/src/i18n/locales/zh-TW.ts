...
export const zhTW: Dict = {
  ...
  'chat.amrCard.switchBody': '可切換到 Z-Design 官方 AMR 模型服務，無需設定 API Key。完成登入、授權與儲值後，將自動重試目前任務。',
  ...
  'settings.onboardingCreateBody': '描述你想做的網站、應用、Deck、圖片或影片，Z-Design 會建立專案，並保留可繼續編輯的產出。',
  ...
  'settings.onboardingCloudTitle': '登入 Z-Design',
  'settings.onboardingCloudBody': '登入後可直接用雲端 AI 開始設計，無需任何複雜設定。',
  'settings.onboardingCloudSignIn': '登入 Z-Design 雲端',
  ...
  'settings.onboardingGateTooltipAmr': '後續流程由 AI 執行，請先登入 Z-Design AMR。',
  ...
  'settings.agentInstall.pathHint': '如果你透過 npm 或 Homebrew 安裝了 CLI，但仍顯示為未安裝，請確認該工具的 bin 目錄已加入 Z-Design daemon 繼承的 PATH（在 macOS 上，Terminal 與 GUI 應用的 PATH 可能不同）。請參閱 QUICKSTART.md（「Local agent CLI and PATH」章節）。',
  'settings.agentInstall.stepOpenLinks': '在目標代理卡片上開啟「安裝」或「文件」連結。',
  'settings.agentInstall.stepAuth': '返回 Z-Design 前，請先在對應 CLI 完成驗證（登入或新增 API 憑證）。',
  ...
  'settings.mcpServerHint': '將 Z-Design 作為 MCP 伺服器暴露給你的編碼代理。',
  ...
  'settings.privacy': '隱私',
  'settings.privacyHint': '與 Z-Design 團隊共享哪些資料',
  'settings.privacyConsentKicker': '協助我們改進 Z-Design',
  'settings.privacyConsentLead': 'Z-Design 可以將使用資料分享給我們的團隊以協助改進。包含:',
  ...
  'settings.mcpHint': '讓其他專案中的程式碼代理（Claude Code、Cursor、VS Code、Antigravity、Zed、Windsurf）讀取您的 Z-Design 專案。您可以直接將設計匯入應用程式，無需先匯出 zip。',
  'settings.mcpDaemonError': '無法連線到本地守護程序以解析安裝路徑（{error}）。請確認 Z-Design 正在執行，然後重新開啟此面板。',
  'settings.mcpBuildDaemon': '請先建置守護行程。',
  'settings.mcpNodeMissing': '缺少 Node 執行檔。',
  'settings.mcpBuildHint': '缺少 apps/daemon/dist/cli.js。請執行 `pnpm --filter @z-design/daemon build` 後重新整理。',
  ...
  'settings.mcpCapabilityDefault': '預設使用您在 Z-Design 中開啟的專案與檔案，因此您可以說「在我的應用程式中建置這個」，無需重新說明是哪個設計。',
  'settings.mcpRunningNote': 'Z-Design 必須正在執行，MCP 工具呼叫才能成功。如果您在開啟 Z-Design 之前就已啟動 coding agent，請重新啟動 agent，使其能夠連線到正在執行的守護行程。',
  ...
  'entry.discordAria': '加入 Z-Design Discord',
  'entry.discordAriaWithOnline': '加入 Z-Design Discord - {online}',
  ...
  'homeHero.subtitlePrefix': 'GLM 原生的智慧型代理人設計工作台。',
  ...
  'homeHero.chip.createPluginHint': '編寫可重複使用的 Z-Design 外掛並將其加入「我的外掛」。',
  ...
  'handoff.promptIntro': '請基於這個 Z-Design 專案的本機資料夾繼續實作：',
  ...
  'pluginsHome.subtitle': '隨附於此執行環境、可立即使用的 Z-Design 工作流程。挑選一個以載入起始提示，或瀏覽登錄檔取得更多。',
  'pluginsHome.browseRegistry': '瀏覽登錄檔',
  'pluginsHome.count': '{total} 個中的 {filtered} 個',
  'pluginsHome.loadingCatalog': '正在載入目錄…',
  'pluginsHome.emptyCatalog': '目錄是空的。內建外掛隨 Z-Design 一同提供，應會自動出現在此處 — 若問題持續發生，請嘗試重新啟動 daemon。',
  ...
  'pluginCard.contributeAria': '將 {title} 貢獻至 Z-Design',
  'pluginCard.contributeTitle': '透過 pull request 將外掛貢獻至 Z-Design',
  ...
  'integrations.lede': '連接外部系統，把 MCP 工具帶入智能體迴圈，並在其他 IDE、腳本與自動化流程中使用 Z-Design。',
  ...
  'mcpClient.daemonError': '無法連線到本機 daemon。請確認 Z-Design 正在執行，然後重新開啟此面板。',
  'mcpClient.saveFailed': '儲存失敗。請確認 daemon 正在執行後再試一次。',
  'useEverywhere.modalAria': '隨處使用 Z-Design',
  'useEverywhere.modalTitle': '隨處使用 Z-Design',
  'useEverywhere.modalSubtitle': '將 Z-Design 接入任何 IDE、Agent 或指令碼 — CLI、HTTP、MCP 及 Skills。使用「複製 Agent 指南」並貼到 Claude Code、Codex、Cursor、openclaw 或 hermes 中，即可設定一切。',
  ...
  'useEverywhere.section.overview.heading': '你的 Agent 在哪裡運作，Z-Design 就在哪裡運作',
  'useEverywhere.section.overview.intro': 'Z-Design 不只是一個視窗 — 它是一個本機特權 daemon（`od`），外加一個 Skills + Design-Systems + Atoms 登錄。一旦它在您的機器上執行，任何程式碼 Agent（Claude Code、Codex、Cursor、OpenCode/openclaw、Hermes、您自己的指令碼）都能透過四個可互換的介面來驅動產生作業、檢視專案並產出設計成品。',
  'useEverywhere.section.overview.bullet1': 'CLI — 以 `od <command>` 用於無頭指令碼、CI 及 shell 自動化。',
  'useEverywhere.section.overview.bullet2': 'MCP server — 將 Z-Design 接成 Model Context Protocol 伺服器，讓任何支援 MCP 的 Agent 都能列出技能、執行情境並讀取成品。',
  'useEverywhere.section.overview.bullet3': 'HTTP API — `http://127.0.0.1:7456/api/*` REST + SSE 端點；與網頁 UI 使用的介面相同。',
  'useEverywhere.section.overview.bullet4': 'Skills — 隨插即用的 `SKILL.md` 套件（相容於 Claude），任何已在您 PATH 上的 Agent 完全無需 Z-Design 即可調用。',
  ...
  'useEverywhere.section.overview.footer': 'daemon 預設會寫入 `./.od/`（專案本機）。設定 `ZD_DATA_DIR=~/.open-design` 即可在多個專案間共用資料。',
  'useEverywhere.section.cli.tab': 'CLI · od',
  'useEverywhere.section.cli.heading': '從任何 shell 操作 Z-Design',
  ...
  'useEverywhere.section.mcp.heading': '將 Z-Design 以 MCP 伺服器形式開放給任何程式設計 Agent',
  'useEverywhere.section.mcp.intro': 'Z-Design 隨附一個 Model Context Protocol 伺服器（`od mcp`），讓任何支援 MCP 的用戶端 — Cursor、Claude Code、Antigravity、VS Code Copilot Chat、openclaw、hermes — 都能探索 Z-Design 工具（列出技能、算繪預覽、產生媒體、執行外掛），而無需手動使用 shell。daemon 透過 `GET /api/mcp/install-info` 為每個主要用戶端發布可直接貼上的安裝程式碼片段。',
  ...
  'useEverywhere.section.mcp.bullet4': '固定 `ZD_DATA_DIR`，讓衍生的 MCP 程序寫入 daemon 已使用的相同位置（避免在 macOS 打包應用程式套件中發生 EPERM）。',
  ...
  'useEverywhere.section.mcp.footer': '在 Z-Design 應用程式中，開啟「設定 → 整合」即可複製特定用戶端的安裝指令（Cursor、Claude Code、Antigravity、VS Code），而無需手動編輯 JSON。',
  'useEverywhere.section.http.tab': 'HTTP API',
  'useEverywhere.section.http.heading': '與 web UI 所用相同的 REST + SSE 介面',
  'useEverywhere.section.http.intro': '本機 daemon 會在 `http://127.0.0.1:7456` 提供 HTTP API（連接埠可設定）。web UI 呼叫的每個端點，你的指令稿同樣可以使用。串流端點（聊天回合、專案執行）會發出 Server-Sent Events，其合約型別定義於 `@z-design/contracts`。',
  ...
  'useEverywhere.section.http.footer': '每個請求／回應的純 TypeScript 型別都位於 `@z-design/contracts` 中 — 在您的指令碼中匯入它們，即可享有完整的自動完成，而無需接上產生器。',
  'useEverywhere.section.skills.tab': '技能與無頭模式',
  'useEverywhere.section.skills.heading': '適用於任何 Agent 的隨插即用 Skills — 即使 Z-Design 未在執行也行',
  'useEverywhere.section.skills.intro': 'Skill 是一個包含相容於 Claude 的 `SKILL.md`（YAML 前置資料 + 內文）的目錄。Z-Design 以 `od:` 命名空間（`mode`、`preview`、`design_system`、`inputs`、…）擴充此格式，因此同一個成品既能在 Z-Design 內使用，也能被像 Claude Code、Codex、openclaw 或 hermes 這類原生 Agent 使用。探索作業遵循一條優先順序鏈，因此專案可覆寫自己的技能。',
  ...
  'useEverywhere.section.skills.snippet1': '最精簡的 SKILL.md（Claude 相容的前置資料 + Z-Design 額外設定）',
  ...
  'entry.githubStarAria': '在 GitHub 為 Z-Design 點亮 Star',
  ...
  'newproj.locationDefault': 'Z-Design 專案',
  ...
  'socialShare.zDesignSection': '分享 Z-Design',
  'socialShare.zDesignTitle': 'GitHub 上的 Z-Design',
  'socialShare.zDesignText': 'Z-Design 是開源的設計智慧工作區，可以生成、編輯、部署並交付設計產物。',
  ...
  'socialShare.projectText': '我用 Z-Design 做了 {title}。查看已部署產物，也推薦收藏倉庫：{repo}',
  'socialShare.projectCopyText': '我用 Z-Design 做了 {title}\n{url}\nZ-Design 倉庫：{repo}',
  ...
  'chat.activeFilePlaceholder': '讓 Z-Design 修改 {file}...',
  ...
  'chat.mode.design.solves': '當你希望 Z-Design 產生檔案、更新 workspace，或把需求變成多模態可交付結果時使用。',
  ...
  'preview.shareTextDefault': 'Z-Design 模板：{title}',
  ...
  'preview.errorBody': '範例 HTML 載入失敗。請確認 Z-Design 正在執行後重試。',
  'preview.retry': '重試',
  ...
  'assistant.shareToZDesign': '分享到 Z-Design',
  'assistant.shareToZDesignBusy': '正在準備投稿…',
  ...
  'pet.navHint': '領養與自訂',
  'pet.tabBuiltIn': '內建',
  'pet.tabBuiltInHint': 'Z-Design 內建的精選寵物 — 一鍵領養。',
  ...
  'pet.codexBundled': '內建',
  'pet.codexBundledTitle': 'Z-Design 內建寵物，無需下載。',
  ...
  'settings.projectLocationsDescription': '新增可包含多個 Z-Design 專案資料夾的工作基目錄。新專案會儲存為所選基目錄中的一個資料夾。',
  ...
  'settings.customInstructionsHint': 'Z-Design 在每次聊天中都應遵循的固定指示。將事實、偏好與專案脈絡儲存為記憶。',
  ...
  'settings.memoryStarterProjectDesc': 'Z-Design v0.5 — 聊天驅動的設計編輯器',
  ...
  'updater.availableBody': 'Z-Design {version} 可用。下載完成後即可開啟安裝器。',
  ...
  'updater.done': '完成',
  'updater.failed': '更新失敗',
  'updater.installerOpenBody': '安裝器已開啟。Z-Design 正在結束，以便你完成更新。',
  ...
  'updater.quitButton': '結束 Z-Design',
  'updater.quitFailedBody': '安裝器已開啟，但 Z-Design 無法結束。替換應用程式前請先結束 Z-Design。',
  'updater.quitFailedTitle': '無法結束',
  'updater.quitting': '正在結束…',
  'updater.ready': '更新已就緒',
  'updater.readyGeneric': '新版本已就緒。Z-Design 會關閉並開啟安裝器。',
  'updater.readyVersion': 'Z-Design {version} 已就緒。Z-Design 會關閉並開啟安裝器。',
  ...
  'dsManager.createBody': '把你的品牌、產品、程式碼、素材與設計參考教給 Z-Design。',
  ...
  'settings.customInstructionsDesc': 'Z-Design 在每次對話中都會遵循的固定指令。它們不是儲存的記憶；事實、偏好與專案脈絡請使用「記憶」。',
