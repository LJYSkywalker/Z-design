...
export const zhCN: Dict = {
  ...
  'chat.amrCard.switchBody': '可切换到 Z-Design 官方 AMR 模型服务，无需配置 API Key。完成登录、授权和充值后，将自动重试当前任务。',
  ...
  'settings.onboardingCreateBody': '描述你想做的网站、应用、Deck、图片或视频，Z-Design 会创建项目，并保留可继续编辑的产物。',
  ...
  'settings.onboardingCloudTitle': '登录 Z-Design',
  'settings.onboardingCloudBody': '登录后可直接用云端 AI 开始设计，无需任何复杂配置。',
  'settings.onboardingCloudSignIn': '登录 Z-Design 云端',
  ...
  'settings.onboardingGateTooltipAmr': '后续流程由 AI 运行，请先登录 Z-Design AMR。',
  ...
  'settings.agentInstall.pathHint': '如果你通过 npm 或 Homebrew 安装了 CLI，但仍显示为未安装，请确认该工具的 bin 目录已加入 Z-Design daemon 继承的 PATH（在 macOS 上，Terminal 与 GUI 应用的 PATH 可能不同）。请参阅 QUICKSTART.md（“Local agent CLI and PATH” 章节）。',
  'settings.agentInstall.stepOpenLinks': '在目标代理卡片上打开“安装”或“文档”链接。',
  'settings.agentInstall.stepAuth': '返回 Z-Design 之前，请先在对应 CLI 中完成认证（登录或添加 API 凭据）。',
  ...
  'settings.mcpServerHint': '将 Z-Design 作为 MCP 服务器暴露给你的编码代理。',
  ...
  'settings.privacy': '隐私',
  'settings.privacyHint': '与 Z-Design 团队共享哪些数据',
  'settings.privacyConsentKicker': '帮助我们改进 Z-Design',
  'settings.privacyConsentLead': 'Z-Design 可以将使用数据共享给我们的团队以协助改进。包括:',
  ...
  'settings.mcpHint': '让其他仓库中的编码助手（Claude Code、Cursor、VS Code、Antigravity、Zed、Windsurf）读取你的 Z-Design 项目。无需先导出 zip，即可将设计拉取到你的应用中。',
  'settings.mcpDaemonError': '无法连接到本地守护进程以解析安装路径（{error}）。请确保 Z-Design 正在运行，然后重新打开此面板。',
  'settings.mcpBuildDaemon': '请先构建守护进程。',
  'settings.mcpNodeMissing': 'Node 二进制文件缺失。',
  'settings.mcpBuildHint': 'apps/daemon/dist/cli.js 缺失。请运行 `pnpm --filter @z-design/daemon build` 并刷新。',
  ...
  'settings.mcpCapabilityDefault': '默认使用你在 Z-Design 中打开的项目和文件，因此你可以直接说"在我的应用中构建这个"，无需重复说明是哪个设计。',
  'settings.mcpRunningNote': 'Z-Design 必须处于运行状态，MCP 工具调用才能成功。如果你在打开 Z-Design 之前启动了编码助手，请重启助手以便它能连接到正在运行的守护进程。',
  ...
  'homeHero.subtitlePrefix': 'GLM 原生的智能体设计工作台。',
  ...
  'homeHero.chip.createPluginHint': '创作可复用的 Z-Design 插件，并添加到我的插件。',
  ...
  'handoff.promptIntro': '请基于这个 Z-Design 项目的本地文件夹继续实现：',
  ...
  'pluginsHome.subtitle': '当前运行环境内置的 Z-Design 工作流。选择一个加载 starter prompt，或浏览插件市场查看更多。',
  'pluginsHome.browseRegistry': '浏览插件市场',
  'pluginsHome.count': '{filtered} / {total}',
  'pluginsHome.loadingCatalog': '正在加载目录…',
  'pluginsHome.emptyCatalog': '目录为空。Z-Design 会随附内置插件，如果这里一直为空，请尝试重启 daemon。',
  ...
  'pluginCard.contributeAria': '将 {title} 贡献到 Z-Design',
  'pluginCard.contributeTitle': '通过 pull request 将插件贡献到 Z-Design',
  ...
  'integrations.lede': '连接外部系统，把 MCP 工具带入智能体循环，并在其它 IDE、脚本和自动化中使用 Z-Design。',
  ...
  'mcpClient.daemonError': '无法连接本地 daemon。请确认 Z-Design 正在运行，然后重新打开此面板。',
  'mcpClient.saveFailed': '保存失败。请检查 daemon 是否正在运行后重试。',
  'useEverywhere.modalAria': '随处使用 Z-Design',
  'useEverywhere.modalTitle': '随处使用 Z-Design',
  'useEverywhere.modalSubtitle': '把 Z-Design 接入任何 IDE、智能体或脚本：CLI、HTTP、MCP 和 Skills。使用“复制智能体指南”，粘贴到 Claude Code、Codex、Cursor、openclaw 或 hermes 中即可完成配置。',
  ...
  'useEverywhere.section.overview.heading': 'Z-Design 可在智能体工作的任何地方运行',
  'useEverywhere.section.overview.intro': 'Z-Design 不只是一个窗口；它包含本地特权 daemon（`od`），以及 Skills + Design Systems + Atoms registry。它在你的机器上运行后，任何代码智能体（Claude Code、Codex、Cursor、OpenCode/openclaw、Hermes 或你自己的脚本）都可以通过四种可互换入口驱动生成、检查项目并产出设计制品。',
  'useEverywhere.section.overview.bullet1': 'CLI — 使用 `od <command>` 支持无头脚本、CI 和 shell 自动化。',
  'useEverywhere.section.overview.bullet2': 'MCP 服务器 — 将 Z-Design 作为 Model Context Protocol 服务器接入，使任何支持 MCP 的智能体都能列出技能、运行场景并读取制品。',
  'useEverywhere.section.overview.bullet3': 'HTTP API — `http://127.0.0.1:7456/api/*` REST + SSE 端点；与 Web UI 使用同一套接口。',
  'useEverywhere.section.overview.bullet4': 'Skills — 可直接放入的 `SKILL.md` 包（兼容 Claude），任何已在 PATH 中的智能体都能调用，甚至不需要打开 Z-Design。',
  ...
  'useEverywhere.section.overview.footer': 'daemon 默认写入项目本地的 `./.od/`。设置 `ZD_DATA_DIR=~/.open-design` 可在多个项目间共享数据。',
  'useEverywhere.section.cli.tab': 'CLI · od',
  'useEverywhere.section.cli.heading': '从任意 shell 驱动 Z-Design',
  ...
  'useEverywhere.section.mcp.heading': '把 Z-Design 暴露为任意代码智能体可用的 MCP 服务器',
  'useEverywhere.section.mcp.intro': 'Z-Design 内置 Model Context Protocol 服务器（`od mcp`），让任何支持 MCP 的客户端（Cursor、Claude Code、Antigravity、VS Code Copilot Chat、openclaw、hermes）都能发现 Z-Design 工具（列出技能、渲染预览、生成媒体、运行插件），无需手动 shell 调用。daemon 会通过 `GET /api/mcp/install-info` 为主流客户端发布可直接粘贴的安装片段。',
  ...
  'useEverywhere.section.mcp.bullet4': '固定 `ZD_DATA_DIR`，让派生的 MCP 进程写入 daemon 已使用的位置，避免打包 macOS app bundle 中的 EPERM 问题。',
  ...
  'useEverywhere.section.mcp.footer': '在 Z-Design 应用中，打开“设置 → 集成”即可复制特定客户端的安装命令（Cursor、Claude Code、Antigravity、VS Code），无需手写 JSON。',
  'useEverywhere.section.http.tab': 'HTTP API',
  'useEverywhere.section.http.heading': '与 Web UI 相同的 REST + SSE 接口',
  'useEverywhere.section.http.intro': '本地 daemon 在 `http://127.0.0.1:7456` 提供 HTTP API（端口可配置）。Web UI 调用的每个端点也都可以供你的脚本使用。流式端点（聊天回合、项目运行）会发出 Server-Sent Events，事件类型定义在 `@z-design/contracts` 中。',
  ...
  'useEverywhere.section.http.footer': '每个请求 / 响应的纯 TypeScript 类型都在 `@z-design/contracts` 中；脚本中导入即可获得完整自动补全，无需接入生成器。',
  'useEverywhere.section.skills.tab': 'Skills 和无头模式',
  'useEverywhere.section.skills.heading': '任意智能体可直接使用的 Skills，即使 Z-Design 未运行',
  'useEverywhere.section.skills.intro': 'Skill 是一个包含 Claude 兼容 `SKILL.md`（YAML front matter + 正文）的目录。Z-Design 通过 `od:` 命名空间（`mode`、`preview`、`design_system`、`inputs` 等）扩展该格式，使同一个制品既能在 Z-Design 内使用，也能被 Claude Code、Codex、openclaw 或 hermes 这类普通智能体使用。发现逻辑遵循优先级链，因此项目可以覆盖自己的 skills。',
  ...
  'useEverywhere.section.skills.snippet1': '最小 SKILL.md（Claude 兼容 front matter + Z-Design 扩展）',
  ...
  'entry.githubStarAria': '在 GitHub 上为 Z-Design 点亮 Star',
  ...
  'newproj.locationDefault': 'Z-Design 项目',
  ...
  'socialShare.zDesignText': 'Z-Design 是开源的设计智能工作区，可以生成、编辑、部署并交付设计产物。',
  ...
  'socialShare.projectText': '我用 Z-Design 做了 {title}。查看已部署产物，也推荐收藏仓库：{repo}',
  ...
  'chat.activeFilePlaceholder': '让 Z-Design 修改 {file}...',
  ...
  'chat.mode.design.solves': '当你希望 Z-Design 生成文件、更新工作区，或把需求变成多模态可交付结果时使用。',
  ...
  'preview.errorBody': '示例 HTML 加载失败。请确认 Z-Design 正在运行后重试。',
  'preview.retry': '重试',
  ...
  'assistant.shareToZDesignBusy': '正在准备投稿…',
  ...
  'pet.navHint': '领养与自定义',
  'pet.tabBuiltIn': '内置',
  'pet.tabBuiltInHint': 'Z-Design 内置的精选宠物 — 一键领养。',
  ...
  'pet.codexBundled': '内置',
  'pet.codexBundledTitle': 'Z-Design 内置宠物，无需下载。',
  ...
  'settings.projectLocationsDescription': '添加可包含多个 Z-Design 项目文件夹的工作基目录。新项目会作为所选基目录内的一个文件夹保存。',
  ...
  'settings.customInstructionsHint': 'ZDesign 每次对话都应遵守的固定指令。事实、偏好和项目上下文请保存为记忆。',
  ...
  'settings.memoryStarterProjectDesc': 'Z-Design v0.5 — 聊天驱动的设计编辑器',
  ...
  'updater.availableBody': 'Z-Design {version} 可用。下载安装完成后即可打开安装器。',
  ...
  'updater.done': '完成',
  'updater.failed': '更新失败',
  'updater.installerOpenBody': '安装器已打开。Z-Design 正在退出，以便你完成更新。',
  ...
  'updater.payloadReadyGeneric': 'Z-Design 更新已就绪。Z-Design 会关闭并自动重启。',
  'updater.payloadReadyVersion': 'Z-Design {version} 已就绪。Z-Design 会关闭并自动重启。',
  'updater.quitButton': '退出 Z-Design',
  'updater.quitFailedBody': '安装器已打开，但 Z-Design 无法退出。替换应用前请先退出 Z-Design。',
  'updater.quitFailedTitle': '无法退出',
  'updater.quitting': '正在退出…',
  'updater.ready': '更新已就绪',
  'updater.readyGeneric': '新版本已就绪。Z-Design 会关闭并打开安装器。',
  'updater.readyVersion': 'Z-Design {version} 已就绪。Z-Design 会关闭并打开安装器。',
  ...
  'dsManager.createBody': '把你的品牌、产品、代码、素材和设计参考教给 Z-Design。',
  ...
  'settings.customInstructionsDesc': 'Z-Design 在每次对话中都会遵循的固定指令。它们不是保存的记忆；事实、偏好和项目上下文请使用「记忆」。',
