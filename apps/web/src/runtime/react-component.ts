    ...
    <script>
      (function(){
        ...
        try {
          ...
          (0, eval)(compiled);
          var Component = window.__ZDesignComponent ||
...
window.__ZDesignComponent = window.__ZDesignComponent || (${componentFallbackExpression(transformed.defaultName)});`;
}
...
function transformExports(source: string): { code: string; defaultName: string | null } {
  ...
  code = code.replace(
    ...
    (_match, name: string | undefined) => {
      defaultName = name || 'ZDesignComponent';
  ...
  code = code.replace(/export\s+default\s+/g, () => {
    defaultName = 'ZDesignComponent';
    return 'const ZDesignComponent = ';
  });
  code = code.replace(
