...
export function buildDesignManifestContent(opts: {
  ...
  const title = opts.title || 'Z-Design artifact';
...
- Keep generated product UI free of Z-Design chrome, preview labels, or design-process annotations.
...
- Do not keep prototype-only annotations, frame labels, or Z-Design chrome in the production UI.
...
export async function captureHostRegionSnapshot(
  ...
  if (!isZDesignHostAvailable()) return null;
...
function injectParentPrintReadyCache(doc: string, nonce: string): string {
  ...
  const script = `<script>window.__odPrintReady=false;window.__odPrintReadyStarted=false;window.__odPrintSize=null;window.addEventListener('message',function(e){if(e.data&&e.data.nonce==='${nonce}'&&(e.source===window||(window.frames&&e.source===window.frames[0]))){if(e.data.type==='ZD_PRINT_READY_STARTED'){window.__odPrintReadyStarted=true;return}if(e.data.type==='ZD_PRINT_READY'){window.__odPrintReady=true;if(Number.isFinite(e.data.width)&&Number.isFinite(e.data.height)&&e.data.width>0&&e.data.height>0)window.__odPrintSize={width:e.data.width,height:e.data.height}}}});<\/script>`;
