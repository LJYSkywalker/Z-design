...
async function buildApiAttachmentContext(
  ...
  for (let index = 0; index < attachments.length; index += 1) {
    ...
    if (remaining <= 0) {
      blocks.push(
        '[Z-Design omitted remaining attached files because the attachment context budget was exhausted.]',
      );
      break;
    }
...
function clipAttachmentText(text: string, maxChars: number): string {
  ...
  return `${text.slice(0, maxChars)}\n\n[Z-Design truncated ${omitted} chars from this attachment before sending it to the API provider.]`;
}
