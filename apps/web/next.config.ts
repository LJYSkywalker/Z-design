...
// dev-all launcher overrides ZD_PORT after probing for a free port; we read
// the same env so /api, /artifacts, and /frames always reach the right
// daemon instance during `next dev`.
const DAEMON_PORT = Number(process.env.ZD_PORT) || 7456;
...
// server runtime with ZD_WEB_OUTPUT_MODE=server; in that mode the web sidecar
// owns the Next.js SSR server and proxies daemon routes at runtime. The
// packaged-size standalone spike uses ZD_WEB_OUTPUT_MODE=standalone to ask
// Next.js for a traced standalone server while keeping the sidecar-owned daemon
// proxy in front of it at runtime.
const isProd = process.env.NODE_ENV !== 'development';
const webOutputMode = process.env.ZD_WEB_OUTPUT_MODE;
...
function resolveWorkspaceRoot(): string {
  ...
  const override = process.env.ZD_WORKSPACE_ROOT;
  if (override && override.trim()) {
    const resolved = isAbsolute(override.trim()) ? override.trim() : resolve(WEB_ROOT, override.trim());
    if (!existsSync(resolved)) {
      throw new Error(
        `ZD_WORKSPACE_ROOT="${override}" resolved to "${resolved}" which does not exist. ` +
    ...
    if (rel.startsWith('..') || isAbsolute(rel)) {
      throw new Error(
        `ZD_WORKSPACE_ROOT="${override}" resolved to "${canonicalResolved}" but WEB_ROOT "${canonicalWebRoot}" ` +
    ...
    // (for example `@z-design/contracts`), and Next would later fail deep
    ...
    if (!existsSync(resolve(canonicalResolved, 'pnpm-workspace.yaml'))) {
      throw new Error(
        `ZD_WORKSPACE_ROOT="${override}" resolved to "${canonicalResolved}" but no ` +
...
function resolveDistDir(defaultValue: string) {
  if (process.env.ZD_WEB_PROD === '1') return defaultValue;
  const configured = process.env.ZD_WEB_DIST_DIR;
  if (!configured) return defaultValue;
  return toPosixPath(isAbsolute(configured) ? relative(WEB_ROOT, configured) || '.' : configured);
}

const DIST_DIR = shouldStaticExport && !process.env.ZD_WEB_DIST_DIR
  ? null
  : resolveDistDir('.next');

function resolveDevTsconfigPath() {
  const configured = process.env.ZD_WEB_TSCONFIG_PATH;
...
function configuredAllowedDevHosts(): string[] {
  const configured = (process.env.ZD_ALLOWED_DEV_ORIGINS ?? '')
    .split(',')
    .map(parseAllowedDevHost)
    .filter((host): host is string => host != null);

  const allowedOrigins = (process.env.ZD_ALLOWED_ORIGINS ?? '')
    .split(',')
    .map(parseAllowedDevHost)
    .filter((host): host is string => host != null);

  const bindHost = parseAllowedDevHost(process.env.ZD_HOST ?? '');
...
const nextConfig: NextConfig = {
  ...
  transpilePackages: ['@z-design/components'],
  turbopack: {
    root: WORKSPACE_ROOT,
  },
