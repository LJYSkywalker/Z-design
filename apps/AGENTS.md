...
- `apps/web`: Next.js 16 App Router + React 18 web runtime. Entrypoints live in `apps/web/app/`; the main client shell is `apps/web/src/App.tsx`. During local `tools-dev` web runs, `apps/web/next.config.ts` rewrites `/api/*`, `/artifacts/*`, and `/frames/*` to `ZD_PORT`.
...
- Packaged web uses Next.js SSR through the web sidecar; do not put Next output under daemon `ZD_RESOURCE_ROOT`.
- Packaged `ZD_RESOURCE_ROOT` is only for daemon non-Next read-only resources: `skills/`, `design-systems/`, and `frames/`.
...
pnpm --filter @z-design/web test
pnpm --filter @z-design/daemon typecheck
pnpm --filter @z-design/daemon test
pnpm --filter @z-design/daemon build
pnpm --filter @z-design/desktop typecheck
pnpm --filter @z-design/desktop build
pnpm --filter @z-design/packaged typecheck
pnpm --filter @z-design/packaged build
```
