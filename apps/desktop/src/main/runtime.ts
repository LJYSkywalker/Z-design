...
import type { ZDesignHostActionResult, ZDesignHostCaptureResult, ZDesignHostUpdaterActionOptions } from "@z-design/host";
...
const DESIGN_BROWSER_PARTITION = "persist:z-design-design-browser";
...
function checkOptionsFromHost(options: unknown): { autoDownload?: boolean } | undefined {
  const input = options as ZDesignHostUpdaterActionOptions | null | undefined;
...
export async function createDesktopRuntime(options: DesktopRuntimeOptions): Promise<DesktopRuntime> {
  ...
  ipcMain.handle("browser:clear-data", async (event, rawOptions: unknown): Promise<ZDesignHostActionResult> => {
  ...
  ipcMain.handle("od:update:quit", async (event): Promise<ZDesignHostActionResult> => {
    requireMainWindowSender(event);
    const status = await (options.updater?.status() ?? unavailableUpdaterStatus());
  ...
  ipcMain.handle('od:capture-page', async (event, rawOptions: unknown): Promise<ZDesignHostCaptureResult> => {
    if (event.sender !== window.webContents) {
