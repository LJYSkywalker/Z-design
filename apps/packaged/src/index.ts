...
import { parseLauncherAfterQuitArgs } from "@z-design/launcher-proto";
import {
  bootstrapSidecarRuntime,
  createSidecarLaunchEnv,
  resolveAppIpcPath,
} from "@z-design/sidecar";
import { applyOsLocaleSwitch, createSplashWindow, setSplashStage } from "@z-design/desktop/main";
import { readProcessStamp } from "@z-design/platform";
...
function applyPackagedUpdaterEnv(updateMetadataUrl: string | null): void {
  ...
  if (process.env.ZD_UPDATE_METADATA_URL != null && process.env.ZD_UPDATE_METADATA_URL.length > 0) return;
  process.env.ZD_UPDATE_METADATA_URL = updateMetadataUrl;
}
...
async function main(): Promise<void> {
  ...
  const { runDesktopMain } = await import("@z-design/desktop/main");
