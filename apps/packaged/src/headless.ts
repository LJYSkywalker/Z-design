...
import { bootstrapSidecarRuntime, createJsonIpcServer, resolveAppIpcPath } from "@z-design/sidecar";
...
function resolveHeadlessNamespaceBaseRoot(): string {
  const odDataDir = process.env.ZD_DATA_DIR;
...
function resolveHeadlessConfig(): PackagedConfig {
  ...
  // ZD_RESOURCE_ROOT may be set by a launcher script; otherwise default to a
  ...
  return {
    ...
    updateMetadataUrl: process.env.ZD_UPDATE_METADATA_URL?.trim() || null,
...
async function main(): Promise<void> {
  ...
  const sidecars = await startPackagedSidecars(runtime, paths, {
    ...
    // Pinning ZD_REQUIRE_DESKTOP_AUTH here would arm a gate no client
