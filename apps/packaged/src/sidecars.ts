 ...
 * boots, but the ZD_LEGACY_DATA_DIR one-shot recovery flow can synch-
...
export function resolveDaemonStatusTimeoutMs(
  ...
  const raw = env.ZD_LEGACY_DATA_DIR;
 ...
 * #710 migrator's LegacyMigrationError on invalid ZD_LEGACY_DATA_DIR,
...
// @z-design/platform's wellKnownUserToolchainBins so the daemon
...
function createPackagedDaemonManagedPathEnv(
  ...
  return {
    ZD_DATA_DIR: paths.dataRoot,
    ZD_RESOURCE_ROOT: paths.resourceRoot,
    ZD_INSTALLATION_DIR: paths.installationRoot,
  };
}
...
export function buildPackagedDaemonSpawnEnv(
  ...
  return {
    ...
    ...(options.requireDesktopAuth ? { ZD_REQUIRE_DESKTOP_AUTH: "1" } : {}),
    ...
    ...(options.appVersion == null ? {} : { ZD_APP_VERSION: options.appVersion }),
    ...
    // ZD_LEGACY_DATA_DIR is the one-shot recovery handle for users
    ...
    ...(options.legacyDataDir == null || options.legacyDataDir.length === 0
      ? {}
      : { ZD_LEGACY_DATA_DIR: options.legacyDataDir }),
...
export async function startPackagedSidecars(
  ...
  try {
    ...
    const daemon = await spawnSidecarChild({
      app: APP_KEYS.DAEMON,
      entryPath: options.daemonSidecarEntry ?? resolveSidecarEntry("@z-design/daemon", "sidecar"),
      env: buildPackagedDaemonSpawnEnv(paths, {
        appVersion: options.appVersion,
        amrProfile: options.amrProfile,
        daemonCliEntry: options.daemonCliEntry,
        legacyDataDir: process.env.ZD_LEGACY_DATA_DIR ?? null,
    ...
    const daemonStatus = await waitForStatus<DaemonStatusSnapshot>(
      ...
      // invalid ZD_LEGACY_DATA_DIR, existing target payload, symlink,
    ...
    const web = await spawnSidecarChild({
      app: APP_KEYS.WEB,
      entryPath: options.webSidecarEntry ?? resolveSidecarEntry("@z-design/web", "sidecar"),
      env: {
        [SIDECAR_ENV.DAEMON_PORT]: extractPort(daemonStatus.url),
        [SIDECAR_ENV.WEB_PORT]: "0",
        ...(options.webStandaloneRoot == null ? {} : { ZD_WEB_STANDALONE_ROOT: options.webStandaloneRoot }),
        ZD_WEB_OUTPUT_MODE: options.webOutputMode,
        PORT: "0",
      },
      nodeCommand: options.nodeCommand,
      paths,
