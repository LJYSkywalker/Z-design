...
import { APP_KEYS, normalizeNamespace } from "@z-design/sidecar-proto";
...
function resolvePackagedDataRoot(
  ...
  const odDataDir = env.ZD_DATA_DIR?.trim();
  if (odDataDir) {
    ...
    if (!isAbs) {
      throw new PackagedPathAccessError(
        [
          "Open Design's packaged runtime requires ZD_DATA_DIR to be an absolute path.",
          "",
          `Configured value: ${odDataDir}`,
          "",
          "Set ZD_DATA_DIR to an absolute path (for example, C:\\\\Users\\\\You\\\\ZDesign on Windows or /Users/you/ZDesign on macOS/Linux) and relaunch Open Design.",
        ].join("\n"),
        { title: "Open Design cannot start with this ZD_DATA_DIR" },
      );
    }
    ...
    if (scopedNamespace) {
      if (scopedNamespace !== namespace) {
        throw new PackagedPathAccessError(
          [
            "Open Design's packaged runtime requires ZD_DATA_DIR to target the active namespace.",
            "",
            `Configured value: ${odDataDir}`,
