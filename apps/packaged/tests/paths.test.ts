...
describe("resolvePackagedNamespacePaths", () => {
  ...
  it("uses ZD_DATA_DIR as a base for the namespace-scoped packaged daemon dataRoot", () => {
    const config = fakeConfig();
    const override = join("C:", "Users", "Fred", "MyProject", "design", ".od");

    expect(
      resolvePackagedNamespacePaths(config, config.namespace, { ZD_DATA_DIR: override }).dataRoot,
    ).toBe(join(override, "namespaces", config.namespace, "data"));
  });

  it("keeps shared ZD_DATA_DIR overrides isolated across packaged namespaces", () => {
    const config = fakeConfig();
    const override = join("C:", "Users", "Fred", "MyProject", "design", ".od");
    const stable = resolvePackagedNamespacePaths(config, "release-stable-win", {
      ZD_DATA_DIR: override,
    });
  ...
  it("preserves already-scoped packaged ZD_DATA_DIR values as the final daemon dataRoot", () => {
    const config = fakeConfig();
    const override = join(
      "C:",
      "Users",
  ...
  it("rejects already-scoped ZD_DATA_DIR values that point at a different packaged namespace", () => {
    const config = fakeConfig();
    const override = join(
      "C:",
      "Users",
  ...
  it("forwards the ZD_DATA_DIR-resolved dataRoot into sidecar launch paths", () => {
    const config = fakeConfig();
    const override = join("C:", "Users", "Fred", "MyProject", "design", ".od");
    const paths = resolvePackagedNamespacePaths(config, config.namespace, {
      ZD_DATA_DIR: override,
  ...
  it("does not read process.env implicitly so headless can keep namespace-root ZD_DATA_DIR semantics", () => {
    const config = fakeConfig();
    const original = process.env.ZD_DATA_DIR;
    try {
      process.env.ZD_DATA_DIR = join("C:", "Users", "Fred", "MyProject", "design", ".od");
      expect(resolvePackagedNamespacePaths(config).dataRoot).toBe(
        join(config.namespaceBaseRoot, config.namespace, "data"),
      );
    } finally {
      if (original == null) delete process.env.ZD_DATA_DIR;
      else process.env.ZD_DATA_DIR = original;
    }
  });

  it("rejects relative ZD_DATA_DIR values instead of resolving them against cwd", () => {
    const config = fakeConfig();

    expect(
      () => resolvePackagedNamespacePaths(config, config.namespace, { ZD_DATA_DIR: "project/.od" }),
    ).toThrow(/ZD_DATA_DIR.*absolute path/);
  });

  it("surfaces the relative-ZD_DATA_DIR rejection as PackagedPathAccessError so packaged main() can show a dialog", () => {
    const config = fakeConfig();
    ...
    expect(err.title).toMatch(/ZD_DATA_DIR/);
    expect(err.message).toContain("project/.od");
    expect(err.message).toMatch(/absolute path/);
  });

  it("rejects Windows-style ZD_DATA_DIR values on non-Windows hosts so the absolute-path guard is platform-correct", () => {
    const config = fakeConfig();
    ...
    try {
      expect(
        () =>
          resolvePackagedNamespacePaths(config, config.namespace, {
            ZD_DATA_DIR: "C:\\Users\\Fred\\OD",
          }),
      ).toThrow(PackagedPathAccessError);
      expect(
        () =>
          resolvePackagedNamespacePaths(config, config.namespace, {
            ZD_DATA_DIR: "\\\\server\\share",
          }),
      ).toThrow(PackagedPathAccessError);
    } finally {
      restore();
