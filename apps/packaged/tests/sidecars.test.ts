/**
 * Regression coverage for the ZD_LEGACY_DATA_DIR migration-aware
 * daemon status timeout in apps/packaged/src/sidecars.ts.
 *
 * Background: when the user is recovering 0.3.x `.od/` data via
 * ZD_LEGACY_DATA_DIR, apps/daemon/src/legacy-data-migrator.ts runs a
 ...
 * @see https://github.com/Aqu-210L/Z-design/issues/710
 */
...
describe('resolveDaemonStatusTimeoutMs', () => {
  ...
  it('treats an empty ZD_LEGACY_DATA_DIR as unset', () => {
    expect(resolveDaemonStatusTimeoutMs({ ZD_LEGACY_DATA_DIR: '' })).toBe(35_000);
  });

  it('extends the budget to 30 minutes when ZD_LEGACY_DATA_DIR is set', () => {
    ...
    const value = resolveDaemonStatusTimeoutMs({
      ZD_LEGACY_DATA_DIR: '/path/to/old/.od',
    });
  ...
  it('falls back to process.env when called with no argument', () => {
    const original = process.env.ZD_LEGACY_DATA_DIR;
    try {
      delete process.env.ZD_LEGACY_DATA_DIR;
      expect(resolveDaemonStatusTimeoutMs()).toBe(35_000);
      process.env.ZD_LEGACY_DATA_DIR = '/some/legacy/path';
      expect(resolveDaemonStatusTimeoutMs()).toBe(30 * 60 * 1000);
    } finally {
      if (original == null) delete process.env.ZD_LEGACY_DATA_DIR;
      else process.env.ZD_LEGACY_DATA_DIR = original;
    }
  });
});
...
describe('buildPackagedDaemonSpawnEnv', () => {
  ...
  it('sets ZD_REQUIRE_DESKTOP_AUTH=1 when requireDesktopAuth=true (Electron entry)', () => {
    ...
    expect(env.ZD_REQUIRE_DESKTOP_AUTH).toBe('1');
    expect(env.ZD_DATA_DIR).toBe('/tmp/od-pkg/data');
    expect(env.ZD_RESOURCE_ROOT).toBe('/tmp/od-pkg/resources');
    expect(env.ZD_APP_VERSION).toBe('1.2.3');
    expect(env.ZD_LEGACY_DATA_DIR).toBeUndefined();
  });

  it('omits ZD_REQUIRE_DESKTOP_AUTH entirely when requireDesktopAuth=false (headless)', () => {
    const env = buildPackagedDaemonSpawnEnv(fakePaths(), {
      appVersion: null,
      daemonCliEntry: null,
      legacyDataDir: null,
    ...
    // the daemon's gate trigger is `process.env.ZD_REQUIRE_DESKTOP_AUTH === '1'`,
    ...
    expect('ZD_REQUIRE_DESKTOP_AUTH' in env).toBe(false);
    expect(env.ZD_DATA_DIR).toBe('/tmp/od-pkg/data');
    expect(env.ZD_APP_VERSION).toBeUndefined();
  });

  it('forwards ZD_LEGACY_DATA_DIR only when set, irrespective of requireDesktopAuth', () => {
    ...
    expect(withLegacy.ZD_LEGACY_DATA_DIR).toBe('/old/.od');
    ...
    expect('ZD_LEGACY_DATA_DIR' in withEmptyLegacy).toBe(false);
  });

  it('forwards daemonCliEntry through ZD_DAEMON_CLI_PATH when set', () => {
    const env = buildPackagedDaemonSpawnEnv(fakePaths(), {
      appVersion: null,
    ...
    expect(env.ZD_DAEMON_CLI_PATH).toBe('/path/to/cli/dist/index.js');
  });
...
describe('waitForStatus child-exit fast-fail', () => {
  // mrcfps round-7: when ZD_LEGACY_DATA_DIR is set the daemon status
