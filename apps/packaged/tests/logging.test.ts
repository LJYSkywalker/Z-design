 ...
 * trips a test.
 *
 * @see https://github.com/Aqu-210L/Z-design/issues/895
 */
...
describe('createPackagedDesktopLogger log-write failures', () => {
  ...
  it('does not let desktop log append failures escape through the logger', () => {
    ...
    const previousEcho = process.env.ZD_DESKTOP_LOG_ECHO;
    process.env.ZD_DESKTOP_LOG_ECHO = '0';
    try {
      ...
      if (previousEcho == null) {
        delete process.env.ZD_DESKTOP_LOG_ECHO;
      } else {
        process.env.ZD_DESKTOP_LOG_ECHO = previousEcho;
      }
