...
import { SIDECAR_MESSAGES } from "@z-design/sidecar-proto";
  ...
  it("focuses an existing namespace desktop and exits", async () => {
    ...
    try {
      ...
      const result = await inspectExistingDesktopForLauncher("release-beta-win", {
        paths,
        requestIpc: (async (_ipcPath: string, message: unknown, options?: { timeoutMs?: number }) => {
        ...
        }) as typeof import("@z-design/sidecar").requestJsonIpc,
      });
