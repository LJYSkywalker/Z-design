// Loader for `plugins/_official/<bucket>/<slug>/z-design.json` —
...
//     Collections; bundled plugins ship `z-design.json` (a
...
function REPO_FOR_BUCKET(bucket: BundledBucket): string {
  return `https://github.com/Aqu-210L/Z-design/tree/main/plugins/_official/${bucket}`;
}
  ...
  for (const bucket of BUNDLED_BUCKETS) {
    ...
    for (const name of readdirSync(dir)) {
      ...
      const record = loadOne({
        manifestPath: path.join(root, bucket, name, 'z-design.json'),
...
export function getDetailPlugins(): ReadonlyArray<BundledPluginRecord> {
  ...
  if (root) {
    for (const bucket of BUNDLED_BUCKETS) {
      ...
      for (const name of readdirSync(dir)) {
        ...
        const record = loadOne({
          manifestPath: path.join(dir, name, 'z-design.json'),
  ...
  if (community) {
    for (const name of readdirSync(community)) {
      ...
      const record = loadOne({
        manifestPath: path.join(dir, 'z-design.json'),
        slugDir: dir,
        slug: name,
        bucket: 'community',
        routeId: `community/${name}`,
        sourceUrl: `https://github.com/Aqu-210L/Z-design/tree/main/plugins/community/${name}`,
      });
      if (record) out.push(record);
    }
  }
