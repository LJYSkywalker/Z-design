...
- **Surface** (\`#f4f5f8\`): Light canvas for content blocks.`,
  ...
  shareTemplate: ({ title, url }) =>
    `🎨 Just discovered ${title} on @ZDesignHQ — the open-source Claude Design alternative.
...
const overrides: Partial<Record<LandingLocaleCode, Partial<PluginsCopy>>> = {
  zh: {
    ...
    shareTemplate: ({ title, url }) => `🎨 安利一个：@ZDesignHQ 上的 ${title} —— Claude Design 的开源替代品。\n✨ 本地优先 · 自带模型 · 让你自己的 agent 做设计。\n\n→ ${url}`,
    ...
    faqItems: [
      ...
      },
      {
        question: '我如何贡献一个模板？',
        answer:
          '向 <a href="https://github.com/Aqu-210L/Z-design/tree/main/plugins/_official" target="_blank" rel="noopener">nexu-io/open-design/plugins/_official</a> 提交 PR，新文件夹包含 <code>z-design.json</code>、<code>SKILL.md</code> 和可运行的 <code>example.html</code>。仓库中的 <code>CONTRIBUTING.md</code> 贡献指南会逐步说明清单字段。被批准的贡献会进入公开目录，在下次部署时自动显示在此处。',
      },
      {
  ...
  'zh-tw': {
    ...
    shareTemplate: ({ title, url }) => `🎨 推薦一個：@ZDesignHQ 上的 ${title} —— Claude Design 的開源替代品。\n✨ 本地優先 · 自帶模型 · 讓你自己的 agent 做設計。\n\n→ ${url}`,
    ...
    faqItems: [
      ...
      },
      {
        question: '我如何貢獻範本？',
        answer:
          '針對 <a href="https://github.com/Aqu-210L/Z-design/tree/main/plugins/_official" target="_blank" rel="noopener">nexu-io/open-design/plugins/_official</a> 開立 PR，其中包含新資料夾，內含 <code>z-design.json</code>、<code>SKILL.md</code> 和可執行的 <code>example.html</code>。回購中 <code>CONTRIBUTING.md</code> 內的貢獻者指南會詳細說明清單欄位。已核准的貢獻將登陸公開目錄，並在下一次部署時自動出現在此處。',
      },
      {
  ...
  ja: {
    ...
    shareTemplate: ({ title, url }) => `🎨 @ZDesignHQ で ${title} を発見 —— オープンソースの Claude Design 代替。\n✨ ローカル優先 · BYOK · あなたのエージェントが設計する。\n\n→ ${url}`,
    ...
    faqItems: [
      ...
      },
      {
        ...
        answer:
          '<a href="https://github.com/Aqu-210L/Z-design/tree/main/plugins/_official" target="_blank" rel="noopener">nexu-io/open-design/plugins/_official</a>に対してPRを開き、<code>z-design.json</code>、<code>SKILL.md</code>、実行可能な<code>example.html</code>を含む新しいフォルダを追加してください。リポジトリの<code>CONTRIBUTING.md</code>に投稿ガイドがあり、マニフェストフィールドについて説明しています。承認された投稿は公開カタログに載り、次のデプロイで自動的にここに表示されます。',
      },
      {
  ...
  ko: {
    ...
    shareTemplate: ({ title, url }) => `🎨 @ZDesignHQ에서 ${title} 발견 —— 오픈 소스 Claude Design 대안.\n✨ 로컬 우선 · BYOK · 에이전트가 디자인합니다.\n\n→ ${url}`,
  ...
  de: {
    ...
    shareTemplate: ({ title, url }) => `🎨 Gerade entdeckt: ${title} auf @ZDesignHQ — die Open-Source-Alternative zu Claude Design.\n✨ Local-first · BYOK · dein Agent designt.\n\n→ ${url}`,
  ...
  fr: {
    ...
    shareTemplate: ({ title, url }) => `🎨 Découvert : ${title} sur @ZDesignHQ — l’alternative open-source à Claude Design.\n✨ Local-first · BYOK · votre agent fait le design.\n\n→ ${url}`,
  ...
  ru: {
    ...
    shareTemplate: ({ title, url }) => `🎨 Нашёл ${title} на @ZDesignHQ — open-source альтернативу Claude Design.\n✨ Локально · BYOK · агент сам делает дизайн.\n\n→ ${url}`,
  ...
  es: {
    ...
    shareTemplate: ({ title, url }) => `🎨 Acabo de descubrir ${title} en @ZDesignHQ — la alternativa open-source a Claude Design.\n✨ Local-first · BYOK · tu agente diseña.\n\n→ ${url}`,
  ...
  'pt-br': {
    ...
    shareTemplate: ({ title, url }) => `🎨 Acabei de descobrir ${title} no @ZDesignHQ — a alternativa open-source ao Claude Design.\n✨ Local-first · BYOK · seu agente faz o design.\n\n→ ${url}`,
  ...
  it: {
    ...
    shareTemplate: ({ title, url }) => `🎨 Ho appena scoperto ${title} su @ZDesignHQ — l’alternativa open-source a Claude Design.\n✨ Local-first · BYOK · il tuo agente progetta.\n\n→ ${url}`,
  ...
  id: {
    ...
    shareTemplate: ({ title, url }) => `🎨 Baru nemu ${title} di @ZDesignHQ — alternatif open-source untuk Claude Design.\n✨ Local-first · BYOK · agent kamu yang nge-desain.\n\n→ ${url}`,
  ...
  pl: {
    ...
    shareTemplate: ({ title, url }) => `🎨 Właśnie odkryłem ${title} na @ZDesignHQ — open-source’ową alternatywę dla Claude Design.\n✨ Local-first · BYOK · twój agent projektuje.\n\n→ ${url}`,
  ...
  ar: {
    ...
    shareTemplate: ({ title, url }) => `🎨 اكتشفت للتو ${title} على @ZDesignHQ — البديل مفتوح المصدر لـ Claude Design.\n✨ محلي أولًا · BYOK · وكيلك يصمّم.\n\n→ ${url}`,
    ...
    faqItems: [
      ...
      },
      {
        ...
        answer:
          'افتح PR ضد <a href="https://github.com/Aqu-210L/Z-design/tree/main/plugins/_official" target="_blank" rel="noopener">nexu-io/open-design/plugins/_official</a> بمجلد جديد يحتوي على <code>z-design.json</code> و <code>SKILL.md</code> و <code>example.html</code> قابل للتشغيل. يرشدك دليل المساهم في <code>CONTRIBUTING.md</code> الخاص بالمستودع عبر حقول البيان. تظهر المساهمات المعتمدة في الكتالوج العام وتظهر هنا تلقائياً عند النشر التالي.',
      },
      {
  ...
  tr: {
    ...
    shareTemplate: ({ title, url }) => `🎨 Yeni keşfettim: ${title} (@ZDesignHQ) — Claude Design’a açık kaynaklı alternatif.\n✨ Local-first · BYOK · ajanın tasarlıyor.\n\n→ ${url}`,
  ...
  uk: {
    ...
    shareTemplate: ({ title, url }) => `🎨 Щойно знайшов ${title} на @ZDesignHQ — open-source альтернативу Claude Design.\n✨ Local-first · BYOK · ваш агент робить дизайн.\n\n→ ${url}`,
  ...
  vi: {
    ...
    shareTemplate: ({ title, url }) => `🎨 Vừa khám phá ${title} trên @ZDesignHQ — giải pháp mã nguồn mở thay thế Claude Design.\n✨ Ưu tiên local · BYOK · agent của bạn thiết kế.\n\n→ ${url}`,
  ...
  nl: {
    ...
    shareTemplate: ({ title, url }) => `🎨 Net ontdekt: ${title} op @ZDesignHQ — het open-source alternatief voor Claude Design.\n✨ Local-first · BYOK · jouw agent ontwerpt.\n\n→ ${url}`,
