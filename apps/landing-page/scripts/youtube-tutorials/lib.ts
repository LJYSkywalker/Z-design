 ...
 */
export async function isAboutZDesign(video: VideoInput): Promise<boolean> {
  const system =
    'You decide whether a YouTube video is specifically about the open-source product "Open Design" ' +
    'by nexu-io (github.com/Aqu-210L/Z-design) — a self-evolving design agent that runs on coding ' +
    ...
    'Reply with strict JSON: {"isZDesign": boolean, "reason": string}.';
  ...
  try {
    const out = extractJson(await callLLM(system, user, 256)) as { isZDesign?: boolean };
    return out.isZDesign === true;
  } catch {
...
export async function scoreCandidate(video: VideoInput): Promise<CandidateScore> {
  const system =
    ...
    'Open Design (github.com/Aqu-210L/Z-design, ~50k stars) is an open-source self-evolving design ' +
    ...
    'Reply with STRICT JSON: {"isZDesign": boolean, "completeness": 0-5, "relevance": 0-5, "reason": string (<=120 chars)}.';
  const user = JSON.stringify({
    title: video.title,
    channel: video.author,
  ...
  try {
    ...
    return { isZDesign: false, overall: 0, completeness: 0, relevance: 0, reach: 0, recommend: false, reason: 'score failed' };
  }
  ...
  return { isZDesign, overall, completeness, relevance, reach, recommend, reason };
}
