 ...
 *
 * Run with: `pnpm --filter @z-design/landing-page previews`
 *
...
/**
 * Bundled plugins (`plugins/_official/<bucket>/<slug>/z-design.json`)
...
async function buildBundledPluginJobs(): Promise<Job[]> {
  ...
  for (const bucket of BUNDLED_BUCKETS) {
    ...
    for (const entry of entries) {
      ...
      const manifestPath = path.join(slugDir, 'z-design.json');
