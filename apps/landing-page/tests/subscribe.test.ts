...
describe("newsletter subscribe welcome email", () => {
  ...
  it("sends and records a welcome email for a new subscriber", async () => {
    ...
    assert.equal(emailBody.from, "Open Design <updates@z-design.ai>");
    assert.equal(emailBody.reply_to, "updates@z-design.ai");
    assert.equal(emailBody.to, "user@example.com");
    assert.equal(emailBody.subject, "Welcome to ZDesign — you're in 🎉");
    assert.match(emailBody.text, /Thanks for subscribing to the ZDesign newsletter/);
    assert.match(emailBody.html, /Welcome to ZDesign/);
