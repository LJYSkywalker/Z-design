...
async function withOnlyFakeAgent<T>(
  ...
  const dir = await fsp.mkdtemp(path.join(os.tmpdir(), 'od-conn-test-bin-'));
  const oldPath = process.env.PATH;
  const oldAgentHome = process.env.ZD_AGENT_HOME;
  ...
  try {
    ...
    process.env.PATH = oldPath;
    if (oldAgentHome === undefined) delete process.env.ZD_AGENT_HOME;
    else process.env.ZD_AGENT_HOME = oldAgentHome;
...
fs.writeFileSync(${JSON.stringify(envFile)}, JSON.stringify({
  ...
  SHOULD_NOT_PASS: process.env.ZD_CONNECTION_TEST_SHOULD_NOT_PASS || null,
}));
console.log(JSON.stringify({ type: 'item.completed', item: { type: 'agent_message', text: 'ok' } }));
setImmediate(() => process.exit(0));
`,
            ...
            body: JSON.stringify({
              ...
              agentCliEnv: {
                codex: {
                  ...
                  ZD_CONNECTION_TEST_SHOULD_NOT_PASS: 'leaked',
                },
                claude: {
...
`,
  ...
  it('reports an early-phase diagnostics block when the agent CLI is missing (#2248)', async () => {
    ...
    // overrides and ZD_AGENT_HOME-scoped toolchain bins.
    const oldPath = process.env.PATH;
    const oldClaudeBin = process.env.CLAUDE_BIN;
    const oldAgentHome = process.env.ZD_AGENT_HOME;
    ...
    process.env.PATH = '';
    delete process.env.CLAUDE_BIN;
    process.env.ZD_AGENT_HOME = emptyHome;
    try {
      const result = await testAgentConnection({ agentId: 'claude' });
      expect(result.ok).toBe(false);
...
describe('connection test timeout overrides', () => {
  it('returns the fallback when the override is missing or empty', () => {
    expect(
      resolveConnectionTestTimeoutMs('ZD_CONNECTION_TEST_PROVIDER_TIMEOUT_MS', 12_000, {}),
    ).toBe(12_000);
    expect(
      resolveConnectionTestTimeoutMs('ZD_CONNECTION_TEST_AGENT_TIMEOUT_MS', 45_000, {
  ...
  it('honors a positive integer override', () => {
    expect(
      resolveConnectionTestTimeoutMs('ZD_CONNECTION_TEST_PROVIDER_TIMEOUT_MS', 12_000, {
        ZD_CONNECTION_TEST_PROVIDER_TIMEOUT_MS: '30000',
      }),
    ).toBe(30_000);
    expect(
      resolveConnectionTestTimeoutMs('ZD_CONNECTION_TEST_AGENT_TIMEOUT_MS', 45_000, {
        ZD_CONNECTION_TEST_AGENT_TIMEOUT_MS: '120000',
      }),
    ).toBe(120_000);
  });
  ...
  it('warns and falls back on non-numeric, zero, negative, or non-integer overrides', () => {
    ...
    try {
      for (const bad of ['fast', '0', '-1', '1.5', 'NaN']) {
        expect(
          resolveConnectionTestTimeoutMs('ZD_CONNECTION_TEST_PROVIDER_TIMEOUT_MS', 12_000, {
            ZD_CONNECTION_TEST_PROVIDER_TIMEOUT_MS: bad,
          }),
        ).toBe(12_000);
      }
  ...
  it('rejects values above the Node setTimeout maximum (2^31-1)', () => {
    ...
    try {
      ...
      expect(
        resolveConnectionTestTimeoutMs('ZD_CONNECTION_TEST_AGENT_TIMEOUT_MS', 45_000, {
          ZD_CONNECTION_TEST_AGENT_TIMEOUT_MS: tooLarge,
        }),
      ).toBe(45_000);
      // The exact maximum is still accepted; anything past it is not.
      expect(
        resolveConnectionTestTimeoutMs('ZD_CONNECTION_TEST_AGENT_TIMEOUT_MS', 45_000, {
          ZD_CONNECTION_TEST_AGENT_TIMEOUT_MS: '2147483647',
        }),
      ).toBe(2_147_483_647);
      expect(
        resolveConnectionTestTimeoutMs('ZD_CONNECTION_TEST_AGENT_TIMEOUT_MS', 45_000, {
          ZD_CONNECTION_TEST_AGENT_TIMEOUT_MS: '2147483648',
        }),
      ).toBe(45_000);
      expect(warn).toHaveBeenCalled();
    } finally {
