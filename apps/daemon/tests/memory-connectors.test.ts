...
const dataDir = path.join(process.env.ZD_DATA_DIR ?? process.cwd(), 'memory-connectors-test');
  ...
  return {
    ...
    execute: async (request: { connectorId: string; toolName: string; input: unknown }) => {
      ...
      return {
        ...
        outputSummary: 'Found ZDesign design memory notes in Notion.',
        output: {
          pages: [
            {
              title: 'ZDesign memory plan',
              text: 'ZDesign connector memory should collect design preferences, UI decisions, and visual references from Notion.',
            },
          ],
        },
      };
...
describe('connector memory extraction', () => {
          ...
          message: {
            content: JSON.stringify({
              entries: [
                {
                  type: 'project',
                  name: 'ZDesign design memory',
                  description: 'Connector memories should stay design-related',
                  body: 'ZDesign connector memories should focus on design preferences, UI decisions, and visual references rather than generic app activity.',
                },
              ],
            }),
          },
  ...
  it('tries additional connector queries when the first read is empty', async () => {
            ...
            pages: [
              {
                ...
                text: 'ZDesign should summarize connector findings when no durable memory is obvious.',
              },
            ],
          },
        };
    ...
    expect(executeCalls[0]).toEqual(expect.objectContaining({
      input: expect.objectContaining({
        query: '设计思路 设计偏好 UI UX 视觉风格 品牌 logo 设计系统 ZDesign',
      }),
    }));
            ...
            pages: [
              {
                ...
                text: 'ZDesign should read the currently available Notion search tool.',
              },
            ],
          },
        };
      ...
      execute: async (request: { connectorId: string; toolName: string; input: unknown }) => ({
        ...
        output: {
          pages: [
            {
              title: 'Memory source notes',
              text: 'ZDesign should summarize connector findings before saving them as memory.',
            },
          ],
        },
      }),
              ...
              entries: [
                {
                  ...
                  body: 'ZDesign read GitHub via List notifications. Summary: Found 5 readable items from GitHub. Save this if it should be reused as context in future chats.',
                },
                {
                  type: 'feedback',
                  name: 'UI density preference',
                  description: 'The user prefers denser design interfaces',
                  body: 'The user prefers ZDesign UI to use higher information density with clear hierarchy instead of spacious marketing-style cards.',
                },
              ],
            }),
          },
      ...
      entries: [
        {
          ...
          body: 'ZDesign connector memory extraction should use the same Claude Code Local CLI selected for chat when the memory model is set to same as chat.',
        },
      ],
    }));
      ...
      entries: [
        {
          ...
          body: 'ZDesign connector memory extraction should use the same OpenCode Local CLI selected for chat instead of falling back to an OpenAI API key.',
        },
      ],
    }));
      ...
      entries: [
        {
          ...
          body: 'ZDesign connector memory extraction should use the same Codex Local CLI selected for chat instead of falling back to an OpenAI API key.',
        },
      ],
    }));
  ...
  it('runs Codex Local CLI through JSON event stream with stdin prompt', async () => {
    ...
    const previousCapture = process.env.ZD_MEMORY_CODEX_ARGS_OUT;
...
fs.writeFileSync(process.env.ZD_MEMORY_CODEX_ARGS_OUT, JSON.stringify({ args, stdin }));
  ...
  item: {
    ...
    text: JSON.stringify({
      entries: [{
        ...
        body: 'ZDesign connector memory extraction should pass the compacted prompt to Codex stdin and parse the JSON event stream response.'
      }]
    })
  }
}) + '\\n');
`,
    ...
    try {
      ...
      process.env.ZD_MEMORY_CODEX_ARGS_OUT = capturePath;
      ...
      expect(captured.stdin).toContain('ZDesign connector memory should collect design preferences');
    } finally {
      ...
      if (previousCapture == null) {
        delete process.env.ZD_MEMORY_CODEX_ARGS_OUT;
      } else {
        process.env.ZD_MEMORY_CODEX_ARGS_OUT = previousCapture;
      }
  ...
  it('runs OpenCode Local CLI memory extraction with the prompt on stdin', async () => {
    ...
    const capturePath = path.join(tempDir, 'capture.json');
    const previousPath = process.env.PATH;
    const previousCapture = process.env.ZD_MEMORY_OPENCODE_ARGS_OUT;
...
fs.writeFileSync(process.env.ZD_MEMORY_OPENCODE_ARGS_OUT, JSON.stringify({ args, stdin, files }));
  ...
  part: {
    ...
    text: JSON.stringify({
      entries: [{
        ...
        body: 'ZDesign connector memory extraction should pass the compacted prompt to OpenCode on stdin and parse the JSON event stream response.'
      }]
    })
  }
}) + '\\n');
`,
    ...
    try {
      process.env.PATH = `${tempDir}${path.delimiter}${previousPath ?? ''}`;
      process.env.ZD_MEMORY_OPENCODE_ARGS_OUT = capturePath;
      ...
      if (previousCapture == null) {
        delete process.env.ZD_MEMORY_OPENCODE_ARGS_OUT;
      } else {
        process.env.ZD_MEMORY_OPENCODE_ARGS_OUT = previousCapture;
      }
      await fsp.rm(tempDir, { recursive: true, force: true });
    }
  });
