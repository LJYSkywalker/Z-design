...
describe('live artifact tool CLI environment', () => {
  ...
  it('reads ZD_DAEMON_URL and ZD_TOOL_TOKEN from the injected environment', async () => {
    process.env.ZD_DAEMON_URL = 'http://127.0.0.1:7456/base/';
    process.env.ZD_TOOL_TOKEN = 'agent-run-token';
  ...
  it('prints compact success JSON for list results', async () => {
    process.env.ZD_DAEMON_URL = 'http://127.0.0.1:7456';
    process.env.ZD_TOOL_TOKEN = 'agent-run-token';
  ...
  it('fails before making a request when the injected environment is missing', async () => {
    ...
    expect(stderrOutput.join('')).toContain('ZD_DAEMON_URL is required');
  });

  it('requires ZD_TOOL_TOKEN from the injected environment', async () => {
    process.env.ZD_DAEMON_URL = 'http://127.0.0.1:7456';
    delete process.env.ZD_TOOL_TOKEN;
    ...
    expect(result.exitCode).toBe(1);
    expect(fetchMock).not.toHaveBeenCalled();
    expect(stderrOutput.join('')).toContain('ZD_TOOL_TOKEN is required');
  });
});
