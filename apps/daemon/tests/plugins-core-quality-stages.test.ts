...
import type { InstalledPluginRecord, PluginManifest, PluginPipeline } from '@z-design/contracts';
...
describe('core quality-stage floor', () => {
  ...
  it.each([
    ['image', 'plugins/_official/examples/image-poster/z-design.json'],
    ['video', 'plugins/_official/examples/vfx-text-cursor/z-design.json'],
    ['audio', 'plugins/_official/examples/audio-jingle/z-design.json'],
  ...
  it('repairs the real bundled web-prototype template (generate-only on disk)', () => {
    const manifest = JSON.parse(
      readFileSync(
        path.join(REPO_ROOT, 'plugins/_official/examples/web-prototype/z-design.json'),
        'utf8',
      ),
    ) as PluginManifest;
