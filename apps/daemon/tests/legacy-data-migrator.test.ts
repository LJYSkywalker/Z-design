/**
 * Unit tests for the ZD_LEGACY_DATA_DIR one-shot migrator. Hermetic:
 * each test runs against a fresh pair of mkdtemp() directories so no
 * test ever touches a real OD install.
 *
 * The migrator's contract is "loud or correct": when ZD_LEGACY_DATA_DIR
...
describe('migrateLegacyDataDirSync', () => {
  ...
  it('returns noop when ZD_LEGACY_DATA_DIR is not set', () => {
  ...
  it('returns noop when ZD_LEGACY_DATA_DIR is the empty string', () => {
    expect(
  ...
  it('marker beats a missing legacyDir on the next boot', async () => {
    ...
    // ZD_LEGACY_DATA_DIR set after success and then deletes or moves
    ...
    // successful migration but leaving ZD_LEGACY_DATA_DIR set.
    await rm(legacyDir, { recursive: true, force: true });
