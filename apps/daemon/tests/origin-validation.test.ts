...
describe('daemon origin validation middleware', () => {
  ...
  it('blocks local guarded routes without Origin when Host only matches a configured deployment origin', async () => {
    process.env.ZD_ALLOWED_ORIGINS = 'https://od.example.com';
    try {
      const res = await request(port, 'POST', '/api/active', {
        headers: {
          Host: 'od.example.com',
      ...
      delete process.env.ZD_ALLOWED_ORIGINS;
    }
  });
  ...
  it('allows local guarded routes without Origin when Host matches a configured non-loopback IP origin', async () => {
    ...
    process.env.ZD_ALLOWED_ORIGINS = `http://${lanHost}`;
    try {
      const res = await request(port, 'POST', '/api/active', {
        headers: {
          Host: lanHost,
  ...
  it('allows explicitly configured deployment origins', async () => {
    process.env.ZD_ALLOWED_ORIGINS = `https://od.example.com,http://203.0.113.10:${port}`;
    try {
      const res = await request(port, 'GET', '/api/projects', {
        origin: 'https://od.example.com',
      });
  ...
  // --- ZD_WEB_PORT (split-port proxy) ---

  it('allows requests from ZD_WEB_PORT (web proxy port)', async () => {
    const webPort = port + 1000;
    process.env.ZD_WEB_PORT = String(webPort);
    const res = await request(port, 'GET', '/api/projects', {
      origin: `http://127.0.0.1:${webPort}`,
    });
    delete process.env.ZD_WEB_PORT;
    expect(res.status).toBe(200);
  });

  it('blocks requests from unknown ports even with ZD_WEB_PORT set', async () => {
    const webPort = port + 1000;
    process.env.ZD_WEB_PORT = String(webPort);
    const res = await request(port, 'GET', '/api/projects', {
...
// browser origin is explicitly listed in ZD_ALLOWED_ORIGINS, because
...
// documented escape-hatch behavior of ZD_ALLOWED_ORIGINS.
describe('isLocalSameOrigin: ZD_ALLOWED_ORIGINS bypass for reverse-proxy deployments', () => {
  const ALLOWED = 'http://192.168.8.168:7457';
  const previousAllowedOrigins = process.env.ZD_ALLOWED_ORIGINS;
  const env: NodeJS.ProcessEnv = {
    ...process.env,
    ZD_ALLOWED_ORIGINS: ALLOWED,
    ZD_BIND_HOST: '0.0.0.0',
  };

  beforeAll(() => {
    process.env.ZD_ALLOWED_ORIGINS = ALLOWED;
  });
  afterAll(() => {
    if (previousAllowedOrigins === undefined) delete process.env.ZD_ALLOWED_ORIGINS;
    else process.env.ZD_ALLOWED_ORIGINS = previousAllowedOrigins;
  });

  it('accepts a request whose Origin matches ZD_ALLOWED_ORIGINS even when the Host header is the proxy upstream', () => {
    const req = {
      headers: {
  ...
  it('still rejects a request whose Origin is not in ZD_ALLOWED_ORIGINS', () => {
    const req = {
      headers: {
...
// public hostname is listed in ZD_ALLOWED_ORIGINS, those legitimate
// same-origin GETs (e.g. /api/app-config) get rejected by the no-Origin
// host check because hostname entries in ZD_ALLOWED_ORIGINS are only
...
describe('isLocalSameOrigin: Sec-Fetch-Site fallback for no-Origin same-origin GETs', () => {
  ...
  it('accepts a no-Origin request whose Host matches ZD_ALLOWED_ORIGINS when Sec-Fetch-Site is same-origin', () => {
    const req = {
      headers: {
