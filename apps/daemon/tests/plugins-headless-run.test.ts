...
describe('Plan §8 e2e-3 (entry slice) — headless install → project → run', () => {
  ...
  it('creates share projects for publishing and contributing a user plugin', async () => {
    ...
    expect(shareBody.prompt).toContain('/api/projects/$ZD_PROJECT_ID/plugins/publish-github');
    ...
    expect(fileNames).toContain('plugin-source/sample-plugin/z-design.json');
    ...
    const contributeResp = await fetch(`${baseUrl}/api/plugins/sample-plugin/share-project`, {
      method:  'POST',
      headers: { 'content-type': 'application/json' },
      body:    JSON.stringify({ action: 'contribute-z-design', locale: 'en' }),
    });
    ...
    expect(contributeBody.actionPluginId).toBe('od-plugin-contribute-z-design');
    ...
    expect(contributeBody.prompt).toContain('/api/projects/$ZD_PROJECT_ID/plugins/contribute-z-design');
    ...
    const previousRealGit = process.env.ZD_REAL_GIT;
    ...
    process.env.ZD_REAL_GIT = realGit;
...
if (args[0] === 'repo' && args[1] === 'clone') {
  ...
  const init = spawnSync(process.env.ZD_REAL_GIT, ['init'], { cwd: dest, stdio: 'inherit' });
  process.exit(init.status ?? 0);
}
if (args[0] === 'pr' && args[1] === 'create') ok('https://github.com/Aqu-210L/Z-design/pull/123');
...
if (args[0] === 'clone') {
  ...
  fs.mkdirSync(dest, { recursive: true });
  const init = spawnSync(process.env.ZD_REAL_GIT, ['init', '-b', 'main'], { cwd: dest, encoding: 'utf8' });
  if (init.status !== 0) {
  ...
  const remoteAdd = spawnSync(process.env.ZD_REAL_GIT, ['remote', 'add', 'origin', remote], { cwd: dest, encoding: 'utf8' });
...
const result = spawnSync(process.env.ZD_REAL_GIT, args, {
  cwd: process.cwd(),
  env: process.env,
  encoding: 'utf8',
});
...
`,
            async () => {
              ...
              const contributeEndpointResp = await fetch(
                `${baseUrl}/api/projects/${encodeURIComponent(contributeBody.project.id)}/plugins/contribute-z-design`,
                {
                  method: 'POST',
                  headers: { 'content-type': 'application/json' },
              ...
              expect(contributeEndpointBody.url).toBe('https://github.com/Aqu-210L/Z-design/pull/123');
            },
          );
        },
      );
      ...
      if (previousRealGit === undefined) {
        delete process.env.ZD_REAL_GIT;
      } else {
        process.env.ZD_REAL_GIT = previousRealGit;
      }
  ...
  it('runs the CLI install → project create → plugin run path with query and local SKILL.md in the agent prompt', async () => {
    ...
    await writeFile(
      path.join(fixture, 'z-design.json'),
      JSON.stringify({
    ...
    try {
      ...
      const previousCapture = process.env.ZD_PROMPT_CAPTURE;
      process.env.ZD_PROMPT_CAPTURE = capturePath;
      try {
        await withFakeAgent(
          'opencode',
          `
...
process.stdin.on('end', () => {
  const capturePath = process.env.ZD_PROMPT_CAPTURE;
...
`,
        ...
        if (previousCapture === undefined) {
          delete process.env.ZD_PROMPT_CAPTURE;
        } else {
          process.env.ZD_PROMPT_CAPTURE = previousCapture;
        }
