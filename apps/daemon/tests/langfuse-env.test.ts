...
const ORIGINAL_OD_TELEMETRY_ENV = process.env.ZD_TELEMETRY_ENV;

afterEach(() => {
  if (ORIGINAL_OD_TELEMETRY_ENV === undefined) {
    delete process.env.ZD_TELEMETRY_ENV;
  } else {
    process.env.ZD_TELEMETRY_ENV = ORIGINAL_OD_TELEMETRY_ENV;
  }
});

describe('langfuse telemetry environment', () => {
  it('stamps trace metadata with env', () => {
    process.env.ZD_TELEMETRY_ENV = 'local_development';
