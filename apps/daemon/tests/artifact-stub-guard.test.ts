...
describe('readArtifactStubGuardConfigFromEnv', () => {
  ...
  it('parses recognised mode values', () => {
    expect(readArtifactStubGuardConfigFromEnv({ ZD_ARTIFACT_STUB_GUARD: 'reject' }).mode).toBe('reject');
    expect(readArtifactStubGuardConfigFromEnv({ ZD_ARTIFACT_STUB_GUARD: 'WARN' }).mode).toBe('warn');
    expect(readArtifactStubGuardConfigFromEnv({ ZD_ARTIFACT_STUB_GUARD: 'off' }).mode).toBe('off');
  });

  it('falls back to default when mode is unrecognised', () => {
    expect(readArtifactStubGuardConfigFromEnv({ ZD_ARTIFACT_STUB_GUARD: 'maybe' }).mode).toBe(
  ...
  it('honours numeric overrides within range', () => {
    const config = readArtifactStubGuardConfigFromEnv({
      ZD_ARTIFACT_STUB_GUARD_MIN_RATIO: '0.35',
      ZD_ARTIFACT_STUB_GUARD_MIN_PRIOR_BYTES: '8192',
    });
  ...
  it('accepts ratio = 1 to reject any shrinkage', () => {
    const config = readArtifactStubGuardConfigFromEnv({
      ZD_ARTIFACT_STUB_GUARD_MIN_RATIO: '1',
    });
  ...
  it('rejects out-of-range numeric overrides', () => {
    const config = readArtifactStubGuardConfigFromEnv({
      ZD_ARTIFACT_STUB_GUARD_MIN_RATIO: '5',
      ZD_ARTIFACT_STUB_GUARD_MIN_PRIOR_BYTES: '-12',
    });
