...
// pure renderer in @z-design/contracts then assembles the stage
...
import { renderActiveStageBlock } from '@z-design/contracts';
...
beforeEach(async () => {
  ...
  await writeFile(path.join(atomA, 'z-design.json'), SAMPLE_MANIFEST('discovery-question-form'));
  await writeFile(path.join(atomA, 'SKILL.md'), SAMPLE_SKILL('discovery-question-form', 'Ask the user about audience.'));
  await writeFile(path.join(atomB, 'z-design.json'), SAMPLE_MANIFEST('todo-write'));
