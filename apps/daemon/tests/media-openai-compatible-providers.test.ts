...
describe('OpenAI-compatible media providers', () => {
  ...
  const originalImageRouterKey = process.env.ZD_IMAGEROUTER_API_KEY;
  const originalCustomImageKey = process.env.ZD_CUSTOM_IMAGE_API_KEY;
  ...
  const originalAllowStubs = process.env.ZD_MEDIA_ALLOW_STUBS;
  ...
  beforeEach(async () => {
    ...
    delete process.env.ZD_IMAGEROUTER_API_KEY;
    delete process.env.ZD_CUSTOM_IMAGE_API_KEY;
    delete process.env.CODEX_BIN;
    delete process.env.CODEX_HOME;
    delete process.env.ZD_MEDIA_CONFIG_DIR;
    delete process.env.ZD_DATA_DIR;
    delete process.env.ZD_MEDIA_MODEL_ALIASES;
    delete process.env.ZD_MEDIA_ALLOW_STUBS;
  ...
  afterEach(async () => {
    ...
    if (originalImageRouterKey == null) {
      ...
      process.env.ZD_IMAGEROUTER_API_KEY = originalImageRouterKey;
    }
    if (originalCustomImageKey == null) {
      delete process.env.ZD_CUSTOM_IMAGE_API_KEY;
    } else {
      process.env.ZD_CUSTOM_IMAGE_API_KEY = originalCustomImageKey;
    }
    ...
    if (originalAllowStubs == null) {
      ...
      process.env.ZD_MEDIA_ALLOW_STUBS = originalAllowStubs;
    }
    for (const key of OPENAI_ENV_KEYS) {
...
`, 'utf8');
  ...
  it('renders ImageRouter images through the OpenAI-compatible JSON endpoint', async () => {
    process.env.ZD_IMAGEROUTER_API_KEY = 'ir-test-key';
...
`, 'utf8');
  ...
  it('uses default app-config Codex CLI env overrides when ZD_DATA_DIR is absent', async () => {
    ...
    await writeFile(wrongCodexBin, `#!/usr/bin/env node
process.stderr.write('wrong codex bin used without ZD_DATA_DIR');
process.exit(24);
`, 'utf8');
    await chmod(wrongCodexBin, 0o755);
    await writeFile(codexBin, `#!/usr/bin/env node
