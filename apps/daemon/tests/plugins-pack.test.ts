...
describe('packPlugin', () => {
  it('writes a .tgz containing every file in the folder', async () => {
    await writeFile(path.join(folder, 'z-design.json'), JSON.stringify({
      name: 'my-plugin',
      version: '0.1.2',
      title: 'Test plugin',
    ...
    expect(result.files.sort()).toEqual([
      'SKILL.md', 'assets/logo.svg', 'z-design.json',
    ]);
  ...
  it('skips node_modules / .git / dist + .DS_Store noise', async () => {
    await writeFile(path.join(folder, 'z-design.json'), JSON.stringify({ name: 'p', version: '0.0.1' }));
    ...
    const result = await packPlugin({ folder });
    const entries = await listArchiveEntries(result.outPath);
    expect(entries).toEqual(['z-design.json']);
  });
  ...
  it('skips symlinks both at walk time and at filter time', async () => {
    ...
    const result = await packPlugin({ folder });
    expect(result.files).toEqual(['z-design.json', 'real.txt']);
    const entries = await listArchiveEntries(result.outPath);
    expect(entries).toEqual(['z-design.json', 'real.txt']);
  });

  it('falls back to <basename>.tgz when the manifest has no version', async () => {
    await writeFile(path.join(folder, 'z-design.json'), JSON.stringify({ name: 'plain' }));
    const result = await packPlugin({ folder });
  ...
  it('round-trips through tar.extract: archive contents match the source tree', async () => {
    await writeFile(path.join(folder, 'z-design.json'), JSON.stringify({ name: 'rt', version: '0.0.1' }));
    ...
    expect((await readFile(path.join(dest, 'z-design.json'), 'utf8')).includes('rt')).toBe(true);
  ...
  it('rejects a folder without z-design.json', async () => {
    await writeFile(path.join(folder, 'SKILL.md'), '---\nname: p\n---\n');
    await expect(packPlugin({ folder })).rejects.toBeInstanceOf(PackPluginError);
  });

  it('rejects a folder whose z-design.json is malformed JSON', async () => {
    await writeFile(path.join(folder, 'z-design.json'), '{ broken');
    await expect(packPlugin({ folder })).rejects.toBeInstanceOf(PackPluginError);
  });
  ...
  it('does not pack the output archive itself when --out lands inside the folder', async () => {
    ...
    const result = await packPlugin({ folder, out });
    expect(result.files).toEqual(['a.txt', 'z-design.json']);
    const entries = await listArchiveEntries(result.outPath);
    expect(entries).toEqual(['a.txt', 'z-design.json']);
  });
});
