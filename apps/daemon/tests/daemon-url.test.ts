...
import { createJsonIpcServer, type JsonIpcServerHandle } from "@z-design/sidecar";
...
// Verifies the resolution chain: --daemon-url > ZD_DAEMON_URL > sidecar
...
describe("resolveDaemonUrl", () => {
  ...
  it("prefers the explicit --daemon-url flag", async () => {
    const url = await resolveDaemonUrl({
      ...
      env: {
        ZD_DAEMON_URL: "http://env.example:2222",
  ...
  it("falls back to ZD_DAEMON_URL when no flag given", async () => {
    const url = await resolveDaemonUrl({
      env: {
        ZD_DAEMON_URL: "http://env.example:2222",
        [SIDECAR_ENV.IPC_PATH]: path.join(ipcBaseDir, "daemon.sock"),
