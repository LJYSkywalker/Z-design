...
describe('artifact stub guard via /api/projects/:id/files', () => {
  ...
  beforeAll(async () => {
    vi.stubEnv('ZD_ARTIFACT_STUB_GUARD', 'reject');
    vi.stubEnv('ZD_ARTIFACT_STUB_GUARD_MIN_PRIOR_BYTES', '1024');
    vi.stubEnv('ZD_ARTIFACT_STUB_GUARD_MIN_RATIO', '0.2');
  ...
  it('warns instead of rejecting when guard mode is warn', async () => {
    vi.stubEnv('ZD_ARTIFACT_STUB_GUARD', 'warn');
  ...
  it('skips the guard entirely when mode is off', async () => {
    vi.stubEnv('ZD_ARTIFACT_STUB_GUARD', 'off');
