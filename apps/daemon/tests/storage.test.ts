...
describe('resolveProjectStorage', () => {
  ...
  it('returns S3ProjectStorage when ZD_PROJECT_STORAGE=s3', () => {
    const storage = resolveProjectStorage({
      projectsRoot: tmp,
      env: {
        ZD_PROJECT_STORAGE:        's3',
        ZD_S3_BUCKET:              'my-bucket',
        ZD_S3_REGION:              'us-east-1',
        ZD_S3_PREFIX:              'tenant',
        ZD_S3_ENDPOINT:            'https://oss.aliyuncs.com',
        ZD_S3_ACCESS_KEY_ID:       'AKIA-FIXTURE',
        ZD_S3_SECRET_ACCESS_KEY:   'shhh',
      },
    });
  ...
  it('falls back to AWS_ACCESS_KEY_ID / AWS_REGION when the ZD_-specific knobs are unset', () => {
    const storage = resolveProjectStorage({
      projectsRoot: tmp,
      env: {
        ZD_PROJECT_STORAGE:    's3',
...
describe('resolveDaemonDbConfig', () => {
  ...
  it('parses postgres env vars when ZD_DAEMON_DB=postgres', () => {
    const cfg = resolveDaemonDbConfig({
      ZD_DAEMON_DB: 'postgres',
      ZD_PG_HOST:   'pg.local',
      ZD_PG_PORT:   '6543',
      ZD_PG_DATABASE: 'open_design',
      ZD_PG_USER:   'od',
      ZD_PG_SSL_MODE: 'disable',
    });
  ...
  it('throws when postgres env vars are incomplete', () => {
    expect(() =>
      resolveDaemonDbConfig({ ZD_DAEMON_DB: 'postgres', ZD_PG_HOST: 'pg.local' }),
    ).toThrow(DaemonDbConfigError);
  });

  it('throws on an unknown ZD_DAEMON_DB value', () => {
    expect(() => resolveDaemonDbConfig({ ZD_DAEMON_DB: 'mongo' })).toThrow(DaemonDbConfigError);
  });
});
