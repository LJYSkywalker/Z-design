...
describe('deploy provider routes', () => {
  ...
  it('dispatches deploy config reads and writes by providerId', async () => {
    ...
    const priorStateRoot = process.env.ZD_USER_STATE_DIR;
    process.env.ZD_USER_STATE_DIR = stateRoot;
    try {
      ...
      if (priorStateRoot === undefined) delete process.env.ZD_USER_STATE_DIR;
      else process.env.ZD_USER_STATE_DIR = priorStateRoot;
