...
test('sandbox runtime registry ignores host-local agent profiles at module load', async () => {
  ...
  try {
    ...
    await withEnvSnapshot(
      ['ZD_SANDBOX_MODE', 'ZD_DATA_DIR', 'ZD_AGENT_PROFILES_CONFIG'],
      async () => {
        process.env.ZD_SANDBOX_MODE = '1';
        process.env.ZD_DATA_DIR = dataDir;
        process.env.ZD_AGENT_PROFILES_CONFIG = hostConfig;
...
test('sandbox runtime registry ignores implicit profiles without ZD_DATA_DIR', async () => {
  ...
  try {
    ...
    await withEnvSnapshot(
      ...
      async () => {
        ...
        delete process.env.ZD_AGENT_PROFILES_CONFIG;
