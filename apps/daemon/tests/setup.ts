...
process.env.ZD_DATA_DIR = globalState[TEST_DATA_DIR_SYMBOL];

// Publish/share endpoints shell out through ZD_NODE_BIN + ZD_BIN (dist/cli.js).
// Build the CLI artifact once per vitest process so package tests do not depend
// on a prior manual `pnpm --filter @z-design/daemon build`.
ensureDaemonCliBuilt();
process.env.ZD_DAEMON_CLI_PATH = daemonCliDist;
