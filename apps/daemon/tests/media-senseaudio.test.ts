...
describe('senseaudio media generation', () => {
  ...
  beforeEach(async () => {
    ...
    delete process.env.ZD_SENSEAUDIO_API_KEY;
  ...
  it('reads the API key from ZD_SENSEAUDIO_API_KEY when storage is empty', async () => {
    process.env.ZD_SENSEAUDIO_API_KEY = 'env-sense-key';
    const fetchMock = vi.fn(async (_input: unknown, init?: RequestInit) => {
      expect(init?.headers).toMatchObject({
