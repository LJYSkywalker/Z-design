...
const SAVED_INSTALL_ENV = process.env.ZD_INSTALLATION_DIR;
...
beforeEach(async () => {
  ...
  process.env.ZD_INSTALLATION_DIR = installDir;
});

afterEach(async () => {
  if (SAVED_INSTALL_ENV == null) {
    delete process.env.ZD_INSTALLATION_DIR;
  } else {
    process.env.ZD_INSTALLATION_DIR = SAVED_INSTALL_ENV;
  }
  if (rootDir != null) {
...
describe('installation.json migration', () => {
  ...
  it('falls back to dataDir when ZD_INSTALLATION_DIR is unset (dev / OSS / tools-dev paths)', async () => {
    delete process.env.ZD_INSTALLATION_DIR;
