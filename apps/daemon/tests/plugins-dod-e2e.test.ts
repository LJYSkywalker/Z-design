...
import { renderPluginBlock } from '@z-design/contracts';
...
async function freshFixture(targetPath: string, version = '1.0.0') {
  ...
  await writeFile(path.join(targetPath, 'z-design.json'), JSON.stringify(manifest, null, 2));
  await writeFile(
...
describe('Plan §8 e2e — daemon-side anchors', () => {
  ...
  it('e2e-6 connector trust gate: restricted plugin without connector:<id> blocks apply + tool token', async () => {
    ...
    await writeFile(path.join(folder, 'z-design.json'), JSON.stringify(manifest));
    await writeFile(
