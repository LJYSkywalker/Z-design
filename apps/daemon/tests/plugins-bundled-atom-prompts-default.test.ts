// Plan §3.V1 — ZD_BUNDLED_ATOM_PROMPTS default is now ON.
//
...
function bundledAtomPromptsEnabled(env: Record<string, string | undefined>): boolean {
  return env.ZD_BUNDLED_ATOM_PROMPTS !== '0';
}

describe('ZD_BUNDLED_ATOM_PROMPTS default policy', () => {
  ...
  it("is ON when the env var is empty (treated as 'not opted out')", () => {
    expect(bundledAtomPromptsEnabled({ ZD_BUNDLED_ATOM_PROMPTS: '' })).toBe(true);
  });

  it("is ON when the env var is explicitly '1'", () => {
    expect(bundledAtomPromptsEnabled({ ZD_BUNDLED_ATOM_PROMPTS: '1' })).toBe(true);
  });

  it("is OFF when the env var is explicitly '0' (documented opt-out)", () => {
    expect(bundledAtomPromptsEnabled({ ZD_BUNDLED_ATOM_PROMPTS: '0' })).toBe(false);
  });

  it("is ON for any non-'0' value (forward-compat)", () => {
    expect(bundledAtomPromptsEnabled({ ZD_BUNDLED_ATOM_PROMPTS: 'true' })).toBe(true);
    expect(bundledAtomPromptsEnabled({ ZD_BUNDLED_ATOM_PROMPTS: 'false' })).toBe(true);
    expect(bundledAtomPromptsEnabled({ ZD_BUNDLED_ATOM_PROMPTS: 'yes' })).toBe(true);
  });
});
