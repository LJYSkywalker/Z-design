...
describe('/api/chat', () => {
  ...
  const originalAgentHome = process.env.ZD_AGENT_HOME;
  ...
  async function createPluginFixture(args: {
    ...
    const manifestPath = resolve(fixtureDir, 'z-design.json');
  ...
  beforeAll(async () => {
    if (process.env.ZD_DATA_DIR) {
      originalMemoryConfig = await readMemoryConfig(process.env.ZD_DATA_DIR);
      await writeMemoryConfig(process.env.ZD_DATA_DIR, {
        enabled: false,
        extraction: null,
      });
    }
  ...
  afterEach(() => {
    ...
    if (originalAgentHome == null) {
      delete process.env.ZD_AGENT_HOME;
    } else {
      process.env.ZD_AGENT_HOME = originalAgentHome;
    }
  });
  ...
  afterAll(async () => {
    ...
    if (process.env.ZD_DATA_DIR && originalMemoryConfig) {
      await writeMemoryConfig(process.env.ZD_DATA_DIR, {
  ...
  it('does not reference an out-of-scope response while starting a run', async () => {
    ...
    process.env.ZD_AGENT_HOME = emptyAgentHome;
...
`,
  ...
  it('passes OPENCODE_CONFIG_CONTENT external_directory rules for the managed project cwd', async () => {
    if (!process.env.ZD_DATA_DIR) {
      throw new Error('ZD_DATA_DIR is required for OpenCode cwd permission tests');
    }
...
`,
  ...
  it('reuses an existing assistant message row instead of creating a duplicate when assistantMessageId is supplied', async () => {
    if (!process.env.ZD_DATA_DIR) {
      throw new Error('ZD_DATA_DIR is required for assistant message reuse tests');
    }
    const projectId = `proj-${randomUUID()}`;
    ...
    const dbFile = resolve(process.env.ZD_DATA_DIR, 'app.sqlite');
...
process.stdin.on('end', () => {
  ...
  fs.writeFileSync(path.join(pluginDir, 'z-design.json'), JSON.stringify({ name: 'generated-plugin' }, null, 2));
...
`,
      async () => {
        ...
        expect(filesBody.files.some((file) => file.name === 'generated-plugin/z-design.json')).toBe(true);
...
`,
  ...
  it('propagates ad-hoc skill critique policy into the chat resolver', async () => {
    if (!process.env.ZD_DATA_DIR) {
      throw new Error('ZD_DATA_DIR is required for user skill critique-policy tests');
    }

    const skillId = `critique-opt-out-${randomUUID()}`;
    const skillDir = resolve(process.env.ZD_DATA_DIR, 'skills', skillId);
    const originalCritiqueEnabled = process.env.ZD_CRITIQUE_ENABLED;
...
`,
    ...
    process.env.ZD_CRITIQUE_ENABLED = 'true';
...
`,
      ...
      if (originalCritiqueEnabled == null) {
        delete process.env.ZD_CRITIQUE_ENABLED;
      } else {
        process.env.ZD_CRITIQUE_ENABLED = originalCritiqueEnabled;
      }
...
`,
  ...
  it('stages colliding plugin and composed skill dirs under distinct aliases', async () => {
    if (!process.env.ZD_DATA_DIR) {
      throw new Error('ZD_DATA_DIR is required for colliding skill-dir staging tests');
    }
    ...
    const projectId = `project-${randomUUID()}`;
    const userSkillDir = resolve(process.env.ZD_DATA_DIR, 'skills', 'sample-plugin');
...
`,
  ...
  it('fails stalled json-stream runs after the inactivity timeout elapses', async () => {
    const previous = process.env.ZD_CHAT_RUN_INACTIVITY_TIMEOUT_MS;
    process.env.ZD_CHAT_RUN_INACTIVITY_TIMEOUT_MS = '500';
    try {
      await withFakeAgent(
        'opencode',
        `
...
`,
      ...
      if (previous == null) {
        delete process.env.ZD_CHAT_RUN_INACTIVITY_TIMEOUT_MS;
      } else {
        process.env.ZD_CHAT_RUN_INACTIVITY_TIMEOUT_MS = previous;
      }
    }
  });
  ...
  it('keeps Claude stream runs alive while structured output is still flowing', async () => {
    const previous = process.env.ZD_CHAT_RUN_INACTIVITY_TIMEOUT_MS;
    process.env.ZD_CHAT_RUN_INACTIVITY_TIMEOUT_MS = '3000';
    try {
      await withFakeAgent(
        'claude',
        `
...
`,
  ...
  it('caps oversized inactivity overrides so Node does not fire the timer immediately', async () => {
    const previous = process.env.ZD_CHAT_RUN_INACTIVITY_TIMEOUT_MS;
    process.env.ZD_CHAT_RUN_INACTIVITY_TIMEOUT_MS = '10000000000';
    try {
      await withFakeAgent(
        'opencode',
        `
...
`,
  ...
  it('marks submitted discovery form answers as the active turn before the transcript', async () => {
    ...
    const previousCapturePath = process.env.ZD_CAPTURE_PROMPT_PATH;
    process.env.ZD_CAPTURE_PROMPT_PATH = capturePath;
    try {
      await withFakeAgent(
        'opencode',
        `
...
process.stdin.on('end', () => {
  fs.writeFileSync(process.env.ZD_CAPTURE_PROMPT_PATH, input, 'utf8');
...
`,
      ...
      if (previousCapturePath == null) {
        delete process.env.ZD_CAPTURE_PROMPT_PATH;
      } else {
        process.env.ZD_CAPTURE_PROMPT_PATH = previousCapturePath;
      }
    }
  });
});
...
describe('daemon run creation during shutdown', () => {
  it('rejects new run creation while shutdown cleanup is still in flight', async () => {
    const previousGrace = process.env.ZD_CHAT_RUN_SHUTDOWN_GRACE_MS;
    process.env.ZD_CHAT_RUN_SHUTDOWN_GRACE_MS = '100';
    const started = await startServer({ port: 0, returnServer: true }) as {
      url: string;
      server: http.Server;
...
`,
      ...
      if (previousGrace == null) {
        delete process.env.ZD_CHAT_RUN_SHUTDOWN_GRACE_MS;
      } else {
        process.env.ZD_CHAT_RUN_SHUTDOWN_GRACE_MS = previousGrace;
      }
