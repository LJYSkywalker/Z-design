...
describe('composeSystemPrompt', () => {
  ...
  it('injects live-artifact skill guidance and metadata intent', () => {
    ...
    expect(prompt).toContain('The wrapper reads injected `ZD_NODE_BIN`, `ZD_BIN`, `ZD_DAEMON_URL`, and `ZD_TOOL_TOKEN`');
    expect(prompt).toContain('Do not include or invent `projectId`; the daemon derives project/run scope from the token.');
    expect(prompt).toContain('"$ZD_NODE_BIN" "$ZD_BIN" tools live-artifacts create --input artifact.json');
  ...
  // `ZD_DESIGN_TOKEN_CHANNEL=0` as the kill switch). These tests pin
