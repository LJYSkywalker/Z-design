...
describe('media-config OpenAI auth-file fallback', () => {
  ...
  const originalMediaConfigDir = process.env.ZD_MEDIA_CONFIG_DIR;
  const originalDataDir = process.env.ZD_DATA_DIR;
  const originalSandboxMode = process.env.ZD_SANDBOX_MODE;
  ...
  beforeEach(async () => {
    ...
    delete process.env.ZD_MEDIA_CONFIG_DIR;
    delete process.env.ZD_DATA_DIR;
    delete process.env.ZD_SANDBOX_MODE;
  });
  ...
  afterEach(async () => {
    ...
    if (originalMediaConfigDir == null) {
      ...
      process.env.ZD_MEDIA_CONFIG_DIR = originalMediaConfigDir;
    }
    if (originalDataDir == null) {
      delete process.env.ZD_DATA_DIR;
    } else {
      process.env.ZD_DATA_DIR = originalDataDir;
    }
    if (originalSandboxMode == null) {
      delete process.env.ZD_SANDBOX_MODE;
    } else {
      process.env.ZD_SANDBOX_MODE = originalSandboxMode;
    }
  ...
  it('detects subscription auth from app-config Codex home overrides', async () => {
    ...
    process.env.ZD_DATA_DIR = dataDir;
  ...
  it('resolves Nano Banana env and stored model overrides', async () => {
    process.env.ZD_NANOBANANA_API_KEY = 'env-nano-key';
    await writeStoredMediaConfig({
      providers: {
        nanobanana: {
    ...
    delete process.env.ZD_NANOBANANA_API_KEY;
  });
  ...
  describe('ZD_MEDIA_CONFIG_DIR / ZD_DATA_DIR storage routing', () => {
    ...
    beforeEach(async () => {
      ...
      originalMediaConfigDir = process.env.ZD_MEDIA_CONFIG_DIR;
      originalDataDir = process.env.ZD_DATA_DIR;
      delete process.env.ZD_MEDIA_CONFIG_DIR;
      delete process.env.ZD_DATA_DIR;
    });
    ...
    it('reads media-config.json from an absolute ZD_MEDIA_CONFIG_DIR', async () => {
      process.env.ZD_MEDIA_CONFIG_DIR = overrideRoot;
    ...
    it('expands a leading ~/ against the user home directory', async () => {
      ...
      process.env.ZD_MEDIA_CONFIG_DIR = `~/${subdir}`;
    ...
    it('resolves a relative override against projectRoot, not process.cwd', async () => {
      ...
      // server.ts anchors ZD_DATA_DIR.
      const relative = 'config/media';
      process.env.ZD_MEDIA_CONFIG_DIR = relative;
    ...
    it('falls back to ZD_DATA_DIR when ZD_MEDIA_CONFIG_DIR is unset', async () => {
      // Packaged daemon (apps/packaged/src/sidecars.ts) and the
      // Home Manager / NixOS modules already set ZD_DATA_DIR for the
      // rest of the daemon's runtime state. media-config should
      // co-locate there without needing a second env var.
      process.env.ZD_DATA_DIR = overrideRoot;
      await writeProvidersAt(overrideRoot, {
        providers: {
          openai: {
            apiKey: 'datadir-key',
    ...
    it('ZD_MEDIA_CONFIG_DIR takes precedence over ZD_DATA_DIR', async () => {
      const dataDir = await mkdtemp(path.join(tmpdir(), 'od-media-data-'));
      try {
        process.env.ZD_DATA_DIR = dataDir;
        process.env.ZD_MEDIA_CONFIG_DIR = overrideRoot;
        // Two competing files; only the ZD_MEDIA_CONFIG_DIR one should
        // be read.
    ...
    it('writeConfig creates the override directory tree on first write', async () => {
      ...
      process.env.ZD_MEDIA_CONFIG_DIR = target;
    ...
    // ZD_DATA_DIR=$HOME/.open-design (and ${HOME}/.open-design) routes
    ...
    it('expands $HOME/... in ZD_DATA_DIR fallback so media-config co-locates with daemon data', async () => {
      const subdir = '.od-test-home';
      process.env.ZD_DATA_DIR = `$HOME/${subdir}`;
      const expandedDir = path.join(homeDir, subdir);
      await writeProvidersAt(expandedDir, {
        providers: {
          openai: {
    ...
    it('expands ${HOME}/... in ZD_DATA_DIR fallback', async () => {
    ...
    it('expands $HOME/... in ZD_MEDIA_CONFIG_DIR (explicit override path)', async () => {
      const subdir = '.od-media-home';
      process.env.ZD_MEDIA_CONFIG_DIR = `$HOME/${subdir}`;
      const expandedDir = path.join(homeDir, subdir);
      await writeProvidersAt(expandedDir, {
        providers: {
          openai: {
...
const GROK_ENV_KEYS = ['ZD_GROK_API_KEY', 'XAI_API_KEY'];
...
describe('media-config model alias resolution (issue #1277)', () => {
  let projectRoot: string;
  const originalEnvAliases = process.env.ZD_MEDIA_MODEL_ALIASES;
  const originalMediaConfigDir = process.env.ZD_MEDIA_CONFIG_DIR;
  const originalDataDir = process.env.ZD_DATA_DIR;

  beforeEach(async () => {
    projectRoot = await mkdtemp(path.join(tmpdir(), 'od-media-alias-'));
    delete process.env.ZD_MEDIA_MODEL_ALIASES;
    delete process.env.ZD_MEDIA_CONFIG_DIR;
    delete process.env.ZD_DATA_DIR;
  });
  ...
  afterEach(async () => {
    if (originalEnvAliases == null) {
      ...
      process.env.ZD_MEDIA_MODEL_ALIASES = originalEnvAliases;
    }
    if (originalMediaConfigDir == null) {
      delete process.env.ZD_MEDIA_CONFIG_DIR;
    } else {
  ...
  it('redirects via the ZD_MEDIA_MODEL_ALIASES env var', async () => {
    process.env.ZD_MEDIA_MODEL_ALIASES = JSON.stringify({
  ...
  it('tolerates malformed env JSON and falls through to the stored map', async () => {
    // A user with a half-typed env var (`ZD_MEDIA_MODEL_ALIASES='{'`)
    // should still get their on-disk aliases, not a hard error mid-
    // generation.
    process.env.ZD_MEDIA_MODEL_ALIASES = '{not valid json';
    await writeStoredMediaConfig({
      providers: {},
      aliases: { 'doubao-seedream-3-0-t2i-250415': 'doubao-seedream-5-0' },
    });
...
describe('seedProviderIfMissing', () => {
  let projectRoot: string;
  const SENSEAUDIO_ENV_KEYS = ['ZD_SENSEAUDIO_API_KEY', 'SENSEAUDIO_API_KEY'];
  const originalEnv = Object.fromEntries(
  ...
  it('no-ops when an env var resolves a key for the provider', async () => {
    process.env.ZD_SENSEAUDIO_API_KEY = 'env-key';
    const wrote = await seedProviderIfMissing(projectRoot, 'senseaudio', {
      apiKey: 'sa-byok-key',
      baseUrl: 'https://api.senseaudio.cn',
    });
