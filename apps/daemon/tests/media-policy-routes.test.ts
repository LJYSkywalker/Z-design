...
describe('run-scoped media policy routes', () => {
  ...
  beforeEach(async () => {
    ...
    oldCapture = process.env.ZD_CAPTURE_MEDIA_RESPONSE;
  ...
  afterEach(async () => {
    ...
    if (oldCapture === undefined) delete process.env.ZD_CAPTURE_MEDIA_RESPONSE;
    else process.env.ZD_CAPTURE_MEDIA_RESPONSE = oldCapture;
    const memoryConfig = memoryConfigPath();
  ...
  function memoryConfigPath(): string {
    return path.join(memoryDir(process.env.ZD_DATA_DIR!), '.config.json');
  }
...
(async () => {
  ...
  const daemonUrl = process.env.ZD_DAEMON_URL;
  const projectId = process.env.ZD_PROJECT_ID;
  ...
  fs.writeFileSync(process.env.ZD_CAPTURE_MEDIA_RESPONSE, JSON.stringify({
  ...
  fs.writeFileSync(process.env.ZD_CAPTURE_MEDIA_RESPONSE, JSON.stringify({
    status: 0,
    tokenAvailable: Boolean(process.env.ZD_TOOL_TOKEN),
    tokenAttached: Boolean(attachToken && process.env.ZD_TOOL_TOKEN),
    endpoint,
...
`;
    ...
    process.env.ZD_CAPTURE_MEDIA_RESPONSE = capturePath;
  }
