...
test('detectAcpModels uses ZD_ACP_TIMEOUT_MS from the ACP probe environment', async () => {
  ...
  try {
    ...
    assert.ok(
      ...
      'expected ZD_ACP_TIMEOUT_MS to bound the probe instead of waiting for the 15s caller timeout',
    );
  } finally {
...
test('detectAcpModels ignores invalid ZD_ACP_TIMEOUT_MS values', async () => {
  const { dir, bin } = writeStallingProbe();
  try {
    await assert.rejects(
      detectAcpModels({
        bin: process.execPath,
        args: [bin],
        env: { ZD_ACP_TIMEOUT_MS: 'not-a-number' },
        timeoutMs: 50,
      }),
...
test('detectAcpModels caps oversized ZD_ACP_TIMEOUT_MS values before scheduling timers', async () => {
  vi.useFakeTimers();
  ...
  try {
    const probe = detectAcpModels({
      ...
      env: { ZD_ACP_TIMEOUT_MS: '10000000000' },
      timeoutMs: 15_000,
    });
    probe.catch(() => {});
...
test('detectAcpModels treats ZD_ACP_TIMEOUT_MS=0 as disabling the ACP probe timeout', async () => {
  const { dir, bin } = writeDelayedSuccessProbe(120);
  try {
    const models = await detectAcpModels({
      bin: process.execPath,
      args: [bin],
      env: { ZD_ACP_TIMEOUT_MS: '0' },
      timeoutMs: 50,
    });
