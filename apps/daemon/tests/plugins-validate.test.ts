...
async function writeManifest(body: Record<string, unknown>) {
  await writeFile(path.join(folder, 'z-design.json'), JSON.stringify(body, null, 2));
}
...
describe('validatePluginFolder', () => {
  ...
  it('rejects malformed z-design.json with a parse error', async () => {
    await writeFile(path.join(folder, 'z-design.json'), '{ this is not json');
    const result = await validatePluginFolder({ folder });
    expect(result.ok).toBe(false);
    expect(result.resolveErrors.some((e) => e.includes('z-design.json'))).toBe(true);
  });
