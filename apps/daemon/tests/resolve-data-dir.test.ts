/**
 * Unit tests for resolveDataDir, the ZD_DATA_DIR path resolver. Covers the
...
describe('resolveDataDir', () => {
  ...
  it('returns <projectRoot>/.od when ZD_DATA_DIR is unset', () => {
  ...
  it('requires an explicit ZD_DATA_DIR when sandbox mode requires one', () => {
    expect(() =>
      resolveDataDir(undefined, projectRoot, { requireExplicit: true }),
    ).toThrow('ZD_DATA_DIR is required when ZD_SANDBOX_MODE is enabled');
    expect(() => resolveDataDir('', projectRoot, { requireExplicit: true })).toThrow(
      'ZD_DATA_DIR is required when ZD_SANDBOX_MODE is enabled',
    );
    expect(() =>
  ...
  it('trims ZD_DATA_DIR before resolving the storage root', () => {
