...
describe('sandbox mode env parsing', () => {
  it('is disabled when ZD_SANDBOX_MODE is unset or false-like', () => {
    expect(isSandboxModeEnabled({})).toBe(false);
    expect(isSandboxModeEnabled({ ZD_SANDBOX_MODE: '0' })).toBe(false);
    expect(isSandboxModeEnabled({ ZD_SANDBOX_MODE: 'false' })).toBe(false);
  });

  it('is enabled for explicit true-like values', () => {
    expect(isSandboxModeEnabled({ ZD_SANDBOX_MODE: '1' })).toBe(true);
    expect(isSandboxModeEnabled({ ZD_SANDBOX_MODE: 'true' })).toBe(true);
    expect(isSandboxModeEnabled({ ZD_SANDBOX_MODE: 'YES' })).toBe(true);
  });

  it('rejects ambiguous non-empty values', () => {
    expect(() => isSandboxModeEnabled({ ZD_SANDBOX_MODE: 'sandbox' })).toThrow(
      'ZD_SANDBOX_MODE must be one of',
    );
  });
});

describe('sandbox runtime roots', () => {
  it('keeps all run-scoped roots under ZD_DATA_DIR', () => {
  ...
  it('pins agent home and tool config env to sandbox roots', () => {
    ...
    expect(env.ZD_AGENT_HOME).toBe(config.roots.agentHomeDir);
