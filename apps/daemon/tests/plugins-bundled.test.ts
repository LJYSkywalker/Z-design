...
import type { InstalledPluginRecord } from '@z-design/contracts';
...
describe('registerBundledPlugins', () => {
  it('registers every <bundledRoot>/<tier>/<id>/ folder under source_kind=bundled', async () => {
    ...
    //   <bundledRoot>/atoms/discovery-question-form/{z-design.json,SKILL.md}
    //   <bundledRoot>/scenarios/od-new-generation/{z-design.json,SKILL.md}
    ...
    await writeFile(path.join(atomDir, 'z-design.json'), SAMPLE_MANIFEST('discovery-question-form'));
    await writeFile(path.join(atomDir, 'SKILL.md'), SAMPLE_SKILL('discovery-question-form'));
    await writeFile(path.join(sceneDir, 'z-design.json'), SAMPLE_MANIFEST('od-new-generation'));
  ...
  it('can stamp official registry provenance on bundled preinstalls', async () => {
    ...
    await writeFile(path.join(folder, 'z-design.json'), SAMPLE_MANIFEST('starter'));
  ...
  it('also registers a direct <bundledRoot>/<plugin-id>/ folder', async () => {
    ...
    await mkdir(folder, { recursive: true });
    await writeFile(path.join(folder, 'z-design.json'), SAMPLE_MANIFEST('sample-plugin'));
  ...
  it('is idempotent — re-running upserts the same row', async () => {
    ...
    await mkdir(folder, { recursive: true });
    await writeFile(path.join(folder, 'z-design.json'), SAMPLE_MANIFEST('sample'));
  ...
  it('skips folders without z-design.json without warning', async () => {
  ...
  it('prunes persisted bundled rows whose bundled folder was removed', async () => {
    ...
    for (const [dir, id] of [[keepDir, 'keep'], [staleDir, 'stale']] as const) {
      ...
      await writeFile(path.join(dir, 'z-design.json'), SAMPLE_MANIFEST(id));
