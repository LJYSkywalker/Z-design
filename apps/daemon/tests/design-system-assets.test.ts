...
describe('isDesignTokenChannelEnabled (PR-D env gate)', () => {
  it('is true when ZD_DESIGN_TOKEN_CHANNEL is unset (PR-D default-on)', () => {
    expect(isDesignTokenChannelEnabled({})).toBe(true);
  });

  it('is true for the legacy explicit opt-in `1`', () => {
    expect(isDesignTokenChannelEnabled({ ZD_DESIGN_TOKEN_CHANNEL: '1' })).toBe(true);
  });

  it('is true for any non-`0` truthy-looking value (forward compatibility)', () => {
    expect(isDesignTokenChannelEnabled({ ZD_DESIGN_TOKEN_CHANNEL: 'true' })).toBe(true);
    expect(isDesignTokenChannelEnabled({ ZD_DESIGN_TOKEN_CHANNEL: 'on' })).toBe(true);
    expect(isDesignTokenChannelEnabled({ ZD_DESIGN_TOKEN_CHANNEL: '2' })).toBe(true);
    expect(isDesignTokenChannelEnabled({ ZD_DESIGN_TOKEN_CHANNEL: 'yes' })).toBe(true);
  });

  it('is true for an empty string (operator typed `=` and forgot the value — fail open, not closed)', () => {
    expect(isDesignTokenChannelEnabled({ ZD_DESIGN_TOKEN_CHANNEL: '' })).toBe(true);
  });

  it('is false ONLY for the literal kill-switch value `0`', () => {
    expect(isDesignTokenChannelEnabled({ ZD_DESIGN_TOKEN_CHANNEL: '0' })).toBe(false);
  });

  it('is true for whitespace-padded `0` — strict literal match prevents accidental kill-switch tripping', () => {
    expect(isDesignTokenChannelEnabled({ ZD_DESIGN_TOKEN_CHANNEL: ' 0' })).toBe(true);
    expect(isDesignTokenChannelEnabled({ ZD_DESIGN_TOKEN_CHANNEL: '0 ' })).toBe(true);
  });
});
...
describe('resolveDesignSystemAssets (PR-D server-layer asset resolution)', () => {
  ...
  it('returns empty (kill switch) when ZD_DESIGN_TOKEN_CHANNEL is `0`, even if files are on disk', async () => {
    const builtInRoot = fresh();
    const userRoot = fresh();
    const dir = brandDir(builtInRoot, 'sample');
    writeFileSync(path.join(dir, 'tokens.css'), ':root { --bg: #fff; }');
  ...
  it('still returns the assets under the legacy explicit opt-in `ZD_DESIGN_TOKEN_CHANNEL=1`', async () => {
    const builtInRoot = fresh();
    const userRoot = fresh();
    const dir = brandDir(builtInRoot, 'sample');
    writeFileSync(path.join(dir, 'tokens.css'), ':root { --bg: #fff; }');
