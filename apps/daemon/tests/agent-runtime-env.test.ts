...
import { SIDECAR_ENV } from '@z-design/sidecar-proto';
...
describe('agent runtime tool environment', () => {
  it('injects daemon URL and run-scoped tool token into agent sessions', () => {
    const env = createAgentRuntimeEnv(
      { PATH: '/bin', ZD_TOOL_TOKEN: 'stale-token' },
      'http://127.0.0.1:7456',
      { token: 'fresh-token' },
    ...
    expect(env).toMatchObject({
      ...
      ZD_NODE_BIN: '/opt/open-design/bin/node',
      ZD_TOOL_TOKEN: 'fresh-token',
    });
  });
  ...
  it('does not leak stale inherited tool tokens when no run token was minted', () => {
    ...
    expect(env.ZD_DAEMON_URL).toBe('http://127.0.0.1:7456');
    expect(env.ZD_NODE_BIN).toBe('/opt/open-design/bin/node');
    expect(env.ZD_TOOL_TOKEN).toBeUndefined();
  });
  ...
  it('pins the daemon runtime data dir into agent sessions', () => {
    ...
    expect(env.ZD_DATA_DIR).toBe(process.env.ZD_DATA_DIR);
  });
  ...
  it('describes daemon URL and token availability without exposing the token', () => {
    ...
    expect(prompt).toContain('`ZD_DAEMON_URL`');
    expect(prompt).toContain('`ZD_NODE_BIN`');
    expect(prompt).toContain('`"$ZD_NODE_BIN" "$ZD_BIN" tools ...`');
    expect(prompt).toContain('& $env:ZD_NODE_BIN $env:ZD_BIN tools ...');
    expect(prompt).toContain('`ZD_TOOL_TOKEN` is available');
  ...
  it('describes missing token availability without exposing stale internals', () => {
    ...
    expect(prompt).toContain('Daemon URL: `http://127.0.0.1:7456`');
    expect(prompt).toContain('`ZD_TOOL_TOKEN` is not available');
