...
describe('POST /api/import/folder', () => {
  ...
  async function withSandboxMode<T>(run: () => Promise<T>): Promise<T> {
    const previous = process.env.ZD_SANDBOX_MODE;
    process.env.ZD_SANDBOX_MODE = '1';
    try {
      return await run();
    } finally {
      if (previous == null) delete process.env.ZD_SANDBOX_MODE;
      else process.env.ZD_SANDBOX_MODE = previous;
    }
  }
  ...
  async function withSandboxImportAllowedRoots<T>(
    ...
    const previous = process.env.ZD_SANDBOX_IMPORT_ALLOWED_ROOTS;
    process.env.ZD_SANDBOX_IMPORT_ALLOWED_ROOTS = roots.join(path.delimiter);
    try {
      return await run();
    } finally {
      if (previous == null) delete process.env.ZD_SANDBOX_IMPORT_ALLOWED_ROOTS;
      else process.env.ZD_SANDBOX_IMPORT_ALLOWED_ROOTS = previous;
    }
  }
  ...
  it('rejects folder imports in sandbox mode', async () => {
    await withSandboxMode(async () => {
      ...
      expect(body.error?.message).toMatch(/ZD_SANDBOX_MODE/i);
    });
  });
  ...
  it('rejects sandbox folder imports outside the explicit import roots', async () => {
    await withSandboxMode(async () => {
      ...
      await withSandboxImportAllowedRoots([allowedRoot], async () => {
        ...
        expect(resp.status).toBe(400);
        const body = (await resp.json()) as { error?: { message?: string } };
        expect(body.error?.message).toMatch(/ZD_SANDBOX_IMPORT_ALLOWED_ROOTS/i);
      });
    });
  });
  ...
  it('rejects relative sandbox import allowed roots', async () => {
    await withSandboxMode(async () => {
      ...
      await withSandboxImportAllowedRoots(['tmp'], async () => {
        ...
        expect(resp.status).toBe(400);
        const body = (await resp.json()) as { error?: { message?: string } };
        expect(body.error?.message).toMatch(/ZD_SANDBOX_IMPORT_ALLOWED_ROOTS.*absolute/i);
      });
    });
  });
  ...
  it('rejects sandbox runs for imported folders before creating a run', async () => {
    ...
    await withSandboxMode(async () => {
      ...
      expect(body.error?.message).toMatch(/imported-folder projects.*ZD_SANDBOX_MODE/i);
    });
  });
  ...
  it('writes generated artifact files into metadata.baseDir instead of the daemon projects dir', async () => {
    ...
    const dataDir = process.env.ZD_DATA_DIR;
    if (!dataDir) throw new Error('ZD_DATA_DIR is required for daemon route tests');
