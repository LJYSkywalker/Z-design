...
import * as platform from '@z-design/platform';
import {
...
test('spawnEnvForAgent keeps sandbox roots pinned to the base ZD_DATA_DIR', () => {
  ...
  try {
    const env = spawnEnvForAgent(
      ...
      {
        CODEX_HOME: '/Users/test/.codex-host',
        ZD_DATA_DIR: '/host/path/.od',
      },
    );

    assert.equal(env.ZD_DATA_DIR, dataDir);
...
test('spawnEnvForAgent resolves relative ZD_DATA_DIR before applying sandbox roots', () => {
...
test('spawnEnvForAgent gives AMR a stable OpenCode home under ZD_DATA_DIR', () => {
...
fsTest('spawnEnvForAgent gives AMR a discovered OpenCode binary under a minimal child PATH', () => {
  ...
  try {
    return withEnvSnapshot(['PATH', 'ZD_AGENT_HOME'], () => {
...
test('detectAgents includes sanitized install and docs metadata from split runtime metadata', async () => {
  ...
  try {
    return await withEnvSnapshot(['PATH', 'ZD_AGENT_HOME'], async () => {
      process.env.PATH = dir;
      process.env.ZD_AGENT_HOME = dir;
...
fsTest('detectAgents marks Codex available when nvm exposes a node shim but launch resolution upgrades it to the native binary', async () => {
  ...
  try {
    return await withEnvSnapshot(['HOME', 'PATH', 'ZD_AGENT_HOME'], async () => {
...
fsTest('detectAgents keeps packaged built-in AMR unavailable when OpenCode cannot be resolved', async () => {
  ...
  try {
    return await withEnvSnapshot(['PATH', 'ZD_AGENT_HOME', 'ZD_RESOURCE_ROOT', 'VELA_OPENCODE_BIN'], async () => {
...
`);
      ...
      process.env.PATH = '';
      process.env.ZD_AGENT_HOME = join(root, 'empty-home');
      delete process.env.ZD_RESOURCE_ROOT;
      delete process.env.VELA_OPENCODE_BIN;
...
test('resolveAgentExecutable ignores Windows CODEX_BIN overrides without executable PATHEXT extension', () => {
  ...
  try {
    return withEnvSnapshot(['PATH', 'PATHEXT', 'ZD_AGENT_HOME'], () => {
...
test('detectAgents applies configured env while probing the CLI', async () => {
  ...
  try {
    await withEnvSnapshot(['PATH', 'ZD_AGENT_HOME'], async () => {
