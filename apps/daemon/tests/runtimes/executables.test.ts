...
fsTest(
  ...
  () => {
    ...
    try {
      return withEnvSnapshot(['PATH', 'ZD_AGENT_HOME', 'ZD_RESOURCE_ROOT', 'VELA_OPENCODE_BIN'], () => {
        ...
        process.env.PATH = '';
        process.env.ZD_AGENT_HOME = join(root, 'empty-home');
        process.env.ZD_RESOURCE_ROOT = resourceRoot;
...
fsTest(
  ...
  () => {
    ...
    try {
      return withEnvSnapshot(['PATH', 'ZD_AGENT_HOME', 'ZD_RESOURCE_ROOT'], () => {
        const resourceRoot = join(root, 'resources', 'open-design');
        const builtInVela = join(resourceRoot, 'bin', 'vela');
  ...
  () => {
    ...
    try {
      return withEnvSnapshot(['PATH', 'ZD_AGENT_HOME', 'ZD_RESOURCE_ROOT'], () => {
        ...
        process.env.ZD_AGENT_HOME = join(root, 'empty-home');
        process.env.ZD_RESOURCE_ROOT = join(root, 'resources', 'open-design');
...
fsTest(
  ...
  () => {
    ...
    try {
      ...
      process.env.ZD_AGENT_HOME = home;
      process.env.PATH = pathBin;
...
// Test isolation: when ZD_AGENT_HOME points at a sandbox, an exported
...
fsTest(
  'ZD_AGENT_HOME isolates resolution from $NPM_CONFIG_PREFIX leakage',
  () => {
    ...
    try {
      // Sandbox is empty — gemini does not exist under ZD_AGENT_HOME.
      ...
      process.env.ZD_AGENT_HOME = sandbox;
      process.env.PATH = '/usr/bin:/bin';
      ...
      assert.equal(
        resolved,
        null,
        `ZD_AGENT_HOME sandbox must not see the real $NPM_CONFIG_PREFIX bin; ` +
          `got ${resolved}`,
      );
    } finally {
...
fsTest(
  'ZD_SANDBOX_MODE scopes fallback toolchain discovery to ZD_DATA_DIR',
  () => {
    ...
    try {
      return withEnvSnapshot(
        ['PATH', 'ZD_AGENT_HOME', 'ZD_DATA_DIR', 'ZD_SANDBOX_MODE', 'NPM_CONFIG_PREFIX'],
        () => {
          mkdirSync(realPrefixBin, { recursive: true });
          writeFileSync(join(realPrefixBin, 'gemini'), '');
          chmodSync(join(realPrefixBin, 'gemini'), 0o755);
...
fsTest(
  'ZD_SANDBOX_MODE resolves relative ZD_DATA_DIR before fallback toolchain discovery',
  () => {
    ...
    try {
      return withEnvSnapshot(
        ...
        () => {
          ...
          process.env.ZD_DATA_DIR = relative(projectRoot, dataDir);
          process.env.ZD_SANDBOX_MODE = '1';
          process.env.PATH = emptyPath;
...
fsTest(
  'ZD_AGENT_HOME isolates resolution from $VP_HOME leakage',
  () => {
    ...
    try {
      ...
      assert.equal(
        resolved,
        null,
        `ZD_AGENT_HOME sandbox must not see the real $VP_HOME bin; got ${resolved}`,
      );
    } finally {
      rmSync(sandbox, { recursive: true, force: true });
