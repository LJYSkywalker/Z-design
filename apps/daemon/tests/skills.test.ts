...
describe('listSkills', () => {
  ...
  it('includes the built-in live-artifact skill catalog entry', async () => {
    ...
    expect(skill.body).toContain('"$ZD_NODE_BIN" "$ZD_BIN" tools live-artifacts create --input artifact.json');
    ...
    expect(skill.body).toContain('`ZD_DAEMON_URL`');
    expect(skill.body).toContain('`ZD_TOOL_TOKEN`');
  });
