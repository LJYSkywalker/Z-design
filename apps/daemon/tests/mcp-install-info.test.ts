...
describe('GET /api/mcp/install-info', () => {
  ...
  afterEach(() => {
    ...
    delete process.env.ZD_BIND_HOST;
  });
  ...
  it('non-sidecar launch bakes --daemon-url so custom ports keep working', async () => {
    ...
    // env always carries ZD_DATA_DIR (issue #848); no sidecar keys in
    // a non-sidecar launch.
    expect(body.env).toEqual({ ZD_DATA_DIR: dataDir });
  ...
  it('pins ZD_DATA_DIR in the env so IDE-spawned MCP processes write to the daemon data dir (issue #848)', async () => {
    const { port } = nonSidecar;
    const res = await fetch(`http://127.0.0.1:${port}/api/mcp/install-info`);
    const body = await readInstallInfo(res);
    expect(body.env).toBeDefined();
    expect(body.env.ZD_DATA_DIR).toBe(dataDir);
  });
  ...
  it('sidecar launch omits --daemon-url and emits the concrete IPC path with ZD_DATA_DIR', async () => {
