// @ts-nocheck
import type { DesktopExportPdfInput, DesktopExportPdfResult } from '@z-design/sidecar-proto';
...
import {
  ...
  type ZDesignDiscordPresenceResponse,
  type ZDesignGithubLatestReleaseResponse,
  type ZDesignGithubRepoResponse,
...
import { runGlmAgent } from './glm-agent-runner.js';
...
import { createCommandInvocation } from '@z-design/platform';
import { SIDECAR_DEFAULTS, SIDECAR_ENV } from '@z-design/sidecar-proto';
import {
...
} from '@z-design/contracts/analytics';
import {
...
import { DIAGNOSTICS_EXPORT_PATH } from '@z-design/diagnostics';
import {
  buildProjectArchive,
  buildBatchArchive,
  createProjectFolder,
...
/** @typedef {import('@z-design/contracts').ApiErrorCode} ApiErrorCode */
/** @typedef {import('@z-design/contracts').ApiError} ApiError */
/** @typedef {import('@z-design/contracts').ApiErrorResponse} ApiErrorResponse */
/** @typedef {import('@z-design/contracts').ChatRequest} ChatRequest */
/** @typedef {import('@z-design/contracts').ChatSseEvent} ChatSseEvent */
/** @typedef {import('@z-design/contracts').ProxyStreamRequest} ProxyStreamRequest */
/** @typedef {import('@z-design/contracts').ProxySseEvent} ProxySseEvent */
/** @typedef {import('@z-design/contracts').ProjectConversationCreatedSsePayload} ProjectConversationCreatedSsePayload */

const __filename = fileURLToPath(import.meta.url);
const __dirname = path.dirname(__filename);
const PROJECT_ROOT = resolveProjectRoot(__dirname);
const RESOURCE_ROOT_ENV = 'ZD_RESOURCE_ROOT';
...
const DAEMON_RESOURCE_ROOT = resolveDaemonResourceRoot({
  safeBases: [
    PROJECT_ROOT,
    resolveProcessResourcesPath(),
    process.env.ZD_INSTALLATION_DIR,
  ],
});
...
const ZD_BIN = resolveDaemonCliPath();
const ZD_NODE_BIN = process.execPath;
...
// so they respect ZD_DATA_DIR overrides (test isolation, packaged runs).
...
const RUNTIME_DATA_DIR = resolveDataDir(process.env.ZD_DATA_DIR, PROJECT_ROOT, {
...
// itself as the stable, user-shaped path so ZD_DATA_DIR resolution stays
...
// One-shot legacy data migration. When ZD_LEGACY_DATA_DIR is set and the
...
// and https://github.com/Aqu-210L/Z-design/issues/710.
migrateLegacyDataDirSync({
  legacyDir: process.env.ZD_LEGACY_DATA_DIR,
 ...
 *
 *   1. `ZD_PUBLIC_BASE_URL` env var. Cloud and packaged-electron deployments
...
function getPublicBaseUrl(req) {
  const env = process.env.ZD_PUBLIC_BASE_URL;
  if (env && /^https?:\/\//i.test(env)) {
    return env.replace(/\/+$/u, '');
  }
  const proto = req.protocol || 'http';
  const host = req.get('host');
  if (!host) return `http://localhost:${process.env.ZD_PORT ?? '7456'}`;
  return `${proto}://${host}`;
}
...
// Load Critique Theater config once at startup so a bad ZD_CRITIQUE_* value
...
export function createAgentRuntimeEnv(
  ...
  const env: NodeJS.ProcessEnv = applySandboxRuntimeEnv(
    {
      ...baseEnv,
      ZD_DATA_DIR: RUNTIME_DATA_DIR,
      ZD_DAEMON_URL: daemonUrl,
      ZD_NODE_BIN: nodeBin,
    },
    SANDBOX_RUNTIME,
  );
  ...
  if (toolTokenGrant?.token) {
    env.ZD_TOOL_TOKEN = toolTokenGrant.token;
  } else {
    delete env.ZD_TOOL_TOKEN;
  }
...
export function createAgentRuntimeToolPrompt(
  ...
  const tokenLine = toolTokenGrant?.token
    ? '- `ZD_TOOL_TOKEN` is available in your environment for this run. Use it only through project wrapper commands; do not print, persist, or override it.'
    : '- `ZD_TOOL_TOKEN` is not available for this run, so `/api/tools/*` wrapper commands may be unavailable.';

  return [
    '## Runtime tool environment',
    '',
    `- Daemon URL: \`${daemonUrl}\` (also available as \`ZD_DAEMON_URL\`).`,
    '- `ZD_NODE_BIN` is the absolute path to the Node-compatible runtime that started the daemon; packaged desktop installs provide this even when the user has no system `node` on PATH.',
    '- `ZD_BIN` is the absolute path to the Open Design CLI script. On POSIX shells run wrappers with `"$ZD_NODE_BIN" "$ZD_BIN" tools ...`; do not call bare `od`, which may resolve to the system octal-dump command on Unix-like systems.',
    '- On PowerShell use `& $env:ZD_NODE_BIN $env:ZD_BIN tools ...`; on cmd.exe use `"%ZD_NODE_BIN%" "%ZD_BIN%" tools ...`.',
    tokenLine,
    '- Prefer project wrapper commands through `ZD_NODE_BIN` + `ZD_BIN` over raw HTTP. The wrappers read these environment values automatically.',
  ].join('\n');
}
...
function renderRunContextPrompt(selection, metadata) {
  ...
  if (Array.isArray(context.connectorIds) && context.connectorIds.length > 0) {
    ...
    lines.push(
      'The user selected these connectors for this run. Discover available read-only connector tools first with `"$ZD_NODE_BIN" "$ZD_BIN" tools connectors list --format compact`, then execute relevant tools through `tools connectors execute`; do not ask for a data source that is already selected.',
    );
...
async function readProjectPluginManifest(folder) {
  const raw = await fs.promises.readFile(path.join(folder, 'z-design.json'), 'utf8');
  ...
  if (/[/\\]/.test(name) || /^\.+$/.test(name)) {
    throw new Error(
      `z-design.json in ${folder}: name "${name}" must not contain path separators or consist only of dots`,
    );
  }
  return {
    name,
...
const PLUGIN_SHARE_ACTION_LABELS = {
  ...
  'contribute-z-design': 'Contribute to Open Design',
};
...
function renderPluginSharePrompt({ action, sourcePlugin, stagedPath }) {
  ...
  if (action === 'publish-github') {
    return [
      ...
      '',
      '```bash',
      `curl -sS -X POST "$ZD_DAEMON_URL/api/projects/$ZD_PROJECT_ID/plugins/publish-github" \\`,
  ...
  return [
    ...
    '',
    '```bash',
    `curl -sS -X POST "$ZD_DAEMON_URL/api/projects/$ZD_PROJECT_ID/plugins/contribute-z-design" \\`,
    `  -H 'content-type: application/json' \\`,
    `  -d '${JSON.stringify({ path: stagedPath })}'`,
    '```',
    '',
...
async function hasGeneratedPluginArtifacts(projectRoot) {
  ...
  const required = [
    path.join(projectRoot, 'generated-plugin', 'z-design.json'),
...
let zDesignGithubRepoCache = null;
let zDesignGithubRepoInflight = null;
let zDesignGithubLatestReleaseCache = null;
let zDesignGithubLatestReleaseInflight = null;
let zDesignDiscordPresenceCache = null;
let zDesignDiscordPresenceInflight = null;

async function readZDesignGithubRepoStats() {
  const now = Date.now();
  if (
    zDesignGithubRepoCache &&
    now - zDesignGithubRepoCache.fetchedAt < OPEN_DESIGN_GITHUB_CACHE_TTL_MS
  ) {
    return { ...zDesignGithubRepoCache, stale: false };
  }

  if (zDesignGithubRepoInflight) {
    return zDesignGithubRepoInflight;
  }

  zDesignGithubRepoInflight = (async () => {
    ...
    try {
      ...
      if (zDesignGithubRepoCache) {
        return { ...zDesignGithubRepoCache, stale: true };
      }
      throw error;
    } finally {
      clearTimeout(timeout);
      zDesignGithubRepoInflight = null;
    }
  })();
...
async function readZDesignLatestReleaseInfo() {
  const now = Date.now();
  if (
    zDesignGithubLatestReleaseCache &&
    now - zDesignGithubLatestReleaseCache.fetchedAt < OPEN_DESIGN_GITHUB_CACHE_TTL_MS
  ) {
    return { ...zDesignGithubLatestReleaseCache, stale: false };
  }

  if (zDesignGithubLatestReleaseInflight) {
    return zDesignGithubLatestReleaseInflight;
  }

  zDesignGithubLatestReleaseInflight = (async () => {
    const ctrl = new AbortController();
    const timeout = setTimeout(() => ctrl.abort(), OPEN_DESIGN_GITHUB_TIMEOUT_MS);
    try {
      ...
      if (zDesignGithubLatestReleaseCache) {
        return { ...zDesignGithubLatestReleaseCache, stale: true };
      }
      throw error;
    } finally {
      clearTimeout(timeout);
      zDesignGithubLatestReleaseInflight = null;
    }
  })();
...
async function readZDesignDiscordPresence() {
  const now = Date.now();
  if (
    zDesignDiscordPresenceCache &&
    now - zDesignDiscordPresenceCache.fetchedAt < OPEN_DESIGN_DISCORD_CACHE_TTL_MS
  ) {
    return { ...zDesignDiscordPresenceCache, stale: false };
  }

  if (zDesignDiscordPresenceInflight) {
    return zDesignDiscordPresenceInflight;
  }

  zDesignDiscordPresenceInflight = (async () => {
    const ctrl = new AbortController();
    ...
    try {
      ...
      if (zDesignDiscordPresenceCache) {
        return { ...zDesignDiscordPresenceCache, stale: true };
      }
      throw error;
    } finally {
      clearTimeout(timeout);
      zDesignDiscordPresenceInflight = null;
    }
  })();
...
//   1. `ZD_CHAT_RUN_INACTIVITY_TIMEOUT_MS` (operator escape hatch).
...
export function resolveChatRunInactivityTimeoutMs(agentDefault?: number) {
  ...
  const env = Number(process.env.ZD_CHAT_RUN_INACTIVITY_TIMEOUT_MS);
...
export function resolveChatRunArtifactQuietPeriodMs() {
  const raw = Number(process.env.ZD_CHAT_RUN_ARTIFACT_QUIET_PERIOD_MS);
...
// period. Extracted so the `ZD_CHAT_RUN_ARTIFACT_QUIET_PERIOD_MS=0`
...
function resolveChatRunShutdownGraceMs() {
  const raw = Number(process.env.ZD_CHAT_RUN_SHUTDOWN_GRACE_MS);
...
function resolveAcpStageTimeoutMs(): number | undefined {
  ...
  // sets `ZD_ACP_STAGE_TIMEOUT_MS`. Bounded to the same 24h ceiling as the
  // outer chat inactivity watchdog so an oversized override doesn't get
  // clamped to 1ms by Node's signed-32-bit delay limit.
  const raw = Number(process.env.ZD_ACP_STAGE_TIMEOUT_MS);
...
export async function startServer({
  port = 7456,
  host = normalizeDaemonBindHost(process.env.ZD_BIND_HOST),
  returnServer = false,
  ...
  // ZD_API_TOKEN is set. This is the spec §16 Phase 5 safety floor:
  // a hosted operator can no longer accidentally publish an unsecured
  // daemon by setting ZD_BIND_HOST=0.0.0.0 without a token.
  //
  // Loopback hosts (127.0.0.1 / ::1 / localhost) are always allowed —
  // the desktop / dev flow remains unchanged. Setting ZD_API_TOKEN is
  ...
  if (!isLoopbackHostname(host) && apiToken.length === 0 && !apiAuthDisabled) {
    throw new Error(
      `ZD_BIND_HOST=${host} requires ZD_API_TOKEN to be set. ` +
      `Generate one with \`openssl rand -hex 32\` and re-launch. ` +
      `(Loopback hosts 127.0.0.1 / ::1 / localhost do not need a token.) ` +
      `Set ZD_DISABLE_API_AUTH=1 only when a trusted reverse proxy already authenticates every request.`,
    );
  }
  ...
  //
  // Active only when ZD_API_TOKEN is set and API auth is not disabled.
  ...
  // value matching `ZD_API_TOKEN`. Health / readiness / version remain
    ...
    app.use('/api', (req, res, next) => {
      ...
      if (!match || match[1] !== apiToken) {
        return res.status(401).json({
          error: { code: 'API_TOKEN_REQUIRED', message: 'Authorization: Bearer <ZD_API_TOKEN> required' },
        });
      }
      return next();
    });
  ...
  if (process.env.ZD_CODEX_DISABLE_PLUGINS === '1') {
    console.log('[od] Codex plugins disabled via ZD_CODEX_DISABLE_PLUGINS=1');
  }
  ...
  // when ZD_SNAPSHOT_GC_INTERVAL_MS is 0; otherwise one-time bootstrap
  ...
  app.get('/api/github/open-design', async (_req, res) => {
    try {
      const stats = await readZDesignGithubRepoStats();
      const payload = /** @type {ZDesignGithubRepoResponse} */ ({
  ...
  app.get('/api/github/open-design/releases/latest', async (_req, res) => {
    try {
      const release = await readZDesignLatestReleaseInfo();
      const payload = /** @type {ZDesignGithubLatestReleaseResponse} */ ({
        repo: 'nexu-io/open-design',
  ...
  app.get('/api/community/discord', async (_req, res) => {
    try {
      const presence = await readZDesignDiscordPresence();
      const payload = /** @type {ZDesignDiscordPresenceResponse} */ ({
  ...
  app.get('/api/daemon/status', async (_req, res) => {
    ...
    res.json({
      ...
      port: resolvedPort,
      dataDir: RUNTIME_DATA_DIR,
      mediaConfigDir: process.env.ZD_MEDIA_CONFIG_DIR ?? null,
  ...
  // set `ZD_METRICS_ENDPOINT=disabled`; the route is registered only
  // when that env value is not the literal string 'disabled'.
  if (process.env.ZD_METRICS_ENDPOINT !== 'disabled') {
  ...
  // design: the recommendation does not flip ZD_CRITIQUE_ROLLOUT_PHASE
  ...
  app.get('/api/critique/conformance', async (req, res) => {
    try {
      ...
      const decision = evaluateRollout({
        current: parseRolloutPhase(process.env.ZD_CRITIQUE_ROLLOUT_PHASE),
        history,
        windowDays,
        shippedThreshold,
        cleanParseThreshold,
  ...
  app.delete('/api/projects/:id', async (req, res) => {
    try {
      ...
      /** @type {import('@z-design/contracts').OkResponse} */
      const body = { ok: true };
      res.json(body);
    } catch (err) {
  ...
  async function resolveAmrModelProbe() {
    ...
    const cacheKey = JSON.stringify({
      ...
      zDesignAmrProfile: env.OPEN_DESIGN_AMR_PROFILE ?? '',
  ...
  const pluginRouteHelpers = {
    ...
    handleShareProject: async (req, res) => {
      try {
        ...
        if (!action) return sendApiError(res, 400, 'BAD_REQUEST', 'action must be publish-github or contribute-z-design');
  ...
  function collectBundledScenarios() {
    type ScenarioEntry = {
      ...
      pipeline: NonNullable<NonNullable<import('@z-design/contracts').PluginManifest['od']>['pipeline']>;
    };
  ...
  const composeDaemonSystemPrompt = async ({
    ...
    //
    // `ZD_DESIGN_TOKEN_CHANNEL=0` is the kill switch: it forces the
    ...
    const critiqueEnabledForRun = isCritiqueEnabled({
      phase: parseRolloutPhase(process.env.ZD_CRITIQUE_ROLLOUT_PHASE),
      skillPolicy: skillCritiquePolicy,
      projectOverride: projectCritiqueOverride,
      envOverride: parseEnvEnabled(process.env.ZD_CRITIQUE_ENABLED),
    });
    ...
    // every Phase 6/7/8 atom); set ZD_BUNDLED_ATOM_PROMPTS=0 to opt
    ...
    const bundledAtomPromptsEnabled = process.env.ZD_BUNDLED_ATOM_PROMPTS !== '0';
    if (
      ...
      try {
        ...
        if (stages.length > 0) {
          ...
          const { renderActiveStageBlock } = await import('@z-design/contracts');
          const blocks = [];
    ...
    const prompt = composeSystemPrompt({
      ...
      // critiqueCfg.enabled is loaded from ZD_CRITIQUE_ENABLED only, so a
  ...
  // the default stage runner; set ZD_PIPELINE_RUNNER=stub to fall
  ...
  const firePipelineForRun = (args) => {
    ...
    const projectIdForRun = run.projectId
      ...
      ?? 'project-unknown';
    const runnerMode = process.env.ZD_PIPELINE_RUNNER === 'stub'
      ? 'stub'
      : 'registry';
    let runStage;
    if (runnerMode === 'stub') {
  ...
  const startChatRun = async (chatBody, run) => {
    ...
    const {
      ...
      await composeDaemonSystemPrompt({
        ...
        // Default ON; set ZD_BUNDLED_ATOM_PROMPTS=0 to opt out.
    ...
    const destroyChildStdio = (child) => {
      ...
      //
      // See: https://github.com/Aqu-210L/Z-design/issues/4100
      if (!child) return;
    ...
    const noteArtifactRegistered = () => {
      ...
      // when `ZD_CHAT_RUN_INACTIVITY_TIMEOUT_MS=0` but
      // `ZD_CHAT_RUN_ARTIFACT_QUIET_PERIOD_MS>0` — noteAgentActivity()
    ...
    // GLM is a daemon-side agentic runtime: it does NOT shell out to a CLI.
    // The `glm-stream` branch drives an in-process tool-use loop against
    // Zhipu's OpenAI-compatible chat-completions API, emitting the same
    // text_delta / tool_use / tool_result / usage event vocabulary the CLI
    // adapters produce. It owns its own start → loop → finish lifecycle, so
    // it short-circuits the binary-resolution, spawn, and streamFormat-dispatch
    // pipeline below. See apps/daemon/src/glm-agent-runner.ts.
    if (def.streamFormat === 'glm-stream') {
      run.status = 'running';
      run.updatedAt = Date.now();
      send('start', {
        runId,
      ...
      // The GLM API key/base URL are read from the per-agent Local-CLI env
      // allowlist (GLM_API_KEY / GLM_BASE_URL) configured in Settings, falling
      // back to the daemon process environment. `configuredAgentEnv` is typed
      // loosely ({}) upstream, so we treat it as a record here.
      const glmEnv: Record<string, string> =
        configuredAgentEnv && typeof configuredAgentEnv === 'object'
          ? configuredAgentEnv as Record<string, string>
          : {};
      const readEnvKey = (key: string): string => {
        const fromAgent = glmEnv[key];
        if (typeof fromAgent === 'string' && fromAgent) return fromAgent;
        const fromProcess = process.env[key];
        return typeof fromProcess === 'string' && fromProcess ? fromProcess : '';
      };
      const glmApiKey = readEnvKey('GLM_API_KEY');
      const glmBaseUrl = readEnvKey('GLM_BASE_URL');
      const glmProjectMetadata = run.projectMetadata;

      try {
        await runGlmAgent({
          apiKey: glmApiKey,
          // Conditional spreads keep these optional fields absent (not
          // `undefined`) when unset — required under exactOptionalPropertyTypes.
          ...(glmBaseUrl ? { baseUrl: glmBaseUrl } : {}),
          model: safeModel,
          systemPrompt: daemonSystemPrompt,
          userMessage: composed,
          projectsRoot: PROJECTS_DIR,
          projectId: typeof projectId === 'string' ? projectId : '',
          ...(glmProjectMetadata != null ? { projectMetadata: glmProjectMetadata } : {}),
          send,
        });
        cleanupPromptFile();
        ...
        revokeToolToken('child_exit');
        unregisterChatAgentEventSink();
        send('error', createSseErrorPayload('AGENT_EXECUTION_FAILED', msg, { retryable: false }));
        return design.runs.finish(run, 'failed', 1, null);
      }
    }
    ...
    const browserUseRuntimeEnv = run.browserUse
      ? {
          ZD_BROWSER_USE_REQUESTED: run.browserUse.requested ? '1' : '0',
          ZD_BROWSER_USE_AVAILABLE: run.browserUse.available ? '1' : '0',
          ...(run.browserUse.reason ? { ZD_BROWSER_USE_UNAVAILABLE_REASON: run.browserUse.reason } : {}),
          ZD_BROWSER_USE_REGISTRY_PATH: run.browserUse.diagnostics?.registryPath ?? '',
        }
      : {};
      ...
      send(
        ...
        createSseErrorPayload(
          ...
          `Run terminated: model emitted fabricated role marker (\`${marker}\`). ` +
            ...
            'See https://github.com/Aqu-210L/Z-design/issues/3247.',
          { retryable: true },
        ),
      );
  ...
  orbitService.setRunHandler(async ({
    ...
    design.runs.start(run, () => startChatRun({
      ...
      systemPrompt: [
        ...
        'Keep connector credentials and ZD_TOOL_TOKEN private; never print or persist secrets.',
      ].join('\n'),
    }, run));
  ...
  app.post('/api/runs', async (req, res) => {
    ...
    /** @type {import('@z-design/contracts').ChatRunCreateResponse} */
    const body = {
      runId: run.id,
  ...
  app.get('/api/runs', (req, res) => {
    ...
    /** @type {import('@z-design/contracts').ChatRunListResponse} */
  ...
  app.get('/api/runs/:id/agui', async (req, res) => {
    const run = design.runs.get(req.params.id);
    if (!run) return sendApiError(res, 404, 'NOT_FOUND', 'run not found');
    const { encodeOdEventForAgui } = await import('@z-design/agui-adapter');
    const sse = createSseResponse(res);
  ...
  app.post('/api/runs/:id/cancel', async (req, res) => {
    ...
    /** @type {import('@z-design/contracts').ChatRunCancelResponse} */
