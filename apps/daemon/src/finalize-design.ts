...
// tsconfig does not resolve the `@z-design/contracts/api/chat` subpath
...
} from '@z-design/contracts/api/finalize';
...
// canonical definitions live in @z-design/contracts/api/finalize
...
function buildFinalizeProviderRequest(params: FinalizeProviderCallParams): FinalizeProviderHttpRequest {
  ...
  // GLM (Zhipu open.bigmodel.cn) is OpenAI-compatible, so it builds the
  // same Bearer-auth /chat/completions request as the OpenAI path.
  if (params.protocol === 'openai' || params.protocol === 'glm') {
    return {
...
export function extractDesignMd(
  ...
  } else if (protocol === 'openai' || protocol === 'azure' || protocol === 'glm') {
...
function extractTokenUsage(
  ...
  if (protocol === 'openai' || protocol === 'azure' || protocol === 'glm') {
