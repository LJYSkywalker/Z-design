...
Environment:
  ZD_NODE_BIN     Node-compatible runtime for agent wrapper invocations
  ZD_BIN          Open Design CLI script for agent wrapper invocations
  ZD_DAEMON_URL   Daemon base URL injected into agent runs
  ZD_TOOL_TOKEN   Bearer token injected into agent runs

Agent runtime invocation:
  "$ZD_NODE_BIN" "$ZD_BIN" tools connectors list --use-case personal_daily_digest --format compact
`;
...
function daemonUrl(): URL | { error: string } {
  ...
  if (!rawUrl) return { error: 'ZD_DAEMON_URL is required' };
  try {
    ...
    return { error: 'ZD_DAEMON_URL must be a valid URL' };
  }
}

function toolToken(): string | { error: string } {
  const token = process.env.ZD_TOOL_TOKEN;
  if (!token) return { error: 'ZD_TOOL_TOKEN is required' };
  return token;
}
  ...
  try {
    ...
    if (connectorReady) {
      ...
      const connectorReason = 'error' in baseUrl
        ? baseUrl.error
        : typeof token === 'string'
          ? 'ZD_TOOL_TOKEN is not available'
          : token.error;
      if (options.requireConnector) {
        return fail('Required GitHub repository intake could not read the repository through git, GitHub CLI, or connector', {
          repo: `${repo.owner}/${repo.repo}`,
