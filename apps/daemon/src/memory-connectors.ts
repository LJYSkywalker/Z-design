...
const DEFAULT_CONNECTOR_MEMORY_QUERY =
  '设计思路 设计偏好 UI UX 视觉风格 品牌 logo 设计系统 ZDesign';
...
const CONNECTOR_MEMORY_SYSTEM_PROMPT = `You are a design-memory extractor for ZDesign connected-app context.

You will receive compacted read-only data from apps such as Notion, Figma, Linear, Google Drive, GitHub, and Slack. Propose only durable memories that would improve future ZDesign design work.
...
- It would change how ZDesign creates, critiques, edits, or explains future design artifacts.
...
- Meta language about extraction or saving, including phrases like "ZDesign read X", "Found N readable items", "Summary from X", "context summary", or "Save this if it should be reused".
...
- feedback: preferences about how ZDesign should work or answer
...
export async function suggestMemoryFromConnectors(
  ...
  const drafts = await suggestWithLLM(
    dataDir,
    {
      userMessage: `Suggest durable ZDesign memories from connected apps. Search hint: ${options.query?.trim().slice(0, 240) || DEFAULT_CONNECTOR_MEMORY_QUERY}`,
...
export async function extractMemoryFromConnectors(
  ...
  const changed = await extractWithLLM(
    dataDir,
    {
      userMessage: `Extract durable ZDesign memory from connected apps. Search hint: ${options.query?.trim().slice(0, 240) || DEFAULT_CONNECTOR_MEMORY_QUERY}`,
      assistantMessage: context.connectorContext,
    },
    {
      projectRoot: options.projectRoot,
