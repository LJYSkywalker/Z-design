...
//                              endpoints
//
// The fallback stub handlers are gated behind ZD_MEDIA_ALLOW_STUBS=1; in
...
type MediaContext = {
   ...
   * to the user-supplied alias from `ZD_MEDIA_MODEL_ALIASES` /
...
// behind ZD_MEDIA_ALLOW_STUBS=1 and otherwise return a 503 (mapped from
...
function stubsAllowed() {
  const v = process.env.ZD_MEDIA_ALLOW_STUBS;
  return v === '1' || v === 'true';
}
...
export async function generateMedia(args: {
  ...
  try {
    if (
      ...
      // stub fallback behind ZD_MEDIA_ALLOW_STUBS so release builds don't
...
function codexImagegenArgs(ctx: MediaContext, generatedRoot: string, env: NodeJS.ProcessEnv): string[] {
  ...
  const model = env.ZD_CODEX_IMAGEGEN_MODEL?.trim() || CODEX_IMAGE_ORCHESTRATOR_MODEL;
  const args = [
    'exec',
    '--json',
    '--skip-git-repo-check',
  ...
  if (env.ZD_CODEX_DISABLE_PLUGINS === '1') args.push('--disable', 'plugins');
...
async function readCodexGeneratedImage(generatedRoot: string, threadId: string): Promise<Buffer> {
  ...
  if (process.env.ZD_CODEX_KEEP_GENERATED_IMAGES !== '1') {
...
async function runCodexImagegen(
  ...
  const stdout: Buffer[] = [];
  const stderr: Buffer[] = [];
  const timeoutMs = Number(process.env.ZD_CODEX_IMAGEGEN_TIMEOUT_MS || 300_000);
...
async function renderCodexImage(ctx: MediaContext): Promise<RenderResult> {
  ...
  return {
    bytes,
    providerNote: `codex/${imageModel} via ${env.ZD_CODEX_IMAGEGEN_MODEL?.trim() || CODEX_IMAGE_ORCHESTRATOR_MODEL} · ${ctx.aspect} · ${bytes.length} bytes`,
...
async function renderImageRouterImage(ctx: MediaContext, credentials: ProviderConfig): Promise<RenderResult> {
  if (!credentials.apiKey) {
    throw new Error(
      'no ImageRouter API key — configure it in Settings or set ZD_IMAGEROUTER_API_KEY',
    );
  }
...
async function renderVolcengineVideo(ctx: MediaContext, credentials: ProviderConfig, onProgress?: ProgressFn): Promise<RenderResult> {
  ...
  const configuredMaxMs = Number(process.env.ZD_VOLCENGINE_VIDEO_MAX_POLL_MS);
  const maxMs =
...
async function renderNanoBananaImage(ctx: MediaContext, credentials: ProviderConfig): Promise<RenderResult> {
  if (!credentials.apiKey) {
    throw new Error(
      'no Nano Banana API key — configure it in Settings or set ZD_NANOBANANA_API_KEY',
    );
  }
...
async function renderOpenRouterImage(
  ...
  // overrides ctx.wireModel (from ZD_MEDIA_MODEL_ALIASES / resolveModelAlias).
...
async function renderOpenRouterVideo(
  ...
  const startedAt = Date.now();
  const configuredMaxMs = Number(process.env.ZD_OPENROUTER_VIDEO_MAX_POLL_MS);
  const maxMs =
    Number.isFinite(configuredMaxMs) && configuredMaxMs >= 60_000
      ? configuredMaxMs
  ...
  if (!videoUrls || videoUrls.length === 0) {
    ...
    throw new Error(
      ...
      + `If your jobs legitimately need longer, raise ZD_OPENROUTER_VIDEO_MAX_POLL_MS.`,
    );
  }
...
async function renderGrokVideo(ctx: MediaContext, credentials: ProviderConfig, onProgress?: ProgressFn): Promise<RenderResult> {
  ...
  if (!videoUrl && requestId) {
    const startedAt = Date.now();
    const configuredMaxMs = Number(process.env.ZD_GROK_VIDEO_MAX_POLL_MS);
    const maxMs =
      Number.isFinite(configuredMaxMs) && configuredMaxMs >= 60_000
        ? configuredMaxMs
        : 8 * 60 * 1000;
    ...
    if (!videoUrl) {
      ...
      throw new Error(
        ...
        + `(last status: ${lastStatus || 'pending'}, ceiling ${ceilingSec}s). `
        + `If your jobs legitimately need longer, raise ZD_GROK_VIDEO_MAX_POLL_MS.`,
      );
    }
  }
...
async function renderElevenLabsTTS(ctx: MediaContext, credentials: ProviderConfig): Promise<RenderResult> {
  if (!credentials.apiKey) {
    throw new Error(
      'no ElevenLabs API key - configure it in Settings or set ZD_ELEVENLABS_API_KEY',
    );
  }
...
async function renderMinimaxTTS(ctx: MediaContext, credentials: ProviderConfig): Promise<RenderResult> {
  if (!credentials.apiKey) {
    throw new Error(
      'no MiniMax API key — configure it in Settings or set ZD_MINIMAX_API_KEY',
    );
  }
...
async function renderSenseAudioTTS(ctx: MediaContext, credentials: ProviderConfig): Promise<RenderResult> {
  if (!credentials.apiKey) {
    throw new Error(
      'no SenseAudio API key — configure it in Settings or set ZD_SENSEAUDIO_API_KEY',
    );
  }
...
//     slot with the TTS path (ZD_SENSEAUDIO_API_KEY / SENSEAUDIO_API_KEY).
...
async function renderAIHubMixImage(ctx: MediaContext, credentials: ProviderConfig): Promise<RenderResult> {
  if (!credentials.apiKey) {
    throw new Error('no AIHubMix API key — configure it in Settings or set ZD_AIHUBMIX_API_KEY');
  }
...
async function renderAIHubMixVideo(
  ...
  const startedAt = Date.now();
  const configuredMaxMs = Number(process.env.ZD_AIHUBMIX_VIDEO_MAX_POLL_MS);
  const maxMs =
    Number.isFinite(configuredMaxMs) && configuredMaxMs >= 60_000
      ? configuredMaxMs
      : 12 * 60 * 1000;
...
async function renderFishAudioTTS(ctx: MediaContext, credentials: ProviderConfig): Promise<RenderResult> {
  if (!credentials.apiKey) {
    throw new Error(
      'no FishAudio API key — configure it in Settings or set ZD_FISHAUDIO_API_KEY',
    );
  }
...
async function falQueueRun(
  ...
  throw new Error(
    ...
    `Raise ZD_FAL_MAX_POLL_MS to extend the ceiling.`,
  );
}

function falMaxPollMs(defaultMs: number): number {
  const v = Number(process.env.ZD_FAL_MAX_POLL_MS);
...
async function renderHyperFramesViaCli(ctx: MediaContext, projectDir: string, onProgress?: ProgressFn): Promise<RenderResult> {
  ...
  if (typeof compRel !== 'string' || !compRel.trim()) {
    throw new Error(
      'hyperframes-html requires --composition-dir <project-relative-path> ' +
        ...
        '$ZD_PROJECT_DIR/.hyperframes-cache/<id>/ and pass that path here.',
    );
  }
  ...
  await assertHyperFramesCompositionFile(
    ...
    compRel,
    'hyperframes.json',
    'Run `npx hyperframes init "$ZD_PROJECT_DIR/$COMP_REL" --example blank --skip-skills --non-interactive` before editing the composition.',
  );
  await assertHyperFramesCompositionFile(
    compAbs,
    compRel,
