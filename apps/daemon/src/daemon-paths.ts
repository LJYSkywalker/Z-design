...
export const DAEMON_CLI_PATH_ENV = 'ZD_DAEMON_CLI_PATH';
export const RESOURCE_ROOT_ENV = 'ZD_RESOURCE_ROOT';
...
export function resolveDaemonCliPath(env: NodeJS.ProcessEnv = process.env): string {
  const configured = cleanOptionalPath(env[DAEMON_CLI_PATH_ENV]) ?? cleanOptionalPath(env.ZD_BIN);
  if (configured) return configured;

  const packageJsonPath = require.resolve('@z-design/daemon/package.json');
...
export function resolveDaemonPluginPreviewsDir({
  ...
  const override = env.ZD_PLUGIN_PREVIEWS_DIR;
  if (override) {
...
export function resolveDataDir(
  ...
  if (!value) {
    if (options.requireExplicit) {
      throw new Error('ZD_DATA_DIR is required when ZD_SANDBOX_MODE is enabled');
    }
  ...
  try {
    ...
    throw new Error(
      [
        `ZD_DATA_DIR "${resolved}" is not writable: ${e.message}`,
