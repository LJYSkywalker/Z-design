...
const AMR_MODELS_RETRY_DELAYS_MS = [250, 750];
...
const PREFERRED_AMR_CHAT_MODEL_RANK = new Map(PREFERRED_AMR_CHAT_MODEL_ORDER.map((id, index) => [id, index]));
...
export function normalizeVelaModelId(rawId) {
    ...
    if (!withoutPrefix)
        return null;
    if (/^deepseek_v3_2$/i.test(withoutPrefix))
        return 'deepseek-v3.2';
    if (/^deepseek-v3-2$/i.test(withoutPrefix))
        return 'deepseek-v3.2';
    if (/^kimi_k2_6$/i.test(withoutPrefix))
        return 'kimi-k2.6';
    if (/^glm_5_1$/i.test(withoutPrefix))
        return 'glm-5.1';
    if (/^glm_5$/i.test(withoutPrefix))
        return 'glm-5';
...
function normalizeKnownVelaVersionId(rawId) {
...
export function parseVelaModels(stdout) {
    const seen = new Set();
    const models = [];
...
export function parseVelaModelJson(stdout, expectedSource) {
    let parsed;
    try {
...
async function fetchVelaModelsWithRetry(resolvedBin, env) {
    let lastError = null;
    for (let attempt = 0; attempt <= AMR_MODELS_RETRY_DELAYS_MS.length; attempt += 1) {
        try {
            ...
            lastError = error;
            if (attempt === AMR_MODELS_RETRY_DELAYS_MS.length ||
...
export async function fetchVelaPresetModels(resolvedBin, env) {
...
export async function fetchVelaRemoteModelsWithRetry(resolvedBin, env) {
    let lastError = null;
    for (let attempt = 0; attempt <= AMR_MODELS_RETRY_DELAYS_MS.length; attempt += 1) {
        try {
