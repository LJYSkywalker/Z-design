...
async function probe(
  ...
  // GLM is a daemon-side HTTP runtime, not a CLI. There is no binary to
  // detect, so it short-circuits the version/auth/model probes below and is
  // always surfaced as available. Auth state is derived from whether a
  // GLM_API_KEY is present in the per-agent env or the daemon process env;
  // the actual key validation happens on the first chat turn.
  if (def.streamFormat === 'glm-stream') {
    const keyFromAgent = configuredEnv?.GLM_API_KEY;
    const keyFromProcess = process.env.GLM_API_KEY;
    const hasKey = Boolean(
      (typeof keyFromAgent === 'string' && keyFromAgent) ||
        (typeof keyFromProcess === 'string' && keyFromProcess),
    );
    return {
      ...stripFns(def),
      models: def.fallbackModels ?? [DEFAULT_MODEL_OPTION],
      ...
      ...(hasKey ? {} : { authMessage: 'GLM_API_KEY not configured. Set it in Settings → Local CLI.' }),
      ...installMetaForAgent(def.id),
    };
  }
...
function rememberDetectedLiveModels(
  ...
  if (agent.models.length === 0) return;
