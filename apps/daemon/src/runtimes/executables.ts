...
import { wellKnownUserToolchainBins } from '@z-design/platform';
...
function userToolchainDirs() {
  ...
  const homeOverride =
    sandboxRuntime?.roots.agentHomeDir ?? process.env.ZD_AGENT_HOME;
  ...
  // When ZD_AGENT_HOME is set, scope the search strictly to the override
...
export function resolveAmrOpenCodeExecutable(
  ...
  // `ZD_RESOURCE_ROOT/bin/libexec/opencode/opencode` so a stale global
  ...
  const resourceRoot = (
    env.ZD_RESOURCE_ROOT ?? process.env.ZD_RESOURCE_ROOT
  )?.trim();
  if (resourceRoot) {
...
function packagedBuiltInExecutable(
  ...
  const resourceRoot = process.env.ZD_RESOURCE_ROOT?.trim();
