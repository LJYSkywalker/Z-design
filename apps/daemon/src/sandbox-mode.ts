...
export const SANDBOX_MODE_ENV = 'ZD_SANDBOX_MODE';
export const SANDBOX_IMPORT_ALLOWED_ROOTS_ENV = 'ZD_SANDBOX_IMPORT_ALLOWED_ROOTS';
...
export function resolveSandboxRuntimeConfigFromEnv(
  ...
  if (!isSandboxModeEnabled(env)) return null;
  const rawDataDir = env.ZD_DATA_DIR?.trim();
  if (!rawDataDir) {
...
export function applySandboxRuntimeEnv(
  ...
  env.ZD_DATA_DIR = config.dataDir;
  env.ZD_AGENT_HOME = roots.agentHomeDir;
  ...
  env.ZD_AGENT_PROFILES_CONFIG = sandboxAgentProfilesConfigPath(config);
