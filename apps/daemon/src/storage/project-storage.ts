...
// adapter — that's an opt-in flag away (`ZD_PROJECT_STORAGE=s3`).
...
export interface S3ProjectStorageOptions {
  ...
  endpoint?: string;
  // AWS access credentials. Read from ZD_S3_ACCESS_KEY_ID /
  // ZD_S3_SECRET_ACCESS_KEY by resolveProjectStorage(); the test
 ...
 * Default is local-disk; setting ZD_PROJECT_STORAGE=s3 pulls the
...
export function resolveProjectStorage(opts: {
  ...
  const kind = (env.ZD_PROJECT_STORAGE ?? 'local').trim().toLowerCase();
  if (kind === 's3') {
    ...
    // without renaming.
    const accessKeyId     = env.ZD_S3_ACCESS_KEY_ID     ?? env.AWS_ACCESS_KEY_ID     ?? '';
    const secretAccessKey = env.ZD_S3_SECRET_ACCESS_KEY ?? env.AWS_SECRET_ACCESS_KEY ?? '';
    const sessionToken    = env.ZD_S3_SESSION_TOKEN     ?? env.AWS_SESSION_TOKEN;
    ...
    return new S3ProjectStorage({
      bucket:   env.ZD_S3_BUCKET ?? '',
      region:   env.ZD_S3_REGION ?? env.AWS_REGION ?? '',
      ...(env.ZD_S3_PREFIX   ? { prefix:   env.ZD_S3_PREFIX }   : {}),
      ...(env.ZD_S3_ENDPOINT ? { endpoint: env.ZD_S3_ENDPOINT } : {}),
      credentials,
    });
  }
