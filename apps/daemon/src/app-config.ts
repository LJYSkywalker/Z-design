...
// to <projectRoot>/.od but follows ZD_DATA_DIR when set, keeping test and
...
export function readPluginEnvKnobs(): PluginEnvKnobs {
  return {
    maxDevloopIterations:        intFromEnv('ZD_MAX_DEVLOOP_ITERATIONS', 10),
    snapshotUnreferencedTtlDays: intFromEnv('ZD_SNAPSHOT_UNREFERENCED_TTL_DAYS', 30),
    snapshotRetentionDays:       nullableIntFromEnv('ZD_SNAPSHOT_RETENTION_DAYS'),
    snapshotGcIntervalMs:        intFromEnv('ZD_SNAPSHOT_GC_INTERVAL_MS', 6 * 60 * 60 * 1000),
  };
}
...
// GLM is the sole agent for GLM Design. The legacy per-agent env allowlists
// (claude/codex/amr/qwen/...) were trimmed when the product narrowed to
// GLM-only. A leftover stored config referencing another agent id is ignored
// by agentCliEnvForAgent (it returns {} for any id not present here).
const AGENT_CLI_ENV_KEYS: ReadonlyMap<string, ReadonlySet<string>> = new Map([
  ['glm', new Set(['GLM_API_KEY', 'GLM_BASE_URL', 'GLM_MODEL', 'GLM_BIN'])],
]);
...
const AGENT_CLI_AUTH_ENV_KEYS: ReadonlyMap<string, {
  ...
  ['glm', {
    auth: new Set(['GLM_API_KEY']),
    baseUrl: new Set(['GLM_BASE_URL']),
  }],
]);
