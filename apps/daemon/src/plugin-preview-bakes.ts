...
//
// Files + a `manifest.json` live under `<dir>` (ZD_PLUGIN_PREVIEWS_DIR, default
...
// previews with zero configuration; ZD_PLUGIN_PREVIEWS_BASE_URL overrides it.
...
export function resolvePluginPreviewsDir(projectRoot: string): string {
  const env = process.env.ZD_PLUGIN_PREVIEWS_DIR;
  ...
  // clips themselves live on R2). Local dev overrides ZD_PLUGIN_PREVIEWS_DIR to
...
export function bakedPreviewBlock(id: string, dir: string): BakedPreviewBlock | null {
  ...
  //   1. an explicit ZD_PLUGIN_PREVIEWS_BASE_URL override;
  ...
  const envBase = process.env.ZD_PLUGIN_PREVIEWS_BASE_URL?.replace(/\/+$/, '');
  const onDisk =
