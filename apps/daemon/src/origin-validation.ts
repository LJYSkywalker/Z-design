...
export function configuredAllowedOrigins(env: NodeJS.ProcessEnv = process.env): string[] {
  const raw = env.ZD_ALLOWED_ORIGINS || '';
  if (!raw.trim()) return [];
  return raw
    .split(',')
    ...
    .map((origin) => {
      ...
      if (parsed.protocol !== 'http:' && parsed.protocol !== 'https:') {
        throw new Error('ZD_ALLOWED_ORIGINS only supports http:// and https:// origins');
      }
...
export function allowedBrowserPorts(
  ...
  const webPort = Number(env.ZD_WEB_PORT);
...
export function isLocalSameOrigin(
  ...
  const bindHost = env.ZD_BIND_HOST || '127.0.0.1';
  ...
  if (origin == null || origin === '') {
    ...
    // ZD_ALLOWED_ORIGINS unreachable for legitimate same-origin GETs
  ...
  // origin in ZD_ALLOWED_ORIGINS. Trust the Origin header in that case:
