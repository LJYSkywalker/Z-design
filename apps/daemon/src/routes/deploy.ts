...
export function registerDeployRoutes(app: Express, ctx: RegisterDeployRoutesDeps) {
  ...
  app.get('/api/deploy/config', async (req, res) => {
    try {
      ...
      /** @type {import('@z-design/contracts').DeployConfigResponse} */
  ...
  app.get('/api/deploy/cloudflare-pages/zones', async (_req, res) => {
    try {
      /** @type {import('@z-design/contracts').CloudflarePagesZonesResponse} */
  ...
  app.get('/api/projects/:id/deployments', (req, res) => {
    try {
      /** @type {import('@z-design/contracts').ProjectDeploymentsResponse} */
  ...
  app.post('/api/projects/:id/deploy', async (req, res) => {
    try {
      ...
      const now = Date.now();
      /** @type {import('@z-design/contracts').DeployProjectFileResponse} */
  ...
  app.post('/api/projects/:id/deploy/preflight', async (req, res) => {
    try {
      ...
      /** @type {import('@z-design/contracts').DeployPreflightResponse} */
    ...
    async (req, res) => {
      try {
        ...
        if (existing.providerId === CLOUDFLARE_PAGES_PROVIDER_ID && existing.cloudflarePages?.pagesDev?.url) {
          ...
          const now = Date.now();
          /** @type {import('@z-design/contracts').CheckDeploymentLinkResponse} */
          const body = upsertDeployment(db, {
            ...existing,
            ...checked,
