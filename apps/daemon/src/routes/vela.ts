...
import { applyAgentLaunchEnv, getAgentDef, resolveAgentLaunch, spawnEnvForAgent, } from '../agents.js';
import { readAnalyticsContext } from '../analytics.js';
import { agentCliEnvForAgent, writeAppConfig } from '../app-config.js';
import { cancelVelaLogin, forgetVelaLogin, mergeVelaEnv, mirrorAmrEntryAnalytics, mirrorAmrOnboardingProfileAnalytics, parseAmrEntryAnalyticsPayload, parseAmrOnboardingProfileAnalyticsPayload, parseVelaLoginAttribution, readVelaCredentialRevision, readVelaLoginStatus, spawnVelaLogin, } from '../integrations/vela.js';
...
function velaApiProxyBaseUrl(req, getPublicBaseUrl) {
...
export function registerVelaRoutes(app, deps) {
    const env = deps.env ?? process.env;
    const { RUNTIME_DATA_DIR } = deps.paths;
    const { readAppConfig } = deps.appConfig;
    const getPublicBaseUrl = deps.http.getPublicBaseUrl ?? ((req) => {
    ...
    async function resolveAmrModelProbe() {
        ...
        if (!launchPath)
            throw new Error('AMR vela binary could not be resolved');
        const spawnEnv = applyAgentLaunchEnv(spawnEnvForAgent(def.id, {
            ...env,
            ...(def.env || {}),
        }, configuredEnv, undefined), agentLaunch);
        const credentialRevision = readVelaCredentialRevision(env, configuredEnv);
        const cacheKey = JSON.stringify({
            launchPath,
            home: spawnEnv.HOME ?? spawnEnv.USERPROFILE ?? '',
            zDesignAmrProfile: spawnEnv.OPEN_DESIGN_AMR_PROFILE ?? '',
        ...
        try {
            ...
            if (status.loggedIn) {
                void resolveAmrModelProbe()
                    .then((probe) => {
                    amrModelLoadingCache.warm(probe.cacheKey, () => fetchVelaRemoteModelsWithRetry(probe.launchPath, probe.env));
                })
    ...
    app.post('/api/integrations/vela/login', async (req, res) => {
        try {
            ...
            try {
                ...
                if (/already running/i.test(directMessage))
                    throw directErr;
                spawned = await spawnVelaLogin({
                    configuredEnv,
                    attribution: loginAttribution,
