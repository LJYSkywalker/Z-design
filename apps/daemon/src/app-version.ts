...
// Keep this structurally aligned with `@z-design/contracts` AppVersionInfo.
...
// inject the version via `ZD_APP_VERSION` (packaged runtime) keep working
...
export function resolveAppVersionInfo({
  ...
  const version = cleanString(env.ZD_APP_VERSION)
    ?? cleanString(packageMetadata?.version)
    ?? APP_VERSION_FALLBACK;
  const inferredChannel = inferReleaseChannelFromVersion(version);
  const channel = cleanString(env.ZD_RELEASE_CHANNEL)
    ?? cleanString(env.ZD_APP_CHANNEL)
    ?? inferredChannel
