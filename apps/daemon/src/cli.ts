...
import { requestJsonIpc } from '@z-design/sidecar';
import { SIDECAR_ENV, SIDECAR_MESSAGES } from '@z-design/sidecar-proto';
import {
  AGENT_SLUGS,
  isAgentSlug,
  planAgentInstall,
...
function printRootHelp() {
  ...
  od media generate --surface <image|video|audio> --model <id> [opts]
      ...
      Designed to be invoked by a code agent - picks up ZD_DAEMON_URL
      and ZD_PROJECT_ID from the env that the daemon injected on spawn.
...
Options:
  --port <n>       Port to listen on (default: 7456, env: ZD_PORT).
  --host <addr>    Interface address to bind to (default: 127.0.0.1, env: ZD_BIND_HOST).
...
Flags:
  ...
  --daemon-url   Local daemon URL. Defaults to ZD_DAEMON_URL, ZD_SIDECAR_IPC_PATH discovery, or http://127.0.0.1:7456.`);
}
...
async function runMediaGenerate(rawArgs) {
  ...
  const daemonUrl = await cliDaemonUrl(flags);
  const projectId = flags.project || process.env.ZD_PROJECT_ID;
  const token = process.env.ZD_TOOL_TOKEN;
  if (!projectId && !token) {
    console.error(
      'project id required. Pass --project <id> or set ZD_PROJECT_ID. The daemon injects this when it spawns the code agent.',
    );
    process.exit(2);
  }
...
async function pollUntilDoneOrBudget(daemonUrl, taskId, sinceStart, options = {}) {
  ...
  process.stderr.write(
    `task ${taskId} still running after ${handoff.elapsed}s. ` +
      `Run \`"$ZD_NODE_BIN" "$ZD_BIN" media wait ${taskId} --since ${since}\` to continue in an agent runtime ` +
      `(${stillRunningHint}).\n`,
  );
...
function printMediaHelp() {
  console.log(`Usage: od media generate --surface <image|video|audio> --model <id> [opts]
       "$ZD_NODE_BIN" "$ZD_BIN" media generate --surface <image|video|audio> --model <id> [opts]

Required:
  --surface  image | video | audio
  --model    Model id from /api/media/models (e.g. gpt-image-2, seedance-2, suno-v5).
  --project  Project id. Auto-resolved from ZD_PROJECT_ID when invoked by the daemon.
...
Options:
  --daemon-url <url>   Open Design daemon HTTP base URL. Resolution
                       order: this flag, ZD_DAEMON_URL, ZD_SIDECAR_IPC_PATH,
...
async function runPlugin(args) {
  ...
  switch (sub) {
    ...
    case 'open-design-pr': return runPluginZDesignPr(rest);
...
//
// Side-effect: writes a SKILL.md + z-design.json starter under
...
Writes <out|cwd>/<id>/{SKILL.md,z-design.json,README.md}.`);
...
Exit codes:
  0  archive written
  2  CLI usage error
  4  pack-time error (missing z-design.json, invalid JSON, etc)`);
    process.exit(rest.length === 0 ? 2 : 0);
  }
  const folder = rest[0];
  try {
...
Common options:
  --daemon-url <url>   Open Design daemon HTTP base (default ZD_DAEMON_URL, ZD_SIDECAR_IPC_PATH discovery, or http://127.0.0.1:7456).
...
// compare the daemon's view to their on-disk z-design.json
...
GitHub API as a last resort. It never publishes to placeholder owners.`);
  ...
  const manifestPath = resolve(absFolder, 'z-design.json');
...
async function runPluginZDesignPr(rest) {
  const flags = parseFlags(rest, {
    string: new Set(['host', 'owner']),
    boolean: new Set(['help', 'h', 'json', 'dry-run']),
  });
...
marks a version unresolvable for new installs while preserving lockfile replay.`);
  ...
  const payload = {
    ...
    version: parsed.range,
    reason,
    url: `https://github.com/Aqu-210L/Z-design/issues/new?${params.toString()}`,
    body,
  };
  if (flags.json) {
    process.stdout.write(JSON.stringify(payload, null, 2) + '\n');
...
status:
  Prints a structured inventory of the daemon's SQLite backend:
    - file path (under .od/ by default; ZD_DATA_DIR overrides)
...
async function runDaemonStart(flags) {
  const port = Number(flags.port ?? process.env.ZD_PORT ?? 7456);
  const host = String(flags.host ?? process.env.ZD_BIND_HOST ?? '127.0.0.1').trim() || '127.0.0.1';
...
diagnostics produces.
  ...
  const { DIAGNOSTICS_EXPORT_PATH, DIAGNOSTICS_FILENAME_PREFIX, diagnosticsFileName } =
    await import('@z-design/diagnostics');
  const fs = await import('node:fs/promises');
  const path = await import('node:path');
