...
import { defaultCritiqueConfig, type CritiqueConfig } from '@z-design/contracts/critique';
import type { ChatSessionMode, MediaExecutionPolicy, MediaSurface } from '@z-design/contracts';
...
- \`ZD_NODE_BIN\`   — absolute path to the Node runtime
- \`ZD_BIN\`        — absolute path to the OD CLI script
- \`ZD_PROJECT_ID\` — the active project id
...
out=\$("$ZD_NODE_BIN" "$ZD_BIN" media generate \\
  --project "$ZD_PROJECT_ID" \\
  --surface image \\
...
since="\${since:-0}"
while [ -n "\$task_id" ]; do
  out=\$("$ZD_NODE_BIN" "$ZD_BIN" media wait "\$task_id" --since "\$since")
  ec=\$?
  last=\$(printf '%s\\n' "\$out" | tail -1)
...
export interface ComposeInput {
  ...
  // `kami`). `ZD_DESIGN_TOKEN_CHANNEL=0` disables the channel as a kill
  ...
  // `renderActiveStageBlock()` when the ZD_BUNDLED_ATOM_PROMPTS env
...
export function composeSystemPrompt({
  ...
  // `@z-design/contracts`'s composer. The daemon hits this path for
  ...
  if (agentId === 'gemini') {
    parts.push(
      "\n\n---\n\n## Gemini todo tool mapping\n\nWhen an Z-Design instruction says to call `TodoWrite`, use Gemini CLI's native `write_todos` tool only if it is present in the current tool list. Pass the full task list as `todos`, with each item using `description` for the task text and `status` set to `pending`, `in_progress`, `completed`, `cancelled`, or `blocked`.\n\nIf `write_todos` is not present, do not simulate it with markdown, plan-mode files, JSON files, TODO files, or shell commands. Continue the work normally without a todo tool.",
    );
  }
...
This conversation is in Z-Design Chat mode. Z-Design is an open-source GLM-native agentic design workspace. Official links: GitHub https://github.com/Aqu-210L/Z-design.
...
instead of \`"$ZD_NODE_BIN" "$ZD_BIN" media generate\` for the first generation
...
\`$ZD_PROJECT_DIR\`. Do not copy every generated variant, do not keep multiple
...
Copy or move the selected generated file into \`$ZD_PROJECT_DIR\` with a short
descriptive filename, then verify the exact destination file exists under
\`$ZD_PROJECT_DIR\` before claiming success. If reading the source path,
...
After the file exists under \`$ZD_PROJECT_DIR\`, reply with the project-local
...
path via \`"$ZD_NODE_BIN" "$ZD_BIN" media generate --surface image --model ${imageModel}\`.
...
function renderMetadataBlock(
  ...
  if (metadata.kind === 'image') {
    ...
    lines.push(renderMediaMetadataAction(
      'image',
      '`"$ZD_NODE_BIN" "$ZD_BIN" media generate --surface image --model <imageModel>`',
      mediaExecution,
    ));
  }
  if (metadata.kind === 'video') {
    ...
    lines.push(renderMediaMetadataAction(
      'video',
      '`"$ZD_NODE_BIN" "$ZD_BIN" media generate --surface video --model <videoModel> --length <seconds> --aspect <ratio>`',
      mediaExecution,
    ));
    if (metadata.videoModel === 'hyperframes-html') {
      lines.push(
        'Special case: `hyperframes-html` is a local HTML-to-MP4 renderer, not a photoreal text-to-video model. Treat it like a motion design renderer, ask at most one clarifying question, then create a HyperFrames composition with `npx hyperframes init` under `.hyperframes-cache/`, edit `index.html`, and dispatch via `"$ZD_NODE_BIN" "$ZD_BIN" media generate --surface video --model hyperframes-html --composition-dir <rel>`. Do not run `npx hyperframes render` yourself.',
      );
    }
  }
  if (metadata.kind === 'audio') {
    ...
    lines.push(renderMediaMetadataAction(
      'audio',
      '`"$ZD_NODE_BIN" "$ZD_BIN" media generate --surface audio --audio-kind <kind> --model <audioModel> --duration <seconds>` and add `--voice <voice-id>` for speech when you have a provider-specific voice id',
      mediaExecution,
    ));
  }
...
function renderMediaMetadataAction(
  ...
  const mode = mediaExecution?.mode ?? 'enabled';
  if (mode === 'disabled') {
    return `This is ${article} **${surface}** project, but Z-Design-owned media execution is disabled for this run. Plan the creative brief only unless an external MCP media tool is explicitly configured. Do NOT call OD media generation tools and do NOT emit \`<artifact>\` HTML for media surfaces.`;
  }
