...
import type { MediaExecutionPolicy, MediaSurface } from '@z-design/contracts';
...
Z-Design-owned media execution is **disabled for this run**. Do not call
\`"$ZD_NODE_BIN" "$ZD_BIN" media generate\`, Codex built-in imagegen, OD media
...
shell command through \`ZD_NODE_BIN\` + \`ZD_BIN\` is HOW you actually produce bytes.
...
- \`ZD_NODE_BIN\`    — absolute path to the Node-compatible runtime that started the daemon. Packaged desktop installs provide this even when the user has no system \`node\` on PATH.
- \`ZD_BIN\`         — absolute path to the OD CLI script. On POSIX shells run with \`"$ZD_NODE_BIN" "$ZD_BIN" …\`.
- \`ZD_PROJECT_ID\`  — the active project's id. Pass it as \`--project "$ZD_PROJECT_ID"\`.
- \`ZD_PROJECT_DIR\` — the project's files folder (your cwd). Generated files land here.
- \`ZD_DAEMON_URL\`  — base URL of the local daemon, e.g. \`http://127.0.0.1:7456\`.
...
\`\`\`bash
"$ZD_NODE_BIN" "$ZD_BIN" media generate \\
...
For media projects, \`"$ZD_NODE_BIN" "$ZD_BIN" media generate …\` is the **only**
...
before the first generate attempt. Treat \`ZD_NODE_BIN\`, \`ZD_BIN\`,
\`ZD_PROJECT_ID\`, and \`ZD_DAEMON_URL\` as the source of truth and try the dispatcher
first.
...
to completion by calling \`media wait <taskId>\` through \`ZD_NODE_BIN\` + \`ZD_BIN\`
...
- \`exit 2\` — still **running**. Final stdout line is
  ...
  \`"$ZD_NODE_BIN" "$ZD_BIN" media wait <taskId> --since <n>\` to continue from where you left
...
\`\`\`bash
out=\$("$ZD_NODE_BIN" "$ZD_BIN" media generate --surface image --model flux-pro-ultra --prompt "…")
ec=\$?
if [ "\$ec" -ne 0 ]; then
  echo "\$out" >&2; exit "\$ec"
fi
...
- **video**:   ${VIDEO_IDS}
  ...
  \`--image <project-relative-path>\` to \`"$ZD_NODE_BIN" "$ZD_BIN" media generate\`. The
...
\`"$ZD_NODE_BIN" "$ZD_BIN" media generate\` as-is via \`--model <id>\`;
...
2. **Dispatch immediately when the brief is complete.** For image and video
   ...
   **Do ask** if the output intent is genuinely ambiguous (e.g. "make something cool"
   ...
   \`"$ZD_NODE_BIN" "$ZD_BIN" media generate --surface video --model hyperframes-html --composition-dir <rel>\`.
...
3. **Generate by shell, reply in one short message.** When you invoke
   \`"$ZD_NODE_BIN" "$ZD_BIN" media generate\`, do it inside a clearly-labelled tool call.
...
4. **Iterate by re-running.** To revise, call \`"$ZD_NODE_BIN" "$ZD_BIN" media generate\` again
...
1. **HTTP status.** When stubs are disabled (the default release-build
   ...
   error message. Set \`ZD_MEDIA_ALLOW_STUBS=1\` to write a labelled
   placeholder instead.
2. **Exit code.** \`"$ZD_NODE_BIN" "$ZD_BIN" media generate\` exits \`0\` for
   both immediate completion and successful queued/running handoff; inspect
   the final stdout JSON for either \`file\` or \`taskId\`. \`"$ZD_NODE_BIN"
   "$ZD_BIN" media wait\` exits \`0\` on terminal **done**, \`2\` when the
