...
export function renderResearchCommandContract(
  ...
  const lines = [
    ...
    '',
    '```bash',
    `"$ZD_NODE_BIN" "$ZD_BIN" research search --query "<search query>" --max-sources ${maxSources}`,
    '```',
    '',
    '```powershell',
    `& $env:ZD_NODE_BIN $env:ZD_BIN research search --query "<search query>" --max-sources ${maxSources}`,
    '```',
    '',
    '```cmd',
    `"%ZD_NODE_BIN%" "%ZD_BIN%" research search --query "<search query>" --max-sources ${maxSources}`,
