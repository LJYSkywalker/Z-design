...
// folder altogether (`ZD_OPENAI_API_KEY=… node daemon/cli.js`).
//
// Storage location (precedence high → low):
//   1. ZD_MEDIA_CONFIG_DIR=DIR   → <DIR>/media-config.json
//   2. ZD_DATA_DIR=DIR           → <DIR>/media-config.json
...
// that already set ZD_DATA_DIR for the rest of the daemon's runtime
...
// semantics as ZD_DATA_DIR in server.ts:resolveDataDir(): the shared
...
//
// Migration note: a workspace install that sets a custom ZD_DATA_DIR
// AND has a pre-existing `<projectRoot>/.od/media-config.json` will
// start reading from `<ZD_DATA_DIR>/media-config.json` instead. Move
// the file once or set ZD_MEDIA_CONFIG_DIR=<projectRoot>/.od to keep
// the old location.
//
...
// to dynamically lift `ZD_MEDIA_MODEL_ALIAS_<id>=value` into a record
...
// their workflow:
//
//   ZD_MEDIA_MODEL_ALIASES='{"doubao-seedream-3-0-t2i-250415":"doubao-seedream-5-0"}'
const ENV_MODEL_ALIASES = 'ZD_MEDIA_MODEL_ALIASES';
...
const ENV_KEYS: Record<string, string[]> = {
  ...
  volcengine: ['ZD_VOLCENGINE_API_KEY', 'ARK_API_KEY', 'VOLCENGINE_API_KEY'],
  // ZD_GROK_API_KEY first (the project-reserved override, same shape as
  ...
  grok: ['ZD_GROK_API_KEY', 'XAI_API_KEY'],
  nanobanana: ['ZD_NANOBANANA_API_KEY', 'GOOGLE_API_KEY', 'GEMINI_API_KEY'],
  imagerouter: ['ZD_IMAGEROUTER_API_KEY', 'IMAGEROUTER_API_KEY'],
  openrouter: ['ZD_OPENROUTER_API_KEY', 'OPENROUTER_API_KEY'],
  'custom-image': ['ZD_CUSTOM_IMAGE_API_KEY', 'CUSTOM_IMAGE_API_KEY'],
  bfl: ['ZD_BFL_API_KEY', 'BFL_API_KEY'],
  fal: ['ZD_FAL_KEY', 'FAL_KEY'],
  replicate: ['ZD_REPLICATE_API_TOKEN', 'REPLICATE_API_TOKEN'],
  google: ['ZD_GOOGLE_API_KEY', 'GOOGLE_API_KEY', 'GEMINI_API_KEY'],
  kling: ['ZD_KLING_API_KEY', 'KLING_API_KEY'],
  midjourney: ['ZD_MIDJOURNEY_API_KEY'],
  minimax: ['ZD_MINIMAX_API_KEY', 'MINIMAX_API_KEY'],
  suno: ['ZD_SUNO_API_KEY'],
  udio: ['ZD_UDIO_API_KEY'],
  elevenlabs: ['ZD_ELEVENLABS_API_KEY', 'ELEVENLABS_API_KEY'],
  fishaudio: ['ZD_FISHAUDIO_API_KEY', 'FISH_AUDIO_API_KEY'],
  senseaudio: ['ZD_SENSEAUDIO_API_KEY', 'SENSEAUDIO_API_KEY'],
  aihubmix: ['ZD_AIHUBMIX_API_KEY', 'AIHUBMIX_API_KEY'],
  tavily: ['ZD_TAVILY_API_KEY', 'TAVILY_API_KEY'],
  leonardo: ['ZD_LEONARDO_API_KEY', 'LEONARDO_API_KEY'],
};

// Resolve an `ZD_*_DIR` env override using the same semantics as
...
function resolveOverrideDir(raw: string, projectRoot: string): string {
  // Share expandHomePrefix with resolveDataDir (server.ts) so ZD_DATA_DIR
  // and ZD_MEDIA_CONFIG_DIR cannot split state under a $HOME-style value.
  // A launcher passing ZD_DATA_DIR=$HOME/.open-design without a shell to
...
export function mediaConfigDir(projectRoot: string): string {
  return (
    envOverrideDir('ZD_MEDIA_CONFIG_DIR', projectRoot)
    ?? envOverrideDir('ZD_DATA_DIR', projectRoot)
