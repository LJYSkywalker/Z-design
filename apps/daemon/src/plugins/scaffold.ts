...
// frontmatter the skills protocol expects) + z-design.json (sidecar
...
export async function scaffoldPlugin(input: ScaffoldInput): Promise<ScaffoldResult> {
  ...
  try {
    ...
    const conflicts = entries.filter((e) =>
      e === 'SKILL.md' || e === 'z-design.json' || e === '.claude-plugin' || e === 'README.md',
    );
  ...
  const manifestPath = path.join(folder, 'z-design.json');
  ...
  const readme = [
    ...
    '- `z-design.json` — the versioned Open Design marketplace sidecar.',
    '',
    'Edit `SKILL.md` to teach the agent how to perform the workflow.',
    'Edit `z-design.json` to refine the marketplace card and inputs.',
    '',
  ].join('\n');
