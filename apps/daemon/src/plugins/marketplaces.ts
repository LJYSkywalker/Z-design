...
// validation lives in `@z-design/plugin-runtime`'s parser and we only
...
function marketplaceRegistryRepo(): string {
  return (process.env.ZD_MARKETPLACE_REPO?.trim() || DEFAULT_MARKETPLACE_REPO)
    .replace(/^\/+|\/+$/g, '');
}

export function marketplaceRegistryBaseUrl(): string {
  const explicit = process.env.ZD_MARKETPLACE_REGISTRY_BASE_URL?.trim();
  if (explicit) return explicit.replace(/\/+$/, '');

  const repo = marketplaceRegistryRepo();
  const ref = (process.env.ZD_MARKETPLACE_REPO_REF?.trim() || DEFAULT_MARKETPLACE_REPO_REF)
    .replace(/^\/+|\/+$/g, '');
  const registryPath = (process.env.ZD_MARKETPLACE_REGISTRY_PATH?.trim() || DEFAULT_MARKETPLACE_REGISTRY_PATH)
    .replace(/^\/+|\/+$/g, '');
