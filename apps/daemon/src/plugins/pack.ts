...
//   - z-design.json (required; this is what the installer
//     resolves first)
...
export interface PackPluginInput {
  // Path to the plugin folder. Must contain z-design.json.
  folder: string;
...
export async function packPlugin(input: PackPluginInput): Promise<PackPluginResult> {
  ...
  // Confirm the folder shape — z-design.json must exist + parse.
  let manifestRaw: string;
  try {
    manifestRaw = await fsp.readFile(path.join(folder, 'z-design.json'), 'utf8');
  } catch (err) {
    if ((err as NodeJS.ErrnoException).code === 'ENOENT') {
      throw new PackPluginError(`folder ${folder} does not contain z-design.json`);
    }
    throw new PackPluginError(`failed to read z-design.json: ${(err as Error).message}`);
  }
  ...
  try {
    ...
    throw new PackPluginError(`z-design.json failed to parse as JSON: ${(err as Error).message}`);
  }
