...
// + z-design.json pair) and register every match into the
...
import type { InstalledPluginRecord, MarketplaceTrust } from '@z-design/contracts';
...
export async function registerBundledPlugins(
  ...
  for (const tier of topLevel) {
    ...
    const tierManifest = path.join(tierAbs, 'z-design.json');
    if (await pathExists(tierManifest)) {
      // Direct: <bundledRoot>/<plugin-id>/z-design.json
    ...
    for (const entry of inner) {
      ...
      const manifest = path.join(folder, 'z-design.json');
