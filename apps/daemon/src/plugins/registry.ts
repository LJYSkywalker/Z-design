...
//   manifest folders.
// - Resolves a plugin folder into either an `z-design.json`-anchored
...
export function defaultRegistryRoots(): RegistryRoots {
  return registryRootsForDataDir(path.resolve(process.env.ZD_DATA_DIR ?? path.join(process.cwd(), '.od')));
}
...
export async function resolvePluginFolder(opts: ResolveOptions): Promise<ResolveResult> {
  ...
  const sidecarPath = path.join(folder, 'z-design.json');
  ...
  if (fs.existsSync(sidecarPath)) {
    ...
    if (!parsed.ok) {
      errors.push(...parsed.errors.map((e) => `z-design.json: ${e}`));
    } else {
  ...
  if (!sidecar && adapters.length === 0) {
    return {
      ok: false,
      errors: [...errors, `Plugin folder contains no SKILL.md, no .claude-plugin/plugin.json, and no z-design.json: ${folder}`],
      warnings,
    };
  }
