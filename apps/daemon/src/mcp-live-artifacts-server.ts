  ...
  return [
    {
      ...
      description: 'Create a project-scoped live artifact through the daemon tool endpoint. POSIX equivalent: `"$ZD_NODE_BIN" "$ZD_BIN" tools live-artifacts create --input artifact.json`.',
      inputSchema: {
        type: 'object',
        additionalProperties: false,
        required: ['input'],
    ...
    },
    {
      ...
      description: 'List compact project-scoped live artifacts through the daemon tool endpoint. POSIX equivalent: `"$ZD_NODE_BIN" "$ZD_BIN" tools live-artifacts list --format compact`.',
      inputSchema: EMPTY_OBJECT_SCHEMA,
    },
    {
      name: 'live_artifacts_update',
      description: 'Update a live artifact through the daemon tool endpoint. POSIX equivalent: `"$ZD_NODE_BIN" "$ZD_BIN" tools live-artifacts update --artifact-id <id> --input artifact.json`.',
      inputSchema: {
        type: 'object',
        additionalProperties: false,
    ...
    },
    {
      ...
      description: 'Refresh a live artifact through the daemon tool endpoint. POSIX equivalent: `"$ZD_NODE_BIN" "$ZD_BIN" tools live-artifacts refresh --artifact-id <id>`.',
      inputSchema: {
        type: 'object',
        additionalProperties: false,
        required: ['artifactId'],
    ...
    },
    {
      ...
      description: 'List connector catalog and available read-only tools through the daemon tool endpoint. Use `{ "useCase": "personal_daily_digest" }` for curated daily-digest tools. POSIX equivalent: `"$ZD_NODE_BIN" "$ZD_BIN" tools connectors list --use-case personal_daily_digest --format compact` or fallback `"$ZD_NODE_BIN" "$ZD_BIN" tools connectors list --format compact`.',
      inputSchema: CONNECTORS_LIST_INPUT_SCHEMA,
    },
    {
      name: 'connectors_execute',
      description: 'Execute an allowed connector read tool through the daemon tool endpoint. POSIX equivalent: `"$ZD_NODE_BIN" "$ZD_BIN" tools connectors execute --connector <id> --tool <name> --input input.json`.',
      inputSchema: {
        type: 'object',
        additionalProperties: false,
...
function daemonUrl(): URL {
  const rawUrl = process.env.ZD_DAEMON_URL;
  if (!rawUrl) throw new Error('ZD_DAEMON_URL is required');
...
function toolToken(): string {
  ...
  if (!token) throw new Error('ZD_TOOL_TOKEN is required');
  return token;
}
