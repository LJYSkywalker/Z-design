...
} from '@z-design/contracts/api/providerModels';
import { isLoopbackApiHost } from '@z-design/contracts/api/connectionTest';
...
function providerModelsUrl(protocol: ConnectionTestProtocol, baseUrl: string, apiKey: string): string {
  ...
  if (protocol === 'openai' || protocol === 'senseaudio' || protocol === 'glm') {
...
function extractModels(protocol: ConnectionTestProtocol, data: unknown): ProviderModelOption[] {
  ...
  if (protocol === 'openai' || protocol === 'senseaudio' || protocol === 'glm') return extractOpenAiModels(data);
