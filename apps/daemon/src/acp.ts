...
// `ZD_ACP_STAGE_TIMEOUT_MS` from the environment.
...
// `ZD_ACP_STAGE_TIMEOUT_MS=0` would call `setTimeout(..., 0)` and fail every
...
function resolveAcpTimeoutMs(env: NodeJS.ProcessEnv, fallbackMs: number): number {
  const raw = Number(env.ZD_ACP_TIMEOUT_MS);
...
export function attachAcpSession({
  ...
  const resetStageTimer = (label: string) => {
    ...
    // Without this, an operator setting `ZD_ACP_STAGE_TIMEOUT_MS=0` would
