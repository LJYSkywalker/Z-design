...
export function registerMcpRoutes(app: Express, ctx: RegisterMcpRoutesDeps) {
  ...
  const { ZD_BIN, RUNTIME_DATA_DIR, PROJECTS_DIR } = ctx.paths;
  ...
  function computeInstallPayload(): McpInstallPayload {
    const cliPath = ZD_BIN;
    // The daemon was bootstrapped as a sidecar (tools-dev, packaged) iff
    // bootstrapSidecarRuntime stamped ZD_SIDECAR_IPC_PATH into the env.
    ...
    // tools-dev / packaged launchers export ZD_WEB_PORT so the daemon
    ...
    const webPortRaw = process.env.ZD_WEB_PORT;
  ...
  // where ZD_PUBLIC_BASE_URL pins the externally-routable URL.
  // ─────────────────────────────────────────────────────────────────
