...
function isAmrOpenCodeBootstrapStderrLine(line) {
    const trimmed = line.trim();
    return (/^Performing one time database migration, may take a few minutes\.\.\.$/i.test(trimmed) ||
...
function coerceStderrChunk(chunk) {
    if (chunk === null || chunk === undefined)
        return '';
    return typeof chunk === 'string' ? chunk : String(chunk);
}
export function createAgentStderrVisibilityFilter(agentId) {
    if (agentId !== 'amr') {
        return {
            write(chunk) {
    ...
    const emitIfVisible = (line, separator = '') => isAmrOpenCodeBootstrapStderrLine(line) ? '' : `${line}${separator}`;
    return {
        write(chunk) {
