 ...
 * way to know about it. See https://github.com/Aqu-210L/Z-design/issues/710.
 *
 * This module gives operators a recovery path. When `ZD_LEGACY_DATA_DIR`
...
export interface MigrateLegacyDataDirOptions {
  /** Path to the legacy `.od/` directory (typically ZD_LEGACY_DATA_DIR). */
...
/**
 * Daemon startup throws this when ZD_LEGACY_DATA_DIR is explicitly set
...
export function migrateLegacyDataDirSync(
  ...
  if (!legacyDirHasPayload(legacyDir)) {
    ...
    throw new LegacyMigrationError(
      'legacy_dir_invalid',
      `ZD_LEGACY_DATA_DIR="${legacyDir}" is not a usable legacy data dir (expected app.sqlite directly inside it). Quit Open Design, fix the path, and relaunch.`,
    );
  }
  ...
  if (existing.length > 0) {
    throw new LegacyMigrationError(
      'data_dir_not_empty',
      `ZD_DATA_DIR="${dataDir}" already contains payload entries (${existing.join(', ')}); refusing to merge legacy data on top. Move the existing data aside or pick a fresh data root before re-running with ZD_LEGACY_DATA_DIR.`,
    );
  }
