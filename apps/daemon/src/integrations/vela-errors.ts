...
export const DEFAULT_AMR_RECHARGE_URL = 'https://open-design.ai/amr/wallet?source=open_design';
const AMR_AUTH_REQUIRED_MESSAGE = 'AMR sign-in is required. Sign in to AMR Cloud again, then retry this run.';
const AMR_INSUFFICIENT_BALANCE_MESSAGE = `AMR Cloud reported insufficient balance for this model. Recharge your AMR wallet at ${DEFAULT_AMR_RECHARGE_URL}, then retry this run.`;
...
export function classifyAmrAccountFailure(text) {
    ...
    if (value.includes('auth_required') ||
...
export function amrAccountFailureDetails(failure) {
    return {
        kind: 'amr_account',
