...
const AMR_ENTRY_SOURCES = new Set([
    'onboarding_amr_card',
...
const AMR_ONBOARDING_PROFILE_SOURCES = new Set([
    'onboarding_amr_card',
    'onboarding_amr_sign_in_continue',
]);
const AMR_ENTRY_SOURCE_PAGES = new Set([
    'onboarding',
    'chat_panel',
    'settings',
    'file_manager',
...
const AMR_ENTRY_SOURCE_PAGE_BY_SOURCE = {
...
const AMR_ANALYTICS_EVENTS_URL = 'https://amr-api.open-design.ai/api/v1/analytics/events';
const AMR_ANALYTICS_TIMEOUT_MS = 1500;
const ZD_DEVICE_ID_MAX_LENGTH = 128;
const AMR_ANALYTICS_ENVS = new Set([
    'local',
    'test',
    'staging',
    'production',
...
export function parseVelaLoginActivation(stdout, stderr) {
...
export function mergeVelaEnv(env = process.env, configuredEnv = {}) {
    return {
        ...env,
        ...configuredEnv,
    };
...
export function readVelaLoginStatus(env = process.env, configuredEnv = {}) {
    ...
    const activationFields = loginInFlight && activeLoginActivation
        ? {
...
export function readVelaCredentialRevision(env = process.env, configuredEnv = {}) {
    const mergedEnv = mergeVelaEnv(env, configuredEnv);
    const status = readVelaLoginStatus(env, configuredEnv);
    const hasEnvCredentials = (mergedEnv.VELA_RUNTIME_KEY?.trim() ?? '').length > 0 &&
    ...
    return {
        ...
        configMtimeMs: hasEnvCredentials || !existsSync(status.configPath)
            ? null
            : statSync(status.configPath).mtimeMs,
    };
}
export function forgetVelaLogin(env = process.env) {
    const file = amrConfigPath();
    if (!existsSync(file))
        return;
    ...
    if (!Object.prototype.hasOwnProperty.call(parsed.profiles, profile))
        return;
    ...
    writeFileSync(file, JSON.stringify({ ...parsed, profiles: nextProfiles }, null, 2), 'utf8');
}
const activeLoginProcs = new Map();
...
function resolveLoginActivationGraceMs(baseEnv) {
    const raw = Number(baseEnv.ZD_AMR_LOGIN_ACTIVATION_GRACE_MS);
...
let activeLoginActivation = null;
...
function beginLoginActivationCapture(child) {
    const activation = {
        activationUrl: null,
        userCode: null,
    ...
    child.stdout?.on('data', (chunk) => {
        ...
        if (parsed.activationUrl)
            activation.activationUrl = parsed.activationUrl;
        if (parsed.userCode)
            activation.userCode = parsed.userCode;
    });
...
export function isVelaLoginInFlight() {
...
async function waitForImmediateLoginFailure(child) {
    let stderr = '';
    let stdout = '';
    child.stderr?.setEncoding('utf8');
    child.stdout?.setEncoding('utf8');
    ...
    const result = await new Promise((resolve) => {
        ...
        const timer = setTimeout(() => finish({ kind: 'running' }), LOGIN_STARTUP_GRACE_MS);
...
async function waitForLoginActivationSteadyState(child, capture, graceMs) {
    if (capture.activation.activationUrl)
        return;
    if (!isChildRunning(child)) {
        if (child.exitCode === 0)
            return;
...
export async function spawnVelaLogin(deps = {}) {
    ...
    const configuredEnv = withDefaultVelaApiUrl(deps.configuredEnv ?? {}, baseEnv, deps.defaultApiUrl);
    ...
    const cleanup = () => {
        if (typeof child.pid === 'number')
            activeLoginProcs.delete(child.pid);
    ...
    if (deps.waitForActivation) {
        await waitForLoginActivationSteadyState(child, activationCapture, resolveLoginActivationGraceMs(baseEnv));
    }
...
function withDefaultVelaApiUrl(configuredEnv, baseEnv, defaultApiUrl) {
    const trimmed = defaultApiUrl?.trim();
    if (!trimmed)
        return configuredEnv;
    if ((configuredEnv.VELA_API_URL ?? '').trim())
        return configuredEnv;
    if ((baseEnv.VELA_API_URL ?? '').trim())
        return configuredEnv;
    return { ...configuredEnv, VELA_API_URL: trimmed };
}
export function parseVelaLoginAttribution(input) {
    ...
    if (typeof value.entryId !== 'string'
        ...
        || !AMR_ENTRY_SOURCES.has(value.sourceDetail)
        || typeof value.occurredAt !== 'string'
        || !Number.isFinite(Date.parse(value.occurredAt))) {
        return null;
    }
    const odDeviceId = sanitizeZDesignDeviceId(value.odDeviceId);
    return {
...
export function parseAmrEntryAnalyticsPayload(input) {
    ...
    if (pageName !== 'open_design'
        || typeof sourcePageName !== 'string'
        || !AMR_ENTRY_SOURCE_PAGES.has(sourcePageName)
        || area !== 'amr_entry'
        || typeof element !== 'string'
        || !AMR_ENTRY_SOURCES.has(element)
        || action !== 'click_amr_entry'
        || typeof entryId !== 'string'
        || entryId.length === 0
        || sourceProduct !== 'open_design'
        || typeof sourceDetail !== 'string'
        || !AMR_ENTRY_SOURCES.has(sourceDetail)
        || sourceDetail !== element
        || sourcePageName
            !== AMR_ENTRY_SOURCE_PAGE_BY_SOURCE[sourceDetail]
...
export function parseAmrOnboardingProfileAnalyticsPayload(input) {
    const raw = isRecord(input) && 'payload' in input ? input.payload : input;
    if (!isRecord(raw))
        return null;
    const pageName = raw.pageName;
    ...
    const odDeviceId = sanitizeZDesignDeviceId(raw.odDeviceId);
    const odRole = sanitizeOptionalProfileValue(raw.odRole);
    const odOrgSize = sanitizeOptionalProfileValue(raw.odOrgSize);
    const odSource = sanitizeOptionalProfileValue(raw.odSource);
    const odUseCase = sanitizeOptionalProfileList(raw.odUseCase);
    if (pageName !== 'open_design'
        ...
        || typeof sourceDetail !== 'string'
        || !AMR_ENTRY_SOURCES.has(sourceDetail)
        || !AMR_ONBOARDING_PROFILE_SOURCES.has(sourceDetail)
        || typeof entryOccurredAt !== 'string'
        || !Number.isFinite(Date.parse(entryOccurredAt))
...
function sanitizeOptionalProfileValue(value) {
...
function sanitizeOptionalProfileList(value) {
    if (value === undefined || value === null)
        return undefined;
...
function sanitizeZDesignDeviceId(value) {
    if (typeof value !== 'string')
        return null;
    const trimmed = value.trim();
    if (!trimmed || trimmed.length > ZD_DEVICE_ID_MAX_LENGTH)
        return null;
    return trimmed;
}
export async function mirrorAmrEntryAnalytics(payload, deps = {}) {
    return mirrorAmrAnalyticsEvent(buildAmrEntryAnalyticsCommon(payload, deps), payload, deps);
}
export async function mirrorAmrOnboardingProfileAnalytics(payload, deps = {}) {
    return mirrorAmrAnalyticsEvent(buildAmrOnboardingProfileAnalyticsCommon(payload, deps), payload, deps);
}
async function mirrorAmrAnalyticsEvent(common, payload, deps) {
...
function velaLoginAttributionEnv(attribution) {
    if (!attribution)
        return {};
    return {
...
function buildAmrEntryAnalyticsCommon(payload, deps) {
...
function buildAmrOnboardingProfileAnalyticsCommon(payload, deps) {
    const context = deps.analyticsContext ?? null;
    const anonymousId = context?.deviceId?.trim() || payload.odDeviceId || payload.entryId;
    const sessionId = context?.sessionId?.trim() || payload.entryId;
    return {
