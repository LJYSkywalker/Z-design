 ...
 * `ZD_CRITIQUE_ENABLED=1` rather than relying on the client toggle,
 ...
 *   3. Environment override (`ZD_CRITIQUE_ENABLED=1`). Useful for
...
export interface RolloutInputs {
  /** Effective rollout phase. Reads from `ZD_CRITIQUE_ROLLOUT_PHASE`
  ...
  /** Environment override (`ZD_CRITIQUE_ENABLED=1`). */
...
/**
 * Parse the `ZD_CRITIQUE_ROLLOUT_PHASE` env var into a `RolloutPhase`.
...
/**
 * Parse `ZD_CRITIQUE_ENABLED`. Recognises the canonical truthy /
