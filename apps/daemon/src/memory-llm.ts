...
function envKeyFor(provider) {
  ...
  if (provider === 'senseaudio') {
    return (
      process.env.ZD_SENSEAUDIO_API_KEY?.trim()
  ...
  if (provider === 'aihubmix') {
    return (
      process.env.ZD_AIHUBMIX_API_KEY?.trim()
      || process.env.AIHUBMIX_API_KEY?.trim()
      || ''
    );
  }
  if (provider === 'glm') {
    // GLM (Zhipu open.bigmodel.cn) is the sole provider for Z-Design.
    return (
...
//
// The `ZD_MEMORY_MODEL` env continues to override the model name across
...
async function pickProvider(projectRoot, dataDir, chatAgentId, chatProvider, chatModel) {
  ...
  const envOverrideModel = (process.env.ZD_MEMORY_MODEL || '').trim();
  ...
  if (chatProtocol) {
    const localCliProvider = localCliProviderFor(
      normalizedChatAgentId,
      chatProtocol,
      process.env.ZD_MEMORY_MODEL || chatModel,
    );
    if (localCliProvider) return localCliProvider;
