...
export function registerChatRoutes(app: Express, ctx: RegisterChatRoutesDeps) {
  ...
  app.post('/api/provider/models', async (req, res) => {
    ...
    if (
      ...
      !['anthropic', 'openai', 'azure', 'google', 'ollama', 'senseaudio', 'aihubmix', 'glm'].includes(protocol)
    ) {
      return sendApiError(
        res,
        400,
        'BAD_REQUEST',
        'protocol must be one of anthropic|openai|azure|google|ollama|senseaudio|aihubmix|glm',
      );
    }
