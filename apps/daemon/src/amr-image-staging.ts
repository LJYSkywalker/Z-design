...
async function pruneStagedAttachments(stagingDir) {
    let entries;
    try {
...
export async function stageAmrImagePaths(cwd, imagePaths, uploadRoot) {
    if (!cwd || !Array.isArray(imagePaths) || imagePaths.length === 0)
        return [];
    ...
    for (const inputPath of imagePaths) {
        ...
        try {
            ...
            if (uploadRootReal && !isWithinRoot(uploadRootReal, real))
                continue;
