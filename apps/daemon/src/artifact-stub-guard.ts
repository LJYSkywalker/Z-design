...
export function readArtifactStubGuardConfigFromEnv(env: NodeJS.ProcessEnv = process.env): ArtifactStubGuardConfig {
  const rawMode = (env.ZD_ARTIFACT_STUB_GUARD ?? '').toLowerCase();
  const mode: ArtifactStubGuardMode =
    rawMode === 'reject' || rawMode === 'warn' || rawMode === 'off'
      ? rawMode
      : DEFAULT_ARTIFACT_STUB_GUARD_CONFIG.mode;

  const ratioRaw = Number(env.ZD_ARTIFACT_STUB_GUARD_MIN_RATIO);
  ...
  const minPriorBytesRaw = Number(env.ZD_ARTIFACT_STUB_GUARD_MIN_PRIOR_BYTES);
  const minPriorBytes =
...
function buildWarning(
  ...
  return {
    ...
    message:
      ...
      'Set ZD_ARTIFACT_STUB_GUARD=warn to record the warning without rejecting, or =off to disable the guard entirely.',
    identifier,
    newSize,
