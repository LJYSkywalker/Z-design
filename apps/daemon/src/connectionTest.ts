...
} from '@z-design/contracts/api/connectionTest';
import { googleGenerateContentUrl } from './google-models.js';

export { validateBaseUrl } from '@z-design/contracts/api/connectionTest';
...
// Override with ZD_CONNECTION_TEST_PROVIDER_TIMEOUT_MS for slow networks
...
// Override with ZD_CONNECTION_TEST_AGENT_TIMEOUT_MS.
...
// the budget — e.g. `ZD_CONNECTION_TEST_AGENT_TIMEOUT_MS=3000000000` —
...
export function resolveConnectionTestTimeoutMs(
  key: 'ZD_CONNECTION_TEST_PROVIDER_TIMEOUT_MS' | 'ZD_CONNECTION_TEST_AGENT_TIMEOUT_MS',
  fallback: number,
...
function providerTimeoutMs(): number {
  return resolveConnectionTestTimeoutMs(
    'ZD_CONNECTION_TEST_PROVIDER_TIMEOUT_MS',
...
function agentTimeoutMs(): number {
  return resolveConnectionTestTimeoutMs(
    'ZD_CONNECTION_TEST_AGENT_TIMEOUT_MS',
...
function inspectProviderCompletion(
  ...
  // GLM (Zhipu open.bigmodel.cn) speaks the OpenAI-compatible
  // chat-completions + /v1/models shape, so it routes through the same
  // branch as openai/azure/senseaudio/aihubmix.
  if (protocol === 'openai' || protocol === 'azure' || protocol === 'senseaudio' || protocol === 'aihubmix' || protocol === 'glm') {
    const responseModel = typeof obj.model === 'string' ? obj.model : '';
    if (
      // AIHubMix is omitted from the strict response-model check (like Azure):
      // its gateway routes by model name and may echo a normalized id. GLM's
      // open platform may also normalize echoed model ids, so it joins the
      // lenient set too.
      (protocol === 'openai' || protocol === 'senseaudio' || protocol === 'glm') &&
...
function buildProviderCall(input: ProviderTestRequest): ProviderCallShape {
  ...
  switch (input.protocol) {
    ...
    case 'glm':
      ...
      // smoke test reuses the same call shape. GLM (Zhipu open.bigmodel.cn)
      // is OpenAI-compatible too and joins this branch. We default the base
      // URL upstream-side in chat-routes; this layer assumes the caller
      // passed a concrete URL via the BYOK form.
      return {
        url: appendVersionedApiPath(baseUrl, '/chat/completions'),
        headers: {
          'content-type': 'application/json',
