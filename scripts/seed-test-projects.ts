...
//
// The daemon URL is resolved in this order: --daemon flag > $ZD_DAEMON_URL >
// http://127.0.0.1:$ZD_PORT > whatever `pnpm tools-dev status --json` reports
...
Options:
  --daemon <url>     Daemon base URL. When omitted, the script reads
                     \$ZD_DAEMON_URL, then \$ZD_PORT, and finally falls back
...
Online daemon URL resolution (first match wins):
  ...
  2. \`ZD_DAEMON_URL\` env var.
  3. \`http://127.0.0.1:\$ZD_PORT\` when \`ZD_PORT\` is set to a real port.
...
function isDiscoverablePort(value: string | undefined): value is string {
  ...
  // tools-dev sets ZD_PORT=0 to mean "ephemeral, look at runtime status",
...
async function resolveDaemonUrl(args: Args, { required }: { required: boolean }): Promise<string | null> {
  ...
  if (process.env.ZD_DAEMON_URL) return process.env.ZD_DAEMON_URL;
  if (isDiscoverablePort(process.env.ZD_PORT)) {
    return `http://127.0.0.1:${process.env.ZD_PORT}`;
  }
  const discovered = await discoverDaemonUrlFromToolsDev(args.namespace);
  if (discovered) return discovered;
  if (!required) return null;
  throw new Error(
    'cannot determine daemon URL: no --daemon flag, no ZD_DAEMON_URL, ' +
      'no usable ZD_PORT, and `pnpm tools-dev status --json` did not report a ' +
...
function resolveDataDir(raw: string | null): string {
  const value = raw ?? process.env.ZD_DATA_DIR ?? path.join(REPO_ROOT, '.od');
...
function assertOfflineDataDirIsExplicit(args: Args): void {
  if (args.namespace && !args.dataDir && !process.env.ZD_DATA_DIR) {
    throw new Error(
      '--namespace is only a tools-dev discovery selector. Offline mode with ' +
        '--namespace requires --data-dir or ZD_DATA_DIR so the script does not ' +
