...
// (the registry only resolves SKILL.md / .claude-plugin / z-design.json
...
export async function runExampleGenerator(opts: ExampleGeneratorOptions): Promise<RunStats> {
  ...
  for (const id of slice) {
    ...
    const manifest = buildManifest({
      ...
      homepage: `https://github.com/Aqu-210L/Z-design/tree/main/plugins/_official/${TIER_EXAMPLES}/${id}`,
      tags: dedupeTags([
        'example',
        'first-party',
        mode,
