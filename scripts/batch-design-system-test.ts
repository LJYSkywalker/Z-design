...
Run/project options:
  --daemon <url>               Daemon base URL. Falls back to ZD_DAEMON_URL,
                               ZD_PORT, then tools-dev status discovery.
...
async function resolveDaemonUrl(config: BatchConfig): Promise<string> {
  ...
  if (isDiscoverablePort(process.env.ZD_PORT)) return `http://127.0.0.1:${process.env.ZD_PORT}`;
...
async function main(): Promise<void> {
  ...
  if (resolved.dryRun) {
    ...
    const daemonUrl =
      config.daemonUrl ??
      process.env.ZD_DAEMON_URL ??
      (isDiscoverablePort(process.env.ZD_PORT) ? `http://127.0.0.1:${process.env.ZD_PORT}` : '(not resolved in dry-run)');
