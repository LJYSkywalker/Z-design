 ...
 *
 * PR-D will flip the `ZD_DESIGN_TOKEN_CHANNEL` env default from off to
...
export async function checkDesignSystemFlagParity(): Promise<boolean> {
  ...
  for (const brand of snapshots) {
    ...
    if (flagOn !== flagOff) {
      violations.push(
        [
          `[${brand.id}] prose-only brand produced different prompts under flag-off vs flag-on — flipping ZD_DESIGN_TOKEN_CHANNEL would silently change behaviour for this brand.`,
  ...
  if (violations.length > 0) {
    ...
    console.error(
      "PR-D's promise is that flipping ZD_DESIGN_TOKEN_CHANNEL on by default is a no-op for the ~138 prose-only brands and a deliberate uplift for the structured tier. Both halves of that promise must keep holding.",
    );
    return false;
  }
  ...
  console.log(
    `Design system flag parity passed: ${proseLabel} produce byte-identical prompts under ZD_DESIGN_TOKEN_CHANNEL flag-off vs flag-on; ${structuredLabel} show the expected token-channel divergence.`,
  );
  return true;
}
