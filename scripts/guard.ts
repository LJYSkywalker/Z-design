...
// add stricter local terms through ZD_PRODUCT_NEUTRALITY_FORBIDDEN_TERMS.
...
function productNeutralityForbiddenTerms(): string[] {
  return String(process.env.ZD_PRODUCT_NEUTRALITY_FORBIDDEN_TERMS ?? "")
    .split(",")
    .map((term) => term.trim())
...
function webImportIsolationViolationReason(fromRepositoryPath: string, specifier: string): string | null {
  ...
  if (webImportIsolationForbiddenDaemonRoots.some((root) => isPathOrDescendant(resolvedPath, root))) {
    return "apps/web must use daemon HTTP APIs or @z-design/contracts instead of daemon private source";
  }
