...
test("cross-app import check rejects another app's package name", () => {
  const violations = collectCrossAppImportViolationsFromSource(
    ...
    [
      "import type { RunRecord } from '@z-design/daemon/src/server.ts';",
      "const mod = await import('@z-design/daemon');",
      "const legacy = require('@z-design/daemon/dist/cli.js');",
    ].join("\n"),
    registry,
  );
...
test("cross-app import check rejects another app's package name in require.resolve", () => {
  const violations = collectCrossAppImportViolationsFromSource(
    ...
    "const daemonManifest = require.resolve('@z-design/daemon/package.json');",
    registry,
  );

  assert.equal(violations.length, 1);
  assert.equal(violations[0]?.specifier, "@z-design/daemon/package.json");
  assert.equal(violations[0]?.targetApp, "daemon");
  assert.equal(violations[0]?.lineNumber, 1);
});
...
test("cross-app import check rejects createRequire-based cross-app resolution", () => {
  const violations = collectCrossAppImportViolationsFromSource(
    ...
    [
      ...
      "const directDaemonManifest = createRequire(import.meta.url).resolve('@z-design/daemon/package.json');",
      "const aliasDaemonManifest = makeRequire(import.meta.url).resolve('@z-design/daemon/package.json');",
      "const nodeRequire = nodeModule.createRequire(import.meta.url);",
      "const daemonCli = nodeRequire('@z-design/daemon/dist/cli.js');",
      "const daemonPackageJson = nodeRequire.resolve('@z-design/daemon/package.json');",
    ].join("\n"),
    registry,
  );
  ...
  assert.deepEqual(
    ...
    [
      "@z-design/daemon/package.json",
      "@z-design/daemon/package.json",
      "@z-design/daemon/dist/cli.js",
      "@z-design/daemon/package.json",
    ],
  );
  assert.deepEqual(
...
test("cross-app import check rejects CommonJS node:module namespace createRequire resolution", () => {
  const violations = collectCrossAppImportViolationsFromSource(
    ...
    [
      ...
      'nodeRequire.resolve("@z-design/daemon/package.json");',
    ].join("\n"),
    registry,
  );
...
test("cross-app import check rejects cross-app imports from app-owned mjs entrypoints", () => {
  ...
  const violations = collectCrossAppImportViolationsFromSource(
    ...
    "import { startDaemon } from '@z-design/daemon/src/server.ts';",
    registry,
  );
...
test("cross-app import check allows packages, same-app relatives, and externals", () => {
  const violations = collectCrossAppImportViolationsFromSource(
    ...
    [
      "import { Button } from '@z-design/components';",
      "import type { ChatRunEvent } from '@z-design/contracts';",
...
test("cross-app import check ignores quoted snippets and comments", () => {
  const violations = collectCrossAppImportViolationsFromSource(
    ...
    [
      "const snippet = \"import { x } from '@z-design/daemon/src/server.ts';\";",
      "// import { y } from '@z-design/daemon/src/server.ts';",
      "/*",
      "require('@z-design/daemon/dist/cli.js');",
      "*/",
    ].join("\n"),
    registry,
  );
...
test("cross-app import check allows allowlisted packaged -> desktop main export", () => {
  const violations = collectCrossAppImportViolationsFromSource(
    ...
    "import { applyOsLocaleSwitch, createSplashWindow } from '@z-design/desktop/main';",
    { packageNameByDirectory: new Map([["packaged", "@z-design/packaged"], ["desktop", "@z-design/desktop"]]) },
  );
