...
import {
  ...
  checkHostUpdater,
  detectZDesignHostClientType,
  getHostUpdaterStatus,
  getZDesignHost,
  installHostUpdater,
  isZDesignHostAvailable,
  isZDesignHostBridge,
  normalizeZDesignHostProjectImportResult,
  openHostExternalUrl,
...
import { createMockZDesignHost, installMockZDesignHost } from "../src/testing.js";
...
describe("open-design host contract", () => {
  it("stays independent from daemon/web contracts", () => {
    ...
    }).not.toHaveProperty("@z-design/contracts");

    const offenders = filesUnder(join(hostRoot, "src")).filter((path) =>
      readFileSync(path, "utf8").includes("@z-design/contracts"),
    );
    expect(offenders).toEqual([]);
  });

  it("recognizes the canonical bridge shape", () => {
    const host = createMockZDesignHost();
    expect(isZDesignHostBridge(host)).toBe(true);
    expect(host.version).toBe(OPEN_DESIGN_HOST_VERSION);
  });

  it("rejects legacy or incomplete bridge shapes", () => {
    expect(isZDesignHostBridge({ version: OPEN_DESIGN_HOST_VERSION })).toBe(false);
    expect(isZDesignHostBridge({ ...createMockZDesignHost(), version: 1 })).toBe(false);
    expect(isZDesignHostBridge({
      ...createMockZDesignHost(),
      browser: {},
    })).toBe(false);
    expect(isZDesignHostBridge({
      ...createMockZDesignHost(),
    ...
    expect(isZDesignHostBridge({
      ...createMockZDesignHost(),
      updater: { status: async () => createMockZDesignHost().updater.status() },
    })).toBe(false);
  });

  it("reads the bridge through the package-owned global accessor", () => {
    const scope: Record<string, unknown> = {};
    scope[OPEN_DESIGN_HOST_GLOBAL] = createMockZDesignHost();
    expect(getZDesignHost(scope)?.client.type).toBe("desktop");
    expect(isZDesignHostAvailable(scope)).toBe(true);
    expect(detectZDesignHostClientType(scope)).toBe("desktop");
  });

  it("falls back to web when no host is installed", () => {
    expect(getZDesignHost({})).toBeNull();
    expect(isZDesignHostAvailable({})).toBe(false);
    expect(detectZDesignHostClientType({})).toBe("web");
  });
  ...
  it("normalizes privileged project-import results into host-owned identifiers", () => {
    const result = normalizeZDesignHostProjectImportResult({
      ok: true,
      response: {
        project: {
          id: "project-1",
  ...
  it("preserves canceled and structured failure project-import results", () => {
    expect(normalizeZDesignHostProjectImportResult({ canceled: true, ok: false })).toEqual({
      canceled: true,
      ok: false,
    });
    expect(normalizeZDesignHostProjectImportResult({
      ok: false,
  ...
  it("installs and restores test hosts without exposing callers to the global key", () => {
    const scope: Record<string, unknown> = {};
    const restore = installMockZDesignHost({ scope });
    expect(getZDesignHost(scope)).not.toBeNull();
    restore();
    expect(getZDesignHost(scope)).toBeNull();
  });
});
