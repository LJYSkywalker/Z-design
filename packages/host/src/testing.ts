import {
  ...
  type ZDesignHostBridge,
  type ZDesignHostGlobalScope,
  type ZDesignHostUpdaterStatusSnapshot,
} from "./index.js";

export type MockZDesignHost = Partial<Omit<ZDesignHostBridge, "capture" | "client" | "pdf" | "pet" | "project" | "shell" | "updater">> & {
  browser?: Partial<ZDesignHostBridge["browser"]>;
  capture?: Partial<ZDesignHostBridge["capture"]>;
  client?: Partial<ZDesignHostBridge["client"]>;
  pdf?: Partial<ZDesignHostBridge["pdf"]>;
  pet?: Partial<ZDesignHostBridge["pet"]>;
  project?: Partial<ZDesignHostBridge["project"]>;
  shell?: Partial<ZDesignHostBridge["shell"]>;
  updater?: Partial<ZDesignHostBridge["updater"]>;
};

export type MockZDesignHostOptions = {
  host?: MockZDesignHost;
  scope?: ZDesignHostGlobalScope;
};

function defaultHost(): ZDesignHostBridge {
  const updaterStatus: ZDesignHostUpdaterStatusSnapshot = {
    arch: "arm64",
    capabilities: {
...
export function createMockZDesignHost(overrides: MockZDesignHost = {}): ZDesignHostBridge {
...
export function installMockZDesignHost(options: MockZDesignHostOptions = {}): () => void {
  const scope = (options.scope ?? globalThis) as ZDesignHostGlobalScope;
  const host = createMockZDesignHost(options.host);
  const windowValue = scope.window;
  const targets = [
    scope,
    ...(typeof windowValue === "object" && windowValue != null && windowValue !== scope
      ? [windowValue as ZDesignHostGlobalScope]
      : []),
  ];
