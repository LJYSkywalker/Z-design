export const OPEN_DESIGN_GITHUB_REPO_URL = 'https://github.com/Aqu-210L/Z-design';
...
export function buildSocialSharePayload(input: SocialShareRequest): SocialShareResponse {
  ...
  const fallbackTitle = kind === 'project-html' ? 'Z-Design project' : 'Z-Design';
  const title = cleanText(input.title, fallbackTitle);
  const fallbackText = kind === 'project-html'
    ? `Built with Z-Design: ${title}. Z-Design repo: ${OPEN_DESIGN_GITHUB_REPO_URL}`
    : 'Z-Design is an open-source workspace for creating, editing, deploying, and handing off design artifacts.';
