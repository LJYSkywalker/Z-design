...
- \`ZD_NODE_BIN\` - absolute path to the Node-compatible runtime that started the daemon.
- \`ZD_BIN\` - absolute path to the OD CLI script. On POSIX shells run with \`"$ZD_NODE_BIN" "$ZD_BIN" ...\`.
- \`ZD_PROJECT_ID\` - active project id. Pass it as \`--project "$ZD_PROJECT_ID"\`.
- \`ZD_PROJECT_DIR\` - active project files directory.
- \`ZD_DAEMON_URL\` - base URL of the local daemon.
...
\`\`\`bash
"$ZD_NODE_BIN" "$ZD_BIN" media wait <taskId> --since <nextSince>
\`\`\`
