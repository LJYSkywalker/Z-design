function Resolve-WindowsSignToolPath([string]$PreferredPath = "") {
  ...
  foreach ($value in @(
    $PreferredPath,
    [Environment]::GetEnvironmentVariable("ZD_WIN_SIGNTOOL_PATH"),
...
function Read-WindowsSigningProbeFromEnvironment {
  $probed = [Environment]::GetEnvironmentVariable("ZD_BETA_WINDOWS_SIGNING_PROBED")
  if (-not (ConvertTo-Boolean $probed)) {
    return $null
  }

  return [ordered]@{
    certificateFound = ConvertTo-Boolean ([Environment]::GetEnvironmentVariable("ZD_BETA_WINDOWS_SIGNING_CERTIFICATE_FOUND"))
    enabled = ConvertTo-Boolean ([Environment]::GetEnvironmentVariable("ZD_BETA_WINDOWS_SIGNING_ENABLED"))
    mode = [Environment]::GetEnvironmentVariable("ZD_BETA_WINDOWS_SIGNING_MODE")
    reason = [Environment]::GetEnvironmentVariable("ZD_BETA_WINDOWS_SIGNING_REASON")
    requested = ConvertTo-Boolean ([Environment]::GetEnvironmentVariable("ZD_BETA_WINDOWS_SIGNING_REQUESTED"))
    signToolFound = ConvertTo-Boolean ([Environment]::GetEnvironmentVariable("ZD_BETA_WINDOWS_SIGNING_SIGNTOOL_FOUND"))
    signToolPath = [Environment]::GetEnvironmentVariable("ZD_BETA_WINDOWS_SIGNTOOL_PATH")
    thumbprint = [Environment]::GetEnvironmentVariable("ZD_BETA_WINDOWS_SIGN_CERT_SHA1")
  }
}
