...
function Get-SmokeSummary {
  if ([string]::IsNullOrWhiteSpace($env:ZD_PACKAGED_E2E_REPORT_DIR)) {
    return $null
  }
  $summaryJsonPath = Join-Path $env:ZD_PACKAGED_E2E_REPORT_DIR "summary.json"
...
function Write-IndexAndSummary([string]$Status) {
  ...
  $index = [ordered]@{
    ...
    reportDir = $env:ZD_PACKAGED_E2E_REPORT_DIR
    smokeMode = $SmokeMode
    smoke = $smokeSummary
...
$env:ZD_PACKAGED_E2E_REPORT_DIR = Join-Path $reportDir "win"
$env:ZD_PACKAGED_E2E_TOOLS_PACK_DIR = $toolsPackDir
...
try {
  ...
  Measure-Step "windows signing capability" {
    ...
    if ($script:windowsSigningEnabled) {
      $env:ZD_WIN_SIGN_CERT_SHA1 = [string]$signingProbe.thumbprint
      $env:ZD_WIN_SIGNTOOL_PATH = [string]$signingProbe.signToolPath
      $env:ZD_WIN_SIGN_TIMESTAMP_URL = "http://timestamp.digicert.com"
  ...
  Measure-Step "tools-pack build" {
    Invoke-Node24 -Arguments @("pnpm.cmd", "--filter", "@z-design/tools-pack", "build")
  }
  ...
  $externalUpdateMetadataUrl = [string]$env:ZD_PACKAGED_E2E_WIN_UPDATE_METADATA_URL
  $externalUpdateArtifactPath = [string]$env:ZD_PACKAGED_E2E_WIN_UPDATE_ARTIFACT_PATH
  $externalUpdateVersion = [string]$env:ZD_PACKAGED_E2E_WIN_UPDATE_VERSION
  ...
  $env:ZD_PACKAGED_E2E_BUILD_JSON_PATH = $buildJsonPath
  $env:ZD_PACKAGED_E2E_WIN = "1"
  $env:ZD_PACKAGED_E2E_WIN_SMOKE_PROFILE = $SmokeMode
  $env:ZD_PACKAGED_E2E_NAMESPACE = $Namespace
  $env:ZD_PACKAGED_E2E_RELEASE_CHANNEL = "beta"
  $env:ZD_PACKAGED_E2E_RELEASE_VERSION = $ReleaseVersion
  if ([string]::IsNullOrWhiteSpace($localUpdateArtifactPath)) {
    if (-not $hasExternalUpdateArtifactPair) {
      Remove-Item Env:ZD_PACKAGED_E2E_WIN_UPDATE_ARTIFACT_PATH -ErrorAction SilentlyContinue
    }
  } else {
    $env:ZD_PACKAGED_E2E_WIN_UPDATE_ARTIFACT_PATH = $localUpdateArtifactPath
  }
  if ([string]::IsNullOrWhiteSpace($localUpdateVersion)) {
    if (-not $hasExternalUpdateArtifactPair -and -not $hasExternalUpdateMetadata) {
      Remove-Item Env:ZD_PACKAGED_E2E_WIN_UPDATE_VERSION -ErrorAction SilentlyContinue
    }
  } else {
    $env:ZD_PACKAGED_E2E_WIN_UPDATE_VERSION = $localUpdateVersion
  }
  if ($SmokeMode -eq "full" -and -not [string]::IsNullOrWhiteSpace($localUpdateArtifactPath)) {
    $env:ZD_PACKAGED_E2E_WIN_UPDATE_BUILD_JSON_PATH = (Join-Path $platformRoot "windows-tools-pack-update-build.json")
  } else {
    Remove-Item Env:ZD_PACKAGED_E2E_WIN_UPDATE_BUILD_JSON_PATH -ErrorAction SilentlyContinue
  }
  ...
  if ($SmokeMode -eq "skip") {
    ...
    Measure-Step "release smoke win" {
      Remove-Item -LiteralPath $env:ZD_PACKAGED_E2E_REPORT_DIR -Recurse -Force -ErrorAction SilentlyContinue
