...
Optional environment:
  ...
  ZD_EXPECT_TIMEOUT_SECONDS=1200
  ZD_SANDBOX_CPUS=4
  ZD_SANDBOX_MEMORY=8g
  ZD_ALLOW_NPX_EXPECT_CLI=1
  ZD_TRACE_R2_UPLOAD=1
  R2_ACCOUNT_ID=...
  R2_ACCESS_KEY_ID=...
...
if ! command -v expect-cli >/dev/null 2>&1 && [ "${ZD_ALLOW_NPX_EXPECT_CLI:-0}" != "1" ]; then
  echo "::error::expect-cli is not installed. Install it on the mini with: npm install -g expect-cli@${ZD_EXPECT_CLI_VERSION:-0.1.3}" >&2
  echo "        For a one-off smoke run, set ZD_ALLOW_NPX_EXPECT_CLI=1 to use the pinned npx fallback." >&2
  exit 1
fi
