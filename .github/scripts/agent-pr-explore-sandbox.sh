...
# sharing across PRs is safe; override with ZD_SANDBOX_PNPM_STORE if needed.
pnpm_store="${ZD_SANDBOX_PNPM_STORE:-$HOME/.cache/agent-pr-explore/pnpm-store}"
...
image="${ZD_SANDBOX_IMAGE:-node:24-bookworm}"
container_web_port=17573
container_daemon_port=17456
container_proxy_port=17574
host_web_port="${ZD_SANDBOX_WEB_PORT:-$((20000 + (PR_NUMBER % 20000)))}"
base_url="http://127.0.0.1:${host_web_port}"
cpus="${ZD_SANDBOX_CPUS:-4}"
memory="${ZD_SANDBOX_MEMORY:-8g}"
expect_timeout_seconds="${ZD_EXPECT_TIMEOUT_SECONDS:-1200}"
expect_cli_version="${ZD_EXPECT_CLI_VERSION:-0.1.3}"
...
# CODEX_HOME). Set ZD_EXPECT_AGENT="" to fall back to expect-cli's default.
expect_agent="${ZD_EXPECT_AGENT-codex}"
expect_agent_args=""
[ -n "$expect_agent" ] && expect_agent_args="-a $expect_agent"
context_max_bytes="${ZD_EXPECT_CONTEXT_MAX_BYTES:-120000}"
file_patch_max_chars="${ZD_EXPECT_FILE_PATCH_MAX_CHARS:-8000}"
ready_timeout_seconds="${ZD_SANDBOX_READY_TIMEOUT_SECONDS:-900}"
...
select_deterministic_verifier() {
  local requested="${ZD_DETERMINISTIC_VERIFIER:-auto}"
...
select_agent_fixture() {
  local requested="${ZD_AGENT_FIXTURE:-auto}"
  if [ "$requested" != "auto" ]; then
    echo "$requested"
    return
  fi
...
EOF
      ;;
    *)
      echo "::error::Unknown ZD_AGENT_FIXTURE: $fixture"
      exit 1
      ;;
  esac
}
...
record_playwright_artifacts() {
  if [ "${ZD_RECORD_PLAYWRIGHT_ARTIFACTS:-1}" = "0" ]; then
  ...
  TRACE_PUBLIC_BASE_URL="${ZD_TRACE_PUBLIC_BASE_URL:-}" \
  TRACE_PUBLIC_TRACE_URL="${ZD_TRACE_PUBLIC_TRACE_URL:-}" \
  node <<'NODE'
...
function ensurePlaywrightBrowserCache() {
  if (process.env.ZD_INSTALL_PLAYWRIGHT_BROWSERS === "0") return;
...
function writeTraceViewerFiles(viewerUrl) {
  ...
  const markdown = viewerUrl
    ...
    : [
        ...
        "```",
        "",
        "To generate a one-click trace link in future runs, upload `playwright-smoke-trace.zip` somewhere browser-readable and set `ZD_TRACE_PUBLIC_BASE_URL` to that artifact directory, or set `ZD_TRACE_PUBLIC_TRACE_URL` to the zip URL.",
        "",
      ].join("\n");
...
publish_trace_artifacts_to_r2() {
  if [ "${ZD_TRACE_R2_UPLOAD:-0}" != "1" ]; then
    return 0
  fi
...
mirror="${ZD_SANDBOX_REPO_MIRROR:-$HOME/.cache/agent-pr-explore/open-design.git}"
git_ssh_key="${ZD_SANDBOX_GIT_SSH_KEY:-$HOME/.ssh/od_agent_deploy}"
pr_src="$root/pr-src"
...
docker run -d \
  ...
  --env "ZD_ALLOWED_ORIGINS=$base_url" \
  --env "ZD_DETERMINISTIC_VERIFIER=$deterministic_verifier" \
  --env "CI=true" \
  ...
  bash -lc '
...

    {
      ...
      pnpm --filter @z-design/tools-dev build

      if [ "${ZD_DETERMINISTIC_VERIFIER}" = "web-static-export" ]; then
        echo "== deterministic verifier: web-static-export =="
        set +e
        (
          set -euo pipefail
          rm -rf apps/web/out apps/web/.next
          ZD_WEB_OUTPUT_MODE=server sh -lc '"'"'ZD_WEB_OUTPUT_MODE= pnpm --filter @z-design/web build && test -d apps/web/out'"'"'
...
- Reproduced the relevant inherited environment condition for this change: \`ZD_WEB_OUTPUT_MODE=server\` is present, then the web build explicitly clears it for static export.
...
\`\`\`bash
rm -rf apps/web/out apps/web/.next
ZD_WEB_OUTPUT_MODE=server sh -c 'ZD_WEB_OUTPUT_MODE= pnpm --filter @z-design/web build && test -d apps/web/out'
test -f apps/web/out/index.html
\`\`\`
...
if command -v expect-cli >/dev/null 2>&1; then
...
elif [ "${ZD_ALLOW_NPX_EXPECT_CLI:-0}" = "1" ] && command -v npx >/dev/null 2>&1; then
  ...
  echo "::error::expect-cli is required on the agent-pr-explore runner. Install expect-cli@${expect_cli_version}, or set ZD_ALLOW_NPX_EXPECT_CLI=1 to use the pinned npx fallback."
  exit 1
fi
...
report_persist_dir="${ZD_SANDBOX_REPORT_DIR:-$HOME/.cache/agent-pr-explore/reports}/pr-${PR_NUMBER}"
