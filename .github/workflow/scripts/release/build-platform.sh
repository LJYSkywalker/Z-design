...
if [ "$RELEASE_SMOKE_MODE" = "skip" ]; then
  ...
  ZD_PACKAGED_E2E_LINUX_APPIMAGE=1 \
  ZD_PACKAGED_E2E_NAMESPACE="$RELEASE_NAMESPACE" \
  ZD_PACKAGED_E2E_SCREENSHOT_PATH="$RELEASE_REPORT_DIR/screenshots/open-design-linux-smoke.png" \
  ZD_PACKAGED_E2E_TOOLS_PACK_DIR="$TOOLS_PACK_DIR" \
  pnpm --dir e2e test specs/linux.spec.ts 2>&1 | tee "$RELEASE_REPORT_DIR/vitest.log"
else
  required RELEASE_REPORT_DIR
  update_build_json_path=""
  update_version=""
  if [ "$RELEASE_SMOKE_MODE" = "full" ] && [ "$RELEASE_TARGET" = "mac_arm64" ] && [ -z "${ZD_PACKAGED_E2E_MAC_UPDATE_METADATA_URL:-}" ]; then
  ...
  ZD_PACKAGED_E2E_BUILD_JSON_PATH="$BUILD_JSON_PATH" \
  ZD_PACKAGED_E2E_BUILD_LOG_PATH="$BUILD_LOG_PATH" \
  ZD_PACKAGED_E2E_MAC=1 \
  ZD_PACKAGED_E2E_MAC_UPDATE_BUILD_JSON_PATH="$update_build_json_path" \
  ZD_PACKAGED_E2E_MAC_UPDATE_METADATA_URL="${ZD_PACKAGED_E2E_MAC_UPDATE_METADATA_URL:-}" \
  ZD_PACKAGED_E2E_MAC_UPDATE_VERSION="${ZD_PACKAGED_E2E_MAC_UPDATE_VERSION:-$update_version}" \
  ZD_PACKAGED_E2E_MAC_SMOKE_PROFILE="$RELEASE_SMOKE_MODE" \
  ZD_PACKAGED_E2E_NAMESPACE="$RELEASE_NAMESPACE" \
  ZD_PACKAGED_E2E_RELEASE_CHANNEL=beta \
  ZD_PACKAGED_E2E_RELEASE_VERSION="$RELEASE_VERSION" \
  ZD_PACKAGED_E2E_REPORT_DIR="$RELEASE_REPORT_DIR" \
  ZD_PACKAGED_E2E_TOOLS_PACK_DIR="$TOOLS_PACK_DIR" \
